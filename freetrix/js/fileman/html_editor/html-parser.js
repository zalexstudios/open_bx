...
(function()
{
	...
	BXEditorParser.prototype = {
					...
					if (!oldNode.__bxparsed)
					{
						...
						if (oldNode && oldNode.id)
						{
							...
							if(bxTag.tag)
							{
								...
								// We've found freetrix-made node
								if (this.bParseBx)
								{
		...
		{
			var
				styleName, styleValue, _styleValue, name, i,
			...
			if (oldNode.style && BX.util.trim(oldNode.style.cssText) != '')
			{
				...
				if (oldNode.style.cssText != '')
				{
					styleValue = _styleValue = oldNode.getAttribute('style');

					// Clean mso-xxx styles
					if (styleValue && styleValue.indexOf('mso-') !== -1)
					{
						styleValue = styleValue.replace(/\s*mso-[^:]+:[^;"]+;?/gi, "");
					}

					// Replace " with ' to prevent specialcharing (" will be replaced with &quote; in FF)
					if (styleValue.indexOf('"') !== -1)
					{
						styleValue = styleValue.replace(/"/gi, "'");
					}

					// Clean vendor styles with prefixes: -webkit, -moz, -ms, -o
					if (styleValue.match(/\-(moz|o|webkit|ms)\-/))
					{
						styleValue = styleValue.replace(/\s*[^:]+:\s*\-(moz|o|webkit|ms)\-[^;"]+;?/gi,"");
						styleValue = styleValue.replace(/\s*\-(moz|o|webkit|ms)\-[^:]+:[^;"]+;?/gi, "");
					}

					if (_styleValue !== styleValue)
					{
						oldNode.setAttribute('style', styleValue);
					}
				}
			}
...
//					return attributeValue.replace(/^https?:\/\//i, function(match){return match.toLowerCase();});
		...
		HandleFreetrixNode: function(node)
		{
			return node;
		},
	...
	BXEditorPhpParser.prototype = {
		ParsePhp: function(content)
		{
			...
			//3. We replace all #BXPHP_IND# by visual custom elements
			if (this.IsAllowed('php'))
			{
		...
		ReplaceIframeBySymCode: function(content)
		{
			...
			var
				_this = this,
				index = 0;
			content = content.replace(/<iframe[\s\S]*?\/iframe>/gi, function(s)
				{
		...
		ParseSymCode: function(content)
		{
			...
			content = content.replace(/#BX(PHP|JAVASCRIPT|HTMLCOMMENT|IFRAME|STYLE)_(\d+)#/g, function(str, type, ind)
			{
				var res;
				switch (type)
				{
		...
		GetPhpCodeHTML: function(code)
		{
			...
			if (component !== false) // It's Freetrix Component
			{
				var
		...
		GetShortTitle: function(str, trim)
		{
			...
			return BX.util.htmlspecialchars(str);
		},
...
//				return '<img style="cursor: default;" id="' + id + '" src="' + icon + '" />';
		...
		OnSurrogateDragEnd: function(e, target, bxTag)
		{
			...
			{
				surr = surrs[i];
				if (surr && surr.id == bxTag.params.origId)
				{
					if (surr.getAttribute('data-bx-paste-flag'))
					{
						BX.remove(surr);
					}
					else
		...
		OnSurrogateKeydown: function(e, keyCode, command, target)
		{
			...
			while (iter <= 4 && node && node.nodeName !== 'BODY')
			{
				if (node.nodeType == 1 && node.id)
				{
					bxTag = this.editor.GetBxTag(node.id);
					if (this.surrogateTags[bxTag.tag])
					{
						if (keyCode === this.editor.KEY_CODES['backspace'] || keyCode === this.editor.KEY_CODES['delete'])
						{
							this.RemoveSurrogate(node, bxTag);
						}
						else if (keyCode === this.editor.KEY_CODES['left'] || keyCode === this.editor.KEY_CODES['up'])
						{
							this.editor.selection.SetBefore(node);
							invisText = this.editor.util.GetInvisibleTextNode();
							this.editor.selection.InsertNode(invisText);
							this.editor.selection.SetBefore(invisText);
						}
						else
						{
