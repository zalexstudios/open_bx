...
//					}
				...
				state: function(command, value, tagName, arStyle, cssClass)
				{
					var
		...
		/*
		* TODO: 1. При создании H1 убирать незначащие span-ы
		* */
...
//					eventListener.stop();
			...
			return {
				exec: function(command, nodeName, className, arStyle)
				{
		...
		GetRemoveFormat: function()
		{
			...
			{
				var parentTable = BX.findParent(node, function(n)
				{
				...
				exec: function(action, value)
				{
					...
					if (range && !_this.editor.iframeView.IsEmpty())
					{
						...
						if (bSurround)
						{
							...
							_this.editor.selection.Surround(tmpNode);
		...
		GetAlign: function()
		{
			...
			return {
				exec: function(action, value)
				{
					...
					if (selectedNode)
					{
						...
						else if (selectedNode.nodeType == 1 && TABLE_NODES[selectedNode.nodeName])
						{
							...
							res = true;
							setTimeout(function(){_this.editor.selection.SelectNode(tableElement);}, 10);
						}
						else
						{
				...
				state: function(action, value)
				{
					...
					{
						return {node: null, value: DEFAULT_VALUE, res: false};
					}
				},
				value: BX.DoNothing
			};
