...
BXEditorTextareaView.prototype.SetValue = function(html, bParse)
{
	if (bParse)
	{
		html = this.editor.Parse(html, true);
	}
...
//			}, 0);
		...
		BX.bind(element, "keyup", function(e)
		{
			...
			else if (!e.shiftKey && (keyCode === editor.KEY_CODES['right'] || keyCode === editor.KEY_CODES['down']))
			{
				editor.selection.SetCursorAfterNode(e);
			}
			else if (!e.shiftKey && (keyCode === editor.KEY_CODES['left'] || keyCode === editor.KEY_CODES['up']))
			{
	...
	BXEditorIframeView.prototype.OnEnterHandler = function(e, keyCode, selectedNode)
	{
		...
		else
		{
			...
			if (blockElement)
			{
				...
				else if (blockElement.nodeName.match(/H[1-6]/) && keyCode === KEY_CODES["enter"])
				{
					setTimeout(function()
					{
 ...
 */
	...
	{
		FromIframeToTextarea: function(bParseHtml)
		{
			var value = this.iframeView.GetValue();
			if (value !== this.lastIframeValue)
			{
				value = BX.util.trim(value);
				this.textareaView.SetValue(value, true);
