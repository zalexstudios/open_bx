...
;(function() {
	...
	TaskbarManager.prototype = {
		...
		OnClick: function(e)
		{
			...
			if (type == 'taskbarmanager' || type == 'taskbarflip' || !type)
			{
				if (this.bShown)
				{
					this.Hide();
			...
			else if(type == 'taskbargroup')
			{
				BX.onCustomEvent(this, 'taskbargroupClick', [target]);
			}
	...
	function Taskbar(editor)
	{
		...
		BX.addCustomEvent(this.manager, 'taskbargroupClick', BX.proxy(this.OnGroupClick, this));
	...
	Taskbar.prototype = {
		...
		BuildSection: function(section)
		{
			var
				...
				title = pGroupTitle.appendChild(BX.create("SPAN", {props: {className: "bxhtmled-tskbr-sect-title"}, text: section.title})),
		...
		BuildElement: function(element)
		{
			var
				_this = this,
				parentCont = this.GetSectionContByPath(element.path, true),
		...
		OnGroupClick: function(pElement)
		{
			if (pElement && pElement.getAttribute('data-bx-taskbar') == this.id)
			{
				return this.ToggleGroup(pElement);
			}
			return true;
		},
	...
	function SnippetsControl(editor)
	{
		...
		this.title = BX.message('SnippetsTitle');
		this.uniqueId = 'taskbar_' + this.editor.id + '_' + this.id;

		this.Init();
	}

	BX.extend(SnippetsControl, Taskbar);
	...
	SnippetsControl.prototype.GetMenuItems = function()
	{
		var arItems = [
			...
			{
				text : BX.message('ManageCategories'),
				title : BX.message('ManageCategories'),
				className : "",
				onclick: function(){alert('ManageCategories');}
			},
			{
				text : BX.message('AddSnippet'),
				title : BX.message('AddSnippet'),
				className : "",
				onclick: function(){alert('AddSnippet');}
			}
		]
		return arItems;
	};
	...
	Toolbar.prototype = {
		...
		GetControlsMap: function()
		{
			var res = [
				...
				{id: 'ChangeView', wrap: 'left', compact: true},
				{id: 'Undo', compact: false},
				{id: 'Redo', compact: false},
				{id: 'StyleSelector', compact: true},
				{id: 'FontSelector', compact: false},
				{id: 'FontSize', compact: false},
				{separator: true, compact: false},
				{id: 'Bold', compact: true},
				{id: 'Italic', compact: true},
				{id: 'Underline', compact: true},
				{id: 'Strikeout', compact: true},
				...
				{id: 'InsertImage', compact: true},
				{id: 'InsertAnchor', compact: false},
				{id: 'InsertTable', compact: false},
				{id: 'InsertChar', compact: false, hidden: true},
				{id: 'PrintBreak', compact: false, hidden: true},
//				{id: 'PageBreak', compact: false, hidden: true},
//				{id: 'InsertHr', compact: false, hidden: true},
				{id: 'TemplateSelector', compact: false},
				{id: 'Fullscreen', compact: true},
				{id: 'More', compact: true}
				//{id: 'Settings',  wrap: 'right', compact: true}
			];
...
//			AlignList: AlignList,
	...
	DropDown.prototype = {
		...
		Open: function ()
		{
			...
			var
				...
				pos = BX.pos(bindCont),
				left = pos.left - this.pValuesCont.offsetWidth / 2 + bindCont.offsetWidth / 2 + this.posOffset.left,
	...
	ImageDialog.prototype.Build = function()
	{
		...
		r.rightCell.appendChild(BX('bx-open-file-medialib-but-' + this.editor.id));
...
*/
	...
	LinkDialog.prototype.GetValues = function()
	{
		...
		else if (type == 'external')
		{
			...
			if(this.pStat.checked)
			{
				value.href = '/freetrix/redirect.php?event1=' + escape(this.pStatEvent1.value) + '&event2=' + escape(this.pStatEvent2.value) + '&event3=' + escape(this.pStatEvent3.value) + '&goto=' + escape(value.href);
			}
		}
		else if (type == 'anchor')
		{
