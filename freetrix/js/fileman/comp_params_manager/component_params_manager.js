/**
 * Freetrix Component Param Manager
 * Date: 26.11.13
 * Time: 4:23
 *
 * ParamsManager
		...
		* */
		...
		BuildComponentParams: function(data, params)
		{
			...
			for (i = 0; i < data.parameters.length; i++)
			{
				...
				this.BuildComponentParameter(param, value, this.groupIndex[param.PARENT].frag);
			}

			for (i = 0; i < data.groups.length; i++)
			{
				group = data.groups[i];
				//if (groupIndex[group.ID].frag.childNodes.length > 0)
				{
		...
		CheckActiveGroup: function(groupId)
		{
			...
			else
			{
				...
				{
					if (this.groupIndex.hasOwnProperty(id))
					{
		...
		BuildComponentParameter: function(param, value, frag)
		{
			...
			var
				tr = BX.adjust(frag.appendChild(BX.create("TR")), {props: {className: 'bxcompprop-prop-tr'}}),
				propId = BX.util.htmlspecialchars(param.ID || Math.round(Math.random() * 10000)) + '_' + this.id,
		...
		DisplayParamList: function(param, value, container)
		{
			...
			var
				...
				pSelect = BX.create("SELECT", {
					...
					events: {change: BX.proxy(this.OnChageParams, this)}
				});
						...
						events: {click: function()
							{
								if(param.ROWS > 1)
								{
									container.appendChild(BX.create("TEXTAREA", {
										props: {cols: param.COLS,name: name, value: ''},
										attrs: {'data-bx-property-id' : param.ID, 'data-bx-comp-prop' : true},
										events: {change: BX.proxy(_this.OnChageParams, _this)}
									}));
								}
								else
								{
		...
		DisplayParamCheckbox: function(param, value, container)
		{
			...
			var
				...
				pCheckbox = BX.create("INPUT", {
					...
					attrs: {'data-bx-property-id' : param.ID, 'data-bx-comp-prop' : true},
					events: {click: BX.proxy(this.OnChageParams, this)}
				});
			...
			{
				param.REFRESH = 'Y';
//				BX.bind(pCheckbox, 'click', function()
//				{
//				});
...
//				pPropTsb.__SEF_MODE = pCheckbox.checked;
		...
		DisplayParamFile: function(param, value, container)
		{
			...
			if (param.FD_USE_MEDIALIB)
			{
				...
				if (fdInputBut)
				{
					fdInputBut.onclick = window['FX_FD_' + param.ID];
				}
			...
			else
			{
				container.appendChild(BX.create("INPUT", {
					props: {type: 'button', value: '...'},
					events: {click: window['FX_FD_' + param.ID]}
				}));
			}

			// Result of selecting file
			window['FX_FD_ONRESULT_' + param.ID] = function(filename, filepath)
			{
		...
		OnChageParams: function(e, propertyId)
		{
			...
			if (propertyId && this.paramsIndex[propertyId])
			{
				var
					_this = this,
					ar = this.paramsIndex[propertyId];
		...
		{
			var result = this.params.currentValues || {};
