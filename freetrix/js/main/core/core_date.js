	...
	var dateFormats = {
		...
		x : function() {
			return BX.date.format([
				...
				["-", BX.date.convertFreetrixFormat(BX.message("FORMAT_DATETIME")).replace(/:s$/g, "")],
				["s", "sago"],
				["i", "iago"],
				["today", "today, H:i"],
		...
		X : function() {
			var day = BX.date.format([
				["tommorow", "tommorow"],
				["-", BX.date.convertFreetrixFormat(BX.message("FORMAT_DATE"))],
				["today", "today"],
				["yesterday", "yesterday"],
				["", BX.date.convertFreetrixFormat(BX.message("FORMAT_DATE"))]
			], date, nowDate, isUTC);
...
BX.date.convertFreetrixFormat = function(format)
{
...
BX.calendar.InsertDaysBack = function(input, days)
{
	if (days != '')
	{
		...
		input.value = BX.date.format(BX.date.convertFreetrixFormat(BX.message('FORMAT_DATE')), d, null);
	}
	else
	{
		input.value = '';
...
{
	return BX.date.format(
		BX.date.convertFreetrixFormat(BX.message(bTime ? 'FORMAT_DATETIME' : 'FORMAT_DATE')),
		value,
		null,
		!!bUTC
	);
...
BX.CClockSelector.prototype.Show = function()
{
	if (!this.isReady)
	{
		...
		return BX.ajax.get('/freetrix/tools/clock_selector.php', {start_time: this.params.start_time, clock_id: this.CLOCK_ID, sessid: BX.freetrix_sessid()}, BX.delegate(this.Ready, this));
	}
