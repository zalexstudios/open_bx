...
{
	(new BX.CAdminDialog({
		'content_url': '/freetrix/components/freetrix/desktop/admin_settings.php?lang='+BX.message('LANGUAGE_ID')+'&bxpublic=Y',
		'content_post': 'sessid='+BX.freetrix_sessid()+'&type=desktop&desktop_page=0&action=new&desktop_backurl=/freetrix/admin/',
		'draggable': true,
		'resizable': true,
...
BX.adminPanel.prototype.hideNotify = function(element)
{
	...
	if (BX.type.isDomNode(element) && element.getAttribute('data-ajax') == "Y")
	{
		...
		{
			BX.ajax({
				url: '/freetrix/admin/admin_notify.php',
				method: 'POST',
				dataType: 'json',
				data: {'ID' : notifyId, 'sessid': BX.freetrix_sessid()}
			});
		}
	}
...
BX.adminFav = {
	url: '/freetrix/admin/favorite_act.php',
	lastId: null,
	...
	del: function(id, callback)
	{
		var urlToSend = BX.adminFav.url + "?act=delete&id="+id,
			data = {sessid: BX.freetrix_sessid()};
...
BX.adminTabControl.prototype.ShowWarnings = function(form_name, warnings)
{
	...
	for(var i in warnings)
	{
		...
		img.src = '/freetrix/panel/main/images_old/icon_warn.gif';
...
BX.adminTabControl.prototype.SaveSettings =  function(el)
{
	...
	var options_url = '/freetrix/admin/user_options.php?lang='+BX.message('LANGUAGE_ID')+'&sessid=' + BX.freetrix_sessid();
			...
			for (var i = 0; i < this.node.rows[0].cells.length; i++)
			{
				this.mirror_thead.rows[0].cells[i].appendChild(
					BX.create('SPAN', {
						...
						html: '<img src="/freetrix/images/1.gif" style="height: 0; width: ' + (this.node.rows[0].cells[i].offsetWidth-4) + 'px;">'
					}));
...
BX.AdminFilter = function(filter_id, aRows)
{
	...
	this.DeleteFromBase = function(id)
	{
		...
		var data = {
			'id': id,
			'action': 'del_filter',
			'sessid': phpVars.freetrix_sessid,
			'lang': BX.message("LANGUAGE_ID")
		};
		...
		BX.ajax.post('/freetrix/admin/filter_act.php', data, callback);
	...
	this.SaveOpenTab = function(id)
	{
		...
		BX.ajax.post('/freetrix/admin/filter_act.php', data);
	};
