...
BX.Finder.OpenCompanyDepartment = function(provider, id, department)
{
	...
	{	
		...
		if (BX.Finder.context == 'access')
			var ajaxSendUrl = '/freetrix/tools/access_dialog.php';
		else
		{
		...
		BX.ajax({
			...
			dataType: 'html',
			processData: true,
			data: {'mode': 'ajax', 'action' : 'structure-item', 'provider' : provider, 'item' : id, 'sessid': BX.freetrix_sessid(), 'site_id': BX.message('SITE_ID')||''},
	...
	if (element.value != '')
	{
		BX.Finder.searchTimeout = setTimeout(function() {
			...
			BX.ajax({
				...
				dataType: 'html',
				processData: true,
				data: {'mode': 'ajax', 'action' : 'search', 'provider' : provider, 'search' : element.value, 'sessid': BX.freetrix_sessid(), 'site_id': BX.message('SITE_ID')||''},
				onsuccess: function(data)	{
					if (data == '')
					{
