...
BXFileDialog.prototype =
{
	Open: function(oConfig, UserConfig, dublReq)
	{
		...
		{
			if(BX("FX_file_dialog"))
				this.Close();

			div = document.body.appendChild(document.createElement("DIV"));
			div.id = "FX_file_dialog";
			...
			div.innerHTML =
				...
				'	<tr>'+
				'		<td width="100%" class="title-text" onmousedown="jsFloatDiv.StartDrag(arguments[0], BX(\'FX_file_dialog\'));" id="FX_file_dialog_title">Title</td>'+
				'		<td width="0%"><a id="FX_file_dialog_close" class="close" href="javascript:oBXFileDialog.Close();" onclick="oBXFileDialog.Close(); return false;"></a></td></tr>'+
				'</table>'+
				'</div>'+
				'<div class="content">'+
				'</div>';
		...
		oDialogTitle = BX('FX_editor_dialog_title');
		var ShowDialog = function(innerHTML)
		{
			...
			if (innerHTML)
			{
				if (innerHTML.indexOf('FX_FD_LOAD_OK') == -1)
	...
	{
		return '/freetrix/admin/file_dialog.php?'
	...
	{
		var
			LSS = 'FX_FD_DUBLICATE_ACTION_REQUEST',
