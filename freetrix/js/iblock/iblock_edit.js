...
JCIBlockProperty.prototype.SetPropInfo = function(ID,arProp,formsess)
{
	...
	if (BX.freetrix_sessid() !== formsess)
	{
		return;
	}
...
JCIBlockProperty.prototype.ShowPropertyDialog = function ()
{
	...
	if (!!target && target.hasAttribute('data-propid'))
	{
		...
		(new BX.CAdminDialog({
			'title': this.TITLE,
			'content_url': '/freetrix/admin/iblock_edit_property.php?lang='+this.LANG+'&propedit='+ID+'&bxpublic=Y&receiver='+this.OBJNAME,
...
JCIBlockAccess.prototype.InsertRights = function(obSelected)
{
	...
	{
		BX.ajax.loadJSON(
			'/freetrix/admin/iblock_edit.php'+
			'?ajax=y'+
			'&sessid='+BX.freetrix_sessid()+
