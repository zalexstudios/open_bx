...
if (isset($_REQUEST[$arParams["ACTION_VARIABLE"]]) && isset($_REQUEST[$arParams["PRODUCT_ID_VARIABLE"]]))
{
	...
	{
		if (\Freetrix\Main\Loader::includeModule("sale") && \Freetrix\Main\Loader::includeModule("catalog"))
		{
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	{
		if (!\Freetrix\Main\Loader::includeModule('currency'))
		{
			$arParams['CONVERT_CURRENCY'] = 'N';
			$arParams['CURRENCY_ID'] = '';
		}
	...
	elseif($arResult["ID"] > 0 && $arParams["ADD_SECTIONS_CHAIN"])
	{
		...
		{
			$ipropValues = new \Freetrix\Iblock\InheritedProperty\SectionValues($arParams["IBLOCK_ID"], $arPath["ID"]);
	...
	if ($arResult["ID"] > 0)
	{
		$ipropValues = new \Freetrix\Iblock\InheritedProperty\SectionValues($arResult["IBLOCK_ID"], $arResult["ID"]);
...
$browserTitle = \Freetrix\Main\Type\Collection::firstNotEmpty(
