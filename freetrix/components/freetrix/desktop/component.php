...
if($USER->IsAuthorized() && $arResult["PERMISSION"]>"R" && check_freetrix_sessid())
{
...
foreach($arGadgets as $gadget)
{
	...
	if ($gadget["SECURITY_ONLY"] == true)
	{
		...
		$bShowSecurity = (file_exists($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/security/install/index.php") && $aGlobalOpt['messages']['security'] <> 'N');
		if (!$bShowSecurity)
			continue;
	}
	...
	{
		$aGlobalOpt = CUserOptions::GetOption("global", "settings", array());
		$bShowPerfmon = (file_exists($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/perfmon/install/index.php") && $aGlobalOpt['messages']['perfmon'] <> 'N');
		if (!$bShowPerfmon)
			continue;
	}
	if (
