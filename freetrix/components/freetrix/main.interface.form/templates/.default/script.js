function BxInterfaceForm(name, aTabs)
{
	...
	this.ToggleTabs = function(bSkipSave)
	{
		...
		if(bSkipSave !== true)
			BX.ajax.get('/freetrix/components'+this.vars.component_path+'/settings.php?FORM_ID='+this.name+'&action=expand&expand='+(this.bExpandTabs? 'Y':'N')+'&sessid='+this.vars.sessid);
	}
	...
	this.SetTheme = function(menuItem, theme)
	{
		...
		BX.ajax.get('/freetrix/components'+_this.vars.component_path+'/settings.php?FORM_ID='+this.name+'&GRID_ID='+this.vars.GRID_ID+'&action=settheme&theme='+theme+'&sessid='+this.vars.sessid);
	}
	...
	this.SaveSettings = function()
	{
		...
		BX.ajax.post('/freetrix/components'+_this.vars.component_path+'/settings.php', data, function(){_this.Reload()});
	}

	this.EnableSettings = function(enabled)
	{
		BX.ajax.get('/freetrix/components'+this.vars.component_path+'/settings.php?FORM_ID='+this.name+'&action=enable&enabled='+(enabled? 'Y':'N')+'&sessid='+this.vars.sessid, function(){_this.Reload()});
	}
