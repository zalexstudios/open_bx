function JCTreeSelectControl(arParams)
{
	...
	this.ElementSelect = function()
	{
		if(_this.multiple)
		{
			...
			{
				if (current_selected[i] == this.FX_ID)
				{
					bFound = true;
					break;
				}
			...
			{
				current_selected[current_selected.length] = this.FX_ID;
	...
	this.ShowSection = function (data)
	{
		...
		if (!obSection.FX_LOADED)
		{
			obSection.FX_LOADED = true;
			...
			if (obSectionDiv)
			{
				...
				for (var i = 0; i < arElements.length; i++)
				{
					...
					obUserRow.FX_ID = arElements[i].ID;
				...
				if (_this.multiple)
				{
					...
					{
						if (obUserRow.FX_ID == current_selected[j])
						{
