...
window.LHEPostForm.prototype = {
	...
	makeButton : function (oldb, newb)
	{
		...
		el.style.backgroundImage = 'url(/freetrix/images/1.gif)';
		el.src = '/freetrix/images/1.gif';
		el.style.width = '25px';
...
window.CustomizeLightEditor = function(editorId, params)
{
				...
				Parse: function(sName, sContent, pLEditor)
				{
					sContent = sContent.replace(/\[VIDEO\s*?width=(\d+)\s*?height=(\d+)\s*\]((?:\s|\S)*?)\[\/VIDEO\]/ig, function(str, w, h, src)
					{
						...
						return '<img id="' + pLEditor.SetBxTag(false, {tag: "postvideo", params: {value : src}}) +
							'" src="/freetrix/images/1.gif" class="bxed-video" width=' + w + ' height=' + h + ' title="' + BX.message.Video + ": " + src + '" />';
					});
					return sContent;
				},
				UnParse: function(bxTag, pNode, pLEditor)
	...
	window.LHEDailogs['InputVideo'] = function(pObj)
	{
		...
		return {
			...
			OnSave: function()
			{
				...
				{
					pObj.pLEditor.InsertHTML('<img id="' + pObj.pLEditor.SetBxTag(false, {tag: "postvideo", params: {value : src}}) + '" src="/freetrix/images/1.gif" class="bxed-video" width=' + w + ' height=' + h + ' title="' + BX.message.Video + ": " + src + '" />');
	...
	window.BXfpdSetLinkName = function(name)
	{
		if (BX.SocNetLogDestination.getSelectedCount(name) <= 0)
			BX('bx-destination-tag').innerHTML = BX.message("FX_FPD_LINK_1");
		else
			BX('bx-destination-tag').innerHTML = BX.message("FX_FPD_LINK_2");
	}
	...
	window.BXfpdSelectCallback = function(item, type, search, bUndeleted)
	{
		if(!BX.findChild(BX('feed-add-post-destination-item'), { attr : { 'data-id' : item.id }}, false, false))
		{
			...
			var stl = '';
			if(bUndeleted)
				stl = ' feed-add-post-destination-undelete';

			var el = BX.create("span", { attrs : { 'data-id' : item.id }, props : { className : "feed-add-post-destination feed-add-post-destination-"+type1+stl }, children: [
			...
			if(!bUndeleted)
			{
				el.appendChild(BX.create("span", { props : { 'className' : "feed-add-post-del-but"}, events : {'click' : function(e){BX.SocNetLogDestination.deleteItem(item.id, type, window.BXSocNetLogDestinationFormName);BX.PreventDefault(e)}, 'mouseover' : function(){BX.addClass(this.parentNode, 'feed-add-post-destination-hover')}, 'mouseout' : function(){BX.removeClass(this.parentNode, 'feed-add-post-destination-hover')}}}));
			}
