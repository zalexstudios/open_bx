...
if (CModule::IncludeModule("form"))
{
	...
	if ($del_id > 0 /* && check_freetrix_sessid()*/)
	{
		$GLOBALS['strError'] = '';
	...
	if ($_REQUEST["delete"] && check_freetrix_sessid())
	{
		$ARR_RESULT = $_REQUEST["ARR_RESULT"];
		if (is_array($ARR_RESULT) && count($ARR_RESULT) > 0 && check_freetrix_sessid())
		{
			$GLOBALS['strError'] = '';
	...
	if ($arParams["arFormInfo"] = $dbres->Fetch())
	{
		...
		$arResult["tf_name"] = COption::GetOptionString("main", "cookie_name", "FREETRIX_SM")."_FORM_RESULT_FILTER";
