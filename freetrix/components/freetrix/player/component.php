...
define(FX_FM_MPLAYER,"/freetrix/components/freetrix/player/mediaplayer/player");
...
if ($player_type == 'flv') // FLASH PLAYER
{
	...
	if ($arParams["USE_PLAYLIST"] == 'Y')
	{
		$jwConfig['players'] = array(
			array('type' => 'flash','src' => FX_FM_MPLAYER)
		);
	...
	else
	{
		...
		{
			$jwConfig['flashplayer'] = FX_FM_MPLAYER;
		}
		else
		{
			$jwConfig['players'] = array(
...
if($arParams["USE_PLAYLIST"] == 'Y')
{
	...
	if ($bShowIcon && strlen($path) > 0)
	{
		$playlist_edit_url = $APPLICATION->GetPopupLink(
			array(
				"URL"=> "/freetrix/components/freetrix/player/player_playlist_edit.php?lang=".LANGUAGE_ID.
