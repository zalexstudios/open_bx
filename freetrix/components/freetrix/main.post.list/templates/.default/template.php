...
$APPLICATION->SetAdditionalCSS("/freetrix/components/freetrix/socialnetwork.log.ex/templates/.default/style.css");
$APPLICATION->SetAdditionalCSS("/freetrix/components/freetrix/socialnetwork.blog.blog/templates/.default/style.css");
$APPLICATION->AddHeadScript("/freetrix/components/freetrix/main.post.list/templates/.default/scripts_for_form.js");
if (CModule::IncludeModule("im"))
	$APPLICATION->AddHeadScript("/freetrix/components/freetrix/main.post.list/templates/.default/scripts_for_im.js");

CUtil::InitJSCore(array("date", "fx", "popup"));
		...
		<div id="record-<?=$arParams["ENTITY_XML_ID"]?>-<?=$res["ID"]?>-cover" class="feed-com-block-cover"><?
			...
			if (is_array($res["UF"]))
			{
				...
				foreach ($res["UF"] as $arPostField)
				{
					...
					{
						$GLOBALS["APPLICATION"]->IncludeComponent("freetrix:system.field.view", $arPostField["USER_TYPE"]["USER_TYPE_ID"],
			...
			foreach(array(
				...
				"AUTHOR_AVATAR_IS" => (empty($res["AUTHOR"]["AVATAR"]) ? "N" : "Y"),
				...
				"SHOW_POST_FORM" => $arParams["SHOW_POST_FORM"]
			) as $pat => $rep)
			{
				$result = str_replace("#".$pat."#", $rep, $result);
			}
...
if ($arParams["SHOW_POST_FORM"] == "Y")
{
	?>
		...
		<div class="feed-com-footer" id="record-<?=$arParams["ENTITY_XML_ID"]?>-switcher" onclick="window['UC']['<?=$arParams["ENTITY_XML_ID"]?>'].reply();" <?
			...
			?><a href="javascript:void(0)"  style="outline: none;" hidefocus="true"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a><?
		?></div><?
}
