;(function(window){
	...
	window.FCList = function (params, add) {
		...
		BX.addCustomEvent(window, 'OnUCUserIsWriting', BX.delegate(function(ENTITY_XML_ID, id) {
			if (this.ENTITY_XML_ID == ENTITY_XML_ID) {
				BX.ajax({
					url: '/freetrix/components/freetrix/main.post.list/templates/.default/activity.php',
					method: 'POST',
					dataType: 'json',
					data: {
						AJAX_POST : "Y",
	...
	window.FCList.prototype = {
		...
		quoteCheck : function() {
			...
			if (text != "")
			{
				...
				if (endParent != startParent || !endParent.hasAttribute("id")) {
					text = "";
				} else {
				...
				this.quote.popup = new MPLQuote({
					...
					events : {
						click : BX.delegate(function(e) {
							BX.onCustomEvent(window, "OnUCUserQuote", [this.ENTITY_XML_ID, params['author'], params['text']]);
							this.quote.popup.hide();
						}, this)
					},
					classEvents : {
						onQuoteHide : BX.proxy(function() { this.quote.popup = null; }, this)
					}
				});
			}
		...
		{
			if (!!node)
				BX.onCustomEvent(window, 'OnUCUserReply', [this.ENTITY_XML_ID, node.getAttribute("bx-mpl-author-id"), node.getAttribute("bx-mpl-author-name")]);
			else
				BX.onCustomEvent(window, 'OnUCUserReply', [this.ENTITY_XML_ID]);
		},
		/*
		* @params array array(
			...
			if (!!container)
			{
				if (!!edit)
				{
					container.parentNode.insertBefore(
						BX.create("DIV", {attrs : {id : ("record-" + id.join('-') + '-cover')}, html : ob.HTML}),
						container );
					BX.remove(container);
				}
			}
			...
			if (ajaxSend)
			{
				...
				BX.ajax({
					...
					onsuccess: BX.delegate(function(data){
						...
						var node = BX('record-' + id.join('-') + '-cover');
						BX.findChild(node, {className: 'feed-com-block'}, true, false).style.backgroundColor = '#fbf2c8';
												...
												children : [
													BX.create('INPUT', {
															attrs : {
																id : id + '-input',
																type : "text",
																value : location.protocol + '//' + location.host + urlView} ,
															style : {
	...
	window["fcParseTemplate"] = function(data, params)
	{
		...
		if (!!res && !!data["messageFields"])
		{
			...
			replacement = {
				...
				"AUTHOR_AVATAR" : (!!res["AUTHOR"]["AVATAR"] ? res["AUTHOR"]["AVATAR"] : '/freetrix/images/1.gif'),
				"AUTHOR_URL" : res["AUTHOR"]["URL"],
				"AUTHOR_NAME" : res["AUTHOR"]["NAME"]
			}
			if (!!res["POST_TIMESTAMP"])
			{
				res["POST_TIMESTAMP"] = parseInt(res["POST_TIMESTAMP"]) + parseInt(BX.message('USER_TZ_OFFSET'));
				if (BX.date.format("d F Y", res["POST_TIMESTAMP"]) == BX.date.format("d F Y"))
					replacement["DATE"] = BX.date.format(params["TIME_FORMAT"], res["POST_TIMESTAMP"]);
				else
					replacement["DATE"] = BX.date.format(params["DATE_TIME_FORMAT"], res["POST_TIMESTAMP"]);
			}
			if (!!params["RIGHTS"])
			{
	...
	window["fcPull"] = function(ENTITY_XML_ID, data) {
		BX.ajax({
			url: '/freetrix/components/freetrix/main.post.list/templates/.default/component_epilog.php',
			method: 'POST',
			data: {
				AJAX_POST :  "Y",
	...
	window.markReadComments = function() {
			...
			(new Date().getTime() - window["UC"]["PULLEDScreenData"]["time"] > 3000))
		{
			...
			var commentsReadToCounter = 0,
				size = BX.GetWindowInnerSize(),
				res = [];
			for(i = 0; i < window["UC"]["PULLED"].length; i++)
			{
				var
