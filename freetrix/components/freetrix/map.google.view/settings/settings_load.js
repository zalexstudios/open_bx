var jsGoogleCE = {
	...
	init: function() 
	{
		BX.loadCSS('/freetrix/components/freetrix/map.google.system/templates/.default/style.css');
	...
	{
		if (null == this.FX_PLACEMARK_INDEX)
			return;
		
		var obPoint = this.getPosition();
		
		jsGoogleCE.arData.PLACEMARKS[this.FX_PLACEMARK_INDEX].LON = obPoint.lng();
		jsGoogleCE.arData.PLACEMARKS[this.FX_PLACEMARK_INDEX].LAT = obPoint.lat();
	},
	...
	__openBalloon: function(obPlacemark)
	{
		...
		obPlacemark.infowin.setContent('<textarea onblur="jsGoogleCE.__updatePointView(this, \'blur\')" onkeyup="jsGoogleCE.__updatePointView(this, \'keyup\')" id="FX_PLACEMARK_TEXT_' + obPlacemark.FX_PLACEMARK_INDEX + '">' + BX.util.htmlspecialchars(jsGoogleCE.arData.PLACEMARKS[obPlacemark.FX_PLACEMARK_INDEX].TEXT) + '</textarea>');
		
		obPlacemark.infowin.open(jsGoogleCE.map, obPlacemark);
		
		setTimeout(function() {
			try {
				BX('FX_PLACEMARK_TEXT_' + obPlacemark.FX_PLACEMARK_INDEX).focus();
			} catch(e) {}
		}, 500);
	},
	...
	__createPlaceMark: function(arPlacemark, index)
	{
		...
		obPointView.id = 'FX_PLACEMARK_' + index;
		...
		obPointView.BXPlacemark.FX_PLACEMARK_INDEX = index;
	...
	__deletePoint: function(e)
	{
		...
		jsGoogleCE.arData.PLACEMARKS[obView.BXPlacemark.FX_PLACEMARK_INDEX].DELETED = 1;
	...
	__updatePointView: function(obTextarea, type)
	{
		...
		var obView = document.getElementById('FX_PLACEMARK_' + index);
