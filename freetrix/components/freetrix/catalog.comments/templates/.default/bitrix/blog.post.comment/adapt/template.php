	...
	?><script>window.BX = top.BX;
		<?if($arResult["use_captcha"]===true)
		{
			?>
				...
				BX('captcha').src='/freetrix/tools/captcha.php?captcha_code='+cc;
	...
	else
	{
		if($arResult["is_ajax_post"] != "Y" && $arResult["CanUserComment"])
		{
			...
			<div id="form_comment_" style="display:none;">
				...
				<form method="POST" name="form_comment" id="form_comment" action="<?=$templateFolder."/ajax.php"?>">
			...
			if($arResult["use_captcha"]===true)
			{
				?>
				<div id="captcha_del">
					<img src="/freetrix/tools/captcha.php?captcha_code=<?=$arResult["CaptchaCode"]?>" width="180" height="40" id="captcha" style="display:none;">
					<script>
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-info">
						...
						if(strlen($comment["urlToBlog"])>0)
						{
							...
							$GLOBALS["APPLICATION"]->IncludeComponent("freetrix:main.user.link",
								'',
								array(
					...
					<div class="blog-comment-content">
						...
						if(strlen($comment["urlToShow"])>0)
						{
							...
							<span class="blog-comment-show">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToShow"]."&".freetrix_sessid_get()?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_SHOW")?>">
								<?else:?>
									<a href="<?=$comment["urlToShow"]."&".freetrix_sessid_get()?>" title="<?=GetMessage("BPC_MES_SHOW")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToHide"])>0)
						{
							...
							<span class="blog-comment-show">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToHide"]."&".freetrix_sessid_get()?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?else:?>
									<a href="<?=$comment["urlToHide"]."&".freetrix_sessid_get()?>" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToDelete"])>0)
						{
							...
							<span class="blog-comment-delete">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) deleteComment('<?=$comment["urlToDelete"]."&".freetrix_sessid_get()?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?else:?>
									<a href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".freetrix_sessid_get()?>&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>' title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?endif;?>
