...
if (strlen($arResult["FatalError"]) <= 0)
{
	...
	if ($bNeedGetUser)
	{
		...
		$path = "/user_card_".intval($arParams["ID"] / TAGGED_user_card_size);

		if($arParams['AJAX_CALL'] == 'INFO' || $obCache->StartDataCache($arParams["CACHE_TIME"], $strCacheID, $path))
		{
			if ($arParams['AJAX_CALL'] != 'INFO' && defined("FX_COMP_MANAGED_CACHE"))
			{
			...
			if (strlen($arResult["FatalError"]) <= 0 && $arParams["USE_THUMBNAIL_LIST"] == "Y" && $arParams['AJAX_CALL'] != 'INFO')
			{
				...
				if (!$bThumbnailFound)
					$imageImg = "<img src='/freetrix/components/freetrix/main.user.link/templates/.default/images/nopic_30x30.gif' width='30' height='30' border='0'>";
		...
		if (strlen($arResult["FatalError"]) <= 0 && $arParams['AJAX_CALL'] == 'INFO')
		{
			...
			require($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/include/epilog_after.php");
			die();
	...
	else
	{
		...
		{
			$arImage = CSocNetTools::InitImage($arParams["PERSONAL_PHOTO_FILE"]["ID"], $arParams["THUMBNAIL_LIST_SIZE"], "/freetrix/images/1.gif", 1, $arParams["~HREF"], $canViewProfile);
