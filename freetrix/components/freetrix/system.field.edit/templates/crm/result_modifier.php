...
{
	if(!$CCrmPerms->HavePerm($supportedType, FX_CRM_PERM_NONE, 'READ'))
	{
...
if (in_array('LEAD', $arParams['ENTITY_TYPE'], true))
{
	...
	if(class_exists('Freetrix\\Crm\\Format\\PersonNameFormatter'))
	{
		$nameFormat = Freetrix\Crm\Format\PersonNameFormatter::getFormat();
	}

	$arSelect = array('ID', 'TITLE', 'NAME', 'SECOND_NAME', 'LAST_NAME', 'STATUS_ID');
	...
	while ($arRes = $obRes->Fetch())
	{
		...
		if (!empty($arRes['PHOTO']) && !isset($arFiles[$arRes['PHOTO']]))
		{
			if(intval($arRes['PHOTO']) > 0)
				$arImg = CFile::ResizeImageGet($arRes['PHOTO'], array('width' => 25, 'height' => 25), FX_RESIZE_IMAGE_EXACT);
		}
	...
	while ($arRes = $obRes->Fetch())
	{
		...
		if (!empty($arRes['LOGO']) && !isset($arFiles[$arRes['LOGO']]))
		{
			if(intval($arRes['LOGO']) > 0)
				$arImg = CFile::ResizeImageGet($arRes['LOGO'], array('width' => 25, 'height' => 25), FX_RESIZE_IMAGE_EXACT);
