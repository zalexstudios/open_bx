 ...
 * @param CFreetrixComponentTemplate $this
 */
			...
			if (document.getElementById('<?echo $name_x?>'))
			{
				...
				if (value_<?=$name_x?> > 0)
				{
					if (old_value != value_<?=$name_x?>)
					{
						...
						document.getElementById("hiddenframe<?=$name_x?>").src='/freetrix/admin/get_user.php?ID=' + value_<?=$name_x?>+'&strName=<?=$name_x?>&lang=<? echo LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")?>';
					}
