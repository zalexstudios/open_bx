	...
	<table cellspacing="0" border="0" class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		if ($arParams["SHOW_AVATAR"] != "N")
		{
			...
			<div class="review-avatar"><?
				...
				else: ?><img src="/freetrix/components/freetrix/forum.topic.reviews/templates/.default/images/noavatar.gif" border="0" /><? endif;
			?></div>
		<?
		}
		?>
		...
		if ($arParams["SHOW_RATING"] == "Y")
		{
			...
			<div class="review-rating rating_vote_graphic">
				...
				$APPLICATION->IncludeComponent( "freetrix:rating.vote", $arParams["RATING_TYPE"], $arRatingParams, $component, array("HIDE_ICONS" => "Y"));
				?>
			</div>
		<?
		}
...
<?			if ($arResult["FORUM"]["ALLOW_QUOTE"] == "Y")
			...
			if ($arResult["PANELS"]["DELETE"] == "Y")
			{
				?>
				<span class="separator"></span>
				<a rel="nofollow" bx-href="<?=htmlspecialcharsbx($res["URL"]["~DELETE"])?>" class="reviews-button-small" bx-act="del"><?=GetMessage("F_DELETE")?></a>
<?			}
			if ($arParams["SHOW_RATING"] == "Y")
			{
				?>
