...
?>
	<div id="bx_tv_block_<?=$arResult['PREFIX']?>" style="width: <?=$arParams['WIDTH']?>px;">
		...
		<div id="freetrix_tv_flv_cont_<?= $arResult["PREFIX"]?>" style="display: none">
		<?$APPLICATION->IncludeComponent(
			...
			Array(
				...
				"PLAYER_ID" => "freetrix_tv_flv_".$arResult["PREFIX"]
			),
			$component,
			Array("HIDE_ICONS" => "Y")
		);?>
		...
		<div id="freetrix_tv_wmv_cont_<?= $arResult["PREFIX"]?>" style="display: none">
		<?$APPLICATION->IncludeComponent(
			"freetrix:player",
			"",
			Array(
				...
				"PLAYER_ID" => "freetrix_tv_wmv_".$arResult["PREFIX"]
			),
			$component,
			Array("HIDE_ICONS" => "Y")
		);?>
...
<script>
	...
	jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases = {
		'duration':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_DURATION")?>',
		'title':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_TITLE")?>',
		'description':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_DESCRIPTION")?>',
		'file':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_FILE")?>',
		'download':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_DOWNLOAD")?>',
		'size_mb':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_BXTV_SIZE_MB")?>',
		'play':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_BXTV_PLAY")?>',
		'edit':'<?=GetMessageJS("FREETRIXTV_TEMPLATE_BXTV_EDIT")?>'
	};
	...
	jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].AddPlayerListener(
		...
		function(txt, i, j)
		{
			...
			'<div onmouseover="jsPublicTVCollector.add[<?=$arResult['PREFIX']?>].HoverListItem(this)" onmouseout="jsPublicTVCollector.add[<?=$arResult['PREFIX']?>].HoverListItem(this)" class="normal-tv-item">'
				+'<div class="freetrix-tv-small-image" onclick="jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].PlayFile('+i+','+j+',true,true)">'
				...
				+'</div>'
				+'<div class="freetrix-tv-tree-item-description">'
	...
	if(jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>])
	{
		jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].Init(
			...
			'tv_description_<?=$arResult['PREFIX']?>',
			{
				block_id:
				{
					wmv: 'freetrix_tv_wmv_cont_<?=$arResult['PREFIX']?>',
					flv: 'freetrix_tv_flv_cont_<?=$arResult['PREFIX']?>'
				},
				obj_id:
				{
					wmv: 'freetrix_tv_wmv_<?=$arResult['PREFIX']?>',
					flv: 'freetrix_tv_flv_<?=$arResult['PREFIX']?>_div'
				},
