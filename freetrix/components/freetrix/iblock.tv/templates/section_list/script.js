		...
		StatEvent: function(player, oldstate, newstate)
		{
			...
			)
			{
				...
				for(var file in this.files)
				{
					...
					{
						BX.ajax.post(
							'/freetrix/components/freetrix/iblock.tv/stat.php',
							{id: this.files[file]},
							function(data){}
						);
...
function jsPublicTV()
{
	...
	{
			id:'-roll',
			styles:['freetrix-tv-section-closed','freetrix-tv-section-title']
	};
...
jsPublicTV.prototype.Init = function(arValues, TBID, DBID, arStart)
{
	...
	this.TreeStyleCodes =
	{
		...
		description_end:'</div>',
		tree_image_start:'<div class="freetrix-tv-small-image">',
		tree_image_end:'</div>',
		tree_description_start:'<div class="freetrix-tv-tree-item-description">',
...
jsPublicTV.prototype.GeneratePlayer = function()
{
	...
	if(this.PlayerConfig.width && this.PlayerConfig.height && this.PlayerConfig.obj_id[curItem.Type])
	{
		if(curItem.Type == 'wmv') //WMV
		{
			...
			this.Player.wmv = new jeroenwijering.Player(BX(this.PlayerConfig.obj_id.wmv), '/freetrix/components/freetrix/player/wmvplayer/wmvplayer.xaml',  this.PlayerConfig);
			this.PlayerConfig.height +=4; //height fix
...
jsPublicTV.prototype.BuildBlock = function(i)
{
	...
	if(CurLevel>0)
	{
		...
		block.className = 'freetrix-tv-section-title';
		//block
		var block = ParentBlock.appendChild(document.createElement('DIV'));
		block.id = this.Prefix + 'bx-tv-section-' + i;
		//set style
		block.className = 'freetrix-tv-section-closed';
	}
...
jsPublicTV.prototype.BuildItem = function(i,j)
{
	...
	if(ParentBlock)
	{
		...
		item.className = 'freetrix-tv-tree-item';
		var txt =
...
jsPublicTV.prototype.EffectCreateOverlay = function()
{
	...
	if (!this.EffectOverlay && this.EffectGetOpacityProperty())
	{
		...
		this.EffectOverlay.className = "freetrix-tv-overlay";
		this.EffectOverlay.id = 'freetrix-tv-overlay';
			...
			if(dxShadow > 0)
			{
				...
				if(!img)
				{
					if(BX.browser.IsIE())
					{
			 			...
			 			img.style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='/freetrix/themes/"+phpVars.ADMIN_THEME_ID+"/images/shadow.png',sizingMethod='scale')";
					}
					else
					{
			 			img = document.createElement("IMG");
						img.src = '/freetrix/themes/'+phpVars.ADMIN_THEME_ID+'/images/shadow.png';
					}
