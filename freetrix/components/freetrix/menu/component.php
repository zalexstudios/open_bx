...
* @var CUser $USER
* @var CFreetrixMenuComponent $this
*/
...
if($this->startResultCache(false, false, ($arParams["MENU_CACHE_USE_USERS"] === "Y"? $this->getGenerationCachePath($USER->GetID()): false)))
{
	if(defined("FX_COMP_MANAGED_CACHE"))
		$CACHE_MANAGER->registerTag("freetrix:menu");
	//Read root menu
	...
	else
	{
		...
		{
			//Menu from iblock (freetrix:menu.sections)
...
if($USER->IsAuthorized())
{
	...
	if($bMenuEdit)
	{
		$menu_edit_url = $APPLICATION->GetPopupLink(array(
			"URL"=> "/freetrix/admin/public_menu_edit.php?lang=".LANGUAGE_ID.
		...
		//panel
		$static_var_name = 'FX_TOPPANEL_MENU_EDIT_'.$menuType;
	...
	if ($bMenuAdd)
	{
		...
		$menu_edit_url = $APPLICATION->GetPopupLink(array(
			"URL" => "/freetrix/admin/public_menu_edit.php?new=Y&lang=".LANGUAGE_ID.
				"&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"]).
		...
		//panel
		$static_var_name = 'FX_TOPPANEL_MENU_ADD_'.$newMenuType;
		...
		if (!defined($static_var_name))
		{
			...
			if (!defined('FX_TOPPANEL_MENU_SEPARATOR_INCLUDED'))
			{
				$APPLICATION->AddPanelButtonMenu($buttonID, array('SEPARATOR' => "Y", "SORT" => "150"));
				define('FX_TOPPANEL_MENU_SEPARATOR_INCLUDED', 1);
			}
	...
	if ($bMenuDelete)
	{
		$menu_del_url = "if(confirm('".CUtil::JSEscape(GetMessage('menu_comp_del_conf', array('#MENU_TITLE#' => (isset($arMenuTypes[$menuType]) ? $arMenuTypes[$menuType] : $menuType)))).
			"')) {BX.showWait(); BX.ajax.get('/freetrix/admin/public_menu_edit.php?lang=".LANGUAGE_ID.
			"&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"]).
			"&path=".urlencode($menuDir)."&name=".$menuType."&action=delete&".freetrix_sessid_get()."')}";
		...
		//panel
		$static_var_name = 'FX_TOPPANEL_MENU_DEL_'.$menuType;
		...
		if (!defined($static_var_name))
		{
			...
			if (!defined('FX_TOPPANEL_DEL_MENU_SEPARATOR_INCLUDED'))
			{
				$APPLICATION->AddPanelButtonMenu($buttonID, array('SEPARATOR' => "Y", "SORT" => "250"));
				define('FX_TOPPANEL_DEL_MENU_SEPARATOR_INCLUDED', 1);
			}
			$APPLICATION->AddPanelButtonMenu($buttonID, $aMenuItem);
		}
	}
