window.bFreetrixTabs = false;
arButtons['tabsection'] = ['BXButton',
	{
		id : 'tabsection',
		codeEditorMode : false,
		src : '/freetrix/images/fileman/htmledit2/insert_tabsection.gif',
		name : 'Вставить область закладок',
		handler : function ()
		{
			this.bNotFocus = true;
			this.pMainObj.insertHTML(
				'<img src="/freetrix/images/1.gif" style="background-image: url(/freetrix/images/fileman/htmledit2/tab_section_begin.gif); height: 17px; width: 100%" __bxtagname="begin_tabsection" __bxcontainer="" /><div _moz_editor_bogus_node="on"></div>' +
				'<br />' +
				'<br />' +
				'<img src="/freetrix/images/1.gif" style="background-image: url(/freetrix/images/fileman/htmledit2/tab_section_end.gif); height: 17px; width: 100%" __bxtagname="end_tabsection" __bxcontainer="" /><div _moz_editor_bogus_node="on"></div>'
			);
			window.bFreetrixTabs = true;
		}
	}
];
...
arButtons['tab'] = ['BXButton',
	{
		id : 'tab',
		codeEditorMode : false,
		src : '/freetrix/images/fileman/htmledit2/insert_tab.gif',
...
arButtons['freetrix_note'] = ['BXButton',
	{
		id : 'freetrix_note',
		codeEditorMode : false,
		...
		{
			this.bNotFocus = true;
			this.pMainObj.OpenEditorDialog("freetrix_note", false, 600, {window: window, document: document});
		}
	}
];
...
if (!window.lightMode)
{
	...
	oBXEditorUtils.appendButton('freetrix_note', arButtons['freetrix_note'], 'standart');
}
else
{
	...
	arGlobalToolbar = arGlobalToolbar.slice(0, bxi).concat([arButtons['tabsection'], arButtons['tab'], arButtons['freetrix_note']], arGlobalToolbar.slice(bxi + 1));
}

arEditorFastDialogs['freetrix_note'] = function(pObj)
{
	...
	'</tr>' +
	'<tr valign="top">' +
		'<td align="right" valign="middle" style="height:40px"><input type="button" id="bx_freetrix_note_save" value="' + FX_MESS.TBSave + '"></td>' +
		'<td align="left" valign="middle" style="height:40px"><input type="button" id="bx_freetrix_note_close" value="' + FX_MESS.TBCancel + '"></td>' +
	'</tr>' +
'</table>';
	...
	var OnSave = function(t)
	{
		...
		pObj.pMainObj.insertHTML('<img src="/images/icons/bxnote.gif" __bxtagname="freetrix_note" __bxcontainer="'+bxhtmlspecialchars(BXSerialize({groups : groups, audience : audience, questions : questions, we_want : we_want, seo : seo, comment : comment}))+'" /><div _moz_editor_bogus_node="on"></div>');
	...
	return {
		...
		OnLoad: function()
		{
			...
			tn.focus();
			var bs = document.getElementById("bx_freetrix_note_save");
			bs.onclick = OnSave;
			document.getElementById("bx_freetrix_note_close").onclick = OnClose;
		}
	};
}
...
arEditorFastDialogs['tab'] = function(pObj)
{
	...
	'</tr>' +
	'<tr valign="top">' +
		'<td align="right" valign="middle" style="height:40px"><input type="button" id="bx_tabsection_save" value="' + FX_MESS.TBSave + '"></td>' +
		'<td align="left" valign="middle" style="height:40px"><input type="button" id="bx_tabsection_close" value="' + FX_MESS.TBCancel + '"></td>' +
	'</tr>' +
'</table>';
	var OnClose = function(){pObj.Close();};
	var OnSave = function(t)
	{
		...
		BXSelectRange(oPrevRange, pObj.pMainObj.pEditorDocument, pObj.pMainObj.pEditorWindow);
		pObj.pMainObj.insertHTML('<img src="/freetrix/images/1.gif" style="background-image: url(/freetrix/images/fileman/htmledit2/tab.gif); height: 20px; width: 100%" __bxtagname="tab" __bxcontainer="'+bxhtmlspecialchars(BXSerialize({name : name, id : id}))+'" /><div _moz_editor_bogus_node="on"></div>');
...
function FreetrixRU_ContentParser(str)
{
	window.bFreetrixTabs = false;
	str = str.replace(/<tabsection>/ig, function(str){
		window.bFreetrixTabs = true;
		return '<img src="/freetrix/images/1.gif" style="background-image: url(/freetrix/images/fileman/htmledit2/tab_section_begin.gif); height: 17px; width: 100%; display:block;" __bxtagname="begin_tabsection" __bxcontainer="" /><div _moz_editor_bogus_node="on"></div>';
	});
	str = str.replace(/<\/tabsection>/ig, function(str)
		{
			return '<img src="/freetrix/images/1.gif" style="background-image: url(/freetrix/images/fileman/htmledit2/tab_section_end.gif); height: 17px; width: 100%" __bxtagname="end_tabsection" __bxcontainer="" /><div _moz_editor_bogus_node="on"></div>';
		}
	);
	str = str.replace(/<tab\s{1}(?:\s|\S)*?>/ig, function(str, b1)
		{
			...
			return '<img src="/freetrix/images/1.gif" style="background-image: url(/freetrix/images/fileman/htmledit2/tab.gif); height: 20px; width: 100%" __bxtagname="tab" __bxcontainer="'+bxhtmlspecialchars(BXSerialize({name : name, id : id}))+'" /><div _moz_editor_bogus_node="on"></div>';
		}
	);

	str = str.replace(/<\?\$APPLICATION->IncludeComponent\(\s*?("|')\S*?freetrix\.comment\1(\S|\s)*?\?>/ig, function(str)
		{
			var groups = '';
			...
			return '<img src="/images/icons/bxnote.gif" __bxtagname="freetrix_note" __bxcontainer="'+bxhtmlspecialchars(BXSerialize({groups : groups, audience : audience, questions : questions, we_want : we_want, seo : seo, comment : comment}))+'" /><div _moz_editor_bogus_node="on"></div>';
		}
	);

	return str;
}
oBXEditorUtils.addContentParser(FreetrixRU_ContentParser);

function FreetrixRU_UnParser(node)
{
	...
	else if(node.arAttributes["__bxtagname"] == 'freetrix_note')
	{
		var par = BXUnSerialize(node.arAttributes["__bxcontainer"]);
...
oBXEditorUtils.addUnParser(FreetrixRU_UnParser);

pPropertybarHandlers['freetrix_note'] = function (bNew, pTaskbar, pElement)
{
	...
	if(bNew)
	{
		...
		if(arBarHandlersCache['freetrix_note'])
		{
			tProp = arBarHandlersCache['freetrix_note'][0];
			pTaskbar.arElements = arBarHandlersCache['freetrix_note'][1];
		}
		else
		{
			...
			arBarHandlersCache['freetrix_note'] = [tProp, pTaskbar.arElements];
		}
