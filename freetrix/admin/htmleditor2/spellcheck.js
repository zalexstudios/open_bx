...
function BXSpellChecker(pMainObj,BXLang,usePspell,useCustomSpell)
{
	...
	//Skip
	BX("FX_dialog_butSkip").onclick = function()
	{
		mObj.pMainObj.SetFocus();
		mObj.skipWord();
	};
	BX("FX_dialog_suggestionsBox").onclick = function()
	{
		mObj.changeReplacementValue();
	};
	BX("FX_dialog_butSkipAll").onclick = function()
	{
		mObj.pMainObj.SetFocus();
		mObj.findSimilarWords('skip');
	};
	BX("FX_dialog_butReplace").onclick = function()
	{
		mObj.pMainObj.SetFocus();
		mObj.replaceWord();
		mObj.skipWord();
	};
	BX("FX_dialog_butReplaceAll").onclick = function()
	{
		mObj.pMainObj.SetFocus();
		mObj.findSimilarWords('replace');
	};
	BX("FX_dialog_butAdd").onclick = function()
	{
		mObj.pMainObj.SetFocus();
		mObj.addWord();
	};
...
BXSpellChecker.prototype.spellCheck = function()
{
	if (this.wordList.length > 0)
	{
		...
		var url = "/freetrix/admin/fileman_spell_checking.php?BXLang="+this.BXLang+"&useCustomSpell="+this.useCustomSpell+"&usePspell="+this.usePspell;
...
BXSpellChecker.prototype.showResult = function()
{
	...
	{
		alert(FX_MESS.DIC_ISNT_INSTALED);
		BXClearSelection(pObj.pMainObj.pEditorDocument,pObj.pMainObj.pEditorWindow);
		window.oBXEditorDialog.Close();
		return;
	}
	var okMessWin = BX("FX_dialog_okMessWin");
	var waitWin = BX("FX_dialog_waitWin");
	var spellResultWin = BX("FX_dialog_spellResultWin");
...
{
	this.highlightWord();
	var wordBox = BX("FX_dialog_wordBox");
	var suggestionsBox = BX("FX_dialog_suggestionsBox");
...
BXSpellChecker.prototype.addWord = function()
{
	...
	var url = "/freetrix/admin/fileman_spell_addWord.php?BXLang="+this.BXLang+"&useCustomSpell="+this.useCustomSpell+"&usePspell="+this.usePspell;
...
function SpellCheck_MS(root)
{
	...
	Word.Quit(0);
	alert(FX_MESS.SpellCheckComplete);
	return true;
};
