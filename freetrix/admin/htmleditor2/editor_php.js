var
	one_gif_src = '/freetrix/images/1.gif',
	image_path = '/freetrix/images/fileman/htmledit2',
	c2wait_path = image_path + '/c2waiter.gif',
	global_iconkit_path = image_path + '/_global_iconkit.gif',
	settings_page_path = '/freetrix/admin/fileman_manage_settings.php?sessid=' + BX.freetrix_sessid(),
	editor_action_path = '/freetrix/admin/fileman_editor_action.php?sessid=' + BX.freetrix_sessid(),
	editor_dialog_path = '/freetrix/admin/fileman_editor_dialog.php',
	flash_preview_path = '/freetrix/admin/fileman_flash_preview.php',
	manage_snippets_path = '/freetrix/admin/fileman_manage_snippets.php?lang=' + window.BXLang + '&site=' + window.BXSite + '&sessid=' + BX.freetrix_sessid(),
	to_template_path = window.FX_PERSONAL_ROOT + "/templates/",
	dxShadowImgPath = '';
...
BXHTMLEditor.prototype.LoadComponents2 = function(oCallBack)
{
	...
	lc.Send('/freetrix/admin/fileman_load_components2.php?lang='+BXLang+'&site='+BXSite+'&load_tree=Y');
};
...
BXHTMLEditor.prototype.OnLoad_ex = function()
{
	...
	if (this.limit_php_access)
	{
		...
		oBXEditorUtils.addPropertyBarHandler('php_disabled', function(bNew, pTaskbar, pElement)
		{
			...
			pTaskbar.pCellProps.appendChild(BX.create("SPAN", {text: FX_MESS.LPA_WARNING, style: {padding: '10px'}}));
		});
	}
};
...
BXHTMLEditor.prototype.SystemParse_ex = function(sContent)
{
	...
	if (this.limit_php_access)
	{
		...
		sContent = sContent.replace(/#PHP(\d{4})#/ig, function(s, s1){
			return "<img src=\"/freetrix/images/fileman/htmledit2/php.gif\" id=\"" + _this.SetBxTag(false, {tag: 'php_disabled', params: {value: s1}}) + "\" border=\"0\"/>";});
	}
	...
	if(arScripts.length > 0)
	{
		...
		for(i = 0; i < arScripts.length; i++)
		{
			...
			else if(!this.pMainObj.limit_php_access || (this.pMainObj.limit_php_access && !this.pMainObj.pComponent2Taskbar))
			{
				...
				newstr += str.substr(plast, arScript[0] - plast) + '<img id="' + id + '" src="/freetrix/images/fileman/htmledit2/php.gif" border="0"/>';
			}
			else
			{
				if (window.BS_MESS)
