...
function BXHTMLEditor(name, start_func)
{
	...
	this.arUndoBuffer = [];
	this.SessionLostStr = 'FX_EDITOR_ERROR_SESSION_EXPIRED';
	this.iUndoPos = -1;
...
BXHTMLEditor.prototype.OnPaste = function (e)
{
	...
	if (AutoDetectWordContent)
	{
		...
		{
			if (confirm(FX_MESS.MaybeTextFromWord))
			{
				this.bNotFocus = true;
...
BXHTMLEditor.prototype.WrapSelectionWith = function (tagName, arAttributes)
{
	...
	this.executeCommand("FontName", "freetrixtemp");
	try{this.pEditorDocument.execCommand("styleWithCSS", false, false);}catch(e){}
	...
	{
		if (arTags[i].getAttribute('face') != 'freetrixtemp')
			continue;
...
BXHTMLEditor.prototype.OnSpellCheck = function()
{
	...
	if (!alreadyCheck)
	{
		...
		{
			alert(FX_MESS.SpellCheckNotInstalled);
		}
	}
};
...
{
	return BX.ajax.post(settings_page_path + '&target=unset&edname=' + this.name, {}, function(){alert(FX_MESS.RestoreSettingsMess);});
};
...
BXHTMLEditor.prototype.AuthFailureHandler = function(name, arAuthResult)
{
	...
	var authDialog = new BX.CAuthDialog({
		content_url: '/freetrix/admin/fileman_editor_dialog.php',
