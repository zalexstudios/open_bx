...
function BXComponents2Taskbar()
{
	...
	BXComponents2Taskbar.prototype.FetchArray = function (clear_cache)
	{
		...
		if (window.arComp2Elements)
			...
			CHttpRequest.Send('/freetrix/admin/fileman_load_components2.php?lang='+BXLang+'&site='+BXSite+'&load_tree=Y'+(clear_cache === true ? '&clear_comp2_cache=Y' : ''));
	}
	...
	BXComponents2Taskbar.prototype.ShowComponentParam = function(P)
	{
		...
		if (!arPropertyParams.PARENT || !arPropertyParams.group)
		{
			...
			arPropertyParams.group_title = FX_MESS.ADD_INSERT;
		}
		...
		case "LIST":
			...
			if(arPropertyParams.ADDITIONAL_VALUES != 'N')
			{
				...
				oOption.text = (isYes(arPropertyParams.MULTIPLE) ? FX_MESS.TPropCompNS : FX_MESS.TPropCompOth)+' ->';
			...
			else
			{
				...
				if (arPropertyParams.TYPE == 'FILE')
				{
					...
					if (arPropertyParams.FD_USE_MEDIALIB && parseInt(arPropertyParams.FD_USE_MEDIALIB) > 0)
					{
						...
						if (xCell)
							xCell.onclick = window['FX_FD_' + propertyID];
					}
					else
					{
					...
					if (isYes(arPropertyParams.REFRESH))
					{
						window['FX_FD_ONRESULT_' + propertyID] = function(filename, filepath)
						{
	...
	BXComponents2Taskbar.prototype.BXShowComponent2Panel = function (_bNew, _pTaskbar, _pElement)
	{
		...
		// Component title
		BX.adjust(tCompTitle.insertRow(-1).insertCell(-1), {props: {className: "titlecell"}, html: "<span class='title'>" + compTitle + "  (" + arAllProps.name + ")</span><br /><span class='description'>" + (bComplex ? FX_MESS.COMPLEX_COMPONENT : "") + compDesc+"</span>"});
		...
		if (temlLen > 0)
		{
			var pParDatacell = oTaskbar.GetPropGroupDataCell('templateParams', FX_MESS.COMPONENT_TEMPLATE, _pTaskbar.pCellProps, [arAllProps.name]);
			...
			BX.adjust(row.insertCell(-1), {props: {className: 'bxtaskbarprop-label'}, html: '<label for="__bx_comp2templ_select">' + FX_MESS.COMPONENT_TEMPLATE + ':</label>'});
			...
			for (j = 0; j < temlLen; j++)
			{
				...
				{
					site_template = ' ('+FX_MESS.BUILD_IN_TEMPLTE+')';
				}
		...
		if (arProps.length == 0 && temlLen == 0)
			_pTaskbar.pCellProps.appendChild(BX.create('DIV', {style: {margin : "5px 0 0 22px"}, text: FX_MESS.CompNoParams}));
	}
	...
	BXComponents2Taskbar.prototype.LoadComp2Params = function (arProps, calbackFunc, calbackObj, calbackParams, method, data)
	{
		...
		var url = '/freetrix/admin/fileman_load_comp2_params.php?lang=' + BXLang + '&site=' + BXSite + '&cname=' + elementName+'&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : '')+"&loadhelp="+loadHelp+'&tname=' + templName + '&bxsender=fileman_html_editor&bxeditor=' + this.pMainObj.name;
	...
	BXComponents2Taskbar.prototype.loadTemplateParams = function(componentName, templateName, calbackFunc, calbackObj, calbackParams, method, data)
	{
		...
		if (method == 'POST' && data)
			_CHttpRequest.Post('/freetrix/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite + '&cname='+componentName+'&tname='+templateName+'&mode=params&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : ''),data);
		else
			_CHttpRequest.Send('/freetrix/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&cname='+componentName+'&tname='+templateName+'&mode=params&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : ''));
	}
	...
	BXComponents2Taskbar.prototype.OnElementClick = function (oEl, arEl)
	{
		...
		cell.innerHTML = "<SPAN class='title'>"+compTitle+"  ("+compName+")</SPAN><BR /><SPAN class='description'>"+(bComplex ? FX_MESS.COMPLEX_COMPONENT : "")+compDesc+"</SPAN>";
...
C2Parser.prototype =
{
	...
	GetRenderedContent : function(P)
	{
		...
		var
			url = '/freetrix/admin/fileman_comp2_render.php?sessid=' + BX.freetrix_sessid() + '&site=' + BXSite,
		...
		var calbackFunc = function(result)
		{
			...
			{
				if (result.length <= 0)
					result  = '#FX_RENDERED_COMPONENT##FX_RENDERED_COMPONENT#';
				...
				if(P.bAllContent)
				{
					...
					result.replace(/(#FX_RENDERED_COMPONENT_(\d+?)#)((?:\S|\s)*?)\1/ig, function(s, b1, b2, b3){_arC2RenderedContent.push(b3); return '';});
				...
				{
					if (result.indexOf('#FX_RENDERED_COMPONENT#') != -1)
					{
						s = result.replace(/(#FX_RENDERED_COMPONENT#)((?:\S|\s)*?)\1/ig, "$2");
						s = s.replace("#FX_RENDERED_COMPONENT#", "");
					}
	...
	RenderComponent : function(id, source, bReRender, repEl)
	{
		...
		{
			if (bReRender)
				source = FX_MESS.Comp2EmptyResult;
			else
		...
		else
		{
			...
			pMoveIcon.title = FX_MESS.MoveComponent;

			var c1 = r.insertCell(-1); // title
			c1.className = 'bxc2-block-title';
			c1.innerHTML = FX_MESS.Comp2Name + ': ' + title;
			...
			c2.appendChild(this.pMainObj.CreateEditorElement("IMG", {src: one_gif_src, className: 'bxc2-block-icon bxc2-flip', title: FX_MESS.FlipComponent}));

			c2.appendChild(this.pMainObj.CreateEditorElement("IMG", {src: one_gif_src, className: 'bxc2-block-icon bxc2-del', title: FX_MESS.DelComponent}));
	...
	FlipComponent : function(pEl)
	{
		...
		if (pContentBlock.style.display == 'none')
		{
			...
			if (pBut)
			{
				...
				pBut.title = FX_MESS.FlipComponent;
			}
		}
		else
		{
			...
			if (pBut)
			{
				...
				pBut.title = FX_MESS.FlopComponent;
			}
		}
	},
	...
	if (_oFunc.name.toUpperCase() == '$APPLICATION->INCLUDECOMPONENT')
	{
		...
		//try{
			var
				...
				icon = (comProps.icon) ? comProps.icon : '/freetrix/images/fileman/htmledit2/component.gif',
...
function loadComp2TemplateLists(siteTemplate,calbackFunc, calbackParams,method,data)
{
	...
	if (method == 'POST' && data)
		_CHttpRequest.Post('/freetrix/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&mode=list&stid='+siteTemplate,data);
	else
		_CHttpRequest.Send('/freetrix/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&mode=list&stid='+siteTemplate);
}
...
oBXEditorUtils.addTaskBar('BXComponents2Taskbar', 2, FX_MESS.CompTBTitle, [], 10);