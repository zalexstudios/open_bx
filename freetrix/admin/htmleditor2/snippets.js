...
function BXSnippetsTaskbar()
{
	...
	BXSnippetsTaskbar.prototype.BuildList = function (__arElements)
	{
		...
		for (var key in __arElements)
		{
			...
			oEl.tagname = 'snippet';
			oEl.isGroup = false;
			oEl.icon = '/freetrix/images/fileman/htmledit2/snippet.gif';
	...
	BXSnippetsTaskbar.prototype.AddSnippet_button = function()
	{
		...
		oDiv.appendChild(BX.create("A", {props: {href: 'javascript:void("")'}, text: FX_MESS.AddSnippet, events: {click: function(){oTaskbar.addSnippet();	return false;}}}));
	};
	...
	BXSnippetsTaskbar.prototype.OnElementClick = function (oEl, arEl)
	{
		...
		if (thumb)
		{
			...
			var thumb_src = FX_PERSONAL_ROOT + "/templates/"+arSnippets[key].template+"/snippets/images/"+(arEl.path == '' ? '' : (arEl.path.replace(',', '/')+'/'))+thumb;
	...
	BXSnippetsTaskbar.prototype.delSnippet = function(oEl, elNode)
	{
		...
		if (confirm(FX_MESS.DEL_SNIPPET_CONFIRM+' "'+oEl.title+'"?'))
		{
...
oBXEditorUtils.addTaskBar('BXSnippetsTaskbar', 2, FX_MESS.SnippetsTB, [], 20);
