...
SetValues: function (values)
{
	...
	if (this.bAdminConfigure && false)
	{
		...
		var pConf = c.appendChild(BX.create("A", {props: {className: "bx-list-conf-link", title: FX_MESS.ListConfigTitle, href: "javascript:void(0);"}, text: FX_MESS.ListConfig}, BXPopupWindow.pDocument));
...
BXStyleList.prototype.FillList = function()
{
	...
	//"clear style" item
	this.CreateListRow('', FX_MESS.DeleteStyleOpt, {value: '', name: FX_MESS.DeleteStyleOptTitle});
...
BXEdColorPicker.prototype = {
	...
	Create: function ()
	{
		...
		var defBut = cell.appendChild(BX.create("SPAN", {props: {className: 'bx-colpic-def-but'}, text: FX_MESS.CPickDef}));
...
Create: function ()
{
	...
	row = tbl.insertRow(-1);
	cell = BX.adjust(row.insertCell(-1), {props: {className: 'bx-alpic-default', colSpan: 3}, html: '<nobr>' + FX_MESS.TAlignDef + '</nobr>'});
...
BXDialog.prototype = {
	_Create: function()
	{
		...
		var
			addUrl = (this.params.PHPGetParams ? this.params.PHPGetParams : '') + '&mode=public' + '&sessid=' + BX.freetrix_sessid() + (this.not_use_default ? '&not_use_default=Y' : ''),
			handler = this.handler ? '/freetrix/admin/' + this.handler : editor_dialog_path,
