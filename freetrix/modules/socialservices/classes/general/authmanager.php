...
class CSocServAuthManager
{
	...
	{
		if($service_id === 'Freetrix24OAuth')
		{
			CSocServFreetrixOAuth::gadgetAuthorize();
		}
	...
	public static function SetUniqueKey()
	{
		if(!isset($_SESSION["UNIQUE_KEY"]))
			$_SESSION["UNIQUE_KEY"] = md5(freetrix_sessid_get().uniqid(rand(), true));
	}
	...
	public static function PostIntoBuzzAsBlog($userTwit, $arSiteId=array(), $userLogin = '')
	{
		...
		if(!CModule::IncludeModule("blog") || !CModule::IncludeModule("socialnetwork"))
			return;
		$arParams = array();
		if((IsModuleInstalled('freetrix24') && defined('BX24_HOST_NAME')) && $userLogin != '')
		{
		...
		if(isset($userTwit['text']))
		{
			...
			if(!$bError)
			{
				preg_match_all("/\[user\s*=\s*([^\]]*)\](.+?)\[\/user\]/ies".FX_UTF_PCRE_MODIFIER, $userTwit['text'], $arMention);
