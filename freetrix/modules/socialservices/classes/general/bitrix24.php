...
class CSocServFreetrixOAuth extends CSocServAuth
{
	const ID = "Freetrix24OAuth";

	/** @var CFreetrixOAuthInterface null  */
	...
	function __construct($appID, $appSecret, $portalURI, $redirectURI, $userId = null)
	{
		...
		$this->userId = $userId;
		$this->entityOAuth = new CFreetrixOAuthInterface($this->appID, $this->appSecret, $this->portalURI);
	}
	...
	public function getStorageToken()
	{
		...
		if($userId > 0)
		{
			$dbSocservUser = CSocServAuthDB::GetList(array(), array('USER_ID' => $userId, 'XML_ID' => $this->appID, "EXTERNAL_AUTH_ID" => "Freetrix24OAuth", 'PERSONAL_WWW' => $this->portalURI), false, false, array("OATOKEN", "REFRESH_TOKEN", "OATOKEN_EXPIRES"));
	...
	public function Authorize()
	{
		...
		{
			$redirect_uri = CSocServUtil::ServerName().'/freetrix/tools/oauth/freetrix24.php';
			$userId = intval($_REQUEST['uid']);
			$appID = trim(COption::GetOptionString("socialservices", "freetrix24_gadget_appid", ''));
			$appSecret = trim(COption::GetOptionString("socialservices", "freetrix24_gadget_appsecret", ''));
			$portalURI = $_REQUEST['domain'];
			if(strpos($portalURI, "http://") === false && strpos($portalURI, "https://") === false)
				$portalURI = "https://".$portalURI;
			$gAuth = new CFreetrixOAuthInterface($appID, $appSecret, $portalURI, $_REQUEST["code"]);
		...
		$url = CSocServUtil::ServerName().FX_ROOT;
		$mode = 'opener';
	...
	public function gadgetAuthorize()
	{
		...
		{
			$userId = intval($_REQUEST['uid']);
			CUserOptions::SetOption('socialservices', 'freetrix24_task_planer_gadget_code', $_REQUEST["code"], false, $userId);
		}
		$url = CSocServUtil::ServerName().FX_ROOT;
		$mode = 'opener';
		$url = CUtil::JSEscape($url);
...
class CFreetrixOAuthInterface
{
	...
	{
		$dbSocUser = CSocServAuthDB::GetList(array(), array('XML_ID' => $this->appID, 'PERSONAL_WWW' => $this->portalURI, 'EXTERNAL_AUTH_ID' => "Freetrix24OAuth"), false, false, array("ID"));
		...
		if($GLOBALS["USER"]->IsAuthorized() && $GLOBALS["USER"]->GetID())
		{
			$arFields = array(
				...
				'XML_ID' => $this->appID,
				'EXTERNAL_AUTH_ID' => "Freetrix24OAuth",
	...
	public function getNewAccessToken($refreshToken, $userId = 0, $save = false, $scope = array())
	{
		...
		if(isset($arResult["access_token"]) && $arResult["access_token"] <> '')
		{
			...
			{
				CUserOptions::SetOption('socialservices', 'freetrix24_task_planer_gadget_token', $this->access_token, false, $userId);
				CUserOptions::SetOption('socialservices', 'freetrix24_task_planer_gadget_token_expire', $this->accessTokenExpires + time(), false, $userId);
				CUserOptions::SetOption('socialservices', 'freetrix24_task_planer_gadget_refresh_token', $this->refresh_token, false, $userId);
			}
			$this->saveDataDB();
			return true;
		}
...
class CFreetrixPHPAppTransport
{
