...
class COpenIDClient
{
	...
	function GetRedirectUrl($identity, $return_to=false)
	{
		...
		$_SESSION['FX_OPENID_IDENTITY'] = $identity;
		...
		if ($arOpenidServerTags = $this->GetOpenIDServerTags($identity))
		{
			...
			$_SESSION['FX_OPENID_RETURN_TO'] = $return_to;
			return $url;
		}
		return false;
	}
	...
	function Validate()
	{
		if(CSocServAuthManager::CheckUniqueKey())
		{
			if ($arOpenidServerTags = $this->GetOpenIDServerTags($_GET['openid_identity']))
			{
				...
				if(isset($_SESSION['FX_OPENID_RETURN_TO']))
				{
					$arParams['openid.return_to'] = $_SESSION['FX_OPENID_RETURN_TO'];
					unset($_SESSION['FX_OPENID_RETURN_TO']);
				}

				$str = CHTTP::sPost($arOpenidServerTags['server'], $arParams, true);

				if (preg_match('/is_valid\s*\:\s*/' . FX_UTF_PCRE_MODIFIER, $str))
				{
					return array(
	...
	function Authorize()
	{
		...
		if ($arOpenID = $this->Validate())
		{
			...
			{
				$fullname = (defined("FX_UTF")? $_GET['openid_sreg_fullname'] : CharsetConverter::ConvertCharset($_GET['openid_sreg_fullname'], 'UTF-8', LANG_CHARSET));
			...
			if (array_key_exists('FX_OPENID_IDENTITY', $_SESSION))
				$arFields['LOGIN'] = $_SESSION['FX_OPENID_IDENTITY'];
			else
