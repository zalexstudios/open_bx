	...
	if(window.lastSeoResult)
	{
		...
		else
		{
			var wnd = new BX.CAdminDialog({
				content_url: "/freetrix/admin/seo_search_google_detail.php?lang="+BX.message('LANGUAGE_ID')+"&domain="+BX.util.urlencode(domain)+"&bxpublic=Y"
			});
			wnd.Show();
		}
	}
...
function bindDomain(domain, dir, node)
{
	...
	BX.ajax.loadJSON('/freetrix/tools/seo_google.php?action=site_add&domain='+BX.util.urlencode(domain)+'&dir='+BX.util.urlencode(dir)+'&sessid=' + BX.freetrix_sessid(), updateCallback);
}

function verifyDomain(domain, dir, node)
{
	node.innerHTML = '<?=CUtil::JSEscape(Loc::getMessage('SEO_LOADING'))?>';
	BX.ajax.loadJSON('/freetrix/tools/seo_google.php?action=site_verify&domain='+BX.util.urlencode(domain)+'&dir='+BX.util.urlencode(dir)+'&sessid=' + BX.freetrix_sessid(), updateCallback);
}

function updateInfo()
{
	BX.ajax.loadJSON('/freetrix/tools/seo_google.php?action=sites_feed&sessid=' + BX.freetrix_sessid(), updateCallback);
}
