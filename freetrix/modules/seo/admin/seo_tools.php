...
require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/img.php");
...
?>
	<tr>
		<td width="50%" align="right"><?echo htmlspecialcharsbx($arSearchers[$key]['NAME'])?>:&nbsp;&nbsp;</td>
...
?>
	<tr>
		<td colspan="2"><img src="/freetrix/admin/searcher_graph.php?&lang=<?echo LANGUAGE_ID?>&find_date1_DAYS_TO_BACK=90<?foreach ($arSearchers as $key => $ar) echo '&find_searchers[]='.$key;?>&mode=list&find_summa=N&width=576&height=300" border="0" width="576" height="300" border="0" /><br /><br />
...
?>
			<tr>
				<td><img src="/freetrix/admin/graph_legend.php?color=<?=$color?>" width="45" height="2"></td>
				<td nowrap="nowrap">[<a href="/freetrix/admin/searcher_list.php?lang=<?=LANGUAGE_ID?>&amp;find_id=<?=$keyL?>&amp;set_filter=Y"><?=$keyL?></a>]&nbsp;<a  href="/freetrix/admin/searcher_dynamic_list.php?lang=<?=LANGUAGE_ID?>&amp;find_searcher_id=<?=$keyL?>&amp;find_date1=<?echo $arFilter["DATE1"]?>&amp;find_date2=<?=$arFilter["DATE2"]?>&amp;set_filter=Y"><?=$arrL["NAME"]?></a></td>
			</tr>
<?
					endforeach;
?>
...
?>
		<tr>
			<td align="left" width="30%"><a href="javascript:void(0)" onclick="BXToggle('bx_phrase_ex_<? echo $arData['ID']?>'); return false;" title="<?echo GetMessage('SEO_PAGE_REFERERS_SEO_PAGE_PHRASES_ALT')?>"><?echo htmlspecialcharsbx($word)?></td>
			<td width="70%">
...
?>
				<tr>
					<td style="text-align: left !important;"><? echo htmlspecialcharsbx($arSearcherData['SEARCHER_NAME']);?></td>
...
?>
		...
		<td colspan="2" align="right"><a href="/freetrix/admin/phrase_list.php?lang=<?echo LANGUAGE_ID?>"><?echo GetMessage('SEO_PAGE_GOTO_CP')?></a></td>
	</tr>
</table>
<?
				}
...
?>
				<tr>
					<td style="text-align: left !important;"><a href="<?echo $url?>"><? echo htmlspecialcharsbx(TruncateText($url, 100));?></td>
...
?>
	...
	<td colspan="2" align="right"><a href="/freetrix/admin/referer_list.php?lang=<?echo LANGUAGE_ID?>"><?echo GetMessage('SEO_PAGE_GOTO_CP')?></a></td>
</tr></table>
<?
			}
			else
...
//HTML output
$aTabs = array(
	array("DIV" => "seo_edit1", "TAB" => GetMessage('SEO_TOOLS_TAB_PAGE'), "ICON" => "main_settings", "TITLE" => GetMessage('SEO_TOOLS_TAB_PAGE_TITLE').' '.htmlspecialcharsbx($back_url, ENT_QUOTES)),
...
?>
<form name="seo_form" method="POST" action="/freetrix/admin/public_seo_tools.php" enctype="multipart/form-data">
...
window.BXCallUpdateKeywordsStats = function(keywords)
{
	...
	BX.ajax({
		url: '/freetrix/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=set_keywords_stats&sessid=' + BX.freetrix_sessid(),
...
window.BXCallPageStats = function()
{
	...
	BX.ajax({
		url: '/freetrix/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&first=Y&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=set_stats&sessid=' + BX.freetrix_sessid(),
		data: 'keywords=' + BX.util.urlencode(keywords),
		method: 'POST',
		dataType: 'script',
		emulateOnload: false
