...
use Freetrix\Seo\RobotsFile;
use Freetrix\Seo\SitemapTable;
use Freetrix\Seo\SitemapEntityTable;
use Freetrix\Seo\SitemapIblockTable;
use Freetrix\Seo\SitemapForumTable;
...
if($ID > 0)
{
	...
	{
		if($_REQUEST['action'] == 'delete' && check_freetrix_sessid())
		{
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_freetrix_sessid() && (strlen($_POST["save"]) > 0 || strlen($_POST['apply']) > 0 || strlen($_POST['save_and_add']) > 0))
{
...
if(isset($_REQUEST['dir']) && check_freetrix_sessid())
{
...
if($bIBlock && isset($_REQUEST['iblock']) && check_freetrix_sessid())
{
...
$APPLICATION->SetAdditionalCSS("/freetrix/panel/seo/sitemap.css");
...
$aMenu[] = array(
	...
	"LINK"	=> "/freetrix/admin/seo_sitemap.php?lang=".LANGUAGE_ID,
	"ICON"	=> "btn_list",
...
if ($ID > 0)
{
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".Loc::getMessage("SITEMAP_DELETE_CONFIRM")."')) window.location='/freetrix/admin/seo_sitemap_edit.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".freetrix_sessid_get()."';",
		"ICON"	=> "btn_delete",
...
function loadDir(bLogical, sw, dir, div, depth, checked)
{
	...
	{
		BX.ajax.get('<?=$APPLICATION->GetCurPageParam('', array('dir', 'depth'))?>', {dir:dir,depth:depth,checked:checked?'Y':'N',log:bLogical?'Y':'N',sessid:BX.freetrix_sessid()}, function(res)
		{
...
function loadIblock(sw, iblock, section, div, section_checked, element_checked)
{
	...
	else
	{
		...
		BX.ajax.get('<?=$APPLICATION->GetCurPageParam('', array('dir', 'iblock', 'section', 'depth'))?>', {iblock:iblock,section:section,section_checked:section_checked?'Y':'N',element_checked:element_checked?'Y':'N',sessid:BX.freetrix_sessid()}, function(res)
		{
			BX(div).innerHTML = res;
			BX(row).style.display = '';
			if(sw)
