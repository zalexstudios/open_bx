...
$aMenu[] = array(
	...
	"LINK"	=> "/freetrix/admin/seo_search_google.php?lang=".LANGUAGE_ID,
	"ICON"	=> "btn_list",
);
...
{
	BX.ajax.loadJSON('/freetrix/tools/seo_google.php?action='+action+'&domain=<?=CUtil::JSEscape(urlencode($arDomain['DOMAIN']))?>&dir=<?=CUtil::JSEscape(urlencode($arDomain['SITE_DIR']))?>&sessid=' + BX.freetrix_sessid(), params, function(res)
		{
			callback(res);
		});
}
...
{
	BX.ajax.loadJSON(
		'/freetrix/tools/seo_google.php?action='+action+'_feed&domain=<?=urlencode($arDomain['DOMAIN'])?>&dir=<?=urlencode($arDomain['SITE_DIR'])?>&<?=freetrix_sessid_get()?>',
		function(res)
		{
