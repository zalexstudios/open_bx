...
if (strlen($counters) > 0):
	$counters = str_replace(array('#DOMAIN#'), array(/*$server_name*/ 'www.freetrixsoft.com'), $counters);
?>
<tr>
...
?>
	<tr>
		<td>Для использования данной функции необходимо задать список используемых поисковых систем в <a href="/freetrix/admin/settings.php?mid=seo&tabControl_active_tab=edit3">настройках модуля SEO</a>.</td>
	</tr>
<?
	else:
...
?>
		...
		<td colspan="2" align="right"><a href="/freetrix/admin/phrase_list.php?lang=<?echo LANGUAGE_ID?>">Посмотреть в панели управления</a></td>
	</tr>
<?
	}
...
?>
	...
	<td colspan="2" align="right"><a href="/freetrix/admin/referer_list.php?lang=<?echo LANGUAGE_ID?>">Посмотреть в панели управления</a></td>
</tr>
<?
	}
...
{
	ShowWaitWindow();
	jsUtils.loadJSFile('/freetrix/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=' + jsUtils.urlencode(callback_name) + '&keywords=' + encodeURIComponent(keywords));
}
...
window.BXCallPageStats = function()
{
	...
	jsUtils.loadJSFile('/freetrix/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&first=Y&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=window.BXSetStats&keywords=' + encodeURIComponent(keywords));
}
