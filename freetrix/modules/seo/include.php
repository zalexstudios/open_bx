...
\Freetrix\Main\Loader::registerAutoLoadClasses(
	"seo",
	array(
...
{
	if (COption::GetOptionString('main', 'vendor', '') == '1c_freetrix')
	{
		define(
	...
	function SeoOnPanelCreate()
	{
		...
		$APPLICATION->AddPanelButton(array(
			"HREF"=> 'javascript:'.$APPLICATION->GetPopupLink(
				array(
					"URL"=>"/freetrix/admin/public_seo_tools.php?lang=".LANGUAGE_ID."&bxpublic=Y&from_module=seo&site=".SITE_ID
...
;(function(){
	...
	var originalTextBtn = new BX.CWindowButton({
			...
			action: function()
			{
				...
				if(content.length < <?=\Freetrix\Seo\Engine\Yandex::ORIGINAL_TEXT_MIN_LENGTH?>)
				{
					alert('<?=GetMessageJS('SEO_YANDEX_ORIGINAL_TEXT_TOO_SHORT', array('#NUM#' => \Freetrix\Seo\Engine\Yandex::ORIGINAL_TEXT_MIN_LENGTH))?>');
				}
				else if(content.length > <?=\Freetrix\Seo\Engine\Yandex::ORIGINAL_TEXT_MAX_LENGTH?>)
				{
					alert('<?=GetMessageJS('SEO_YANDEX_ORIGINAL_TEXT_TOO_LONG', array('#NUM#' => \Freetrix\Seo\Engine\Yandex::ORIGINAL_TEXT_MAX_LENGTH))?>');
				}
				else
				{
					...
					BX.ajax({
						method: 'POST',
						dataType: 'json',
						url: '/freetrix/tools/seo_yandex.php',
						data: {
	...
	var originalTextHandler = function()
	{
		...
		{
			BX.showWait();
			BX.ajax.get('/freetrix/tools/seo_yandex.php?get=original_text_form&sessid=' + BX.freetrix_sessid(), BX.proxy(originalTextLoader, this));
		}
	...
	{
		id : 'SeoUniqText',
		src : '/freetrix/panel/seo/images/icon_editor_toolbar.png',
