...
BX.ready(function(){
	var tabArea = BX.findParent(BX("FX_GMAP_<?=$MAP_ID?>"),{className:"adm-detail-content"});
...
BX.loadCSS('/freetrix/components/freetrix/map.google.view/settings/settings.css');
...
function saveYandexKey(domain, input)
{
	...
	var data = 'key_type=yandex&domain=' + domain + '&key=' + value + '&<?echo freetrix_sessid_get()?>';
	ShowWaitWindow();
	CHttpRequest.Post('/freetrix/admin/settings.php?lang=<?echo LANGUAGE_ID?>&mid=fileman&save_map_key=Y', data);
}
</script>
<?
...
<script type="text/javascript">
BX.ready(function(){
	var tabArea = BX.findParent(BX("FX_YMAP_<?=$MAP_ID?>"),{className:"adm-detail-content"});
	var tabButton = BX("tab_cont_"+tabArea.id);
...
jsUtils.loadCSSFile('/freetrix/components/freetrix/map.yandex.view/settings/settings.css');
...
<script>
jsUtils.loadCSSFile("/freetrix/components/freetrix/player/js/skin_selector.css");
jsUtils.loadJSFile("/freetrix/components/freetrix/player/js/prop_skin_selector.js", function()
{
...
<table class="bx-video-prop-tbl">
	...
	<? if ($controlMode != "iblock_element_admin"): ?>
		...
		<tr id="bx_video_new_path_row_<?= $id?>" <?if (strlen($path) > 0){ echo 'style="display: none;"'; }?>>
			...
			<img src="/freetrix/images/1.gif" class="spacer" />
...
<?
	...
	{
		return "/freetrix/components/freetrix/player/mediaplayer/skins";
	}
