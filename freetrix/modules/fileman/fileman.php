...
// AddEventHandler("fileman", "OnBeforeHTMLEditorScriptsGet", "freetrix_tabs");
// function freetrix_tabs($editorName, $arEditorParams){return array("JS" => array('freetrix_tabs.js'));}
...
class CFileMan
{
	...
	{ 
		...
		{
			// New page
			$href = "/freetrix/admin/fileman_".$editor_type."_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&path=".UrlEncode($APPLICATION->GetCurDir())."&new=Y&templateID=".urlencode(SITE_TEMPLATE_ID)."&back_url=".UrlEncode($REQUEST_URI);
			...
			//New folder
			$href = "/freetrix/admin/fileman_newfolder.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&path=". UrlEncode($APPLICATION->GetCurDir())."&back_url=".UrlEncode($REQUEST_URI);
		...
		{
			$href = "/freetrix/admin/fileman_".$editor_type."_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&templateID=".urlencode(SITE_TEMPLATE_ID).$full_src."&path=".UrlEncode(isset($_SERVER["REAL_FILE_PATH"]) && $_SERVER["REAL_FILE_PATH"]<>""? $_SERVER["REAL_FILE_PATH"] : $cur_page)."&back_url=".UrlEncode($REQUEST_URI);
		...
		if ($sect_permission>="W")
		{
			$href = "/freetrix/admin/fileman_folder.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&path=".UrlEncode($APPLICATION->GetCurDir())."&back_url=".UrlEncode($REQUEST_URI);
	...
	function GetVersion()
	{
		@include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/fileman/install/version.php");
			...
			BX.ready(
				function()
				{
					window.changeType_<?= $name?> = function(bSave)
					{
						...
						if (bSave !== false)
						{
							BX.ajax.get('/freetrix/admin/fileman_manage_settings.php?<?= freetrix_sessid_get()?>&target=text_type&edname=<?= $name?>&key=<?= $key?>&type=' + curType);
						}
						<?endif;?>
					};
	...
	)
	{
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST" && !check_freetrix_sessid())
			return;
	...
	function ShowHTMLEditControl($name, $content, $arParams = Array())
	{
		...
		$arParams["light_mode"] = (defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1) || (isset($arParams["light_mode"]) && $arParams["light_mode"] == 'Y');
		...
		if(!$bFirstUsed)
		{
			...
			<script>
				var
					...
					spellcheck_js_v = "<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/freetrix/admin/htmleditor2/spellcheck.js')?>",
					FX_PERSONAL_ROOT = "<?=FX_PERSONAL_ROOT?>";
				...
				window.FX_PERSONAL_ROOT = top.FX_PERSONAL_ROOT = FX_PERSONAL_ROOT;
			</script>
			<?
			...
			for($i = 0, $c = count($arJS); $i < $c; $i++)
			{
				...
				if(file_exists($_SERVER['DOCUMENT_ROOT'].'/freetrix/admin/htmleditor2/'.$arJS[$i]))
					$arr[] = $arJS[$i];
			}
			?>
			<script type="text/javascript" src="/freetrix/admin/fileman_js.php?lang=<?=LANGUAGE_ID?>&v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/freetrix/modules/fileman/lang/'.LANGUAGE_ID.'/admin/fileman_js.php')?>"></script>
			<script type="text/javascript" src="/freetrix/admin/fileman_common_js.php?s=<?=$str_taskbars?>"></script>
			<?
			for($i = 0; $i < count($arr); $i++)
			{
				$script_filename = $arr[$i];
				?><script type="text/javascript" src="/freetrix/admin/htmleditor2/<?=$script_filename?>?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/freetrix/admin/htmleditor2/'.$script_filename)?>"></script><?
			}
			?>
			<script type="text/javascript" src="/freetrix/js/main/popup_menu.js?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/freetrix/js/main/popup_menu.js')?>"></script>
			<?
			for($i=0; $i< count($arCSS); $i++) // Additional CSS files from event OnBeforeHtmlEditorScriptGet
			{
				...
				if(!file_exists($_SERVER['DOCUMENT_ROOT'].'/freetrix/admin/htmleditor2/'.$arCSS[$i]))
					continue;
				?><link rel="stylesheet" type="text/css" href="/freetrix/admin/htmleditor2/<?=$arCSS[$i]?>?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/freetrix/admin/htmleditor2/'.$arCSS[$i])?>"/><?
			}
						...
						<table class="dim100x100">
							<tr>
								...
								<td id="<?= $name?>_taskBarSet2" class="bxedtaskbarset" style="width:0%; display: none;">
									<table>
										<tr><td class="bx-move-col-v" rowSpan="3"><img src="/freetrix/images/1.gif" /></td><td style="height:26px;"></td></tr>
							...
							<tr style="height:0%; display: none;">
								<td id="<?= $name?>_taskBarSet3" colspan="2">
									<table>
										<tr><td class="bx-move-col-h"><img src="/freetrix/images/1.gif" /></td></tr>
		...
		<script>
			BX.loadCSS('/freetrix/admin/htmleditor2/editor.css');
	...
	function ParseFileContent($filesrc, $bCheckProlog = false)
	{
		...
		if ($bCheckProlog)
		{
			...
			if (strlen($prolog) > 0 &&
				...
				strpos($prolog, 'freetrix/header.php') === false &&
			...
			if (strlen($epilog) > 0 &&
				strpos($epilog, 'freetrix/footer.php') === false &&
	...
	function GetAllTemplateParams($templateID, $site, $findcomponent = true, $arAdditionalParams = Array())
	{
				...
				if (preg_match_all($pattern, $ar_templ["STYLES"], $matches))
				{
					for ($j = 0; $j < count($matches[0]); $j++)
					{
						...
						if (substr($url, -5) != 'print')
						{
							...
							$url = trim(trim($url), '\'";');
							if (substr($url, 0, 1) != '/' && file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$ar_templ["ID"]."/".$url))
								$css = "\n".$APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$ar_templ["ID"]."/".$url)."\n";
		...
		{
			if(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/".(strlen($site)<=0?LANGUAGE_ID:$site)."/styles.css"))
			{
				$arResult["STYLES"] = $APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/".(strlen($site) <= 0?LANGUAGE_ID : $site)."/styles.css");
				$arResult["STYLES_TITLE"] = CSiteTemplate::__GetByStylesTitle($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/".(strlen($site)<=0?LANGUAGE_ID : $site)."/.styles.php");
			}
			elseif(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/styles.css"))
			{
				$arResult["STYLES"] = $APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/styles.css");
				$arResult["STYLES_TITLE"] = CSiteTemplate::__GetByStylesTitle($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/.styles.php");
			}
			else
			{
				$arResult["STYLES"] = $APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/.default/styles.css");
				$arResult["STYLES_TITLE"] = CSiteTemplate::__GetByStylesTitle($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/.default/.styles.php");
			}
		}
		...
		if(strlen($templateID)>0 && file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$templateID."/editor.css"))
			$arResult["STYLES"] .= "\r\n".$APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$templateID."/editor.css");
		elseif(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/.default/editor.css"))
			$arResult["STYLES"] .= "\r\n".$APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/.default/editor.css");
		elseif(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/".$site."/editor.css"))
			$arResult["STYLES"] .= "\r\n".$APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/".(strlen($site)<=0?LANGUAGE_ID:$site)."/editor.css");
		elseif(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/editor.css"))
			$arResult["STYLES"] .= "\r\n".$APPLICATION->GetFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/editor.css");
