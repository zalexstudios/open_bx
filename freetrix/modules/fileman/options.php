<?
$module_id = "fileman";
$dicsRelPath = '/freetrix/modules/fileman/dictionaries';
$gzDicsRelPath = FX_PERSONAL_ROOT.'/tmp/dics';
require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/fileman/include.php");
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/admin/task_description.php");
...
if($REQUEST_METHOD == "POST" && strlen($Update)>0 && $USER->CanDoOperation('fileman_edit_all_settings') && check_freetrix_sessid())
{
	...
	if (!$useEditor3)
	{
		...
		if (isset($_POST['user_dics_path']) && $_POST['user_dics_path']!='')
			...
			COption::SetOptionString($module_id, "user_dics_path","/freetrix/modules/fileman/u_dics");
		...
		if (isset($_POST['dic_lang']) && isset($_FILES['dic_aff']) && isset($_FILES['dic_base']) && $_FILES['dic_aff']['name'] != '' && 	$_FILES['dic_base']['name'] != '')
		{
			...
			if (isValidLang($dic_lang))
			{
				...
				if (!file_exists($lang_dir))
					mkdir($lang_dir, FX_DIR_PERMISSIONS);
				...
				if (!file_exists($dics_dir))
					mkdir($dics_dir, FX_DIR_PERMISSIONS);
		...
		if (isset($_POST['index_dic']))
		{
			...
			if (file_exists($lang_dir) && is_dir($lang_dir))
			{
				...
				require($_SERVER['DOCUMENT_ROOT'].FX_ROOT.'/modules/fileman/admin/spell_createDictionary.php');
...
table.bxopt-tbl td.bxopt-but-icon .bxed-button{width: 20px; height: 20px; background-image: url(/freetrix/images/fileman/htmledit2/_global_iconkit.gif); background-repeat: norepeat;}
...
table.bxopt-list td.bx-listbutton{background:url("/freetrix/images/fileman/htmledit2/_global_iconkit.gif") no-repeat scroll -63px -59px #E0DEDA !important; border-left:1px solid #A5ACB2 !important; font-size:1pt !important; height:18px !important; width:14px !important;}
...
function _GtFMess()
{
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/freetrix/modules/fileman/lang/'.LANGUAGE_ID.'/admin/fileman_js.php'))
		include($_SERVER['DOCUMENT_ROOT'].'/freetrix/modules/fileman/lang/'.LANGUAGE_ID.'/admin/fileman_js.php');
	else
		include($_SERVER['DOCUMENT_ROOT'].'/freetrix/modules/fileman/lang/en/admin/fileman_js.php');
	return $MESS;
}
...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/freetrix/modules/fileman/admin/fileman_js.php');
$l = count($aMess);
for($i = 0; $i < $l; $i++)
...
BXButtonConfig.prototype =
{
	Init: function()
	{
		...
		// Load buttons
		BX.loadScript('/freetrix/admin/htmleditor2/toolbarbuttons.js', function()
			{
	...
	AddButton: function(typeCode, but, ind, toolbarId)
	{
		...
		else
		{
			...
			{
				var pIcon = iconCell.appendChild(BX.create("IMG", {props: {id: "bx_btn_" + but[1].id, src: "/freetrix/images/1.gif", className: "bxed-button"}}));
...
<? if (!$useEditor3):?>
	...
	<tr>
		...
		<td>
			<input type="text" name="user_dics_path" style="width: 100%" value="<? echo COption::GetOptionString($module_id, "user_dics_path", "/freetrix/modules/fileman/u_dics")?>">
		</td>
	</tr>
	<tr>
...
?>
	...
	<tr>
		...
		<img src="/freetrix/images/1.gif" style="width: 110px; height: 1px;" />
		</td>
	</tr>
...
<?$tabControl->BeginNextTab();?>
<?require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/admin/group_rights2.php");?>
...
<script>
	function RestoreDefaults()
	{
		if(confirm('<?= AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
			window.location = "<?= $APPLICATION->GetCurPage()?>?RestoreDefaults=Y&lang=<?echo LANG?>&mid=<?echo urlencode($mid)?>&<?=freetrix_sessid_get()?>";
	}
</script>
