...
if(($extended=="Y" || $extended=="N") && $extended != ${COption::GetOptionString("main", "cookie_name", "FREETRIX_SM")."_menumode"})
	setcookie(COption::GetOptionString("main", "cookie_name", "FREETRIX_SM")."_menumode", $extended, time()+60*60*24*30*60 ,'/');
else
	$extended = ${COption::GetOptionString("main", "cookie_name", "FREETRIX_SM")."_menumode"};
		...
		if($REQUEST_METHOD=="POST" && strlen($save) >0 && is_array($ids) && check_freetrix_sessid())
		{
			...
			if(strlen($apply)<=0)
			{
				if(strlen($back_url)>0)
					...
					LocalRedirect("/freetrix/admin/fileman_admin.php?".$addUrl."&site=".$site."&path=".UrlEncode($path));
			}
			else
				LocalRedirect("/freetrix/admin/fileman_menu_edit.php?".$addUrl."&site=".$site."&path=".UrlEncode($path)."&name=".$name);
		}
	}
}
...
if ($bEdit && !$only_edit)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessage("FILEMAN_DEL_CONF")."')) window.location='fileman_admin.php?path=".UrlEncode($path)."&action=delete&ID[]=".UrlEncode(".".$name.".menu.php")."&".$addUrl."&".freetrix_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
