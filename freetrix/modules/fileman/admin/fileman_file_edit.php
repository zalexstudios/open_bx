	...
	if($REQUEST_METHOD == "POST" && strlen($save) > 0 && strlen($propeditmore) <= 0 && !$only_read)
	{
		...
		if(strlen($strWarning) <= 0)
		{
			...
			if (strlen($strWarning) <= 0)
			{
				if(strlen($apply) <= 0)
				{
					if(strlen($back_url)>0 && strpos($back_url, "/freetrix/admin/fileman_file_edit.php")!==0)
						LocalRedirect("/".ltrim($back_url, "/"));
					LocalRedirect("/freetrix/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"]));
				}
				else
				{
					LocalRedirect("/freetrix/admin/fileman_file_edit.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($path).($full_src=="Y"?"&full_src=Y":""));
				}
			}
...
if($bEdit)
{
	...
	if($USER->CanDoFileOperation('fm_delete_file', $arPath))
	{
		...
		$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("FILEMAN_FILE_DELETE_CONFIRM")."')) window.location='/freetrix/admin/fileman_admin.php?ID=".urlencode($id)."&action=delete&".$addUrl."&site=".urlencode($site)."&path=".urlencode($folder_path)."&".freetrix_sessid_get()."';",
...
$tabControl->BeginNextTab();?>
	...
	<?if(!$bEdit):?>
		...
		<tr>
			...
			<td width="70%">
				<select name="template" onchange="window.location='/freetrix/admin/fileman_file_edit.php?lang=<?= LANG?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($path)?><? echo ($full_src=="Y" ? "&full_src=Y" : "")?>&new=y&template='+escape(this[this.selectedIndex].value)">
					<?for($i=0; $i<count($arTemplates); $i++):?>
		...
		<tr><td></td><td style="padding: 0!important;">
			...
			<IMG src="/freetrix/themes/.default/images/icon_warn.gif" title="<?=GetMessage("FILEMAN_NAME_ERROR");?>">
			</td><td class="jserror">
...
$tabControl->Buttons(
	array(
		"disabled" => $only_read,
		"back_url" => (strlen($back_url)>0 && strpos($back_url, "/freetrix/admin/fileman_file_edit.php")!==0) ? htmlspecialcharsbx($back_url) : "/freetrix/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"])
	)
);
