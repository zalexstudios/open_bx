...
if(strlen($back_url)>0 && strpos($back_url, "/freetrix/admin/fileman_file_edit.php")!==0)
	...
	$url = "/freetrix/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"]);
		...
		if(strlen($strWarning) <= 0)
		{
			...
			else
			{
				...
				if(strlen($strWarning)<=0 && strlen($apply)<=0 && strlen($apply2)<=0)
					...
					LocalRedirect("/freetrix/admin/fileman_html_edit.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($path)."&back_url=".UrlEncode($back_url)."&fullscreen=".($bFullScreen?"Y":"N")."&tabControl_active_tab=".urlencode($tabControl_active_tab));
			}
...
?>
	<?if(!$bEdit):?>
		...
		function templateOnChange(_this)
		{
			...
			window.location='/freetrix/admin/fileman_html_edit.php?lang=<?= LANG?><?=$logic?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($folderPath)?>&new=y&template='+encodeURIComponent(_this[_this.selectedIndex].value)+_name+_title;
		}
		</script>
...
{
	window.location='/freetrix/admin/fileman_html_edit.php?lang=<?= LANG?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($path)?>&new=y&template='+id;
