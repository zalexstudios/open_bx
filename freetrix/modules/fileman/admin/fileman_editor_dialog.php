...
function OnLoad()
{
	...
	if(pElement && bxTag) /* Link selected*/
	{
		...
		else if (href.indexOf("://") !== -1 || href.substr(0, 'www.'.length) == 'www.' || href.indexOf("&goto=") !== -1)
		{
			...
			if(href.substr(0, '/freetrix/redirect.php'.length) == '/freetrix/redirect.php')
			{
				BX("bx_fixstat").checked = true;
				ChangeFixStat();
				var sParams = href.substring('/freetrix/redirect.php'.length);
	...
	switch(BX('bx_link_type').value)
	{
		...
		case 't2':
			...
			if(BX("bx_fixstat").checked)
				href = '/freetrix/redirect.php?event1=' + escape(BX("event1").value) + '&event2=' + escape(BX("event2").value) + '&event3=' + escape(BX("event3").value) + '&goto=' + escape(href);
			break;
		case 't3':
...
function OnLoad()
{
	...
	BX('FX_dialog_title').value = BX('title').value;
	BX("FX_more_prop_but").onclick = function(e) {AppendRow('', '');};
...
function AppendRow(code, value, name)
{
	var
		tbl = BX('pageprops_t1'),
		cnt = parseInt(BX("FX_dialog_maxind").value) + 1;
		r = tbl.insertRow(tbl.rows.length - 1),
		c = r.insertCell(-1);

	c.className = "bx-par-title";
	if(name)
		c.innerHTML = '<input type="hidden" id="FX_dialog_CODE_'+cnt+'" name="FX_dialog_CODE_'+cnt+'" value="'+bxhtmlspecialchars(code)+'">'+bxhtmlspecialchars(name)+':';
	else
	{
		c.innerHTML = '<input type="text" id="FX_dialog_CODE_'+cnt+'" name="FX_dialog_CODE_'+cnt+'" value="'+bxhtmlspecialchars(code)+'" size="30">:';
		if(!finput)
			finput = BX('FX_dialog_CODE_'+cnt);
	}

	c = r.insertCell(-1);
	c.className = "bx-par-val";
	c.innerHTML = '<input type="text" name="FX_dialog_VALUE_'+cnt+'" id="FX_dialog_VALUE_'+cnt+'" value="'+bxhtmlspecialchars(value)+'" size="55">';

	if(!finput)
		finput = BX('FX_dialog_VALUE_'+cnt);

	BX("FX_dialog_maxind").value = cnt;
...
function AppendTagPropertyRow(code, value, name)
{
	...
	var cnt = parseInt(BX("FX_dialog_maxind").value)+1;
	...
	c = r.insertCell(-1);
	c.className = "bx-par-val";
	id = 'FX_dialog_VALUE_' + cnt;
	name = 'FX_dialog_VALUE_' + cnt;
...
function OnSave()
{
	...
	var cnt = parseInt(BX("FX_dialog_maxind").value);

	for(var i=0; i<=edcnt; i++)
	{
		if(BX("CODE_"+i).value != BX("FX_dialog_CODE_"+i).value)
			BX("CODE_"+i).value = BX("FX_dialog_CODE_"+i).value;
		if(BX("VALUE_"+i).value != BX("FX_dialog_VALUE_"+i).value)
			BX("VALUE_"+i).value = BX("FX_dialog_VALUE_"+i).value;
	}

	for(i = edcnt+1; i<=cnt; i++)
		window._MoreRProps(BX("FX_dialog_CODE_"+i).value, BX("FX_dialog_VALUE_"+i).value);

	BX("maxind").value = cnt;
	BX('title').value = BX('FX_dialog_title').value;
...
<table id="pageprops_t1" class="bx-par-tbl">
	<tr>
		<td class="bx-par-title"><label for="FX_dialog_title"><b><?= GetMessage("FILEMAN_DIALOG_TITLE")?></b></label></td>
		<td class="bx-par-val"><input type="text" id="FX_dialog_title" value="" size="30"></td>
	</tr>
	<tr>
		<td></td>
		<td class="bx-par-val"><input id="FX_more_prop_but" type="button" value="<?= GetMessage("FILEMAN_DIALOG_MORE_PROP")?>"></td>
	</tr>
</table>
<input type="hidden" value="-1" id="FX_dialog_maxind">
...
<?ob_start();?>
<div>
<div id="FX_dialog_waitWin" style="display: block; text-align: center; vertical-align: middle;">
	<table border="0" width="100%" height="100%" style="vertical-align: middle">
		<tr><td height="60"></td></tr>
		<tr>
			<td align="center" valign="top">
				<img style="vertical-align: middle;" src="/freetrix/themes/.default/images/wait.gif" />
				<span style="vertical-align: middle;"><?= GetMessage("FILEMAN_ED_WAIT_LOADING")?></span>
			</td>
		</tr>
	</table>
</div>
<div id="FX_dialog_okMessWin" style="display: none;">
	<table border="0" width="100%" height="100%">
		<tr>
			<td align="center">
				<span style="vertical-align: middle;"><?= GetMessage("FILEMAN_ED_SPELL_FINISHED")?></span>
				<br><br>
				<input id="FX_dialog_butClose" type="button" value="<?= GetMessage("FILEMAN_ED_CLOSE")?>" style="width:150">
			</td>
		</tr>
	</table>
</div>
<div id="FX_dialog_spellResultWin" style="display: none">
<table width="380" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr><td colspan="4" height="5"></td></tr>
	<tr>
		<td width="224" valign="top"><input id="FX_dialog_wordBox" type="text" style="width:100%;"></td>
		<td width="8"></td>
		<td width="140" valign="top"><input id="FX_dialog_butSkip" type="button" value="<?= GetMessage("FILEMAN_ED_SKIP")?>" style="width:100%;"></td>
		<td width="8"></td>
	</tr>
	<tr><td colspan="4" height="7"></td></tr>
	<tr>
		<td rowspan="9" valign="top"><select id="FX_dialog_suggestionsBox" size="8" style="width:100%;"></select></td>
		<td></td>
		<td><input id="FX_dialog_butSkipAll" type="button" value="<?= GetMessage("FILEMAN_ED_SKIP_ALL")?>" style="width:100%;"></td>
		<td></td>
	</tr>
	<tr height="5"><td colspan="2" height="5"></td></tr>
	<tr>
		<td></td>
		<td><input id="FX_dialog_butReplace" type="button" value="<?= GetMessage("FILEMAN_ED_REPLACE")?>" style="width:100%;"></td>
		<td></td>
	</tr>
	<tr height="5"><td colspan="2" height="5"></td></tr>
	<tr>
		<td></td>
		<td><input id="FX_dialog_butReplaceAll" type="button" value="<?= GetMessage("FILEMAN_ED_REPLACE_ALL")?>" style="width:100%;"></td>
		<td></td>
	</tr>
	<tr height="5"><td colspan="2" height="5"></td></tr>
	<tr>
		<td></td>
		<td><input id="FX_dialog_butAdd" type="button" value="<?= GetMessage("FILEMAN_ED_ADD")?>" style="width:100%;"></td>
		<td></td>
	</tr>
	<tr height="5"><td colspan="2" height="5"></td></tr>
	<tr>
		<td></td>
		<td><input id="FX_dialog_butClose" type="button" value="<?= GetMessage("FILEMAN_ED_CLOSE")?>" style="width:100%;" onClick="pObj.Close();"></td>
		<td></td>
	</tr>
</table>
</div>
...
function _displayTaskbarList(oCont)
{
	...
	if(pObj.pMainObj.allowedTaskbars['BXComponents2Taskbar'])
	{
		...
		if (!window.BXComponents2Taskbar || !ar_BXTaskbarS["BXComponents2Taskbar_" + pObj.pMainObj.name])
		{
			...
			{
				_displayTaskbarRow(oTable,{name:'BXComponents2Taskbar', title:FX_MESS.CompTBTitle}, settings);
	...
	if(pObj.pMainObj.allowedTaskbars['BXSnippetsTaskbar'])
	{
		...
		if (!ar_BXTaskbarS["BXSnippetsTaskbar_" + pObj.pMainObj.name])
		{
			...
			{
				_displayTaskbarRow(oTable,{name:'BXSnippetsTaskbar',title:FX_MESS.SnippetsTB}, settings);
	...
	if (!compareObj(Settings.arTaskbarSettings, window.temp_arTaskbarSettings))
	{
		...
		if(temp_arTaskbarSettings['BXSnippetsTaskbar'].show)
		{
			if (window.BXSnippetsTaskbar_need_preload)
				arScripts.push("/freetrix/admin/htmleditor2/snippets.js");
		}
		...
		if (temp_arTaskbarSettings['BXComponents2Taskbar'].show)
		{
			if (window.BXComponents2Taskbar_need_preload)
				arScripts.push("/freetrix/admin/htmleditor2/components2.js");
...
function OnLoad()
{
	...
	oDiv.innerHTML = '<table width="100%" border="0" height="260">'+
					'<tr>'+
						'<td align="right" width="40%">' + FX_MESS.PATH2SWF + ':</td>'+
						'<td width="60%">'+
					...
					'</tr>'+
					'<tr>'+
						'<td align="right">' + FX_MESS.TPropSize + ':</td>'+
	...
	oDiv.innerHTML = '<table width="100%" border="0" height="260">'+
				'<tr>'+
					'<td align="right" width="40%" colspan="2">' + FX_MESS.SWF_ID + ':</td>'+
					'<td width="60%" colspan="2">'+
						'<input type="text" size="30" value="" id="_flash_id">'+
					'</td>'+
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_TITLE + ':</td>'+
					'<td colspan="2">'+
						'<input type="text" size="30" value="" id="_flash_title">'+
					'</td>'+
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_CLASSNAME + ':</td>'+
					'<td colspan="2">'+
						'<input type="text" size="30" value="" id="_flash_classname">'+
					'</td>'+
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.TPropStyle + '</td>'+
					'<td colspan="2">'+
						'<input type="text" size="30" value="" id="_flash_style">'+
					'</td>'+
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_QUALITY + ':</td>'+
					'<td colspan="2">'+
				...
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_WMODE + ':</td>'+
					'<td colspan="2">'+
				...
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_SCALE + ':</td>'+
					'<td colspan="2">'+
				...
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_SALIGN + ':</td>'+
					'<td colspan="2">'+
				...
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_AUTOPLAY + ':</td>'+
					'<td colspan="2">'+
						'<input type="checkbox" value="" id="_flash_autoplay">'+
					'</td>'+
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_LOOP + ':</td>'+
					'<td colspan="2">'+
						'<input type="checkbox" value="" id="_flash_loop">'+
					'</td>'+
				'</tr>'+
				'<tr>'+
					'<td align="right" colspan="2">' + FX_MESS.SWF_SHOW_MENU + ':</td>'+
					'<td colspan="2">'+
	...
	if(pElement && bxTag) // Edit flash
	{
		...
		window.oBXEditorDialog.SetTitle(FX_MESS.FLASH_MOV);
	...
	BX.ajax.post(manage_snippets_path + '&target=add',
		...
		function()	{setTimeout(function(){
			...
			var __arEl =
			{
				...
				thumb:thumb,
				isGroup:false,
				icon:'/freetrix/images/fileman/htmledit2/snippet.gif',
