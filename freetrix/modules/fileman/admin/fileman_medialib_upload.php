...
if (isset($_GET['action']) && $_GET['action'] == 'upload' && check_freetrix_sessid())
{
...
elseif(isset($_GET['action']) && $_GET['action'] == 'redirect' && check_freetrix_sessid())  //Redirect after files uploading
{
	$APPLICATION->SetTitle(GetMessage('FM_ML_UPL_TITLE2'));
	$APPLICATION->SetAdditionalCSS('/freetrix/js/fileman/medialib/medialib_admin.css');
	...
	if ($len > 0)
	{
		...
		<form name="ml_mu_form" action="/freetrix/admin/fileman_medialib_upload.php?action=postsave" method="POST">
		...
		for($i = 0; $i < $len; $i++)
		{
			...
			<tr>
				...
				<?else:?>
					<img src="/freetrix/images/1.gif" class="ml-item-thumb ml-item-no-thumb" />
				<?endif;?>
				</td>
			</tr>
			<tr>
		...
		<input type="button" title="<?= GetMessage('admin_lib_edit_cancel_title')?>" onclick="window.location='/freetrix/admin/fileman_medialib_admin.php?lang=<?= LANGUAGE_ID?>';" name="cancel" value="<?= GetMessage('admin_lib_edit_cancel')?>" />
		</form>
		<script>
		...
		function addCollToItem(ItemId, id, pSel)
		{
			...
			var
				...
				pDel = pDiv.appendChild(jsUtils.CreateElement("IMG", {src: '/freetrix/images/1.gif', className: 'mu-col-del', id: 'mu_it_' + id, title: '<?= GetMessage('ML_DEL_COL2ITEM')?>'}));
...
iu.addParam("LicenseKey", "Freetrix");
...
iu.addParam("Action", "/freetrix/admin/fileman_medialib_upload.php?action=upload&<?=freetrix_sessid_get()?>&lang=<?=LANGUAGE_ID?>&ml_type=<?= intVal($_GET['type'])?>");
...
function ImageUploaderML_AfterUpload(Html)
{
	try
	{
		...
		return jsUtils.Redirect([], "/freetrix/admin/fileman_medialib_upload.php?action=redirect&<?=freetrix_sessid_get()?>&lang=<?= LANGUAGE_ID?>" + sGet);
	}
	catch(e)
	{
		return jsUtils.Redirect([], "/freetrix/admin/fileman_medialib_upload.php?<?=freetrix_sessid_get()?>&lang=<?=LANGUAGE_ID?>");
	}
}
