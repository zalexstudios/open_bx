...
{
	var sURL = '/freetrix/admin/fileman_get_xml.php?op=sitetemplateparams&lang='+BXLang+'&site='+BXSite+'&templateID='+templateID;
...
return;
	...
	*/
	...
	if(this.pEditorDocument.selection)
	{
		...
		{
			if(arB[i].face && arB[i].face=='freetrixtemp')
			{
	...
	BXContextMenu.prototype._Create = function ()
	{
		...
		for(i=0; i<arMenuItems.length; i++)
		{
			if(arMenuItems[i] == 'separator')
			{
				...
				c.innerHTML = '<img src="/freetrix/images/1.gif" width="1" height="1"/>';
			}
			else
			{
				r = t.insertRow(-1);
