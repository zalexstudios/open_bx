...
function CFormValidatorSettings(bSimple)
{
	...
	{
		if(document.getElementById("form_settings_float_div"))
			return;
		...
		CHttpRequest.Action = function(result)
		{
			...
			div.id = "form_settings_float_div";
			...
			jsUtils.addEvent(document, "keypress", oCFVS.SettingsOnKeyPress);
		}
		ShowWaitWindow();
	...
	{
		jsUtils.removeEvent(document, "keypress", oCFVS.SettingsOnKeyPress);
		var div = document.getElementById("form_settings_float_div");
		jsFloatDiv.Close(div);
		div.parentNode.removeChild(div);
	}

	CFormValidatorSettings.prototype.SettingsOnKeyPress = function(e)
	{
		if(!e) e = window.event
		if(!e) return;
		if(e.keyCode == 27)
	...
	CFormValidatorSettings.prototype.UpdateSelect = function()
	{
				...
				if (oTypeSelect && oMessage && oMessage.value.length != 0)
				{
					...
					else
					{
						if (sType != oTypeSelect.value)
						{
							...
							_container.appendChild(document.createTextNode(arMess.ERROR_MULTITYPE));
		...
		if (arValidatorsType[sType] && arValidatorsType[sType].length > 0)
		{
			...
			_button.setAttribute('TITLE', arMess.ADD_TITLE);
			...
			var __cell = __row.insertCell(-1);
			__cell.appendChild(document.createTextNode(arMess.ADD_TITLE));
		...
		{
			var _container = document.createElement('SPAN');
			_container.appendChild(document.createTextNode(arMess.NO_REGISTERED_VALS_TYPE));
	...
	CFormValidatorSettings.prototype.addValidator = function()
	{
		...
		if (oValidatorList)
		{
			...
			if (arValidators[current_item])
			{
				...
				{
					var url = '/freetrix/admin/form_validator_props.php?validator=' + current_item;
	...
	CFormValidatorSettings.prototype.UpdateList = function()
	{
		...
		if (oOutput)
		{
			...
			if (arCurrentValidators.length > 0)
			{
				...
				_cell = _row.insertCell(-1);
				_cell.appendChild(document.createTextNode(arMess.LIST_HEAD_VAL));
				_cell = _row.insertCell(-1);
				_cell.appendChild(document.createTextNode(arMess.LIST_HEAD_PARAMS));
				_cell = _row.insertCell(-1);
				_cell.appendChild(document.createTextNode(arMess.LIST_HEAD_REMOVE));
				...
				for (var i=0; i<arCurrentValidators.length; i++)
				{
					...
					_img.src = '/freetrix/images/form/' + (arCurrentValidators[i].VALID ? "green.gif" : "red.gif");
					_img.style.height = "14px";
					_img.style.width = "14px";
					_img.alt = arMess[arCurrentValidators[i].VALID ? 'VALID' : 'INVALID'];
					...
					_button.style.width = '20px';
					_button.style.cursor = 'pointer';
					_button.setAttribute('title', arMess.DEL_TITLE);
					//_button.setAttribute('onClick', 'jsFormValidatorSettings.RemoveValidator(' + i + ')');

					_button.setAttribute('__FX_val_index', i);
			...
			{
				oOutput.appendChild(document.createTextNode(arMess.NO_CURRENT));
			}
		}
	}
	...
	{
		var index = this.getAttribute('__FX_val_index');
