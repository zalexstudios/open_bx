...
class CAllForm extends CForm_old
{
	...
	{
		$module_id = "form";
		@include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/".$module_id."/install/version.php");
	...
	function Delete($ID, $CHECK_RIGHTS="Y")
	{
		...
		if ($CHECK_RIGHTS!="Y" || CForm::IsAdmin())
		{
			...
			if (CForm::Reset($ID, "N"))
			{
				...
				$tmp_filename = $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/tmp/form/form_".$ID.".php";
	...
	function SetMailTemplate($WEB_FORM_ID, $ADD_NEW_TEMPLATE="Y", $old_SID="", $bReturnFullInfo = false)
	{
		...
		if ($arrForm = $q->Fetch())
		{
			...
			{
				IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/form/admin/form_mail.php", $arLang["LID"]);
			...
			if ($ADD_NEW_TEMPLATE=="Y")
			{
				...
				if (is_array($arrFormSite) && count($arrFormSite)>0)
				{
					...
					{
						IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/form/admin/form_mail.php", $arrSiteLang[$sid]);
...
http://#SERVER_NAME#/freetrix/admin/form_result_view.php?lang=".$arrSiteLang[$sid]."&WEB_FORM_ID=#RS_FORM_ID#&RESULT_ID=#RS_RESULT_ID#
	...
	function GetByID_admin($WEB_FORM_ID, $current_section = false)
	{
		...
		if ($arForm = $dbForm->Fetch())
		{
			...
			$arForm['ADMIN_MENU'][] = array(
				...
				"LINK"	=> "/freetrix/admin/form_edit.php?lang=".LANGUAGE_ID."&ID=".$WEB_FORM_ID,
