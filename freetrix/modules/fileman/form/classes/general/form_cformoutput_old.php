	...
	function Init($arParams, $admin = false)
	{
		...
		if ($this->WEB_FORM_ID > 0)
		{
			...
			if ($this->isAccessForm())
			{
				...
				if (!$this->comp2 || $this->arParams["COMPONENT"]["componentName"] != "freetrix:form.result.list" || $this->isAccessFormResultList())
				{
					if ($this->RESULT_ID)
					{
						if ($this->isAccessFormResult($arrResult))
						{
							...
							if (
								...
								$this->comp2 && // for components2 - check whether there's editing or viewing and check rights for it
									(
										$this->arParams["COMPONENT"]["componentName"] == "freetrix:form.result.edit" && !$this->isAccessFormResultEdit()
										||
										$this->arParams["COMPONENT"]["componentName"] == "freetrix:form.result.view" && !$this->isAccessFormResultView()
									)
							)
							{
							...
							else
							{
								...
								if ($this->arResult)
								{
									if ($this->comp2 && $this->arParams["COMPONENT"]["componentName"] == "freetrix:form.result.view")
									{
...
/*****************************************/
	...
	function Out()
	{
					...
					if ($this->RESULT_ID)
					{
						...
						if (defined("ADMIN_SECTION") && ADMIN_SECTION === true)
						{
							if (strlen($_REQUEST["web_form_submit"])>0)
								LocalRedirect(FX_ROOT."/admin/form_result_list.php?lang=".LANG."&WEB_FORM_ID=".$this->WEB_FORM_ID."&strFormNote=".urlencode($this->strFormNote));
							elseif (strlen($_REQUEST["web_form_apply"])>0)
								LocalRedirect(FX_ROOT."/admin/form_result_edit.php?lang=".LANG."&WEB_FORM_ID=".$this->WEB_FORM_ID."&RESULT_ID=".$this->RESULT_ID."&strFormNote=".urlencode($this->strFormNote));
							die();
						}
						/*
						else
		...
		$path = FX_PRESONAL_ROOT."/templates/".SITE_TEMPLATE_ID."/".$rel_path;
		$href = "javascript:window.location='".$editor."path=".urlencode($path)."&lang=".LANGUAGE_ID."&back_url=".urlencode($back_url)."'";

		if(!file_exists($_SERVER["DOCUMENT_ROOT"].$path))
		{
			$path = FX_PRESONAL_ROOT."/templates/.default/".$rel_path;
			$href = "javascript:window.location='".$editor."path=".urlencode($path)."&lang=".LANGUAGE_ID."&back_url=".urlencode($back_url)."'";
			if(!file_exists($_SERVER["DOCUMENT_ROOT"].$path))
			{
				$path = "/freetrix/modules/form/install/templates/".$rel_path;
				$href = "javascript:if(confirm('".GetMessage("MAIN_INC_BLOCK_COMMON")."')) window.location='".$editor."path=".urlencode(FX_PERSONAL_ROOT.'/templates/'.SITE_TEMPLATE_ID.'/'.$rel_path)."&template=".urlencode($path)."&lang=".LANGUAGE_ID."&back_url=".urlencode($back_url)."'";
			}
		}
		...
		if ($USER->IsAdmin())
		{
			$APPLICATION->AddPanelButton(array(
				"SORT" => 100,
				"MAIN_SORT" => 1000,
				"HREF" => "/freetrix/admin/form_edit.php?lang=".LANGUAGE_ID."&amp;ID=".$this->WEB_FORM_ID. "&amp;tabControl_active_tab=edit5&back_url=".urlencode($back_url),
				"SRC"  => "/freetrix/images/form/edit_templ.gif",
			...
			$APPLICATION->AddPanelButton(array(
				...
				"MAIN_SORT" => 1000,
				"HREF" => $href,
				"SRC"  => "/freetrix/images/form/edit_default_templ.gif",
			...
			if($APPLICATION->GetShowIncludeAreas())
			{
				if ($this->arForm["USE_DEFAULT_TEMPLATE"] == "N")
				{
					...
					$arIcons[] =
							Array(
								"URL" => "/freetrix/admin/form_edit.php?lang=".LANGUAGE_ID."&amp;ID=".$this->WEB_FORM_ID. "&amp;tabControl_active_tab=edit5&back_url=".urlencode($back_url),
								"ICON" => 'form-edit-tpl',
								"ALT" => GetMessage("FORM_PUBLIC_ICON_EDIT_TPL")
								);
		...
		if (is_array($arrResultAnswer))
		{
			...
			foreach ($arrResultAnswer as $key => $arrA)
			{
				...
				if (intval($arrA["USER_FILE_ID"])>0)
				{
					...
					{
						$file_link = "/freetrix/tools/form_show_file.php?rid=".$this->RESULT_ID."&hash=".$arrA["USER_FILE_HASH"]."&lang=".LANGUAGE_ID;
