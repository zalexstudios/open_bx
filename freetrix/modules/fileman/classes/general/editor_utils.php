...
class CEditorUtils
{
	...
	function _RenderOneComponent($arParams, $bLPA)
	{
		...
		$arProperties['FX_EDITOR_RENDER_MODE'] = Array('NAME' => 'Workin in render mode *For Visual editor rendering only*', 'TYPE' => 'CHECKBOX', 'DEFAULT' => 'Y'); // Add description of the system parameter
		...
		$arProps['FX_EDITOR_RENDER_MODE'] = 'Y'; //Set system parameter
		$params = PHPParser::ReturnPHPStr2($arProps, $arParameters);
		$code .= 'Array('.$params.')';
		$code .= ');';

		ob_start();
		echo '#FX_RENDERED_COMPONENT#';
		eval($code);
		echo  '#FX_RENDERED_COMPONENT#';
		$s = ob_get_contents();
		ob_end_clean();
		...
		if ($l > 0)
		{
			...
			for ($n = 0; $n<$l; $n++)
			{
				...
				if ($arRes)
				{
					...
					$arParams['FX_EDITOR_RENDER_MODE'] = 'Y';
					...
					echo '#FX_RENDERED_COMPONENT_'.$comp_count.'#';
					eval($code);
					echo  '#FX_RENDERED_COMPONENT_'.$comp_count.'#';
				}
				$comp_count++;
