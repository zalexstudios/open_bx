...
class CSnippets
{
	...
	{
		$basePath = $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$template."/snippets";
	...
	{
		$path = $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$template."/snippets";
	...
	function Edit($Params)
	{
		...
		$site = $Params['site'];
		$code = $Params['code'];
		$contPath = $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$template."/snippets";
		$thumb = $Params['thumb'] === false ? false : CFileMan::SecurePathVar($Params['thumb']);

		if (!file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$template))
		{
		...
		if ($Params['bNew'])
		{
			...
			if ($name == '')
			{
				$name = CSnippets::GetDefaultFileName($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$template."/snippets/".$path);
		...
		if ($thumb !== false)
		{
			...
			$img_path1 = FX_PERSONAL_ROOT.'/templates/'.$template.'/snippets/images/'.$path.($path == '' ?  '' : '/').$f_name;
	...
	function Delete($Params)
	{
		...
		$snPath = FX_PERSONAL_ROOT."/templates/".CFileMan::SecurePathVar($Params['template'])."/snippets";
...
<?
	...
	{
		$contPath = $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$Params['template']."/snippets";
	...
	function DisplayJSGroups($template, $ar = array())
	{
		...
		window.rootDefaultName['<?= $template?>'] = '<?= CSnippets::GetDefaultFileName($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$template."/snippets")?>';
		<?
