...
class CFileInput
{
	...
	private static function GetFile($fileId = "")
	{
		...
		if(
			$arFile["HANDLER_ID"]
			|| (defined("FX_IMG_SERVER") && substr($sImagePath, 0, strlen(FX_IMG_SERVER)) === FX_IMG_SERVER)
	...
	private static function DisplayControl($inputs = array())
	{
		...
		if (self::$bUseCloud)
			$arConfig['cloudDialogPath'] = '/freetrix/admin/clouds_file_search.php?lang='.LANGUAGE_ID.'&n=';
	...
	private static function DisplayFile($arFile = array(), $ind = 0)
	{
		...
		{
			$file = CFile::ResizeImageGet($arFile['ID'], array('width' => self::$maxPreviewWidth, 'height' => self::$maxPreviewHeight), FX_RESIZE_IMAGE_PROPORTIONAL, true);
			?>
