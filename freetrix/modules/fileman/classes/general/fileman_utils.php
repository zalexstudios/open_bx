...
class CFilemanUtils
{
	function InitScript($Params)
	{
		...
		?>
		<script type="text/javascript" src="/freetrix/js/fileman/fileman_utils.js?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/freetrix/js/fileman/fileman_utils.js')?>"></script>
	...
	function Search($file)
	{
		...
		if ($this->Params['phrase'] != "")
		{
			...
			if ($this->bReplace) // Replace
			{
				...
				$pattern = '/'.preg_quote($this->Params['phrase'], '/').'/'.$I_PCRE_MODIFIER.FX_UTF_PCRE_MODIFIER;
		...
		$this->Result[] = array(
			...
			'type_src' => "/freetrix/images/fileman/types/".($bIsDir ? "folder" : CFileMan::GetFileTypeEx($file)).".gif",
...
function InitTranslitInputs(params)
{
	...
	if (params.bLinkInputs)
	{
		...
		var
			pLink1 = BX.create("IMG", {props: {src: "/freetrix/images/1.gif", className: "bxfm-linked", title: params.linkedTitle}}),
			pLink2 = BX.create("IMG", {props: {src: "/freetrix/images/1.gif", className: "bxfm-linked", title: params.linkedTitle}});
