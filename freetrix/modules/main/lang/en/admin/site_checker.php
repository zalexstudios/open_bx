...
$MESS["SC_CRON_WARN"] = "The constant FX_CRONTAB_SUPPORT is defined in /freetrix/php_interface/dbconn.php, this requires running agents using cron.";
...
$MESS["SC_T_MBSTRING"] = "UTF configuration parameters (mbstring and FX_UTF)";
$MESS["SC_T_SITES"] = "Website Parameters";
$MESS["SC_FX_UTF"] = "Use the following code in <i>/freetrix/php_interface/dbconn.php</i>:
<code>define('FX_UTF', true);</code> ";
$MESS["SC_FX_UTF_DISABLE"] = "The FX_UTF constant must not be defined";
...
$MESS["SC_HELP_CHECK_INSTALL_SCRIPTS"] = "Users may occasionally forget to delete the installation scripts (restore.php, freetrixsetup.php) after system recovery or installation. This may become a serious security threat and result in website hijacking. If you have ignored the autodelete warning, remember to remove these files manually.";
...
All the Freetrix modules use the <i>FX_UTF</I> constant to resolve the current encoding. A UTF-8 website requires the following code in <i>/freetrix/php_interface/dbconn.php</i>:
<code>define('FX_UTF', true);</code>
";
$MESS["SC_HELP_CHECK_SITES"] = "Verifies general multisite parameters. If a website specifies the root directory path (which is required only for websites existing on different domains), that directory must contain a symbolic link to writable \"freetrix\" folder.

All the websites that share the same Freetrix system instance must use the same encoding: either UTF-8 or single byte.";
...
$MESS["SC_HELP_CHECK_UPLOAD"] = "This will attempt to connect to the web server and send a chunk of binary data as a file. The server will then compare the received data with the original sample. If a problem arises, it may be caused by some parameter in <i>php.ini</I> prohibiting binary data transfer, or by inaccessible temporary folder (or <i>/freetrix/tmp</i>).
...
$MESS["SC_HELP_CHECK_MAIL"] = "This will send an e-mail message to hosting_test@freetrixsoft.com using the standard PHP function \"mail\". A special mailbox exists to make the test conditions as real-life as possible.
...
Alternatively, you can use cron to send the e-mails. To do so, add <code>define('FX_CRONTAB_SUPPORT', true);</code> to dbconn.php. Then, set cron to execute <i>php /var/www/freetrix/modules/main/tools/cron_events.php</I> every minute (replace <i>/var/www</i> with your website root).

If the call to mail() has failed, you cannot send e-mail from your server using conventional methods.

If your hosting provider offers alternative e-mail sending services, you can use them by calling the function \"custom_mail\". Define this function in <i>/freetrix/php_interface/dbconn.php</I>. If the system find this function definition, it will use the latter instead of PHP's \"mail\" with the same input parameters.";
$MESS["SC_HELP_CHECK_MAIL_BIG"] = "This will test bulk e-mails by sending the same message as in the previous text (the site check script) 10 times. Additionally, a newline character is inserted into the message subject, and the message is BCC'ed to noreply@freetrixsoft.com.
...
If the current value is less than that, add this line of code to <i>/freetrix/php_interface/dbconn.php</i>.
";
...
If this test fails, contact your hosting techsupport for necessary file permissions and set the contants <b>FX_FILE_PERMISSIONS</b> and <b>FX_DIR_PERMISSIONS</b> in <i>dbconn.php</i> accordingly.

Configure PHP to run as an Apache module if possible.";
$MESS["SC_HELP_CHECK_FX_CRONTAB"] = "To migrate the non-periodic agents and e-mail to cron, add the following constant to <i>/freetrix/php_interface/dbconn.php</i>:
<code>define('FX_CRONTAB_SUPPORT', true);</code>

With this constant set to \"true\", the system will run only the periodic agents when a hit occurs. Now add a task to cron to execute <i>/var/www/freetrix/modules/main/tools/cron_events.php</i> every minute (replace <i>/var/www</i> with the website root path).

The script defines the constant <b>FX_CRONTAB</b> which indicates that the script is activated by cron and runs only non-periodic agents. If you define this constant in <i>dbconn.php</i> by mistake, periodic agents will never run.";
...
Set the PHP time zone in <i>/freetrix/php_interface/dbconn.php</i>:
<code>date_default_timezone_set(&quot;Europe/London&quot;);</code> (use your region and city).

Set the database time zone by adding the following code to <i>/freetrix/php_interface/after_connect.php</i>:
<code>\$connection = FreetrixMainApplication::getConnection(); 
...
$MESS["SC_HELP_CHECK_MYSQL_MODE"] = "The parameter <i>sql_mode</i> specifies the MySQL operation mode. Note that it may accept values incompatible with Freetrix solutions. Add the following code to <i>/freetrix/php_interface/after_connect.php</I> to set the default mode:
<code>\$connection = FreetrixMainApplication::getConnection(); \$connection->queryExecute(&quot;SET sql_mode=''&quot;);</code>";
...
To change the connection charset (for example, set it to UTF-8), add the following code to <i>/freetrix/php_interface/after_connect.php</i>:
<code>\$connection = FreetrixMainApplication::getConnection();
...
$MESS["ERR_OLD_VM"] = "You are running an outdated version of Freetrix Environment. Please install the most recent version to prevent configuration issues.";
...
$MESS["SC_HELP_CHECK_PERF"] = "Server performance evaluation as provided by <a href=\"http://www.freetrixsoft.com/support/training/course/index.php?COURSE_ID=20&CHAPTER_ID=04955\">Performance Monitor</a>.
...
The reference <a href=\"http://www.freetrixsoft.com/products/virtual_appliance/\">Freetrix Virtual Appliance</a> usually scores 30 points.
...
$MESS["SC_HELP_CHECK_CA_FILE"] = "The test attempts to connect to www.freetrixsoft.com. 
...
Then, the test downloads a list of certification centers from the Freetrix server which is required by the SSL certificate test.
";
...
";
$MESS["SC_HELP_CHECK_PULL_STREAM"] = "The <a href=\"http://www.freetrixsoft.com/support/training/course/index.php?COURSE_ID=26&LESSON_ID=5144\">Push and Pull</a> module requires that your server supports this feature.
...
<a href=\"http://www.freetrixsoft.com/products/virtual_appliance/\">Freetrix Virtual Appliance</a> supports this module since version 4.2.
";
$MESS["SC_HELP_CHECK_PULL_COMMENTS"] = "To make comments in Avtivity Stream available to all readers right away, the Push and Pull module may require additional configuration. Namely, your Nginx instance needs to have push-stream-module installed, and then activated in the <a href=\"http://www.freetrixsoft.com/support/training/course/index.php?COURSE_ID=26&LESSON_ID=5144\">Push and Pull</a> module settings.

<a href=\"http://www.freetrixsoft.com/products/virtual_appliance/index.php\">Freetrix Virtual Appliance</a> comes fully preconfigured to support this feature since version 4.2.
";
...
$MESS["SC_HELP_CHECK_SOCNET"] = "To receive updates from social resources, the <a href=\"http://www.freetrixsoft.com/company/blog/news/integration-with-social-networks.php\">Social Website Integration</a> module has to be configured providing authentication keys for each service that are going to be used.";
$MESS["SC_HELP_CHECK_EXTRANET"] = "The <a href=\"http://www.freetrixsoft.com/products/intranet/features/collaboration/extranet.php\">Extranet</a> module requires that your Intranet is externally accessible via the Internet.

If you don't need features provided by this module, simply <a href=\"/freetrix/admin/module_admin.php\">uninstall it</a>.";
...
Notice that some extra configuration might be required <a href=\"http://www.freetrixsoft.com/support/training/course/index.php?COURSE_ID=27&LESSON_ID=1466#office\">on the client side</a> to support direct editing, and there's no way to verify it remotely.
";
$MESS["SC_HELP_CHECK_AD"] = "If a Windows AD or LDAP server is set up on your local network, it is advised to check that AD is <a href=\"http://www.freetrixsoft.com/support/training/course/index.php?COURSE_ID=20&CHAPTER_ID=04264\">properly configured</a>.
...
Setting up NTLM on Linux is definitely not a trivial task; <a href=\"http://www.freetrixsoft.com/products/virtual_appliance/\">Freetrix Virtual Appliance</a> includes NTLM authentication fully configured since version 4.2.";
...
Freetrix Inc. provides the preconfigured TURN server free of charge at turn.calls.freetrix24.com. 

Alternatively, you can set up your own server and specify the server URL in the Web Messenger module settings.";
$MESS["SC_HELP_CHECK_PUSH_FREETRIX"] = "The <a href=\"http://www.freetrixsoft.com/support/training/course/index.php?COURSE_ID=26&LESSON_ID=5144\">Push and Pull</a> module handles the delivery of instant messages (Pull), and sends Push notifications to mobile devices (<a href=\"http://www.freetrixsoft.com/products/intranet/features/freetrixmobile.php\">Freetrix mobile application</a>).

Sending notification to Apple and Android devices is performed using the secure (HTTPS) Freetrix messaging center https://cloud-messaging.freetrix24.com.
...
The test employs a special server at checker.internal.freetrix24.com that attempts a connection to your Intranet using the current Freetrix24 URL as provided by the web browser. No user data is transmitted while the connection with the remote server is active.

Instant messaging requires that the read port of Nginx's push-stream-module can be connected to. The port number comes from the <a href=\"http://www.freetrixsoft.com/support/training/course/index.php?COURSE_ID=26&LESSON_ID=5144\">Push and Pull</a> module settings.";
...
Once a request has been served, PHP resources are freed to process a subsequent request in the queue. This significantly improves Intranet performance and boosts file download speed when accessed via Freetrix.Drive, Document Library or when downloading attachments from Activity Stream posts.

Enable this option in the <a href=\"/freetrix/admin/settings.php?mid=main\">Kernel settings</a>. <a href=\"http://www.freetrixsoft.com/products/virtual_appliance/\">Freetrix Virtual Appliance</a> supports fast file downloads by default.
...
If the server does not support HTML page compression, the Freetrix Compression module is used instead. Remember that this module <a href=\"/freetrix/admin/module_admin.php\">should not be installed</a> otherwise.";
...
$MESS["SC_HELP_CHECK_SEARCH"] = "The system can search text in documents in Open XML format (introduced in Microsoft Office 2007) out of the box. To support other file formats, specify paths to parsing applications <a href=\"/freetrix/admin/settings.php?mid=intranet\">in the Intranet module settings</a>. Otherwise, the system will be able to search filenames only.

<a href=\"http://www.1c-freetrix.ru/products/vmfreetrix/index.php\">Freetrix Virtual Appliance</a> supports it by default.";
$MESS["MAIN_SC_AGENTS_CRON"] = "Use cron to run agents";
$MESS["MAIN_SC_PERF_TEST"] = "Server performance test";
$MESS["MAIN_SC_COMP_DISABLED"] = "The server doesn't support compression, using the Freetrix Compression module instead (PHP)";
$MESS["MAIN_SC_COMP_DISABLED_MOD"] = "The server doesn't support compression, the compression module disabled";
$MESS["MAIN_SC_ENABLED"] = "The server supports compression, the Freetrix Compression module needs to be uninstalled";
$MESS["MAIN_SC_ENABLED_MOD"] = "Using the web server module for compression";
$MESS["MAIN_SC_TEST_SSL1"] = "Secure HTTPS connection was established but the SSL certificate could not be verified because the list of certification authorities was not downloaded from Freetrix server";
...
$MESS["MAIN_FX_CRONTAB_DEFINED"] = "Defined the FX_CRONTAB constant which can only be done in scripts running on cron.";
...
$MESS["MAIN_SC_CLOUD_TEST"] = "Access to Freetrix cloud services";
...
$MESS["MAIN_SC_NO_ACCESS"] = "Cannot access Freetrix, Inc. server. Updates and Freetrix Cloud Service are unavailable.";
...
$MESS["MAIN_SC_CORRECT_DESC"] = "Intranet requires special configuration of the server environment. <a href=\"http://www.freetrixsoft.com/products/virtual_appliance/\" target=\"_blank\">Freetrix Virtual Appliance</a> is configured properly out-of-the-box. Some features may be unavailable if you fail to adjust the required parameters.";
...
$MESS["MAIN_SC_TEST_MOBILE"] = "Freetrix24 mobile app";
...
$MESS["MAIN_SC_TEST_FAST_FILES"] = "Freetrix24.Drive. Fast file operations";
