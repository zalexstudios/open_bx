...
$MESS["SC_CRON_WARN"] = "Определена константа FX_CRONTAB_SUPPORT в /freetrix/php_interface/dbconn.php, при этом должен быть настроен вызов агентов на cron.";
...
$MESS["SC_T_MBSTRING"] = "Параметры настройки UTF (mbstring и константа FX_UTF)";
$MESS["SC_T_SITES"] = "Настройки сайтов";
$MESS["SC_FX_UTF"] = "Необходимо установить в <i>/freetrix/php_interface/dbconn.php</i> код
<code>define('FX_UTF', true);</code> ";
$MESS["SC_FX_UTF_DISABLE"] = "Константа FX_UTF не должна быть установлена";
...
В качестве индикатора кодировки для всех модулей 1С-Битрикс является константа <i>FX_UTF</i>. Для сайта в UTF-8 необходимо установить в <i>/freetrix/php_interface/dbconn.php</i> код:
<code>define('FX_UTF', true);</code>
";
$MESS["SC_HELP_CHECK_SITES"] = "Проверяются базовые настройки многосайтовости. Если для сайтов указан путь к корневой папке (а это необходимо делать только для сайтов на разных доменах), в каждой такой папке должна быть символьная ссылка на папку freetrix, доступную на запись.
...
$MESS["SC_HELP_CHECK_MAIL"] = "Осуществляется передача тестового письма на почтовый адрес hosting_test@freetrixsoft.com через стандартную php функцию mail. Чтобы максимально приблизить тест к реальной работы почты заведен служебный ящик.
...
Затем на cron добавьте ежеминутное выполнение скрипта <i>php /var/www/freetrix/modules/main/tools/cron_events.php</i>.
...
Если хостер предоставляет альтернативные способы отправки почты с сайта, их можно использовать через функцию custom_mail. Определите эту функцию в файле <i>/freetrix/php_interface/dbconn.php</i>, она будет использоваться вместо встроенной mail функции с тем же набором параметров.";
$MESS["SC_HELP_CHECK_MAIL_BIG"] = "При отправке большого сообщения текст предыдущего письма (исходный код скрипта проверки сайта) повторяется 10 раз. Кроме того, тема письма разрывается на две строки и добавляется передача скрытой копии на адрес noreply@freetrixsoft.com.
...
Если текущее значение меньше, установите указанную строку в <i>/freetrix/php_interface/dbconn.php</i>.
";
...
Если тест не отрабатывает, узнайте необходимые права для скриптов и установите в <i>dbconn.php</i> для констант <b>FX_FILE_PERMISSIONS</b> и <b>FX_DIR_PERMISSIONS</b>.

Если есть возможность, используйте режим работы php как модуль Apache.";
$MESS["SC_HELP_CHECK_FX_CRONTAB"] = "Чтобы перенести работу непериодических агентов (и передачу почты) на cron, необходимо в <i>/freetrix/php_interface/dbconn.php</i> установить константу:
<code>define('FX_CRONTAB_SUPPORT', true);</code>

После этого на хитах будут выполняться только периодические агенты. Затем в cron поставить задание на ежеминутный запуск скрипта <i>/var/www/freetrix/modules/main/tools/cron_events.php</i>.
(замените <i>/var/www</i> на свой путь к корню сайта).

Внутри скрипта определена константа <b>FX_CRONTAB</b>, которая является индикатором того, что скрипт работает из cron и приводит к выполнению только непериодических агентов. Если эта константа ошибочно определена в <i>dbconn.php</i>, периодические агенты полностью перестают выполняться.";
...
Установить часовой пояс php можно в <i>/freetrix/php_interface/dbconn.php</i>, например:
<code>date_default_timezone_set(&quot;Europe/Moscow&quot;);</code>

Для базы данных добавьте в <i>/freetrix/php_interface/after_connect_d7.php</i>:
...
$MESS["SC_HELP_CHECK_MYSQL_MODE"] = "Параметр <i>sql_mode</i> задаёт режим работы MySQL. Может принимать значения, несовместимые с 1С-Битрикс. Чтобы установить режим работы по умолчанию, добавьте в <i>/freetrix/php_interface/after_connect_d7.php</i>:
<code>\$connection = FreetrixMainApplication::getConnection(); 
...
Чтобы изменить кодировку соединения, установите в <i>/freetrix/php_interface/after_connect_d7.php</i> код (пример для <i>utf8</i>):
<code>\$connection = FreetrixMainApplication::getConnection(); 
...
$MESS["SC_HELP_CHECK_PERF"] = "Оценка производительности сервера методом <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=32&CHAPTER_ID=1146\">монитора производительности</a>.
...
Эталонная <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=37\">виртуальная машина &quot;1С-Битрикс&quot;</a> имеет оценку около 30 единиц.
...
$MESS["SC_HELP_CHECK_CA_FILE"] = "Тест пытается подключиться к сайту www.1c-freetrix.ru. 
...
";
$MESS["SC_HELP_CHECK_PULL_STREAM"] = "Для полноценной работы чата необходима настройка модуля <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=41&LESSON_ID=2033\">Push and Pull</a>.
...
Для этого должен быть настроен модуль сервера Nginx: push-stream-module. <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=37\">Виртуальная машина &quot;1С-Битрикс&quot;</a> с версии 4.2 имеет полную поддержку этого модуля.
";
$MESS["SC_HELP_CHECK_PULL_COMMENTS"] = "Для того чтобы комментарии в живой ленте были видны всем пользователям сразу после добавления, необходима настройка модуля <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=41&LESSON_ID=2033\">Push and Pull</a>.
...
$MESS["SC_HELP_CHECK_SOCNET"] = "Чтобы получать в живую ленту портала сообщения из социальных сетей, необходимо <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=48&LESSON_ID=3040\">настроить</a> модуль социальных сервисов, указав ключи для каждого сервиса отдельно. ";
$MESS["SC_HELP_CHECK_EXTRANET"] = "Для работы модуля <a href=\"http://dev.1c-freetrix.ru/learning/course/?COURSE_ID=45&CHAPTER_ID=794\">экстранет</a> необходим доступ к порталу из Интернета. 

Если вы не используете этот модуль, удалите его из <a href=\"/freetrix/admin/module_admin.php\">списка модулей</a>.";
...
Кроме этого нужна дополнительная <a href=\"http://dev.1c-freetrix.ru/learning/course/?COURSE_ID=45&LESSON_ID=2262#office\">настройка</a> на стороне пользователя, проверить которую возможности нет.
";
$MESS["SC_HELP_CHECK_AD"] = "Если в локальной сети настроен Windows Active Directory или LDAP сервер, желательно <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=41&CHAPTER_ID=726\">настроить</a> с AD чтобы обеспечить единое место хранения паролей и структуру компании. 

Для этого в php должен быть установлен модуль ldap.";
$MESS["SC_HELP_CHECK_NTLM"] = "<a href=\"http://wikipedia.org/wiki/SSO\">Технология единого входа</a> требует настройки специальных модулей на стороне веб-сервера и <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=32&LESSON_ID=5510\">настройку</a> портала.

Задача настройки NTLM в среде Linux нетривиальна, но на <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=37\">виртуальной машине &quot;1С-Битрикс&quot;</a> с версии 4.2 это настроено, нужно только включить через мастер настройки машины.";
...
На сервере должен быть настроен модуль Nginx: push-stream-module. <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=37\">Виртуальная машина &quot;1С-Битрикс&quot;</a> с версии 4.2 имеет полную поддержку этого модуля.

Если прямое сетевое соединение браузеров невозможно, нужен специальный TURN сервер. Компания &quot;1С-Битрикс&quot; бесплатно предоставляет для своих клиентов настроенный TURN сервер по адресу: turn.calls.freetrix24.com, который используется по умолчанию. Вы можете настроить собственный сервер и указать его адрес в настройках модуля &quot;веб-мессенджер&quot; (im).";
$MESS["SC_HELP_CHECK_PUSH_FREETRIX"] = "Модуль <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=41&LESSON_ID=2033\">Push and Pull</a> обеспечивает мгновенную передачу сообщений по технологии Pull и отправку уведомлений на мобильные устройства по технологии Push в <a href=\"http://www.1c-freetrix.ru/products/intranet/features/mobileapp.php\">мобильное приложение 1С-Битрикс</a>.

Отправка уведомлений на Apple и Android устройства осуществляется через единый сервис 1С-Битрикс https://cloud-messaging.freetrix24.com через HTTPS.
...
Тест использует специальный сервер checker.internal.freetrix24.com, который пытается осуществить подключение к порталу, используя адрес портала из браузера. При этом никакие пользовательские данные не передаются на наш сервер.

Для работы мгновенных сообщений необходима также возможность подключения к порту чтения сообщений модуля Nginx push-stream-module. Порт подключения берется из настроек модуля <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=41&LESSON_ID=2033\">Push and Pull</a>.";
...
В настройках <a href=\"/freetrix/admin/settings.php?mid=main\">главного модуля</a> необходимо включить соответствующую опцию. <a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=37\">Виртуальная машина &quot;1С-Битрикс&quot;</a> имеет поддержку этой возможности по умолчанию.
...
Если такой возможности нет, используется сжатие модулем битрикс compression, иначе модуль compression должен быть <a href=\"/freetrix/admin/module_admin.php\">не установлен</a>.";
...
Чтобы этот механизм корректно работал, необходимо наличие к порталу достпа из Интернета.";
$MESS["SC_HELP_CHECK_SEARCH"] = "Для поиска по содержимому документов требуется <a href=\"http://dev.1c-freetrix.ru/learning/course/?COURSE_ID=48&CHAPTER_ID=04759\">настройка</a> внешних инструментов. В настройках модуля <a href=\"/freetrix/admin/settings.php?mid=intranet\">intranet</a> должны быть указаны корректные пути к этим приложениям. Иначе поиск документов на портале возможен только по имени файла.

<a href=\"http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=37\">Виртуальная машина &quot;1С-Битрикс&quot;</a> имеет поддержку этой возможности по умолчанию.";
...
$MESS["MAIN_FX_CRONTAB_DEFINED"] = "Определена константа FX_CRONTAB, она может определяться только в скриптах, работающих на cron.";
