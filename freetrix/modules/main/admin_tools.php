...
class CMainAdmin
{
	function GetTemplateList($rel_dir)
	{
		...
		$path = FX_PERSONAL_ROOT."/templates/";
		...
		{
			$module_id = $arrS[0];
			$path = "/freetrix/modules/".$module_id."/install/templates/";
...
class CTemplates
{
	function GetList($arFilter = array(), $arCurrentValues = array(), $template_id = array())
	{
		...
		foreach($arFilter["FOLDER"] as $folder)
		{
			...
			$arPath = array(
				"/freetrix/modules/".$folder."/install/templates/",
				FX_PERSONAL_ROOT."/templates/.default/",
			);

			if(is_array($template_id))
			{
				foreach($template_id as $v)
					$arPath[] = FX_PERSONAL_ROOT."/templates/"._normalizePath($v)."/";
			}
			elseif(strlen($template_id)>0)
			{
				$arPath[] = FX_PERSONAL_ROOT."/templates/"._normalizePath($template_id)."/";
			}
	...
	function GetFolderList($template_id = false)
	{
		...
		$arPath = array(FX_PERSONAL_ROOT."/templates/.default");
		if($template_id)
			$arPath[] = FX_PERSONAL_ROOT."/templates/".$template_id;
...
class CPageTemplate
{
	function GetList($arSiteTemplates=array())
	{
		...
		{
			$path = getLocalPath($dir, FX_PERSONAL_ROOT);
			if($path === false)
				continue;
...
function GetTemplateContent($filename, $lang=LANG, $arTemplates=array())
{
	...
	foreach($arDirs as $dir)
	{
		$path = getLocalPath($dir."/".$filename, FX_PERSONAL_ROOT);
...
function GetFileTemplates($lang = LANG, $arTemplates = array())
{
	...
	foreach($arDirs as $dir)
	{
		$templDir = getLocalPath($dir, FX_PERSONAL_ROOT);
		if($templDir === false)
			continue;
...
function ParsePath($path, $bLast=false, $url=false, $param="", $bLogical = false)
{
	...
	if($url===false)
		$url = FX_ROOT."/admin/fileman_admin.php";
...
function GetDirList($path, &$arDirs, &$arFiles, $arFilter=array(), $sort=array(), $type="DF", $bLogical=false,$task_mode=false)
{
	...
	foreach ($arChildren as $child)
	{
		...
		if(substr($arFile["ABS_PATH"], 0, strlen(FX_ROOT."/modules"))==FX_ROOT."/modules" && !$USER->CanDoOperation('edit_php') && !$task_mode)
			continue;
