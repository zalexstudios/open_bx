...
class CPackageLicense extends CWizardStep
{
	...
	function InitStep()
	{
		...
		$this->SetCancelStep(FX_WIZARD_CANCEL_ID);
	}
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if ($siteID != "" && array_key_exists($siteID, $package->arSites))
			{
				if ($package->_InitSubStep("select", $package->arSites[$siteID], false))
				{
					...
					$package->__obFirstStep->SetPrevStep(FX_WIZARD_SELECT_SITE_ID);
				}
				else
				{
					if ($package->groupExists && $package->templateExists)
						$wizard->SetCurrentStep(FX_WIZARD_SELECT_GROUP_ID);
					elseif ($package->templateExists)
						$wizard->SetCurrentStep(FX_WIZARD_SELECT_TEMPLATE_ID);
					elseif ($package->serviceExists)
						$wizard->SetCurrentStep(FX_WIZARD_SELECT_SERVICE_ID);
					elseif ($package->structureExists)
						$wizard->SetCurrentStep(FX_WIZARD_SELECT_STRUCTURE_ID);
					else
						$wizard->SetCurrentStep(FX_WIZARD_START_INSTALL_ID);
				}
			}
			else
	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if ($templateID != "" && array_key_exists($templateID, $package->arTemplates))
			{
				...
				{
					$wizard->SetCurrentStep($package->__obFirstStep->GetStepID());
					$package->__obFirstStep->SetPrevStep(FX_WIZARD_SELECT_TEMPLATE_ID);
				}
				else
				{
	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			foreach ($arServices as $service)
			{
				...
				{
					$wizard->SetCurrentStep($package->__obFirstStep->GetStepID());
					$package->__obFirstStep->SetPrevStep(FX_WIZARD_SELECT_SERVICE_ID);
					break;
				}
				else
				{
...
class CPackageSelectStructure extends CWizardStep
{
	...
	function DisplayTree(&$arStructure, $systemTree = false)
	{
		...
		foreach ($arStructure as $pageID => $arPage)
		{
			...
			{
				if ($isService)
					$strTree .= '<img src="/freetrix/images/main/wizard/service.gif" width="16" height="16" border="0" alt="" />';
				else
					$strTree .= '<img src="/freetrix/images/main/wizard/page.gif" width="16" height="16" border="0" alt="" />';
			}
