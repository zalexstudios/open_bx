...
global $FX_EVENT_SITE_PARAMS;
$FX_EVENT_SITE_PARAMS = array();
...
class CAllEvent
{
	...
	{
		global $FX_EVENT_SITE_PARAMS;
		if($site_id !== false && isset($FX_EVENT_SITE_PARAMS[$site_id]))
			return $FX_EVENT_SITE_PARAMS[$site_id];
		...
		if(strlen($site_id)>0)
		{
			...
			{
				$FX_EVENT_SITE_PARAMS[$site_id] = array(
	...
	function HandleEvent($arEvent)
	{
		...
		while($db_mail_result_array = $db_mail_result->Fetch())
		{
			...
			if(defined("FX_MS_SMTP") && FX_MS_SMTP===true)
			{
