...
class CModule
{
	...
	{
		\Freetrix\Main\Loader::autoLoad($className);
	}

	function _GetCache()
	{
		return \Freetrix\Main\ModuleManager::getInstalledModules();
	}
	...
	{
		return \Freetrix\Main\ModuleManager::isModuleInstalled($this->MODULE_ID);
	}
	...
	function Remove()
	{
		\Freetrix\Main\ModuleManager::delete($this->MODULE_ID);
	}

	function Add()
	{
		\Freetrix\Main\ModuleManager::add($this->MODULE_ID);
	}
	...
	{
		return \Freetrix\Main\Loader::includeSharewareModule($module_name);
	}
...
{
	\Freetrix\Main\ModuleManager::registerModule($id);
}

function UnRegisterModule($id)
{
	\Freetrix\Main\ModuleManager::unRegisterModule($id);
}

function AddEventHandler($FROM_MODULE_ID, $MESSAGE_ID, $CALLBACK, $SORT=100, $FULL_PATH = false)
{
	$eventManager = \Freetrix\Main\EventManager::getInstance();
...
function ExecuteModuleEvent($arEvent, $param1=NULL, $param2=NULL, $param3=NULL, $param4=NULL, $param5=NULL, $param6=NULL, $param7=NULL, $param8=NULL, $param9=NULL, $param10=NULL)
{
	...
	elseif($arEvent["TO_PATH"] <> '' && file_exists($_SERVER["DOCUMENT_ROOT"].FX_ROOT.$arEvent["TO_PATH"]))
	{
		$r = include_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT.$arEvent["TO_PATH"]);
	}
...
function ExecuteModuleEventEx($arEvent, $arParams = array())
{
	...
	elseif(
		...
		&& $arEvent["TO_PATH"]<>""
		&& file_exists($_SERVER["DOCUMENT_ROOT"].FX_ROOT.$arEvent["TO_PATH"])
	)
	{
		$r = include_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT.$arEvent["TO_PATH"]);
	}
...
{
	return \Freetrix\Main\ModuleManager::isModuleInstalled($module_id);
}
