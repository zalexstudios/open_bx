	...
	class CBXSanitizer
	{
		...
		protected function IsValidAttr(&$arAttr)
		{
			...
			switch (strtolower($arAttr[1]))
			{
				case 'src':
				case 'href':
					if(!preg_match("#^(http://|https://|ftp://|file://|mailto:|callto:|skype:|\\#|/)#i".FX_UTF_PCRE_MODIFIER, $attrValue))
						$arAttr[3] = "http://".$arAttr[3];

					$valid = (!preg_match("#javascript:|data:|[^\\w".$this->localAlph.":/\\.=@;,!~\\*\\&\\#\\)(%\\s\\+\$\\?\\-]#i".FX_UTF_PCRE_MODIFIER, $attrValue)) ? true : false;
					break;
				...
				case 'cellpadding':
				case 'cellspacing':
					$valid = !preg_match("#^[^0-9\\-]+(px|%|\\*)*#i".FX_UTF_PCRE_MODIFIER, $attrValue) ? true : false;
					break;

				case 'title':
				case 'alt':
					$valid = !preg_match("#[^\\w".$this->localAlph."\\.\\?!,:;\\s\\-]#i".FX_UTF_PCRE_MODIFIER, $attrValue) ? true : false;
					break;

				case 'style':
					$valid = !preg_match("#(behavior|expression|position|javascript)#i".FX_UTF_PCRE_MODIFIER, $attrValue) && !preg_match("#[^\\w\\s)(,:\\.;\\-]#i".FX_UTF_PCRE_MODIFIER, $attrValue) ? true : false;
					break;

				case 'coords':
					$valid = !preg_match("#[^0-9\\s,\\-]#i".FX_UTF_PCRE_MODIFIER, $attrValue) ? true : false;
					break;

				default:
					$valid = !preg_match("#[^\\#\\w".$this->localAlph."\\-\\#\\.]#i".FX_UTF_PCRE_MODIFIER, $attrValue) ? true : false;
					break;
			}
		...
		public function SanitizeHtml($html)
		{
			...
			$seg = array();
			$arData = preg_split('/(<[^<>]+>)/si'.FX_UTF_PCRE_MODIFIER, $html, -1, PREG_SPLIT_DELIM_CAPTURE);
			...
			for($i=0; $i<$segCount; $i++)
			{
				...
				elseif($seg[$i]['segType'] == 'tag')
				{
					...
					preg_match('#^<\s*(/)?\s*([a-z0-9]+)(.*?)>$#si'.FX_UTF_PCRE_MODIFIER, $seg[$i]['value'], $matches);
					...
					if($seg[$i]['tagType'] == 'open')
					{
						...
						else
						{
							...
							preg_match_all('#([a-z_]+)\s*=\s*([\'\"])\s*(.*?)\s*\2#i'.FX_UTF_PCRE_MODIFIER, $matches[3], $arTagAttrs, PREG_SET_ORDER);
							$attr = array();
			...
			for ($tElCategory=self::TABLE_COLS;$tElCategory>self::TABLE_TOP;$tElCategory--)
			{
				...
				for($j=$segIndex-1;$j>=0;$j--)
				{
					...
					for($k=$segIndex-1;$k>$j;$k--)
					{
						...
						if($seg[$k]['segType'] == 'text' && !preg_match("#[^\n\r\s]#i".FX_UTF_PCRE_MODIFIER, $seg[$k]['value']))
							continue;
