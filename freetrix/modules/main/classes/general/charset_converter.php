<?
define("PATH2CONVERT_TABLES", $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/cvtables/");
global $FX_CHARSET_TABLE_CACHE;
$FX_CHARSET_TABLE_CACHE = Array();
...
class CharsetConverter
{
	...
	public static function ConvertCharset($string, $charset_in, $charset_out, &$errorMessage = "")
	{
		...
		if (!defined("FX_ICONV_DISABLE") || FX_ICONV_DISABLE !== true)
		{
			$utf_string = false;
			if (strtoupper($charset_in) == "UTF-16")
			{
	...
	protected function BuildConvertTable()
	{
		...
		for ($i = 0; $i < func_num_args(); $i++)
		{
			...
			if(isset($FX_CHARSET_TABLE_CACHE[$fileName]))
				continue;

			$FX_CHARSET_TABLE_CACHE[$fileName] = Array();
				...
				if ($line = trim(fgets($hFile, 1024)))
				{
					if (substr($line, 0, 1) != "#")
					{
						...
						if (substr($hexValue[1], 0, 1) != "#")
						{
							...
							$FX_CHARSET_TABLE_CACHE[func_get_arg($i)][$key] = $value;
						}
					}
				}
			}
