...
# mailto:sources@freetrixsoft.com              #
##############################################
...
class CControllerClient
{
	...
	{
		return file_exists($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/update_db_updater.php");
	}
...
class __CControllerPacket
{
	...
	function Debug($sText)
	{
		...
		if(!defined($this->debug_file_const))
			$dirname = $_SERVER['DOCUMENT_ROOT'].'/freetrix/controller_logs/';
		else
...
class __CControllerPacketRequest extends __CControllerPacket
{
	...
	function Send($url, $page)
	{
		...
		$strRequest .= "User-Agent: FreetrixControllerMember\r\n";
		$strRequest .= "Accept: */*\r\n";
...
class CControllerClientRequestTo extends __CControllerPacketRequest
{
	...
	function SendWithCheck($page="/freetrix/admin/controller_ws.php")
	{
	...
	function Send($page="/freetrix/admin/controller_ws.php")
	{
		$this->Sign();
