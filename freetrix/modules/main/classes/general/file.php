...
class CAllFile
{
	...
	function SaveFile($arFile, $strSavePath, $bForceMD5=false, $bSkipExt=false)
	{
		...
		if(!$bExternalStorage)
		{
			...
			@chmod($strPhysicalFileNameX, FX_FILE_PERMISSIONS);
	...
	function GetFileSRC($arFile, $upload_dir = false, $external = true)
	{
		...
		if(!$src)
		{
			...
			$src = str_replace("//", "/", $src);
			if(defined("FX_IMG_SERVER"))
				$src = FX_IMG_SERVER.$src;
		}
	...
	function CheckFile($arFile, $intMaxSize=0, $strMimeType=false, $strExt=false, $bForceMD5=false, $bSkipExt=false)
	{
		if($arFile["name"] == "")
		{
			return "";
...
function ImgShw(ID, width, height, alt)
{
	...
	function ResizeImage(&$arFile, $arSize, $resizeType = FX_RESIZE_IMAGE_PROPORTIONAL)
	{
	...
	function ResizeImageGet($file, $arSize, $resizeType = FX_RESIZE_IMAGE_PROPORTIONAL, $bInitSizes = false, $arFilters = false, $bImmediate = false, $jpgQuality = false)
	{
		...
		if ($resizeType !== FX_RESIZE_IMAGE_EXACT && $resizeType !== FX_RESIZE_IMAGE_PROPORTIONAL_ALT)
			$resizeType = FX_RESIZE_IMAGE_PROPORTIONAL;
			...
			if ($arSize["width"] > 0 && $arSize["height"] > 0)
			{
				switch ($resizeType)
				{
					case FX_RESIZE_IMAGE_EXACT:
						...
						break;
					default:
						if ($resizeType == FX_RESIZE_IMAGE_PROPORTIONAL_ALT)
						{
	...
	function ResizeImageFile($sourceFile, &$destinationFile, $arSize, $resizeType = FX_RESIZE_IMAGE_PROPORTIONAL, $arWaterMark = array(), $jpgQuality=false, $arFilters=false)
	{
		$io = CBXVirtualIo::GetInstance();
