	...
	implements IBXVirtualIO, IBXGetErrors
{
	...
	public static function ConvertCharset($string, $direction = 1, $skipEvents = false)
	{
		...
		{
			self::$systemEncoding = strtolower(defined("FX_FILE_SYSTEM_ENCODING") ? FX_FILE_SYSTEM_ENCODING : "");
		...
		if (is_null(self::$serverEncoding))
		{
			...
			elseif (defined("FX_DEFAULT_CHARSET"))
				self::$serverEncoding = FX_DEFAULT_CHARSET;
			else
		...
		include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/charset_converter.php");
	...
	function ValidatePathString($path)
	{
		...
		if(defined("FX_UTF") && !mb_check_encoding($path, "UTF-8"))
			return false;
	...
	function ValidateFilenameString($filename)
	{
		...
		if(defined("FX_UTF") && !mb_check_encoding($filename, "UTF-8"))
			return false;
	...
	private function CopyDirFiles($pathFrom, $pathTo, $bRewrite = true, $bDeleteAfterCopy = false)
	{
		...
		elseif (is_file($pathFromEncoded))
		{
			...
			{
				@chmod($pathToEncoded, FX_FILE_PERMISSIONS);
		...
		if ($handle = @opendir($pathFromEncoded))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				elseif (is_file($pathFromEncoded."/".$file))
				{
					...
					@chmod($pathToEncoded."/".$file, FX_FILE_PERMISSIONS);
	...
	extends CBXVirtualFile
{
	...
	{
		if ($this->IsExists())
			@chmod($this->GetPathWithNameEncoded(), FX_FILE_PERMISSIONS);
	}
	...
	extends CBXVirtualDirectory
{
	...
	public function Create()
	{
		if (!file_exists($this->GetPathWithNameEncoded()))
			return mkdir($this->GetPathWithNameEncoded(), FX_DIR_PERMISSIONS, true);
		else
	...
	{
		if ($this->IsExists())
			@chmod($this->GetPathWithNameEncoded(), FX_DIR_PERMISSIONS);
	}
