...
class CWizardUtil
{
	...
	function GetWizardList($filterNamespace = false, $bLoadFromModules = false)
	{
		...
		if ($bLoadFromModules)
		{
			$modulesPath = $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules";
	...
	function ReplaceMacros($filePath, $arReplace, $skipSharp = false)
	{
		...
		@chmod($filePath, FX_FILE_PERMISSIONS);
	...
	function ReplaceMacrosRecursive($filePath, $arReplace)
	{
		...
		if ($handle = @opendir($filePath))
		{
			while (($file = readdir($handle)) !== false)
			{
				if ($file == "." || $file == ".." || (trim($filePath, "/") == trim($_SERVER["DOCUMENT_ROOT"], "/") && ($file == "freetrix" || $file == "upload"))) 
					continue;
				...
				elseif (is_file($filePath."/".$file))
				{
					...
					@chmod($filePath."/".$file, FX_FILE_PERMISSIONS);
	...
	function GetModules()
	{
		...
		$handle=@opendir($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules");
		if($handle)
		{
			while (false !== ($dir = readdir($handle)))
			{
				if(is_dir($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$dir) && $dir!="." && $dir!="..")
				{
					$module_dir = $_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$dir;
