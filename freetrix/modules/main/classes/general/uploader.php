...
class CImageUploader
{
	public static function ShowScript($Params = array())
	{
		...
		$APPLICATION->AddHeadScript('/freetrix/image_uploader/bximageuploader.js');
		$APPLICATION->SetAdditionalCSS('/freetrix/image_uploader/bximageuploader.css');

		$APPLICATION->AddHeadScript('/freetrix/image_uploader/aurigma.uploader.js');
		$APPLICATION->AddHeadScript('/freetrix/image_uploader/aurigma.uploader.installationprogress.js');
		$id = self::GetId();
		...
		var bxp = {
			...
			activeXControl:
			{
				...
				codeBase: '/freetrix/image_uploader/ImageUploader7.cab',
				codeBase64: '/freetrix/image_uploader/ImageUploader7_x64.cab',
				progId: 'Freetrix.ImageUploader.7',
				version: '7.0.38.0'
			},
			javaControl:
			{
				className: 'com.freetrixsoft.imageuploader.ImageUploader',
				codeBase: '/freetrix/image_uploader/ImageUploader7.jar',
				version: '7.0.38.0'
			},
			metadata: {
	...
	{
		include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/image_uploader/ImageUploaderPHP/UploadHandler.class.php");
		//include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/image_uploader/ImageUploaderPHP/UploadedFile.class.php");
	}
	...
	public static function SetTmpPath($PackageGuid, $pathToTmp)
	{
		...
		self::$sTmpPath = $pathToTmp.md5(serialize(array("PackageGuid " => $PackageGuid, "sessid" => freetrix_sessid())));
	...
	public static function SetSavedData($savedData = array())
	{
		...
		$_SESSION['FX_PHOTO_TMP_SAVED_DATA'] = $savedData;
		return true;
	}
	...
	public static function GetSavedData()
	{
		...
		$savedData = $_SESSION['FX_PHOTO_TMP_SAVED_DATA'];
	...
	{
		unset($_SESSION['FX_PHOTO_TMP_SAVED_DATA']);
	}
...
class CFlashUploader extends CImageUploader
{
	public static function ShowScript($Params = array())
	{
		...
		$APPLICATION->AddHeadScript('/freetrix/image_uploader/bximageuploader.js');
		$APPLICATION->SetAdditionalCSS('/freetrix/image_uploader/bximageuploader.css');
		$APPLICATION->AddHeadScript('/freetrix/image_uploader/flash/aurigma.imageuploaderflash.js');
		$id = self::GetId();
		?>
		<script>
		...
		var bxp = {
			...
			converters: [],
			flashControl: {
				codeBase: '/freetrix/image_uploader/flash/aurigma.imageuploaderflash.swf',
				wmode: "opaque"
			},
			uploadSettings: {
				actionUrl: '<?= CUtil::JSEscape(self::StrangeUrlEncode($Params['actionUrl']))?>',
	...
	public static function InitUploaderHandler()
	{
		include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/image_uploader/ImageUploaderFlashPHP/UploadHandler.class.php");
	}
...
use \Freetrix\Main\UI\FileInputUtility;
class CFileUploader
{
	public $files = array();
	public $error = "";
...
/*		"allowUploadExt" => "",
	...
	function setIntoCache($hash, $data)
	{
		$io = CBXVirtualIo::GetInstance();
		$directory = $io->GetDirectory($this->path.$hash);
		if ($directory->Create())
	...
	{
		$request = \Freetrix\Main\Context::getCurrent()->getRequest();
	...
	public function getData($data)
	{
		array_walk_recursive($data, create_function('&$v,$k',
					'if($k=="error"){$v=preg_replace("/<(.+?)>/is".FX_UTF_PCRE_MODIFIER, "", $v);}'));
