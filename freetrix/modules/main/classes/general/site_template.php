...
class CSiteTemplate
{
	...
	function CheckFields($arFields, $ID=false)
	{
		...
		if($ID === false)
		{
			...
			elseif(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]))
	...
	function Add($arFields)
	{
		...
		CheckDirPath($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]);
		if(isset($arFields["CONTENT"]))
		{
			$p = strpos($arFields["CONTENT"], "#WORK_AREA#");
			$header = substr($arFields["CONTENT"], 0, $p);
			$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]."/header.php", $header);
			$footer = substr($arFields["CONTENT"], $p + strlen("#WORK_AREA#"));
			$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]."/footer.php", $footer);
		}
		if(isset($arFields["STYLES"]))
		{
			$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]."/styles.css", $arFields["STYLES"]);
		}

		if(isset($arFields["TEMPLATE_STYLES"]))
		{
			$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]."/template_styles.css", $arFields["TEMPLATE_STYLES"]);
		}

		if(isset($arFields["NAME"]) || isset($arFields["DESCRIPTION"]) || isset($arFields["SORT"]))
		{
			$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]."/description.php",
				'<'.'?'.
				'$arTemplate = array('."\n".
		...
		if(isset($arFields["STYLES_DESCRIPTION"]) && is_array($arFields["STYLES_DESCRIPTION"]))
		{
			...
			$str .= ");\n".'?'.'>';
			$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$arFields["ID"]."/.styles.php", $str);
		}
	...
	function Update($ID, $arFields)
	{
		...
		$path = getLocalPath("templates/".$ID, FX_PERSONAL_ROOT);
		if($path === false)
		{
			return false;
		}
	...
	function DirsRecursive($ID, $path="", $depth=0, $maxDepth=1)
	{
		...
		$templPath = getLocalPath("templates/".$ID, FX_PERSONAL_ROOT);
		if($templPath === false)
		{
			return $arRes;
		}
