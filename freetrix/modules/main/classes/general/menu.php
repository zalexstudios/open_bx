...
class CMenu
{
	...
	function RecalcMenu($bMultiSelect = false, $bCheckSelected = true)
	{
		...
		else
		{
			...
			elseif(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/".LANG."/".$this->type.".menu_template.php"))
			{
				$this->template = FX_PERSONAL_ROOT."/php_interface/".LANG."/".$this->type.".menu_template.php";
				$this->MenuTemplate = $_SERVER["DOCUMENT_ROOT"].$this->template;
			}
			else
			{
				$this->template = FX_PERSONAL_ROOT."/templates/.default/".$this->type.".menu_template.php";
				$this->MenuTemplate = $_SERVER["DOCUMENT_ROOT"].$this->template;
			}
		}
	...
	function GetMenuHtmlEx()
	{
		...
		if($APPLICATION->GetShowIncludeAreas())
		{
			...
			{
				$arIcons[] = array(
					"URL"=>"/freetrix/admin/fileman_menu_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&path=".urlencode($this->MenuDir)."&name=".$this->type,
					"ICON"=>"menu-edit",
			...
			{
				$arIcons[] = array(
					"URL"=>"/freetrix/admin/fileman_file_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&full_src=Y&path=".urlencode($this->template),
					"ICON"=>"menu-template",
...
global $FX_MENU_CUSTOM;
$FX_MENU_CUSTOM = new CMenuCustom;
