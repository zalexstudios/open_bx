...
class CComponentAjax
{
	...
	function CComponentAjax($componentName, $componentTemplate, &$arParams, $parentComponent)
	{
		...
		if ($USER->IsAdmin())
		{
			if ($_GET['freetrix_disable_ajax'] == 'N')
			{
				unset($_SESSION['freetrix_disable_ajax']);
			}

			if ($_GET['freetrix_disable_ajax'] == 'Y' || $_SESSION['freetrix_disable_ajax'] == 'Y')
			{
				$_SESSION['freetrix_disable_ajax'] = 'Y';
				return null;
			}
		}
	...
	function LocalRedirectHandler(&$url)
	{
		...
		{
			if (!$this->bIFrameMode)
				Header('X-Freetrix-Ajax-Status: OK');
		}
		else
		{
			...
			if (!$this->bIFrameMode)
				Header('X-Freetrix-Ajax-Status: Redirect');
	...
	function __isAjaxURL($url)
	{
		...
		if ($this->arParams['SEF_MODE'] == 'Y')
		{
			...
			$test_str = '/freetrix/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=';
	...
	function __PrepareLinks(&$data)
	{
		...
		$regexp_links = '/(<a[^>]*?>.*?<\/a>)/is'.FX_UTF_PCRE_MODIFIER;
		$regexp_params = '/([\w]+)\s*=\s*([\"\'])(.*?)\2/is'.FX_UTF_PCRE_MODIFIER;
		...
		{
			if(!preg_match('/^<a([^>]*?)>(.*?)<\/a>$/is'.FX_UTF_PCRE_MODIFIER, $arData[$iData], $match))
				continue;
	...
	{
		$this->_checkPcreLimit($data);
		$arData = preg_split('/(<form([^>]*)>)/i'.FX_UTF_PCRE_MODIFIER, $data, -1, PREG_SPLIT_DELIM_CAPTURE);
	...
	{
		$regexp = '/(<script(?:[^>]*)?>)(.*?)<\/script>/is'.FX_UTF_PCRE_MODIFIER;
...
";
	...
	function _PrepareAdditionalData()
	{
		...
		{
			$additional_data .= 'top.BX.ajax.history.put(window.AJAX_PAGE_STATE.getState(), \''.CUtil::JSEscape(CAjax::encodeURI($APPLICATION->GetCurPageParam('', array(FX_AJAX_PARAM_ID), false))).'\')'.";\r\n";
		}
	...
	function _PrepareData()
	{
		...
		if (!$this->bAjaxSession)
		{
			...
			{
				$data =
					'<script type="text/javascript">if (window.location.hash != \'\' && window.location.hash != \'#\') top.BX.ajax.history.checkRedirectStart(\''.CUtil::JSEscape(FX_AJAX_PARAM_ID).'\', \''.CUtil::JSEscape($this->componentID).'\')</script>'
					.$data
					.'<script type="text/javascript">if (top.BX.ajax.history.bHashCollision) top.BX.ajax.history.checkRedirectFinish(\''.CUtil::JSEscape(FX_AJAX_PARAM_ID).'\', \''.CUtil::JSEscape($this->componentID).'\');</script>'
