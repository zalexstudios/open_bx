 ...
 * @author Freetrix <support@freetrixsoft.com>
 * @version 1.0
 * @package usertype
...
class CAllUserTypeManager
{
	...
	*/
	...
	function EditFormShowTab($entity_id, $bVarsFromForm, $ID)
	{
		...
		{
			echo "<tr colspan=\"2\"><td align=\"left\"><a href=\"/freetrix/admin/userfield_edit.php?lang=".LANG."&ENTITY_ID=".urlencode($entity_id)."&back_url=".urlencode($APPLICATION->GetCurPageParam()."&tabControl_active_tab=user_fields_tab")."\">".GetMessage("USER_TYPE_EDIT_TAB_HREF")."</a></td></tr>";
		}
	...
	function GetEditFormHTML($bVarsFromForm, $form_value, $arUserField)
	{
		...
		if($arUserField["USER_TYPE"])
		{
			if($this->GetRights($arUserField["ENTITY_ID"]) >= "W")
				$edit_link = ($arUserField["HELP_MESSAGE"]? htmlspecialcharsex($arUserField["HELP_MESSAGE"]).'<br>': '').'<a href="'.htmlspecialcharsbx('/freetrix/admin/userfield_edit.php?lang='.LANG.'&ID='.$arUserField["ID"].'&back_url='.urlencode($APPLICATION->GetCurPageParam().'&tabControl_active_tab=user_fields_tab')).'">'.htmlspecialcharsex(GetMessage("MAIN_EDIT")).'</a>';
			else
				$edit_link = '';
