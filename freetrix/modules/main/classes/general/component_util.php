...
class CComponentUtil
{
								...
								if (array_key_exists("PATH", $arComponentDescription) && array_key_exists("ID", $arComponentDescription["PATH"]))
								{
									...
									if (array_key_exists("ICON", $arComponentDescription))
									{
										...
										if($arComponentDescription["ICON"] != "" && $io->FileExists($io->RelativeToAbsolutePath($componentFolder."/".$file."/".$arComponentDescription["ICON"])))
											...
											$arComponent["ICON"] = "/freetrix/images/fileman/htmledit2/component.gif";
									}
	...
	function GetComponentProps($componentName, $arCurrentValues = array(), $templateProperties = array())
	{
					...
					foreach ($arSavedParams as $keyTmp => $valueTmp)
					{
						if ($keyTmp == "CACHE_TIME")
						{
							...
							$arComponentParameters["PARAMETERS"]["CACHE_NOTES"] = array(
								"PARENT" => "CACHE_SETTINGS",
								"TYPE" => "CUSTOM",
								"JS_FILE" => "/freetrix/js/main/comp_props.js",
								...
								"JS_DATA" => GetMessage("COMP_PROP_CACHE_NOTE", array(
									...
									"#MANAGED_MODE#" =>(defined("FX_COMP_MANAGED_CACHE")? GetMessage("COMP_PARAM_CACHE_MANAGED_ON"):GetMessage("COMP_PARAM_CACHE_MANAGED_OFF")),
								)),
							);
						}
						else
	...
	function GetTemplateProps($componentName, $templateName, $siteTemplate = "", $arCurrentValues = array())
	{
		...
		if ($siteTemplate <> "")
		{
			$folders[] = FX_PERSONAL_ROOT."/templates/".$siteTemplate."/components".$path2Comp."/".$templateName;
		}
		$folders[] = FX_PERSONAL_ROOT."/templates/.default/components".$path2Comp."/".$templateName;
		$folders[] = "/freetrix/components".$path2Comp."/templates/".$templateName;
	...
	function CopyTemplate($componentName, $templateName, $siteTemplate, $newSiteTemplate, $newName = false, $bRewrite = false)
	{
		...
		{
			$siteTemplateDir = getLocalPath("templates/".$siteTemplate, FX_PERSONAL_ROOT);
		...
		if ($siteTemplate != false)
			$path = getLocalPath("templates/".$siteTemplate."/components".$path2Comp."/".$templateName, FX_PERSONAL_ROOT);
		else
		...
		$newSiteTemplateDir = getLocalPath("templates/".$newSiteTemplate, FX_PERSONAL_ROOT);
