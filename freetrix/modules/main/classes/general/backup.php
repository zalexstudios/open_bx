...
class CBackup
{
	...
	function ignorePath($path)
	{
		...
		if (!$ignore_path)
			$ignore_path = array(
				FX_PERSONAL_ROOT."/cache",
				FX_PERSONAL_ROOT."/cache_image",
				FX_PERSONAL_ROOT."/managed_cache",
				FX_PERSONAL_ROOT."/managed_flags",
				FX_PERSONAL_ROOT."/stack_cache",
				FX_PERSONAL_ROOT."/html_pages",
				FX_PERSONAL_ROOT."/tmp",
				FX_ROOT."/tmp",
				FX_ROOT."/help",
				FX_ROOT."/updates",
		...
		{
			$clouds = DOCUMENT_ROOT_SITE.FX_ROOT.'/backup/clouds/';
			if (strpos($path, $clouds) === 0 || strpos($clouds, $path) === 0)
				return false;
		}
		
		## Backups
		if (strpos($path, DOCUMENT_ROOT_SITE.FX_ROOT.'/backup/') === 0)
			return true;
		...
		if (strpos(DOCUMENT_ROOT_SITE.FX_ROOT, $path) !== false) // на пути к /freetrix
			return false;

		if (strpos($path, DOCUMENT_ROOT_SITE.FX_ROOT) === false) // за пределами /freetrix 
			return !$dump_file_public;

		$path_root = substr($path, strlen(DOCUMENT_ROOT_SITE));
		if (preg_match('#^/freetrix/(.settings.php|php_interface|templates)/([^/]*)#',$path_root.'/',$regs))
			return !$dump_file_public;
	
		if (preg_match('#^/freetrix/(activities|components|gadgets|wizards)/([^/]*)#',$path_root.'/',$regs))
		{
			if (!$regs[2])
				return false;
			if ($regs[2] == 'freetrix')
		...
		// всё остальное в папке freetrix - ядро
		return !$dump_file_kernel;
	}
	...
	{
		$arc_name = DOCUMENT_ROOT.FX_ROOT."/backup/".date("Ymd_His");
...
class CDirRealScan extends CDirScan
{
	function ProcessFile($f)
	{
		...
		{
			$f = str_replace('\\', '/', $f);
			if (preg_match('#/freetrix/(php_interface/dbconn.php|.settings.php)$#', $f, $regs))
			{
...
class CPasswordStorage
{
	...
	function getEncryptKey()
	{
		...
		if (!$LICENSE_KEY)
		{
			if (file_exists($file = $_SERVER['DOCUMENT_ROOT'].FX_ROOT.'/license_key.php'))
				include($file);
			if (!$LICENSE_KEY)
				$LICENSE_KEY = 'DEMO';
		}
	...
	function Scan($path)
	{
		...
		if ($arCloudFiles = CBackup::GetBucketFileList($this->id, $path))
		{
			foreach($arCloudFiles['file'] as $k=>$file)
			{
				...
				$HTTP = new CHTTP;
				if ($HTTP->Download($this->obBucket->GetFileSRC(array("URN" => $name)), DOCUMENT_ROOT.FX_ROOT.'/backup/clouds/'.$this->id.$name))
				{
