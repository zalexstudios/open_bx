...
define("FX_WIZARD_WELCOME_ID", "__welcome");
define("FX_WIZARD_LICENSE_ID", "__license");
define("FX_WIZARD_SELECT_SITE_ID", "__select_site");
define("FX_WIZARD_SELECT_GROUP_ID", "__select_group");
define("FX_WIZARD_SELECT_TEMPLATE_ID", "__select_template");
define("FX_WIZARD_SELECT_SERVICE_ID", "__select_service");
define("FX_WIZARD_SELECT_STRUCTURE_ID", "__select_structure");
define("FX_WIZARD_START_INSTALL_ID", "__start_install");
define("FX_WIZARD_INSTALL_SITE_ID", "__install_site");
define("FX_WIZARD_INSTALL_TEMPLATE_ID", "__install_template");
define("FX_WIZARD_INSTALL_SERVICE_ID", "__install_service");
define("FX_WIZARD_INSTALL_STRUCTURE_ID", "__install_structure");
define("FX_WIZARD_FINISH_ID", "__finish");
define("FX_WIZARD_CANCEL_ID", "__install_cancel");
...
class CWizard
{
	...
	function Install()
	{
		...
		elseif ($this->pathToScript)
		{
			...
			{
				$lang = LANGUAGE_ID;
				$wizardPath = $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main";
	...
	function __Install()
	{
		...
		$wizard->AddStep($step, FX_WIZARD_WELCOME_ID);
		$this->_SetNextStep($step, FX_WIZARD_WELCOME_ID, "select");
		$this->__SetStepDescription($step, "WELCOME");
		$step->SetCancelStep(FX_WIZARD_CANCEL_ID);
		...
		if ($this->_InitSubStep("static", $this->arDescription["STEPS_SETTINGS"]["WELCOME"]) )
		{
			...
			$this->_SetNextStep($this->__obLastStep, FX_WIZARD_WELCOME_ID, "select");
			$this->__obFirstStep->SetPrevStep(FX_WIZARD_WELCOME_ID);
		...
		if ($this->licenseExists)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_LICENSE_ID);
			$this->_SetNextStep($step, FX_WIZARD_LICENSE_ID, "select");
			...
			{
				$lastStepID = $this->__obLastStep->GetStepID();
				$this->_SetNextStep($this->__obLastStep, FX_WIZARD_LICENSE_ID, "select");
				$this->__obFirstStep->SetPrevStep(FX_WIZARD_LICENSE_ID);
				$step->SetNextStep($this->__obFirstStep->GetStepID());
			}
		}
		...
		if ($this->siteExists)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_SELECT_SITE_ID);
			$this->_SetNextStep($step, FX_WIZARD_SELECT_SITE_ID, "select");
			...
			{
				$lastStepID = $this->__obLastStep->GetStepID();
				$this->_SetNextStep($this->__obLastStep, FX_WIZARD_SELECT_SITE_ID, "select");
				$this->__obFirstStep->SetPrevStep(FX_WIZARD_SELECT_SITE_ID);
				$step->SetNextStep($this->__obFirstStep->GetStepID());
			}
		}
		...
		if ($this->groupExists && $this->templateExists)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_SELECT_GROUP_ID);
			$this->_SetNextStep($step, FX_WIZARD_SELECT_GROUP_ID, "select");
		...
		if ($this->templateExists)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_SELECT_TEMPLATE_ID);
			$this->_SetNextStep($step, FX_WIZARD_SELECT_TEMPLATE_ID, "select");
			...
			{
				$lastStepID = $this->__obLastStep->GetStepID();
				$this->_SetNextStep($this->__obLastStep, FX_WIZARD_SELECT_TEMPLATE_ID, "select");
				$this->__obFirstStep->SetPrevStep(FX_WIZARD_SELECT_TEMPLATE_ID);
				$step->SetNextStep($this->__obFirstStep->GetStepID());
			}
		}
		...
		if ($this->serviceExists)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_SELECT_SERVICE_ID);
			$this->_SetNextStep($step, FX_WIZARD_SELECT_SERVICE_ID, "select");
			...
			if ($this->serviceSelected)
			{
				foreach ($arServices as $service)
				{
					...
					if ($this->_InitSubStep("select", $this->arServices[$service]))
					{
						...
						$this->_SetNextStep($this->__obLastStep, FX_WIZARD_SELECT_SERVICE_ID, "select");
						$lastStepID = $this->__obLastStep->GetStepID();
		...
		if ($this->structureExists)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_SELECT_STRUCTURE_ID);
			$this->_SetNextStep($step, FX_WIZARD_SELECT_STRUCTURE_ID, "select");
			...
			{
				$lastStepID = $this->__obLastStep->GetStepID();
				$this->_SetNextStep($this->__obLastStep, FX_WIZARD_SELECT_STRUCTURE_ID, "select");
				$this->__obFirstStep->SetPrevStep(FX_WIZARD_SELECT_STRUCTURE_ID);
				$step->SetNextStep($this->__obFirstStep->GetStepID());
			}
		}
		...
		$wizard->AddStep($step, FX_WIZARD_START_INSTALL_ID);
		$step->SetPrevStep($lastStepID);
		$step->SetCancelStep(FX_WIZARD_CANCEL_ID);
		$this->__SetStepDescription($step, "START_INSTALL");
		$this->_SetNextStep($step, FX_WIZARD_START_INSTALL_ID, "install");

		//#NEW
		if ($this->_InitSubStep("static", $this->arDescription["STEPS_SETTINGS"]["START_INSTALL"]) )
		{
			$lastStepID = $this->__obLastStep->GetStepID();
			$this->_SetNextStep($this->__obLastStep, FX_WIZARD_START_INSTALL_ID, "install");
			$this->__obFirstStep->SetPrevStep(FX_WIZARD_START_INSTALL_ID);
			$step->SetNextStep($this->__obFirstStep->GetStepID());
		}
		//
		...
		if ($this->siteSelected)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_INSTALL_SITE_ID);
			$this->_SetNextStep($step, FX_WIZARD_INSTALL_SITE_ID, "install");
			$this->__SetStepDescription($step, "INSTALL_SITE");

			if ($this->_InitSubStep("install", $this->arSites[$siteID]))
			{
				$this->_SetNextStep($this->__obLastStep, FX_WIZARD_INSTALL_SITE_ID, "install");
				$step->SetNextStep($this->__obFirstStep->GetStepID());
			}
		}
		...
		if ($this->templateSelected)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_INSTALL_TEMPLATE_ID);
			$this->_SetNextStep($step, FX_WIZARD_INSTALL_TEMPLATE_ID, "install");
			$this->__SetStepDescription($step, "INSTALL_TEMPLATE");

			if ($this->_InitSubStep("install", $this->arTemplates[$templateID]))
			{
				$this->_SetNextStep($this->__obLastStep, FX_WIZARD_INSTALL_TEMPLATE_ID, "install");
				$step->SetNextStep($this->__obFirstStep->GetStepID());
			}
		}
		...
		if ($this->serviceSelected)
		{
			...
			foreach ($arServices as $service)
			{
				...
				if ($obLastStep !== null)
					$obLastStep->SetNextStep(FX_WIZARD_INSTALL_SERVICE_ID.$number);

				$step = new CPackageInstallService($this, $service);
				$wizard->AddStep($step, FX_WIZARD_INSTALL_SERVICE_ID.$number);
				...
				if ($this->_InitSubStep("install", $this->arServices[$service]))
				{
					...
					$this->_SetNextStep($this->__obLastStep, FX_WIZARD_INSTALL_SERVICE_ID, "install");
				...
				else
				{
					...
					$this->_SetNextStep($step, FX_WIZARD_INSTALL_SERVICE_ID, "install");
				}
		...
		if ($this->structureSelected)
		{
			...
			$wizard->AddStep($step, FX_WIZARD_INSTALL_STRUCTURE_ID);
			$this->_SetNextStep($step, FX_WIZARD_INSTALL_STRUCTURE_ID, "install");
			$this->__SetStepDescription($step, "INSTALL_STRUCTURE");

			//#NEW
			if ($this->_InitSubStep("install", $this->arStructure["SETTINGS"]))
			{
				$this->_SetNextStep($this->__obLastStep, FX_WIZARD_INSTALL_STRUCTURE_ID, "install");
				$step->SetNextStep($this->__obFirstStep->GetStepID());
			}
		}
		...
		$wizard->AddStep($step, FX_WIZARD_FINISH_ID);
		$this->__SetStepDescription($step, "FINISH");

		if (!$isUserStep)
			$step->SetCancelStep(FX_WIZARD_FINISH_ID);

		//#NEW
		if ($this->_InitSubStep("end", $this->arDescription["STEPS_SETTINGS"]["FINISH"]) )
		{
			$this->__obFirstStep->SetPrevStep(FX_WIZARD_FINISH_ID);
			$step->SetNextStep($this->__obFirstStep->GetStepID());
		}
		...
		$wizard->AddStep($step, FX_WIZARD_CANCEL_ID);
		...
		{
			$this->__obFirstStep->SetPrevStep(FX_WIZARD_CANCEL_ID);
			$step->SetNextStep($this->__obFirstStep->GetStepID());
		}
	...
	function __SetTemplate()
	{
		...
		foreach ($this->arDescription["TEMPLATES"] as $arTemplate)
		{
			if($arTemplate["SCRIPT"]=="wizard_sol")
			{
				...
				include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/install/wizard_sol/template.php");
	...
	function _SetNextStep($obStep, $currentStep, $stepType = "select")
	{
		if ($stepType == "select")
			$arWizardStep = Array(
				FX_WIZARD_WELCOME_ID => true,
				FX_WIZARD_LICENSE_ID => $this->licenseExists,
				FX_WIZARD_SELECT_SITE_ID => $this->siteExists,
				FX_WIZARD_SELECT_GROUP_ID => ($this->groupExists && $this->templateExists),
				FX_WIZARD_SELECT_TEMPLATE_ID => $this->templateExists,
				FX_WIZARD_SELECT_SERVICE_ID => $this->serviceExists,
				FX_WIZARD_SELECT_STRUCTURE_ID => $this->structureExists,
				FX_WIZARD_START_INSTALL_ID => true,
			);

		else
			$arWizardStep = Array(
				FX_WIZARD_START_INSTALL_ID => true,
				FX_WIZARD_INSTALL_SITE_ID => $this->siteSelected,
				FX_WIZARD_INSTALL_TEMPLATE_ID => $this->templateSelected,
				FX_WIZARD_INSTALL_SERVICE_ID => $this->serviceSelected,
				FX_WIZARD_INSTALL_STRUCTURE_ID => $this->structureSelected,
				FX_WIZARD_FINISH_ID => true,
			);
	...
	function _InitSubStep($stepType, &$arInstallation, $bInitStep = true)
	{
		...
		foreach ($arSteps["STEPS"] as $stepID => $stepClass)
		{
			...
			if ($stepType == "select")
			{
				$subStep->SetCancelStep(FX_WIZARD_CANCEL_ID);
			}
