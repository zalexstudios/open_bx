...
class CUndo
{
	...
	function ShowUndoMessage($ID)
	{
		$_SESSION['FX_UNDO_ID'] = $ID;
	}

	function CheckNotifyMessage()
	{
		global $USER, $APPLICATION;
		if(!is_array($_SESSION) || !array_key_exists("FX_UNDO_ID", $_SESSION))
			return;

		$ID = $_SESSION['FX_UNDO_ID'];
		unset($_SESSION['FX_UNDO_ID']);
...
window.BXUndoLastChanges = function()
{
	...
	BX.ajax.get(\"/freetrix/admin/public_undo.php?undo=".$ID."&".freetrix_sessid_get()."\", null, function(result)
	{
...
<?
	...
	public static function Allowed()
	{
		...
		if (self::$bAllowed == null)
		{
			...
			self::$bAllowed = $arOpt['autosave'] != 'N' && $APPLICATION->GetCurPage() != '/freetrix/admin/update_system.php';
		}
