...
class CTextParser
{
	...
	protected function initSmilePatterns()
	{
		...
		foreach ($this->smiles as $row)
		{
			if($row["TYPING"] <> '' && $row["IMAGE"] <> '')
			{
				...
				$this->smilePatterns[] = "/(?<=".$pre.")".$patt."(?=.\\W|\\W.|\\W$)/ei".FX_UTF_PCRE_MODIFIER;
	...
	{
		$text = preg_replace(array("#([?&;])PHPSESSID=([0-9a-zA-Z]{32})#is", "/\\x{00A0}/".FX_UTF_PCRE_MODIFIER), array("\\1PHPSESSID1=", " "), $text);
		...
		{
			$text = preg_replace(
				"#(\\[code(?:\\s+[^\\]]*\\]|\\]))(.+?)(\\[/code(?:\\s+[^\\]]*\\]|\\]))#ise".FX_UTF_PCRE_MODIFIER,
		...
		if ($this->allow["HTML"] != "Y")
		{
			if ($this->allow["ANCHOR"]=="Y")
			{
				$text = preg_replace(
					array(
						"#<a[^>]+href\\s*=\\s*('|\")(.+?)(?:\\1)[^>]*>(.*?)</a[^>]*>#is".FX_UTF_PCRE_MODIFIER,
						"#<a[^>]+href(\\s*=\\s*)([^'\">]+)>(.*?)</a[^>]*>#is".FX_UTF_PCRE_MODIFIER),
					"[url=\\2]\\3[/url]", $text
				);
			}
			if ($this->allow["BIU"]=="Y")
			{
				...
				{
					$text = preg_replace(
						"/<([busi])[^>a-z]*>(.+?)<\\/(\\1)[^>a-z]*>/is".FX_UTF_PCRE_MODIFIER,
						"[\\1]\\2[/\\1]",
					$text, -1, $replaced);
				}
				while($replaced > 0);
			...
			{
				$text = preg_replace(
					"#<img[^>]+src\\s*=[\\s'\"]*(((http|https|ftp)://[.-_:a-z0-9@]+)*(\\/[-_/=:.a-z0-9@{}&?%]+)+)[\\s'\"]*[^>]*>#is".FX_UTF_PCRE_MODIFIER,
					"[img]\\1[/img]", $text
				);
			}
			if ($this->allow["FONT"]=="Y")
			{
				$text = preg_replace(
					array(
						"/\\<font[^>]+size\\s*=[\\s'\"]*([0-9]+)[\\s'\"]*[^>]*\\>(.+?)\\<\\/font[^>]*\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<font[^>]+color\\s*=[\\s'\"]*(\\#[a-f0-9]{6})[^>]*\\>(.+?)\\<\\/font[^>]*>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<font[^>]+face\\s*=[\\s'\"]*([a-z\\s\\-]+)[\\s'\"]*[^>]*>(.+?)\\<\\/font[^>]*>/is".FX_UTF_PCRE_MODIFIER
					),
					array(
						"[size=\\1]\\2[/size]",
						"[color=\\1]\\2[/color]",
			...
			if ($this->allow["TABLE"]=="Y")
			{
				$text = preg_replace(
					array(
						"/\\<table((\\s[^>]*)|(\\s*))\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<\\/table([^>]*)\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<tr((\\s[^>]*)|(\\s*))\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<\\/tr([^>]*)\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<td((\\s[^>]*)|(\\s*))\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<\\/td([^>]*)\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<th((\\s[^>]*)|(\\s*))\\>/is".FX_UTF_PCRE_MODIFIER,
						"/\\<\\/th([^>]*)\\>/is".FX_UTF_PCRE_MODIFIER,
					),
					array(
						"[table]",
						"[/table]",
		...
		if ($this->allow["ANCHOR"]=="Y")
		{
			$text = preg_replace(
				array(
						"/\\[url\\](.*?)\\[\\/url\\]/ie".FX_UTF_PCRE_MODIFIER,
						"/\\[url\\s*=\\s*(
							(?:
								[^\\[\\]]++
								|\\[ (?: (?>[^\\[\\]]+) | (?:\\1) )* \\]
							)+
							)\\s*\\](.*?)\\[\\/url\\]/iexs".FX_UTF_PCRE_MODIFIER,
					),
				array(
			...
			$text = preg_replace(
				"/(?<=^|[".$word_separator."]|\\s)(?<!\\[nomodify\\]|<nomodify>)((".$this->getAnchorSchemes()."):\\/\\/[._:a-z0-9@-].*?)(?=[\\s'\"{}\\[\\]]|&quot;|\$)/is".FX_UTF_PCRE_MODIFIER,
				"[url]\\1[/url]", $text
			);
		...
		$text = preg_replace("/<\\/?nomodify>/i".FX_UTF_PCRE_MODIFIER, "", $text);
		...
		foreach ($this->allow as $tag => $val)
		{
			...
			{
				$text = preg_replace(
					"/<nomodify>(.*?)<\\/nomodify>/ies".FX_UTF_PCRE_MODIFIER,
			...
			switch ($tag)
			{
				case "CODE":
					$text = preg_replace(
						"/\\[code[^\\]]*\\](.*?)\\[\\/code[^\\]]*\\]/ies".FX_UTF_PCRE_MODIFIER,
				...
				case "VIDEO":
					...
					break;
				case "IMG":
					$text = preg_replace("/\\[img([^\\]]*)\\](.+?)\\[\\/img\\]/ies".FX_UTF_PCRE_MODIFIER, "\$this->convert_image_tag('\\2', '\\1')", $text);
					break;
				case "ANCHOR":
					$arUrlPatterns = array(
						"/\\[url\\](.*?)\\[\\/url\\]/ie".FX_UTF_PCRE_MODIFIER,
				...
				case "BIU":
					...
					{
						$text = preg_replace(
							"/\\[([busi])\\](.+?)\\[\\/(\\1)\\]/is".FX_UTF_PCRE_MODIFIER,
							"<\\1>\\2</\\1>",
						$text, -1, $replaced);
					}
					while($replaced > 0);
					break;
				case "LIST":
					while (preg_match("/\\[list\\s*=\\s*(1|a)\\s*\\](.+?)\\[\\/list\\]/is".FX_UTF_PCRE_MODIFIER, $text))
					{
						$text = preg_replace(
							array(
				...
				case "ALIGN":
					...
					do
					{
						$text = preg_replace(
							array(
								...
								"/\\[justify\\](.*?)\\[\\/justify\\]/is".FX_UTF_PCRE_MODIFIER,
							),
							array(
					...
					break;
				case "QUOTE":
					while (preg_match("/\\[quote[^\\]]*\\](.*?)\\[\\/quote[^\\]]*\\]/ies".FX_UTF_PCRE_MODIFIER, $text))
					{
						$text = preg_replace(
							"/\\[quote[^\\]]*\\](.*?)\\[\\/quote[^\\]]*\\]/ies".FX_UTF_PCRE_MODIFIER,
			...
			foreach($this->allow["USERFIELDS"] as $userField)
			{
					...
					method_exists($userField["USER_TYPE"]["CLASS_NAME"], "GetPublicViewHTML") )
				{
					...
					$text = preg_replace_callback(
						"/\\[".$userField["TAG"]."\\s*=\\s*([0-9]+)([^\\]]*)\\]/is".FX_UTF_PCRE_MODIFIER,
		...
		if ($this->allow["HTML"] != "Y" || $this->allow['NL2BR'] == 'Y')
		{
			...
			$text = preg_replace(array(
				"/\\<br \\/\\>(\\<\\/table[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<thead[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<\\/thead[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<tfoot[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<\\/tfoot[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<tbody[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<\\/tbody[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<tr[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<\\/tr[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<td[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
				"/\\<br \\/\\>(\\<\\/td[^>]*\\>)/is".FX_UTF_PCRE_MODIFIER,
			),
			"\\1", $text);
		}
		...
		{
			$text = preg_replace(
				"/<nosmile>(.*?)<\\/nosmile>/ies".FX_UTF_PCRE_MODIFIER,
				"\$this->defended_tags('\\1', 'replace')",
			$text);
		}
	...
	function convert4mail($text)
	{
		...
		$arPattern[] = "/\\[(\\/?)(color|font|size|left|right|center)([^\\]]*)\\]/is".FX_UTF_PCRE_MODIFIER;
		$arReplace[] = "";
		...
		$arPattern[] = "/\\[user([^\\]]*)\\](.+?)\\[\\/user\\]/is".FX_UTF_PCRE_MODIFIER;
		$arReplace[] = "\\2";

		$arPattern[] = "/\\[DOCUMENT([^\\]]*)\\]/is".FX_UTF_PCRE_MODIFIER;
		$arReplace[] = "";
	...
	function convert_video($params, $path)
	{
		...
		preg_match("/preview\\='([^']+)'/is".FX_UTF_PCRE_MODIFIER, $params, $preview);
		if (empty($preview))
			preg_match("/preview\\=\"([^\"]+)\"/is".FX_UTF_PCRE_MODIFIER, $params, $preview);
	...
	function convert_emoticon($code = "", $image = "", $description = "", $width = "", $height = "", $descriptionDecode = false)
	{
		...
		{
			$this->preg["cache"][$cacheKey] = "<\018#".$this->preg["counter"].">";
			$this->preg["pattern"][] = $this->preg["cache"][$cacheKey];
			$this->preg["replace"][] = $html;
			$this->preg["counter"]++;
		}
	...
	function pre_convert_code_tag ($text = "")
	{
		...
		$text = preg_replace(
			"/(?<=^|[^a-z])((".$this->getAnchorSchemes()."):\\/\\/[._:a-z0-9@-].*?)(?=[\\s'\"\\[\\]{}]|&quot;|\\[\\/?nomodify\\]|<\\/?nomodify>)/is".FX_UTF_PCRE_MODIFIER,
			"[nomodify]\\1[/nomodify]",
			$text
		);
		return $text;
	...
	function convert_image_tag($url = "", $params = "")
	{
		...
		if(strlen($this->serverName) <= 0 || preg_match("/^(http|https|ftp)\\:\\/\\//i".FX_UTF_PCRE_MODIFIER, $url))
	...
	function convert_userfields($matches)
	{
		...
		$id = intval($matches[1]);
		if ($id > 0 && in_array($id, $userField["VALUE"]))
		{
			...
			return call_user_func_array(
				...
				array($userField, $id, $matches[2], $vars)
			);
		}
		return $matches[0];
	}
	...
	function part_long_words($str)
	{
		...
		if (($this->MaxStringLen > 0) && (strlen(trim($str)) > 0))
		{
			...
			$str = preg_replace(
				"/(?<=[".$word_separator."]|^)(([^".$word_separator."]+))(?=[".$word_separator."]|$)/ise".FX_UTF_PCRE_MODIFIER,
	...
	function convert_anchor_tag($url, $text, $pref="")
	{
		...
		if (substr($url, 0, 1) != "/" && !preg_match("/^(".$this->getAnchorSchemes().")\\:/i".FX_UTF_PCRE_MODIFIER, $url))
			$url = "http://".$url;
		...
		if ($bShortUrl && strlen($text) < $this->MaxAnchorLength)
			$bShortUrl = false;
		if ($bShortUrl && !preg_match("/^(".$this->getAnchorSchemes()."):\\/\\//i".FX_UTF_PCRE_MODIFIER, $text))
			$bShortUrl = false;

		if($bTextUrl)
		{
			$lastInd = substr($url, -1);
			if(in_array($lastInd, array(")", "”", "\"", "'")))
			{
				$url = substr($url, 0, -1);
		...
		if ($bShortUrl)
		{
			preg_match("/^(".$this->getAnchorSchemes()."):\\/\\/(\\S+)$/i".FX_UTF_PCRE_MODIFIER, $text, $matches);
			$uri_type = $matches[1];
			$stripped = $matches[2];
	...
	function pre_convert_anchor_tag($url, $text = "")
	{
		...
		{
			$this->defended_urls[$tag] = "[url][nomodify]".$url."[/nomodify][/url]";
		}
		return $tag;
	}
	...
	function post_convert_anchor_tag($str)
	{
		if (!empty($this->defended_urls))
			return str_replace(array_keys($this->defended_urls), array_values($this->defended_urls), $str);
		else
			return $str;
	}
	...
	function TextParserConvertVideo($arParams)
	{
		...
		$APPLICATION->IncludeComponent(
			...
			array(
				...
				"LOGO" => "",
				"FULLSCREEN" => "Y",
				"SKIN_PATH" => "/freetrix/components/freetrix/player/mediaplayer/skins",
				"SKIN" => "freetrix.swf",
				"CONTROLBAR" => "bottom",
				"WMODE" => "transparent",
				"HIDE_MENU" => "N",
