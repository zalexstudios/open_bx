...
class CPHPCacheMemcache implements ICacheBackend
{
	...
	function CPHPCacheMemcache()
	{
		...
		if(!defined("FX_MEMCACHE_CONNECTED"))
		{
			if(self::$obMemcache->connect(FX_MEMCACHE_HOST, $port))
			{
				define("FX_MEMCACHE_CONNECTED", true);
				register_shutdown_function(array("CPHPCacheMemcache", "close"));
			}
		}

		if(defined("FX_CACHE_SID"))
			$this->sid = FX_CACHE_SID;
		else
			$this->sid = "BX";
	}
	...
	function close()
	{
		if(defined("FX_MEMCACHE_CONNECTED") && is_object(self::$obMemcache))
			self::$obMemcache->close();
	}

	function IsAvailable()
	{
		return defined("FX_MEMCACHE_CONNECTED");
	}
