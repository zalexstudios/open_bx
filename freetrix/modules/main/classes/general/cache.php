...
{
	/**
	 * @var Freetrix\Main\Data\Cache
	 */
	private $cache;

	public function __construct()
	{
		$this->cache = \Freetrix\Main\Data\Cache::createInstance();
	}
...
class CPageCache
{
	...
	{
		$this->_cache = \Freetrix\Main\Data\Cache::createCacheEngine();
	}
	...
	function Clean($uniq_str, $initdir = false, $basedir = "cache")
	{
		...
		{
			$basedir = FX_PERSONAL_ROOT."/".$basedir."/";
			$filename = CPageCache::GetPath($uniq_str);
			if (\Freetrix\Main\Data\Cache::getShowCacheStat())
				\Freetrix\Main\Diag\CacheTracker::add(0, "", $basedir, $initdir, "/".$filename, "C");
	...
	function CleanDir($initdir = false, $basedir = "cache")
	{
		$basedir = FX_PERSONAL_ROOT."/".$basedir."/";
		if (\Freetrix\Main\Data\Cache::getShowCacheStat())
			\Freetrix\Main\Diag\CacheTracker::add(0, "", $basedir, $initdir, "", "C");
	...
	function InitCache($TTL, $uniq_str, $initdir = false, $basedir = "cache")
	{
		...
		$this->basedir = FX_PERSONAL_ROOT."/".$basedir."/";
...
//		$GLOBALS["CACHE_STAT_BYTES"] += $this->_cache->read;
		if (\Freetrix\Main\Data\Cache::getShowCacheStat())
		{
			...
			\Freetrix\Main\Diag\CacheTracker::addCacheStatBytes($read);
			\Freetrix\Main\Diag\CacheTracker::add($read, $path, $this->basedir, $this->initdir, $this->filename, "R");
		}
		return true;
	}
...
function BXClearCache($full=false, $initdir="")
{
	...
	$path = $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/cache".$initdir;
...
class CCacheManager
{
	...
	{
		return false; //return $this->managedCache->read($ttl, $uniqid, $table_id);
	}
	...
	{
		return false;//$this->managedCache->set($uniqid, $val);
	}
	...
	{
		\Freetrix\Main\Data\ManagedCache::finalize();
	}
...
class CStackCacheEntry
{
	...
	function Save()
	{
		if (defined("FREETRIX_SKIP_STACK_CACHE") && FREETRIX_SKIP_STACK_CACHE)
			return;
