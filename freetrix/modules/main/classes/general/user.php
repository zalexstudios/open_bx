...
global $FX_GROUP_POLICY;
$FX_GROUP_POLICY = array(
...
abstract class CAllUser extends CDBResult
{
	...
	function RequiredHTTPAuthBasic($Realm = "Freetrix")
	{
	...
	function LoginByDigest($arDigest)
	{
		...
		if($arUser = $res->Fetch())
		{
			...
			{
				$_SESSION["FX_HTTP_DIGEST_ABSENT"] = false;
			...
			else
			{
				...
				$_SESSION["FX_HTTP_DIGEST_ABSENT"] = true;
			}
		}
	...
	{
		$cookie_md5pass = COption::GetOptionString("main", "cookie_name", "FREETRIX_SM")."_UIDH";
...
class CAllGroup
{
	...
	/* by ivanbogomoloff@yandex.ru
	* @param int $ID int|array
	*/
	function getCodeByID($ID = null)
	{
		if($ID == null) 
			throw new \Exception("Set fisrt parameter");

		global $DB;

		if(is_array($ID))
			$strSql = "SELECT G.STRING_ID FROM b_group G WHERE G.ID IN ('".$DB->ForSQL(implode(',',$ID) )."')";
		else
			$strSql = "SELECT G.STRING_ID FROM b_group G WHERE G.ID='".$DB->ForSQL($ID)."'";	
		$db_res = $DB->Query($strSql);

		if(is_array($ID)) {
			$codes = [];
			while($ar_fields = $db_res->GetNext())
			{
			  $codes[] = $ar_fields['STRING_ID'];
			}
			return $codes;
		}
		else {
			if($ar_res = $db_res->Fetch())
				return $ar_res["STRING_ID"];
