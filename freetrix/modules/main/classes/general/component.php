...
class CFreetrixComponent
{
	...
	/** @var CFreetrixComponentTemplate */
	 ...
	 *
	 * @param CFreetrixComponent $component
	 */
	public function __construct($component = null)
	{
		self::$__componentCounter++;
		if(is_object($component) && ($component instanceof cfreetrixcomponent))
		{
	...
	/**
	 * Function returns component name in form freetrix:component.name
	 *
	...
	/**
	 * Function returns path to component in form /freetrix/component.name
	 *
	 * <p>Note: component must be inited by initComponent method.</p>
	 * @return string
	 *
	...
	/**
	 * Function returns path to component relative to Web server DOCUMENT_ROOT in form /freetrix/components/freetrix/component.name
	 *
	 * <p>Note: component must be inited by initComponent method.</p>
	 * @return string
	 *
	 ...
	 *
	 * <p>Note: component must be inited by initComponent method.</p>
	 * @return CFreetrixComponentTemplate
	 *
	 */
	...
	{
		if ($this->__bInited)
			CFreetrixComponentTemplate::ApplyCachedData($templateCachedData);
	}
	/**
	 ...
	 * Then if there is subsclass of CFreetrixComponent found? it's name is returned.</p>
	...
	final private function __getClassForPath($componentPath)
	{
		if (!isset(self::$__classes_map[$componentPath]))
		{
			...
			if (file_exists($fname) && is_file($fname))
			{
				...
				{
					if (is_subclass_of($afterClasses[$i], "cfreetrixcomponent"))
	...
	final public function addDeleteAction($entryId, $deleteLink, $deleteTitle = false, $arParams = array())
	{
		...
		if (substr($deleteLink, 0, 11) != 'javascript:')
		{
			...
			$deleteLink.= '&'.freetrix_sessid_get();
	...
	public function randString($length = 6)
	{
		...
		if (!$this->randomSequence)
		{
			...
			$this->randomSequence = new \Freetrix\Main\Type\RandomSequence($seed);
		}
