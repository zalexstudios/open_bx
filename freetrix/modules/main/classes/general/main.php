...
define('FX_SPREAD_SITES', 999 );
define('FX_SPREAD_DOMAIN', 999 );

define('FX_RESIZE_IMAGE_PROPORTIONAL_ALT', 0);
define('FX_RESIZE_IMAGE_PROPORTIONAL', 1);
define('FX_RESIZE_IMAGE_EXACT', 2);

global $FX_CACHE_DOCROOT;
$FX_CACHE_DOCROOT = array();
...
abstract class CAllMain
{
	...
	function GetCurPage($get_index_page=null)
	{
		...
		{
			if (defined('FX_DISABLE_INDEX_PAGE'))
				$get_index_page = !FX_DISABLE_INDEX_PAGE;
			else
				$get_index_page = true;
		}
	...
	function AuthForm($mess, $show_prolog=true, $show_epilog=true, $not_show_links="N", $do_die=true)
	{
		...
		if(substr($this->GetCurDir(), 0, strlen(FX_ROOT."/admin/")) == FX_ROOT."/admin/" || (defined("ADMIN_SECTION") && ADMIN_SECTION===true))
			$isAdmin = "_admin";
		else
			$isAdmin = "";
		...
		$last_login = ${COption::GetOptionString("main", "cookie_name", "FREETRIX_SM")."_LOGIN"};
		...
		{
			Header('X-Freetrix-Ajax-Status: Authorize');
		...
		if($show_prolog)
		{
			...
			define("FX_AUTH_FORM", true);
			include($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/include/prolog".$isAdmin. "_after.php");
		}
		...
		if($isAdmin == "")
		{
			...
			{
				$this->IncludeComponent("freetrix:".$comp_name, "", array(
		...
		{
			include($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/auth/wrapper.php");
		}

		if($show_epilog)
			include($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/include/epilog".$isAdmin.".php");
	...
	function NeedCAPTHAForLogin($login)
	{
		...
		if(isset($_SESSION["FX_LOGIN_NEED_CAPTCHA"]) && $_SESSION["FX_LOGIN_NEED_CAPTCHA"])
		{
			return true;
		}
		...
		if(
			!array_key_exists("FX_LOGIN_NEED_CAPTCHA_LOGIN", $_SESSION)
			|| $_SESSION["FX_LOGIN_NEED_CAPTCHA_LOGIN"]["LOGIN"] !== $login
		)
		{
			$POLICY_ATTEMPTS = 0;
			if($login <> '')
			...
			$_SESSION["FX_LOGIN_NEED_CAPTCHA_LOGIN"] = array(
				"LOGIN" => $login,
		...
		if(
			$login <> ''
			&& $_SESSION["FX_LOGIN_NEED_CAPTCHA_LOGIN"]["POLICY_ATTEMPTS"] > 0
		)
		{
			...
			if($USER_ATTEMPTS >= $_SESSION["FX_LOGIN_NEED_CAPTCHA_LOGIN"]["POLICY_ATTEMPTS"])
				return true;
		}
	...
	function SetTitle($title, $arOptions = null)
	{
		...
		{
			$arTrace = array_reverse(Freetrix\Main\Diag\Helper::getBackTrace(0, DEBUG_BACKTRACE_PROVIDE_OBJECT | DEBUG_BACKTRACE_IGNORE_ARGS));

			foreach ($arTrace as $arTraceRes)
			{
				if (isset($arTraceRes['class']) && isset($arTraceRes['function']))
				{
					if (ToUpper($arTraceRes['class']) == 'CFREETRIXCOMPONENT' && ToUpper($arTraceRes['function']) == 'INCLUDECOMPONENT' && is_object($arTraceRes['object']))
					{
						/** @var CFreetrixComponent $comp */
	...
	function GetCSS($cMaxStylesCnt=true, $bXhtmlStyle=true)
	{
		...
		if(!$this->bInAjax && (!defined("ADMIN_SECTION") || ADMIN_SECTION !== true))
		{
			...
			{
				$path = FX_PERSONAL_ROOT."/tmp/templates/__bx_preview/";
			...
			else
			{
				...
				$path = FX_PERSONAL_ROOT."/templates/.default";
				$arTemplateCss[] = $path."/styles.css";
				$arTemplateCss[] = $path."/template_styles.css";
			}
		}
		...
		foreach($arCSS as $cssKey => $css_path)
		{
			...
			$optimize = (
				$optimizeCSS &&
				(
					strncmp($css_path, '/freetrix/panel/', 14) != 0
					&& strncmp($css_path, '/freetrix/themes/', 15) != 0
					&& strncmp($css_path, '/freetrix/modules/', 16) != 0
				)
			);
			...
			elseif(file_exists($filename) && filesize($filename) > 0)
			{
				...
				elseif($optimize)
				{
					if(strncmp($css_path, '/freetrix/js/', 11) != 0)
					{
	...
	private function __OptimizeCssJs($arFile = array(), $arSrcFile = array(), $site_template, $unique = false, $sufix = 'default', $type = 'css', $bXhtmlStyle=true)
	{
		...
		$noCheckOnly = !defined('FX_HEADFILES_CACHE_CHECK_ONLY');
		...
		$optimPath = FX_PERSONAL_ROOT.'/cache/'.$type.'/'.SITE_ID.'/'.$site_template.'/'.$sufix.'/';
		$infoFile = FX_PERSONAL_ROOT.'/managed_cache/'.$GLOBALS['DB']->type.'/'.$type.'/'.SITE_ID.'/'.$site_template.'/'.$sufix.'/info'.$fileMTime.'.php';
	...
	function GetHeadScripts($type = 0)
	{
		...
		if($firstExec)
		{
			...
			$additionalJS = $this->arHeadAdditionalScripts;//array_unique($this->arHeadAdditionalScripts);
			...
			{
				$arScripts = array_merge($this->arHeadScripts, $additionalJS); //array_unique(array_merge($this->arHeadScripts, $additionalJS));
			}
			...
			foreach($arScripts as $jsKey => $src)
			{
				...
				if($optimJS)
				{
					if(strncmp($src, '/freetrix/js/', 11) != 0)
					{
	...
	function GetTemplatePath($rel_path)
	{
		...
		{
			$path = getLocalPath("templates/".SITE_TEMPLATE_ID."/".$rel_path, FX_PERSONAL_ROOT);
			if($path !== false)
				return $path;

			$path = getLocalPath("templates/.default/".$rel_path, FX_PERSONAL_ROOT);
			if($path !== false)
				return $path;
	...
	function OnChangeFileComponent($path, $site)
	{
		...
		{
			\Freetrix\Main\Component\ParametersTable::deleteByFilter(
		...
		for ($i = 0, $cnt = count($arComponents); $i < $cnt; $i++)
		{
			if (class_exists("\\Freetrix\\Main\\Application", false))
			{
				...
				\Freetrix\Main\Component\ParametersTable::add(
					array(
						'SITE_ID' => $site,
						'COMPONENT_NAME' => $arComponents[$i]["DATA"]["COMPONENT_NAME"],
						'TEMPLATE_NAME' => $arComponents[$i]["DATA"]["TEMPLATE_NAME"],
						'REAL_PATH' => $path,
						'SEF_MODE' => ($isSEF? \Freetrix\Main\Component\ParametersTable::SEF_MODE : \Freetrix\Main\Component\ParametersTable::NOT_SEF_MODE),
	...
	function IncludeFile($rel_path, $arParams = array(), $arFunctionParams = array())
	{
		...
		if($bDrawIcons)
		{
			...
			if($USER->CanDoOperation('edit_php') && $bComponent)
			{
				...
				$arIcons[] = array(
					'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
						'URL' => "/freetrix/admin/component_props.php?".
			...
			{
				$arIcons[] = array(
					'URL'=>'javascript:if(confirm(\''.GetMessage("MAIN_INC_BLOCK_MODULE").'\'))window.location=\''.$editor.'&path='.urlencode(FX_PERSONAL_ROOT.'/templates/'.SITE_TEMPLATE_ID.'/'.$rel_path).$arFunctionParams["BACK_URL"].$arFunctionParams["LANG"].'&template='.$path.'\';',
					'ICON'=>'copy',
			...
			elseif($sType == "DEFAULT")
			{
				...
				$arIcons[] = array(
					'URL'=>$editor.'&path='.urlencode(FX_PERSONAL_ROOT.'/templates/'.SITE_TEMPLATE_ID.'/'.$rel_path).$arFunctionParams["BACK_URL"].$arFunctionParams["LANG"].'&template='.$path,
					'ICON'=>'copy',
			...
			else
			{
				...
				{
					$arIcons[] = array(
						'URL'=>'/freetrix/admin/workflow_edit.php?'.$arFunctionParams["LANG"].'&fname='.urlencode($path).$arFunctionParams["TEMPLATE"].$arFunctionParams["BACK_URL"],
						'ICON'=>'bx-context-toolbar-edit-icon',
	...
	function GetNavChain($path=false, $iNumFrom=0, $sNavChainPath=false, $bIncludeOnce=false, $bShowIcons = true)
	{
		...
		if(!file_exists($strChainTemplate))
			$strChainTemplate = $DOC_ROOT.FX_PERSONAL_ROOT."/templates/.default/chain_template.php";
		...
		if($APPLICATION->GetShowIncludeAreas() && $USER->CanDoOperation('edit_php') && $bShowIcons)
		{
			...
			if(strpos($strChainTemplate, $DOC_ROOT)===0)
			{
				...
				if((!defined("ADMIN_SECTION") || ADMIN_SECTION!==true) && $templ_perm>="W")
				{
					$arIcons = array();
					$arIcons[] = array(
						"URL"=>"/freetrix/admin/fileman_file_edit.php?lang=".LANGUAGE_ID."&site=".$site."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&full_src=Y&path=".urlencode($path),
						"ICON"=>"nav-template",
	...
	function ProcessLPA($filesrc = false, $old_filesrc = false)
	{
		...
		if (strpos($filesrc, '#PHP') !== false && $old_filesrc !== false) // We have to handle php fragments
		{
			...
			while (preg_match('/#PHP\d{4}#/i'.FX_UTF_PCRE_MODIFIER, $filesrc, $res))
			{
	...
	function GetSiteSwitcherArray()
	{
		...
		$bAdmin = (substr($cur_dir, 0, strlen(FX_ROOT."/admin/")) == FX_ROOT."/admin/");
	...
	*/
	function get_cookie($name, $name_prefix=false)
	{
		if($name_prefix===false)
			$name = COption::GetOptionString("main", "cookie_name", "FREETRIX_SM")."_".$name;
		else
	...
	*/
	function set_cookie($name, $value, $time=false, $folder="/", $domain=false, $secure=false, $spread=true, $name_prefix=false, $httpOnly=false)
	{
		...
		if($spread === "Y" || $spread === true)
			$spread_mode = FX_SPREAD_DOMAIN | FX_SPREAD_SITES;
		elseif($spread >= 1)
			$spread_mode = $spread;
		else
			$spread_mode = FX_SPREAD_DOMAIN;

		//current domain only
		if($spread_mode & FX_SPREAD_DOMAIN)
			setcookie($name, $value, $time, $folder, $domain, $secure, $httpOnly);

		//spread over sites
		if($spread_mode & FX_SPREAD_SITES)
	...
	function GetSpreadCookieUrls()
	{
		...
		if(COption::GetOptionString("main", "ALLOW_SPREAD_COOKIE", "Y")=="Y")
		{
			...
			if(!empty($this->arrSPREAD_COOKIE))
			{
				...
				$salt = $_SERVER["REMOTE_ADDR"]."|".@filemtime($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/version.php")."|".LICENSE_KEY;
				...
				if(count($arrDomain)>0)
				{
					...
					foreach($arUniqDomains as $domain)
					{
						...
						{
							$url = $protocol.$domain."/freetrix/spread.php?".$params;
	...
	function AddBufferContent($callback)
	{
		...
		if(!defined("FX_BUFFER_USED") || FX_BUFFER_USED!==true)
		{
	...
	function ConvertCharset($string, $charset_in, $charset_out)
	{
		...
		if (!$result && !empty($error))
			$this->ThrowException($error, "ERR_CHAR_FX_CONVERT");
	...
	{
		include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/captcha.php");
	...
	*/
	...
	static function ForkActions($func = false, $args = array())
	{
		...
		if(
			!defined("FX_FORK_AGENTS_AND_EVENTS_FUNCTION")
			|| !function_exists(FX_FORK_AGENTS_AND_EVENTS_FUNCTION)
		...
		if(defined("FX_FORK_AGENTS_AND_EVENTS_FUNCTION_STARTED"))
			return false;
		...
		$func = FX_FORK_AGENTS_AND_EVENTS_FUNCTION;
		$pid = $func();
		...
		define("FX_FORK_AGENTS_AND_EVENTS_FUNCTION_STARTED", true);
...
class CAllSite
{
	...
	function CheckFields($arFields, $ID=false)
	{
		...
		if(isset($arFields["TEMPLATE"]))
		{
			...
			{
				if($val["TEMPLATE"] <> '' && getLocalPath("templates/".$val["TEMPLATE"], FX_PERSONAL_ROOT) !== false)
				{
	...
	function GetSiteDocRoot($site)
	{
		...
		global $FX_CACHE_DOCROOT;
		if(!array_key_exists($site, $FX_CACHE_DOCROOT))
		{
			$ar = CSite::getArrayByID($site);
			if($ar && strlen($ar["DOC_ROOT"])>0)
				$FX_CACHE_DOCROOT[$site] = Rel2Abs($_SERVER["DOCUMENT_ROOT"], $ar["DOC_ROOT"]);
			else
				$FX_CACHE_DOCROOT[$site] = rtrim($_SERVER["DOCUMENT_ROOT"], "/\\");
		}

		return $FX_CACHE_DOCROOT[$site];
	}
...
abstract class CAllFilterQuery
{
	...
	*/
	...
	function ParseQ($q)
	{
		...
		$q="( $q )";
		$q = preg_replace("/\\s+/".FX_UTF_PCRE_MODIFIER, " ", $q);
...
class CDebugInfo
{
	...
	function Start()
	{
		...
		{
			$this->arCacheDebugSave = \Freetrix\Main\Diag\CacheTracker::getCacheTracking();
			\Freetrix\Main\Diag\CacheTracker::setCacheTracking(array());
			$this->cache_size = \Freetrix\Main\Diag\CacheTracker::getCacheStatBytes();
			\Freetrix\Main\Diag\CacheTracker::setCacheStatBytes($CACHE_STAT_BYTES = 0);
		}
		$this->arResult = array();
	...
	function Stop($rel_path="", $path="", $cache_type="")
	{
		...
		$this->arResult = array(
			...
			"FX_STATE" => $GLOBALS["FX_STATE"],
			"CACHE_TYPE" => $cache_type,
			"CACHE_SIZE" => \Freetrix\Main\Data\Cache::getShowCacheStat() ? \Freetrix\Main\Diag\CacheTracker::getCacheStatBytes() : 0,
			"LEVEL" => self::$level,
		);
	...
	function Output($rel_path="", $path="", $cache_type="")
	{
		...
		{
			$result .= '; <a title="'.GetMessage("main_incl_file_sql_stat").'" href="javascript:FX_DEBUG_INFO_'.$this->index.'.Show(); FX_DEBUG_INFO_'.$this->index.'.ShowDetails(\'FX_DEBUG_INFO_'.$this->index.'_1\'); ">'.GetMessage("main_incl_file_sql").' '.($this->arResult["QUERY_COUNT"]).' ('.round($this->arResult["QUERY_TIME"], 4).' '.GetMessage("main_incl_file_sec").')</a>';
		}
		if($this->arResult["CACHE_SIZE"])
		{
			if ($this->arResult["CACHE"] && !empty($this->arResult["CACHE"]))
				$result .= '<nobr>; <a href="javascript:FX_DEBUG_INFO_CACHE_'.$this->index.'.Show(); FX_DEBUG_INFO_CACHE_'.$this->index.'.ShowDetails(\'FX_DEBUG_INFO_CACHE_'.$this->index.'_0\');">'.GetMessage("main_incl_cache_stat").'</a> '.CFile::FormatSize($this->arResult["CACHE_SIZE"], 0).'</nobr>';
			else
