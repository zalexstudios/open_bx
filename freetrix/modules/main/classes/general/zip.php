...
include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/archive.php");
...
class CZip implements IBXArchive
{
	...
	*/
	public function Pack($arFileList, $startFile = "")
	{
		...
		$isNewArchive = true;
		if (file_exists($this->io->GetPhysicalName($this->zipname)) && is_file($this->io->GetPhysicalName($this->zipname)) && ($startFile != ""))
			$isNewArchive = false;
	...
	private function _processFiles($arFileList, $addPath, $removePath, $startFile = "")
	{
		...
		while ($j++ < count($arFileList) && ($this->tempres === "started"))
		{
			...
			if (_normalizePath($filename) == _normalizePath($this->_strArchiveName))
				continue;
	...
	*/
	...
	private function _checkDirPath($path, $bPermission=true)
	{
		...
		if(!file_exists($this->io->GetPhysicalName($path)))
			return mkdir($this->io->GetPhysicalName($path), FX_DIR_PERMISSIONS, true);
		else
	...
	{
		$this->fileSystemEncoding = strtolower(defined("FX_FILE_SYSTEM_ENCODING") ? FX_FILE_SYSTEM_ENCODING : "");
