...
class CAdminFileDialog
{
	function ShowScript($arConfig)
	{
		...
		if (strlen($functionError) <= 0)
		{
			?>
			<script>
			var mess_SESS_EXPIRED = '<?=GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_SESS_EXPIRED')?>';
			var mess_ACCESS_DENIED = '<?=GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_NO_PERMS')?>';
...
?>
	...
	{
		if(!defined("FX_B_FILE_DIALOG_SCRIPT_LOADED"))
		{
			define("FX_B_FILE_DIALOG_SCRIPT_LOADED", true);
?>
if (window.jsUtils)
{
	jsUtils.addEvent(window, 'load', function(){jsUtils.loadJSFile("/freetrix/js/main/file_dialog.js?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/freetrix/js/main/file_dialog.js')?>");}, false);
}
<?
		}
	}
			...
			<table class="bx-fd-top-contr-tbl">
				<tr>
					...
					<td nowrap style="width:170px !important; padding: 0px 2px 0px 2px !important;">
						<img src="/freetrix/images/1.gif" class="fd_iconkit go_button" id="__bx_dir_path_go" title="<?=GetMessage("FD_GO_TO")?>"/>
						<img src="/freetrix/images/1.gif" __bx_disable="Y" class="fd_iconkit path_back_dis" title="<?=GetMessage("FD_GO_BACK")?>" id="__bx_dir_path_back"/>
						<img src="/freetrix/images/1.gif" __bx_disable="Y" class="fd_iconkit path_forward_dis" title="<?=GetMessage("FD_GO_FORWARD")?>" id="__bx_dir_path_forward"/>
						<img src="/freetrix/images/1.gif" class="fd_iconkit dir_path_up" title="<?=GetMessage("FD_GO_UP")?>" id="__bx_dir_path_up" />
						<img src="/freetrix/images/1.gif" class="fd_iconkit dir_path_root" title="<?=GetMessage("FD_GO_TO_ROOT")?>" id="__bx_dir_path_root" />
						<img src="/freetrix/images/1.gif" class="fd_iconkit new_dir" title="<?=GetMessage("FD_NEW_FOLDER")?>" id="__bx_new_dir" />
						<img src="/freetrix/images/1.gif" class="fd_iconkit refresh" title="<?=GetMessage("FD_REFRESH")?>" onclick="oBXDialogControls.RefreshOnclick(this);"/>
						<?
						$arSitesPP = Array();
						$arViews = Array(
						...
						$u->Show();
						?>
						<img onclick="oBXDialogControls.ViewSelector.OnClick();" src="/freetrix/images/1.gif" id="__bx_view_selector" class="fd_iconkit view_selector"  title="<?=GetMessage("FD_SELECT_VIEW")?>"/>
					</td>
					...
					<td style="width:20px !important; padding: 0px 6px 0px 3px !important;">
						<img src="/freetrix/images/1.gif" class="fd_iconkit sort_up" title="<?=GetMessage("FD_CHANGE_SORT_ORDER")?>" __bx_value="asc" id="__bx_sort_order" />
					</td>
				</tr>
			</table>
		</div>
			...
			<tr>
				<td style="height:30px">
					<table class="fd_tab_title">
						<tr>
							<td class="icon"><img class="bxfd-add-to-menu-icon" src="/freetrix/images/1.gif" width="32" height="32"/></td>
...
<div id="__bx_get_real_size_cont"></div>
	...
	{
		global $APPLICATION;
		$fd_engine_js_src = '/freetrix/js/main/file_dialog_engine.js';
		$fd_css_src = '/freetrix/themes/.default/file_dialog.css';
		$arSites = $Params['arSites'];
		?>
<script>
...
var FD_MESS =
{
	...
	FD_SESS_EXPIRED : '<?=GetMessageJS('FX_FD_ERROR').': '.GetMessageJS('FX_FD_SESS_EXPIRED')?>',
	FD_ERROR : '<?=GetMessageJS('FX_FD_ERROR')?>',
...
<?
	...
	function GetMenuTypes($site, $path, $bEchoResult = false)
	{
		...
		if(!CModule::IncludeModule("fileman"))
			return $APPLICATION->ThrowException(GetMessage("FX_FD_NO_FILEMAN"));
	...
	function GetItems($Params)
	{
		...
		if(strlen($Params["site"]) > 2)
		{
			if (!$USER->CanDoOperation('clouds_browse'))
				return $APPLICATION->ThrowException(GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_ACCESS_DENIED'), 'access_denied');

			if($Params['operation'] !== 'O')
				return $APPLICATION->ThrowException(GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_PATH_CORRUPT').' [clouds 04]', 'path_corrupt');

			if(!CModule::IncludeModule('clouds'))
				return $APPLICATION->ThrowException(GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_PATH_CORRUPT').' [clouds 01]', 'path_corrupt');
			...
			if(!$obBucket)
				return $APPLICATION->ThrowException(GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_PATH_CORRUPT').' [clouds 02]', 'path_corrupt');

			if(!$obBucket->Init())
				return $APPLICATION->ThrowException(GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_PATH_CORRUPT').' [clouds 03]', 'path_corrupt');
			...
			if(!is_array($arFiles))
				return $APPLICATION->ThrowException(GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_PATH_CORRUPT').' [clouds 05]', 'path_corrupt');
?>
arFDDirs['<?=$path_js?>'] = [];
arFDFiles['<?=$path_js?>'] = [];
<?
...
<?
		...
		{
			return $APPLICATION->ThrowException(GetMessage('FX_FD_ERROR').': '.GetMessage('FX_FD_PATH_CORRUPT'), 'path_corrupt');
		}
		...
		if ($Params['getFiles'])
		{
			...
			foreach ($arFiles as $File)
			{
				...
				if ($genTmb)
				{
					...
					{
						$tmbPath = FX_PERSONAL_ROOT."/tmp/fd_tmb".$path_i;
...
<?
	...
	function MakeNewDir($Params)
	{
		...
		{
			$strWarning = GetMessage("FX_FD_NO_FILEMAN");
		}
...
<form name="frmLoad" action="file_dialog.php?action=uploader&lang=<?=$lang?>&site=<?=$site?>&<?=freetrix_sessid_get()?>" onsubmit="return parent.oBXDialogControls.Uploader.OnSubmit();" method="post" enctype="multipart/form-data">
