...
require($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/database.php");
	...
	function DoConnect()
	{
		...
		if (!$this->bNodeConnection)
		{
			...
			if ($app != null)
			{
				...
				if ($con->isConnected()
					&& ($con instanceof Freetrix\Main\DB\Connection)
		...
		if(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/after_connect.php"))
			include($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/after_connect.php");
	...
	function Query($strSql, $bIgnoreErrors=false, $error_position="", $arOptions=array())
	{
		...
		if(!$result)
		{
			...
			if(!$bIgnoreErrors)
			{
				...
				if(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/dbquery_error.php"))
					include($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/dbquery_error.php");
				elseif(file_exists($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/include/dbquery_error.php"))
					include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/include/dbquery_error.php");
				else
...
//		static $replace = array("%Y", "%m", "%d", "%H", "%i", "%s");
	...
	function SlaveConnection()
	{
		...
		if(isset($_SESSION["FX_REDIRECT_TIME"]))
		{
			$redirect_delay = time() - $_SESSION["FX_REDIRECT_TIME"] + 1;
			if(
				$redirect_delay > 0
		...
		{
			if(defined("FX_CLUSTER_GROUP") && FX_CLUSTER_GROUP != $slave["GROUP_ID"])
			{
				unset($arSlaves[$i]);
			}
