...
require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/Helpers.php");
...
define("FX_URLREWRITE", true);

$requestUri = urldecode($_SERVER["REQUEST_URI"]);

$bUTF = (!defined("FX_UTF") && CUtil::DetectUTF8($_SERVER["REQUEST_URI"]));
if($bUTF)
	$requestUri = CharsetConverter::ConvertCharset($requestUri, "utf-8", (defined("FX_DEFAULT_CHARSET")? FX_DEFAULT_CHARSET : "windows-1251"));

include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/virtual_io.php");
...
if(isset($_SERVER['REDIRECT_STATUS']) && $_SERVER['REDIRECT_STATUS'] == '404' || isset($_GET["SEF_APPLICATION_CUR_PAGE_URL"]))
{
	...
	if (isset($_GET["SEF_APPLICATION_CUR_PAGE_URL"])
		&& (
			...
			|| ($requestUriWithoutParams != "/freetrix/urlrewrite.php")
		)
	)
	{
	...
	define("POST_FORM_ACTION_URI", htmlspecialcharsbx("/freetrix/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=".urlencode($sUrlPath)));
}
...
if (!CHTTP::isPathTraversalUri($_SERVER["REQUEST_URI"]))
{
	foreach($arUrlRewrite as $val)
	{
		if(preg_match($val["CONDITION"], $requestUri))
		{
			...
			if (($urlTmp == "upload/" || ($urlTmp == "freetrix/" && $bxUrlTmp != "freetrix/services/")))
				continue;
...
//admin section 404
if(strpos($requestUri, "/freetrix/admin/") === 0)
{
	include($_SERVER["DOCUMENT_ROOT"]."/freetrix/admin/404.php");
	die();
}

define("FX_CHECK_SHORT_URI", true);
?>