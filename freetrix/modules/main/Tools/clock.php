...
class CClock
{
	...
	function Show($arParams)
	{
		...
		$GLOBALS['APPLICATION']->AddHeadScript('/freetrix/js/main/utils.js');
		...
		{
			?><a href="javascript:void(0);" onclick="bxShowClock_<?=$arParams['inputId']?>()" title="<?=GetMessage('FX_CLOCK_TITLE')?>" onmouseover="this.className='bxc-icon-hover';" onmouseout="this.className='';"><img id="<?=$arParams['inputId']?>_icon" src="/freetrix/images/1.gif" class="bx-clock-icon bxc-iconkit-c"></a><?
		}

		//Init JS and append CSS
		?><script>
		BX.loadCSS("/freetrix/themes/.default/clock.css");
		...
		function bxLoadClock_<?=$arParams['inputId']?>(callback)
		{
			...
			if (!window.JCClock)
			{
				...
				else
				{
					...
					return BX.loadScript(['<?=CUtil::GetAdditionalFileURL("/freetrix/js/main/clock.js")?>'], function() {bxLoadClock_<?=$arParams['inputId']?>(callback)});
				}
			}
			...
			window[obId] = new JCClock({
				...
				MESS: {
					Insert: '<?=GetMessageJS('FX_CLOCK_INSERT')?>',
					Close: '<?=GetMessageJS('FX_CLOCK_CLOSE')?>',
					Hours: '<?=GetMessageJS('FX_CLOCK_HOURS')?>',
					Minutes: '<?=GetMessageJS('FX_CLOCK_MINUTES')?>',
					Up: '<?=GetMessageJS('FX_CLOCK_UP')?>',
					Down: '<?=GetMessageJS('FX_CLOCK_DOWN')?>'
				}
				});
