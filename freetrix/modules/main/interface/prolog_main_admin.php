...
?>
window.document.title = '<?echo CUtil::JSEscape($APPLICATION->GetTitle(false, true))?> - <?echo CUtil::JSEscape(COption::GetOptionString("main","site_name", $_SERVER["SERVER_NAME"]));?>';
</script>
<?
endif;
...
if(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/admin_header.php"))
	include($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/admin_header.php");
...
require($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/top_panel.php");
require($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/favorite_menu.php");
...
<?
	...
	if(isset($GLOBALS["FX_FAVORITE_MENU_ACTIVE_ID"]) && $GLOBALS["FX_FAVORITE_MENU_ACTIVE_ID"])
...
<?
		...
		{
			require($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/desktop_menu.php");
...
<?
	...
	if(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/this_site_logo.php"))
	{
		include($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/this_site_logo.php");
	}
?>
					</div>
				</div></div>
			</td>
			<td class="adm-workarea-wrap <?=defined('FX_ADMIN_SECTION_404') && FX_ADMIN_SECTION_404 == 'Y' ? 'adm-404-error' : 'adm-workarea-wrap-top'?>">
				<div class="adm-workarea adm-workarea-page" id="adm-workarea">
<?
//Title
if ($APPLICATION->GetCurPage(true) != "/freetrix/admin/index.php")
{
	$currentFavId = null;
	$currentItemsId = '';
	...
	if (!defined('FX_ADMIN_SECTION_404') || FX_ADMIN_SECTION_404 != 'Y')
	{
		$arLastItem = null;
		//Navigation chain
		$adminChain->Init();
...
if($USER->IsAuthorized()):
	if(defined("DEMO") && DEMO == "Y"):
		$vendor = COption::GetOptionString("main", "vendor", "1c_freetrix");
