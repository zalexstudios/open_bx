...
require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/auth/authorize.php");
?>
...
window.BXAuthForm = function()
{
	...
	if (!form)
		return;
	var editorDialogDiv = document.getElementById("FX_editor_dialog");
	...
	else if(editorDialogDiv) // Handle authorization lost in editor dialogs
	{
		...
		document.getElementById('FX_editor_dialog_title').innerHTML = '<?=GetMessage("AUTH_PLEASE_AUTH")?>';
		...
		form.onsubmit = function()
		{
			...
			r.Post('/freetrix/admin/fileman_admin.php?login=yes', window.GetParameters('form_auth'));
			return false;
		};
	}
	else
