...
if($bShowExtTime)
{
	...
	foreach($GLOBALS["DB"]->arQueryDebug as $arQueryDebug)
	{
		foreach($arAreas as $i => $arArea)
		{
			if(in_array($arQueryDebug["FX_STATE"], $arArea["FLT"]))
			{
	...
	foreach($APPLICATION->arIncludeDebug as $i => $arIncludeDebug)
	{
		foreach($arAreas as $i => $arArea)
		{
			if(in_array($arIncludeDebug["FX_STATE"], $arArea["FLT"]))
			{
	...
	echo '<a href="javascript:jsDebugTimeWindow.Show(); jsDebugTimeWindow.ShowDetails(\'FX_DEBUG_TIME_1_1\')">'.GetMessage("debug_info_cr_time").'</a> <span id="bx_main_exec_time">'.round($PAGE, 4).'</span> '.GetMessage("debug_info_sec").'<br>';
}
elseif($bShowTime)
{
...
if($bShowStat || $bShowCacheStat)
{
	if ($bShowStat)
	{
		...
		echo '<a title="'.GetMessage("debug_info_query_title").'" href="javascript:FX_DEBUG_INFO_'.count($APPLICATION->arIncludeDebug).'.Show(); FX_DEBUG_INFO_'.count($APPLICATION->arIncludeDebug).'.ShowDetails(\'FX_DEBUG_INFO_'.count($APPLICATION->arIncludeDebug).'_1\');">'.GetMessage("debug_info_total_queries")."</a> ".intval($totalQueryCount)."<br>";
	...
	if($GLOBALS["CACHE_STAT_BYTES"] || $bShowCacheStat)
	{
		...
		{
			echo '<a title="'.GetMessage("debug_info_query_title").'" href="javascript:FX_DEBUG_INFO_CACHE.Show(); FX_DEBUG_INFO_CACHE.ShowDetails(\'FX_DEBUG_INFO_CACHE_m_0\');">'.GetMessage("debug_info_cache_size")."</a> "." ",CFile::FormatSize(\Freetrix\Main\Diag\CacheTracker::getCacheStatBytes(), 0)." (".count($arCacheDebug).")<br>";
		}
		else
		{
			echo GetMessage("debug_info_cache_size")." ",CFile::FormatSize(\Freetrix\Main\Diag\CacheTracker::getCacheStatBytes(), 0)."<br>";
		}
	}
}
...
if ($bShowStat || $bShowCacheStat) //2
{
	...
	if (!empty($arCacheDebug))
	{
		...
		<script type="text/javascript">
			...
			FX_DEBUG_INFO_CACHE = new BX.CDebugDialog();
		</script>
		<?
		$obJSPopup = new CJSPopupOnPage('', array());
		$obJSPopup->jsPopup = 'FX_DEBUG_INFO_CACHE';
		$obJSPopup->StartDescription('bx-debug-window');
		?>
		<p><?echo GetMessage("debug_info_cache_size")?> <?=CFile::FormatSize(\Freetrix\Main\Diag\CacheTracker::getCacheStatBytes(), 0)?></p>
		<?
		...
		<div class="bx-debug-content bx-debug-content-table">
			<table id="cacheDebug" cellpadding="2" cellspacing="0" border="0">
				...
				foreach($arCacheDebug as $j => $cacheDebug)
				{
					if (substr($cacheDebug["path"], 0, $DOCUMENT_ROOT_LEN) === $_SERVER["DOCUMENT_ROOT"])
						$path = '<a target="blank" href="/freetrix/admin/fileman_file_view.php?path='.urlencode(substr($cacheDebug["path"], $DOCUMENT_ROOT_LEN)).'&lang='.LANGUAGE_ID.'">'.htmlspecialcharsEx(substr($cacheDebug["path"], $DOCUMENT_ROOT_LEN)).'</a>';
					else
						$path = '&nbsp;';
				?>
				<tr class="cache-row">
					<td class="number"><?echo $j+1?></td>
					<td><a href="javascript:FX_DEBUG_INFO_CACHE.ShowDetails('FX_DEBUG_INFO_CACHE_m_<?=$j?>')"><?echo $cacheDebug["callee_func"]?></a></td>
			...
			<?
				foreach($arCacheDebug as $j => $cacheDebug)
				{
					?><div id="FX_DEBUG_INFO_CACHE_m_<?=$j?>" style="display:none">
	...
	foreach($APPLICATION->arIncludeDebug as $i=>$arIncludeDebug)
	{
		?>
		<script type="text/javascript">
			FX_DEBUG_INFO_<?=$i?> = new BX.CDebugDialog();
		</script>
		<?
		$obJSPopup = new CJSPopupOnPage('', array());
		$obJSPopup->jsPopup = 'FX_DEBUG_INFO_'.$i;
		...
		if(count($arIncludeDebug["QUERIES"]) > 0)
		{
				...
				?><table cellpadding="0" cellspacing="0" border="0"><?
					...
					foreach($arQueries as $strSql=>$query)
					{
						?><tr>
							...
							<td><a href="javascript:FX_DEBUG_INFO_<?=$i?>.ShowDetails('FX_DEBUG_INFO_<?=$i."_".$j?>')"><?echo htmlspecialcharsbx(substr($strSql, 0, 100))."..."?></a>&nbsp;(<?echo $query["COUNT"]?>) </td>
			...
			<?
				...
				foreach($arQueries as $strSql=>$query)
				{
					?><div id="FX_DEBUG_INFO_<?=$i."_".$j?>" style="display:none">
		...
		?>
		<script type="text/javascript">
			FX_DEBUG_INFO_CACHE_<?=$i?> = new BX.CDebugDialog();
		</script>
		<?
		$obJSPopup = new CJSPopupOnPage('', array());
		$obJSPopup->jsPopup = 'FX_DEBUG_INFO_CACHE_'.$i;
		$obJSPopup->StartDescription('bx-core-debug-info');
		?>
		...
		if($arIncludeDebug["CACHE"] && !empty($arIncludeDebug["CACHE"]))
		{
			...
			<div class="bx-debug-content bx-debug-content-table">
				<table id="cacheDebug<?=$i?>" cellpadding="2" cellspacing="0" border="0">
					...
					<tr class="cache-row">
						<td class="number"><?echo $j+1?></td>
						<td><a href="javascript:FX_DEBUG_INFO_CACHE_<?=$i?>.ShowDetails('FX_DEBUG_INFO_CACHE_<?=$i."_".$j?>')"><?echo $cacheDebug["callee_func"]?></a></td>
						<td class="number" sort="<?echo $cacheDebug["cache_size"]?>"><?=CFile::FormatSize($cacheDebug["cache_size"], 0)?></td>
						<td><?=$path?></td>
					</tr>
					<?
			...
			<?
				foreach($arIncludeDebug["CACHE"] as $j => $cacheDebug)
				{
					?><div id="FX_DEBUG_INFO_CACHE_<?=$i?>_<?=$j?>" style="display:none">
					<b><?echo GetMessage("debug_info_query_from")?></b>
					<?
					$k=1;
					foreach($cacheDebug["TRACE"] as $n => $tr)
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	<tr class="heading heading-bottom">
		...
		<td>
			<?if($bShowComps):?>
				<a style="font-weight:bold !important" href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_1')"><?echo GetMessage("debug_info_whole_page")?></a>
			<?else:?>
	...
	<tr valign="top">
		...
		<td>
			<?if($bShowComps):?>
				<p><a style="font-weight:bold !important" href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_2')"><?echo GetMessage("debug_info_prolog")?></a></p>
				<p>
					&nbsp;&nbsp;<a href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_3')"><?echo GetMessage("debug_info_prolog_before")?></a><br>
					&nbsp;&nbsp;<a href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_4')"><?echo GetMessage("debug_info_agents")?></a><br>
					&nbsp;&nbsp;<a href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_5')"><?echo GetMessage("debug_info_prolog_after")?></a><br>
				</p>
			<?else:?>
	...
	<tr valign="top">
		...
		<td>
			<?if($bShowComps):?>
				<p><a style="font-weight:bold !important" href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_6')"><?echo GetMessage("debug_info_work_area")?></a></p>
			<?else:?>
	...
	<tr valign="top">
		...
		<td>
			<?if($bShowComps):?>
				<p><a style="font-weight:bold !important" href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_7')"><?echo GetMessage("debug_info_epilog")?></a></p>
				<p>
					&nbsp;&nbsp;<a href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_8')"><?echo GetMessage("debug_info_epilog_before")?></a><br>
					&nbsp;&nbsp;<a href="javascript:jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_9')"><?echo GetMessage("debug_info_epilog_after")?></a><br>
				</p>
			<?else:?>
		...
		<td>&nbsp;</td>
			...
			<div id="FX_DEBUG_TIME_1_<?echo $arArea["NUM"]?>" style="display:none">
				<table cellpadding="0" cellspacing="0" border="0" width="100%">
				<?
				$tim = 0;
				foreach($APPLICATION->arIncludeDebug as $i=>$arIncludeDebug)
				{
					if(isset($arIncludeDebug["REL_PATH"]) && in_array($arIncludeDebug["FX_STATE"], $arArea["FLT"]))
					{
				...
				?>
					<tr>
						...
						<td>
						<?if($bShowStat):?>
							<a title="<?echo GetMessage("debug_info_query_title")?>" href="javascript:FX_DEBUG_INFO_<?echo $arArea["IND"]?>.Show(); FX_DEBUG_INFO_<?echo $arArea['IND']?>.ShowDetails('FX_DEBUG_INFO_<?echo $arArea['IND']?>_1');"><?echo GetMessage("debug_info_raw_code")?></a>
						<?else:?>
				...
				<?$j=1;$k=1;foreach($APPLICATION->arIncludeDebug as $i=>$arIncludeDebug):?>
					...
					<tr>
						...
						<?if($bShowStat):?>
							<a title="<?echo GetMessage("debug_info_query_title")?>" href="javascript:FX_DEBUG_INFO_<?echo $i?>.Show(); FX_DEBUG_INFO_<?echo $i?>.ShowDetails('FX_DEBUG_INFO_<?echo $i?>_1');"><?echo htmlspecialcharsbx($arIncludeDebug["REL_PATH"])?></a>
						<?else:?>
...
<?
	if(
		...
		echo "<script>BX.ready(function() {jsDebugTimeWindow.Show(); jsDebugTimeWindow.ShowDetails('FX_DEBUG_TIME_1_1');});</script>";
}
?>