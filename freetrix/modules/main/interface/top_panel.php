...
//					"ACTION"=>"jsUtils.Redirect([], '".CUtil::addslashes(FX_ROOT."/admin/favorite_act.php?lang=".LANG."&act=add&name=".urlencode($APPLICATION->GetTitle()))."&".freetrix_sessid_get()."&addurl='+encodeURIComponent(document.getElementById('navchain-link').getAttribute('href', 2)));"
				...
				array(
					...
					"ACTION"=>"jsUtils.Redirect([], '".CUtil::addslashes(FX_ROOT."/admin/favorite_list.php?lang=".LANG)."');"
				),
			);
	...
	*/
	...
	{
		//Settings
		$settingsUrl = FX_ROOT."/admin/settings.php?lang=".LANG."&mid=".(defined("ADMIN_MODULE_NAME")? ADMIN_MODULE_NAME:"main").($APPLICATION->GetCurPage() <> FX_ROOT."/admin/settings.php"? "&back_url_settings=".urlencode($_SERVER["REQUEST_URI"]):"");
		$arPanelButtons[] = array(
...
/*			"MENU"=> array(
				...
				array(
					...
					"LINK" => "/freetrix/admin/user_settings.php?lang=".LANGUAGE_ID,
...
var jsControl = new JCAdminTitleSearch({
	'AJAX_PAGE' : '/freetrix/admin/get_search.php?lang=<?=LANGUAGE_ID?>',
...
?><a hidefocus="true" href="/freetrix/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&amp;ID=<?=$USER->GetID()?>" class="adm-header-user-block" id="bx-panel-user"><?=htmlspecialcharsbx($userName);?></a><?
...
?><a hidefocus="true" href="<?=htmlspecialcharsbx((defined('FX_ADMIN_SECTION_404') && FX_ADMIN_SECTION_404 == 'Y' ? '/freetrix/admin/' : $APPLICATION->GetCurPage())).'?logout=yes'.htmlspecialcharsbx(($s=DeleteParam(array("logout"))) == ""? "":"&".$s)?>" class="adm-header-exit" id="bx-panel-logout" title="<?=GetMessage('admin_panel_logout_title')?>"><?=GetMessage("admin_panel_logout")?></a><?
...
/*
	<div id="bx-panel-top"><?
		...
		$userInfo = "";
		if(!defined("FX_AUTH_FORM") && $USER->IsAuthorized())
		{
		...
		if($USER->IsAuthorized())
		{
			...
			if ($bCanProfile)
				$userInfo .= '<a href="/freetrix/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$USER->GetID().'" id="bx-panel-user">'.htmlspecialcharsbx($USER->GetFullName()).' ('.htmlspecialcharsbx($USER->GetLogin()).')</a><span class="bx-panel-userinfo-separator"></span>';
			else
