	...
	function _SetActiveItems(&$aMenu, $aSections=array())
	{
		...
		if($aMenu["url"] <> "" || $bMoreUrl)
		{
			...
			for($j = 0; $j < $n; $j++)
			{
				...
				if(strpos($all_links[$j], "/freetrix/admin/") !== 0)
					$tested_link = "/freetrix/admin/".$all_links[$j];
				else
...
class CAdminList
{
	...
	function ShowSettings($aAllCols, $aCols, $aOptions)
	{
		...
		require($_SERVER['DOCUMENT_ROOT']."/freetrix/modules/main/interface/settings_admin_list.php");
	...
	function EditAction()
	{
		if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['save'])  && check_freetrix_sessid())
		{
...
class CAdminMainChain extends CAdminChain
{
	...
	function Init()
	{
		...
		parent::AddItem(array("TEXT"=> GetMessage("admin_lib_navchain_first"), "LINK"=>"/freetrix/admin/index.php?lang=".LANGUAGE_ID, "CLASS" => "adm-navchain-item-desktop"));
...
class CAdminCalendar
{
	...
	public function Calendar($sFieldName, $sFromName="", $sToName="", $bTime=false)
	{
		...
		ob_start();
		$APPLICATION->IncludeComponent('freetrix:main.calendar', '', array(
			'RETURN' => 'Y',
			'SHOW_INPUT' => 'N',
	...
	public function CalendarPeriodCustom($sFromName, $sToName, $sFromVal="", $sToVal="", $bSelectShow=false, $size="10", $bTime=false, $arPeriod = false)
	{
			...
			foreach($arTabs as $customFields)
			{
				...
				foreach($arCustomFields as $customField)
				{
					...
					else
					{
						...
						$arCustomFieldName = ltrim($arCustomFieldName, defined("FX_UTF")? "* -\xa0\xc2": "* -\xa0");
...
class CAdminForm extends CAdminTabControl
{
	...
	function ShowSettings()
	{
		...
		require($_SERVER['DOCUMENT_ROOT']."/freetrix/modules/main/interface/settings_admin_form.php");
			...
			?>
				<tr>
					<td colspan="2" align="left">
						<a href="/freetrix/admin/userfield_edit.php?lang=<?echo LANGUAGE_ID?>&amp;ENTITY_ID=<?echo urlencode($PROPERTY_ID)?>&amp;back_url=<?echo urlencode($APPLICATION->GetCurPageParam()."&tabControl_active_tab=user_fields_tab")?>"><?echo $this->GetCustomLabelHTML()?></a>
					</td>
				</tr>
			<?
...
function ShowJSHint($text, $arParams=false)
{
	...
	$res = '<img src="/freetrix/images/1.gif" onload="BX.hint_replace(this, \''.htmlspecialcharsbx(CUtil::JSEscape($text)).'\')" />';
