...
if (isset($_REQUEST['bxsender']))
{
	if ($_REQUEST['bxsender'] != 'core_autosave')
		require($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/auth/wrapper_popup.php");
...
if ($arAuthResult && defined('ADMIN_SECTION_LOAD_AUTH') && ADMIN_SECTION_LOAD_AUTH || $_REQUEST['AUTH_FORM'])
{
	...
	include($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/auth/wrapper_auth_result.php");
	die();
}
...
$authUrl = (defined('FX_ADMIN_SECTION_404') && FX_ADMIN_SECTION_404 == 'Y') ? '/freetrix/admin/' : $sDocPath;
?>
			...
			<td class="login-popup-alignment-2" id="popup_alignment">
				...
				<div class="login-footer">
					...
					<div class="login-footer-right">
						<?if(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/this_site_support.php")):?><?include($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/this_site_support.php");?><?else:?><?echo $sLinks?><?endif;?>
					</div>
				</div>
				<form name="form_auth" method="post" target="auth_frame" class="bx-admin-auth-form" action="" novalidate>
					<input type="hidden" name="AUTH_FORM" value="Y">

					<div id="auth_form_wrapper"><?require($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/auth/".$inc_file.'.php')?></div>
...
foreach ($arFormsList as $form)
{
	if ($form != $inc_file)
	{
		require($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/auth/".$form.".php");
	}
}
?>
