...
function Calendar($sFieldName, $sFormName="skform", $sFromName="", $sToName="")
{
	...
	if(!$bCalendarCode)
	{
		...
		$func =
			...
			"	}\n".
			"	window.open('/freetrix/tools/calendar.php?lang=".LANGUAGE_ID.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"&admin_section=N")."&'+params+'&date='+escape(dateVal)+'&initdate='+escape(dateVal),'','scrollbars=no,resizable=yes,width='+width+',height='+height+',left='+left+',top='+top);\n".
			"}\n".
			"//-->\n".
			"</script>\n";
	}
	return $func."<a href=\"javascript:void(0);\" onclick=\"window.Calendar('name=".urlencode($sFieldName)."&amp;from=".urlencode($sFromName)."&amp;to=".urlencode($sToName)."&amp;form=".urlencode($sFormName)."', document['".$sFormName."']['".$sFieldName."'].value);\" title=\"".GetMessage("TOOLS_CALENDAR")."\"><img src=\"".FX_ROOT."/images/icons/calendar.gif\" alt=\"".GetMessage("TOOLS_CALENDAR")."\" width=\"15\" height=\"15\" border=\"0\" /></a>";
}
...
function ParseDateTime($datetime, $format=false)
{
	...
	{
		$dt_args = array();
		if(preg_match_all("~([^:\\\\/\\s.0-9-]+|[^:\\\\/\\s.a-z-]+)~i".FX_UTF_PCRE_MODIFIER, $datetime, $dt_args))
		{
			$arrResult = array();
...
function FormatDateEx($strDate, $format=false, $new_format=false)
{
	...
	$new_format = preg_replace("/([DMYIHGST])\\1+/is".FX_UTF_PCRE_MODIFIER, "\\1", $new_format);
...
function InsertSpaces($sText, $iMaxChar=80, $symbol=" ", $bHTML=false)
{
	...
	if ($iMaxChar > 0 && strlen($sText) > $iMaxChar)
	{
		...
		{
			return preg_replace("/([^() \\n\\r\\t%!?{}\\][-]{".$iMaxChar."})/".FX_UTF_PCRE_MODIFIER,"\\1".$symbol, $sText);
		}
	}
	return $sText;
}
...
function ToUpper($str, $lang = false)
{
	static $lower = array();
	static $upper = array();
	if(!defined("FX_CUSTOM_TO_UPPER_FUNC"))
	{
		if(defined("FX_UTF"))
		{
...
function ToLower($str, $lang = false)
{
	static $lower = array();
	static $upper = array();
	if(!defined("FX_CUSTOM_TO_LOWER_FUNC"))
	{
		if(defined("FX_UTF"))
		{
...
function convert_to_href($url, $link_class="", $event1="", $event2="", $event3="", $script="", $link_target="_self")
{
	...
	{
		$script = strlen($script)>0 ? $script : "/freetrix/redirect.php";
		$goto = $script.
...
function TxtToHTML(
	...
	$arUrlEvent				= array(),			// массив в нем если заданы ключи EVENT1, EVENT2, EVENT3 то ссылки будут через
												// $arUrlEvent["SCRIPT"] (по умолчанию равен "/freetrix/redirect.php")
...
function htmlspecialcharsbx($string, $flags=ENT_COMPAT)
{
	...
	return htmlspecialchars($string, $flags, (defined("FX_UTF")? "UTF-8" : "ISO-8859-1"));
}
...
function CheckDirPath($path, $bPermission=true)
{
	...
	if(!file_exists($path))
		return mkdir($path, FX_DIR_PERMISSIONS, true);
	else
		return is_dir($path);
}
...
function CopyDirFiles($path_from, $path_to, $ReWrite = True, $Recursive = False, $bDeleteAfterCopy = False, $strExclude = "")
{
	...
	elseif(is_file($path_from))
	{
		...
		if(is_file($path_to))
			@chmod($path_to, FX_FILE_PERMISSIONS);
	...
	if ($handle = @opendir($path_from))
	{
		while (($file = readdir($handle)) !== false)
		{
			...
			elseif (is_file($path_from."/".$file))
			{
				...
				@chmod($path_to."/".$file, FX_FILE_PERMISSIONS);
...
function RewriteFile($abs_path, $strContent)
{
	...
	if(file_exists($abs_path) && !is_writable($abs_path))
		@chmod($abs_path, FX_FILE_PERMISSIONS);
...
function GetLangFileName($before, $after, $lang=false)
{
	...
	if(strpos($before, "/freetrix/modules/")===false)
		return $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/lang/en/tools.php";
	...
	IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/".$module.$path.$after, $lang);
	return $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/".$module."/lang/".$lang.$path.$after;
}
...
function IncludeTemplateLangFile($filepath, $lang=false)
{
	...
	$module_path = "/freetrix/modules/";
	$module_name = $templ_path = $file_name = $template_name = "";

	$dirs = array(
		"/local/templates/",
		FX_PERSONAL_ROOT."/templates/",
	);
	foreach($dirs as $dir)
	{
	...
	$FX_DOC_ROOT = rtrim(preg_replace("'[\\\\/]+'", "/", $_SERVER["DOCUMENT_ROOT"]), "/ ");
	$module_path = $FX_DOC_ROOT.$module_path;
	...
	if($templ_path <> "")
	{
		$templ_path = $FX_DOC_ROOT.$templ_path;
		$checkDefault = true;
...
//		{
//			\Freetrix\Main\Localization\Loc::loadMessages($filepath);
//			return true;
//		}
//	}
	...
	if(strpos($filepath, $module_path) !== false)
	{
		...
		$FX_DOC_ROOT = rtrim(preg_replace("'[\\\\/]+'", "/", $_SERVER["DOCUMENT_ROOT"]), "/ ");
		$module_path = $FX_DOC_ROOT.getLocalPath($module_path.$module_name);
	}
...
function SendError($error)
{
	...
	{
		$from = (defined('ERROR_EMAIL_FROM') && ERROR_EMAIL_FROM <> ''? ERROR_EMAIL_FROM : 'error@freetrix.ru');
		$reply_to = (defined('ERROR_EMAIL_REPLY_TO') && ERROR_EMAIL_REPLY_TO <> ''? ERROR_EMAIL_REPLY_TO : 'admin@freetrix.ru');
...
function UnQuoteAll()
{
	...
	UnQuoteArr($_GET);
	if(!defined("FX_SKIP_POST_UNQUOTE") || FX_SKIP_POST_UNQUOTE !== true)
	{
...
function LocalRedirect($url, $skip_security_check=false, $status="302 Found")
{
	...
	if(!defined("FX_UTF") && defined("LANG_CHARSET"))
	{
	...
	$_SESSION["FX_REDIRECT_TIME"] = time();
...
function FindUserID($tag_name, $tag_value, $user_name="", $form_name = "form1", $tag_size = "3", $tag_maxlength="", $button_value = "...", $tag_class="typeinput", $button_class="tablebodybutton", $search_page="/freetrix/admin/user_search.php")
{
	/** @global CMain $APPLICATION */
	global $APPLICATION;
	...
	if (!!DV_".$tag_name_x.")
	{
		if (tv".$tag_name_x."!=document.".$form_name."['".$tag_name."'].value)
		{
			...
			if (tv".$tag_name_x."!='')
			{
				...
				BX(\"hiddenframe".$tag_name."\").src='/freetrix/admin/get_user.php?ID=' + tv".$tag_name_x."+'&strName=".$tag_name."&lang=".LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")."';
			}
			else
			{
...
{
	include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/countries.php");
...
/**
 * @deprecated Use Freetrix\Main\Web\HttpClient
 */
...
function Help($module="", $anchor="", $help_file="")
{
	...
	if($IS_HELP!==true)
	{
		...
		echo "<script type=\"text/javascript\">
			...
			{
				window.open('".FX_ROOT."/tools/help_view.php?local=Y&file='+file+'&module='+module+'&lang=".LANGUAGE_ID."'+anchor, '','scrollbars=yes,resizable=yes,width=".$width.",height=".$height.",top='+Math.floor((screen.height - ".$height.")/2-14)+',left='+Math.floor((screen.width - ".$width.")/2-5));
			}
			//-->
			</script>";
		$IS_HELP=true;
	}
	echo "<a href=\"javascript:Help('".urlencode($help_file)."','".$module."','".$anchor."')\" title='".GetMessage("TOOLS_HELP")."'><img src='".FX_ROOT."/images/main/show_help.gif' width='16' height='16' border='0' alt='".GetMessage("TOOLS_HELP")."' align='absbottom' vspace='2' hspace='1'></a>";
}
...
function freetrix_sessid_set($val=false)
{
	if($val === false)
		$val = freetrix_sessid_val();
...
{
	global $USER;
	if(defined("FREETRIX_STATIC_PAGES") && (!is_object($USER) || !$USER->IsAuthorized()))
		return true;
	else
		return $_REQUEST[$varname] == freetrix_sessid();
}

function freetrix_sessid_get($varname='sessid')
{
	return $varname."=".freetrix_sessid();
}

function freetrix_sessid_post($varname='sessid')
{
	return '<input type="hidden" name="'.$varname.'" id="'.$varname.'" value="'.freetrix_sessid().'" />';
}
...
function IncludeAJAX()
{
	...
	$APPLICATION->AddHeadScript('/freetrix/js/main/cphttprequest.js', true);
}
...
class CJSCore
{
	...
	ex: CJSCore::RegisterExt('timeman', array(
		'js' => '/freetrix/js/timeman/core_timeman.js',
		'css' => '/freetrix/js/timeman/css/core_timeman.css',
		'lang' => '/freetrix/modules/timeman/lang/#LANG#/js_core_timeman.php',
	...
	*/
	...
	public static function Init($arExt = array(), $bReturn = false)
	{
		...
		{
			$ret .= self::_loadCSS('/freetrix/js/main/core/css/core.css', $bReturn);
			$ret .= self::_loadJS('/freetrix/js/main/core/core.js', $bReturn);
			$ret .= self::_loadLang(FX_ROOT.'/modules/main/lang/'.LANGUAGE_ID.'/js_core.php', $bReturn);
	...
	public static function GetCoreMessagesScript()
	{
		static $first = true;
		if ($first)
		{
	...
	{
		require_once($_SERVER['DOCUMENT_ROOT'].FX_ROOT.'/modules/main/jscore.php');
	}
...
class CUtil
{
	...
	{
		preg_match_all("#<([a-z0-9]+)([^>]*)(?<!/)>#i".FX_UTF_PCRE_MODIFIER, $html, $result);
		$openedtags = $result[1];
	...
	public static function ConvertToLangCharset($string)
	{
		...
		$fromCP = $toCP = false;
		if(defined("FX_UTF") && !$bUTF)
		{
			$fromCP = (defined("FX_DEFAULT_CHARSET")? FX_DEFAULT_CHARSET : "Windows-1251");
			$toCP = "UTF-8";
		}
		elseif(!defined("FX_UTF") && $bUTF)
		{
			$fromCP = "UTF-8";
			$toCP = (defined("LANG_CHARSET")? LANG_CHARSET : (defined("FX_DEFAULT_CHARSET")? FX_DEFAULT_CHARSET : "Windows-1251"));
		}
	...
	public static function translit($str, $lang, $params = array())
	{
		...
		{
			$mess = IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/js_core_translit.php", $lang, true);
		...
		for($i = 0; $i < $len; $i++)
		{
			...
			if(preg_match("/[a-zA-Z0-9]/".FX_UTF_PCRE_MODIFIER, $chr) || strpos($params["safe_chars"], $chr)!==false)
			{
				$chr_new = $chr;
			}
			elseif(preg_match("/\\s/".FX_UTF_PCRE_MODIFIER, $chr))
			{
				if (
...
class CHTTP
{
	...
	function CHTTP()
	{
		$this->user_agent = 'FreetrixSM ' . __CLASS__ . ' class';
	}
	...
	/**
	 * @deprecated Use Freetrix\Main\Web\Uri
	 */
	...
	public static function SetStatus($status)
	{
		...
		if($bCgi && (!defined("FX_HTTP_STATUS") || FX_HTTP_STATUS == false))
	...
	public static function SetAuthHeader($bDigestEnabled=true)
	{
		...
		if(defined('FX_HTTP_AUTH_REALM'))
			$realm = FX_HTTP_AUTH_REALM;
		else
		...
		if($bDigestEnabled !== false && COption::GetOptionString("main", "use_digest_auth", "N") == "Y")
		{
			...
			if($_SESSION["FX_HTTP_DIGEST_ABSENT"] !== true)
...
function CheckSerializedData($str, $max_depth = 200)
{
	...
	{
		$str1 = preg_replace('/[^{}]+/'.FX_UTF_PCRE_MODIFIER, '', $str);
		$cnt = 0;
	...
	function CheckUpdates()
	{
		...
		if($days_check > 0)
		{
			...
			if(time() > $update_res["check_date"]+$days_check*86400)
			{
				...
				{
					require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/update_client.php");
		...
		if($update_res["result"] == true || $update_res["error"] <> '')
		{
			...
			if($updOptions['display'] <> 'off')
			{
				...
				elseif($update_res["error"] <> '')
					$update_res['tooltip'] = GetMessage("top_panel_updates_err").' '.$update_res["error"].'<br><a href="/freetrix/admin/settings.php?lang='.LANGUAGE_ID.'&amp;mid=main&amp;tabControl_active_tab=edit5">'.GetMessage("top_panel_updates_settings").'</a>';
			}
		}
...
class CSpacer
{
	...
	{
		return $arMatch[1].preg_replace("/([^() \\n\\r\\t%!?{}\\][-]{".$this->iMaxChar."})/".FX_UTF_PCRE_MODIFIER,"\\1".$this->symbol, $arMatch[2]).$arMatch[3];
	}
}
...
function getLocalPath($path, $baseFolder = "/freetrix")
{
