...
function repair_db()
{
	 ...
	 * @param $DBPassword
	 */
	@include($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/dbconn.php");
	if($DBType == "mysql")
	{
...
if(isset($_REQUEST["login"]) && isset($_REQUEST["password"]) && $_REQUEST["login"] <> '' && $_REQUEST["password"] <> '')
{
	...
	{
		include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/lang/en/admin/repair_db.php");
		?>
		...
		<link rel="stylesheet" type="text/css" href="/freetrix/themes/.default/adminstyles.css">
		</head>
		<body>
...
else
{
	...
	elseif(isset($_REQUEST["table_name"]) && check_freetrix_sessid())
	{
	...
	else
	{
		...
		{
			if($_REQUEST["check_tables"]=="Y" && check_freetrix_sessid())
			{
				repair_db();
			}
			elseif($_REQUEST["optimize_tables"]=="Y")
			{
				...
				function Optimize(table_name)
				{
					...
					else
					{
						...
						var url = 'repair_db.php?lang=<?echo LANGUAGE_ID?>&<?echo freetrix_sessid_get()?>&table_name='+table_name;
