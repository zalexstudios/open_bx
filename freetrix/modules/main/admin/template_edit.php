...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST["save"] <> '' || $_POST["apply"] <> '') && check_freetrix_sessid() && ($edit_php || $lpa))
{
	$strError = "";
	if ($lpa)
	{
	...
	if($strError == "")
	{
		...
		else
		{
			...
			if ($_POST["save"] <> '')
				LocalRedirect(FX_ROOT."/admin/template_admin.php?lang=".LANGUAGE_ID.$useeditor_param);
			else
				LocalRedirect(FX_ROOT."/admin/template_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&".$tabControl->ActiveTabParam().$useeditor_param);
		}
	}
}

if($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["action"]=="export" && $edit_php && check_freetrix_sessid())
{
	if (strlen($tpath)<=0)
...
if($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["action"]=="import" && $edit_php && check_freetrix_sessid())
{
	$DATA_FILE_NAME = "";
	...
	if(strlen($strError)<=0)
	{
		...
		$tres = $oArchiver->extractFiles($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$ID);
		if($tres)
	...
	if(strlen($strError)>0)
		...
		LocalRedirect(FX_ROOT."/admin/template_edit.php?lang=".LANG."&ID=".$ID."&".$tabControl->ActiveTabParam().$useeditor_param);
}
...
$APPLICATION->AddHeadScript("/freetrix/js/main/template_edit.js");
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/freetrix/admin/template_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
...
if (strlen($ID)>0 && $edit_php)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/freetrix/admin/template_edit.php?lang=".LANGUAGE_ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/freetrix/admin/template_admin.php?lang=".LANGUAGE_ID."&ID=".urlencode($ID)."&action=copy&".freetrix_sessid_get(),
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/freetrix/admin/template_admin.php?ID=".urlencode(urlencode($ID))."&lang=".LANGUAGE_ID."&".freetrix_sessid_get()."&action=delete';",
...
<?
$loadEditor = false;
if((!defined("FX_DISABLE_TEMPLATE_EDITOR") || FX_DISABLE_TEMPLATE_EDITOR == false) && COption::GetOptionString("main", "templates_visual_editor", "N") == "Y" && IsModuleInstalled('fileman'))
{
...
if($loadEditor):
	...
	?><script>
		TE_MESS = {};
		TE_MESS.FILEMAN_EDIT_HBF = '<?=GetMessageJS("FILEMAN_EDIT_HBF")?>';
		TE_MESS.FILEMAN_INSERT_WA = '<?=GetMessageJS("FILEMAN_INSERT_WA")?>';
		TE_MESS.FILEMAN_TOOLBAR_TITLE = '<?=GetMessageJS("FILEMAN_TOOLBAR_TITLE")?>';
		TE_MESS.FILEMAN_PREVIEW_TEMPLATE = '<?=GetMessageJS("FILEMAN_PREVIEW_TEMPLATE")?>';
		TE_MESS.FILEMAN_PREVIEW_TEMPLATE_TITLE = '<?=GetMessageJS("FILEMAN_PREVIEW_TEMPLATE_TITLE")?>';
		__ID = '<?=CUtil::JSEscape($ID)?>';
		var SITE_TEMPLATE_PATH = '<?=CUtil::JSEscape($templFields["PATH"])?>';
	</script>
	<?
	$template_styles_path = $_SERVER["DOCUMENT_ROOT"].$templFields["PATH"]."/template_styles.css";
...
<input type="button" value="<?=GetMessage('FILEMAN_PREVIEW_TEMPLATE')?>" name="template_preview" onclick="preview_template('<?=htmlspecialcharsbx(CUtil::JSEscape($ID))?>', '<?= freetrix_sessid()?>');" title="<?=GetMessage('FILEMAN_PREVIEW_TEMPLATE_TITLE')?>">
<?endif;?>
