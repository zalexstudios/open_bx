...
//authorize as user
if($_REQUEST["action"] == "authorize" && check_freetrix_sessid() && $USER->CanDoOperation('edit_php'))
{
	$USER->Logout();
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if($USER->CanDoOperation('edit_php'))
	{
		...
		$arActions[] = array("ICON"=>"", "TEXT"=>GetMessage("MAIN_ADMIN_AUTH"), "TITLE"=>GetMessage("MAIN_ADMIN_AUTH_TITLE"), "LINK"=>"user_admin.php?lang=".LANGUAGE_ID."&ID=".$f_ID."&action=authorize&".freetrix_sessid_get());
	}
