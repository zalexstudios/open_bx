...
if($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["action"]=="import" && $isAdmin && check_freetrix_sessid())
{
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if($oArchiver->extractFiles($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$ID))
				{
					$strOK .= str_replace("#TEMPLATE_NAME#", $ID, GetMessage("MAIN_TEMPLATE_LOAD_OK"));

					if($dh = opendir($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$ID))
					{
						while(($file = readdir($dh)) !== false)
						{
							...
							if(is_file($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$ID."/".$file))
								break;
	...
	elseif($_POST["goto_edit"] == "Y")
		LocalRedirect(FX_ROOT."/admin/template_edit.php?lang=".LANGUAGE_ID."&ID=".$ID);
	else
		LocalRedirect(FX_ROOT."/admin/template_admin.php?lang=".LANGUAGE_ID);
}
