...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($edit_php || $lpa) && check_freetrix_sessid())
{
	...
	if ($USER->CanDoFileOperation('fm_view_file', array(SITE_ID, FX_PERSONAL_ROOT."/templates/".$ID)) && strlen($ID) > 0)
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/templates/".$ID, $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/tmp/templates/__bx_preview", false, true);
	...
	if(strlen($CONTENT) > 0)
	{
		...
		if (!$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/tmp/templates/__bx_preview/header.php", $header))
			$strWaring .= GetMessage('MAIN_TP_ERROR_SAVE_FILE', Array('#FILE#' => 'header.php'));

		$footer = substr($CONTENT, $p + strlen("#WORK_AREA#"));
		if (!$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/tmp/templates/__bx_preview/footer.php", $footer))
			$strWaring .= GetMessage('MAIN_TP_ERROR_SAVE_FILE', Array('#FILE#' => 'footer.php'));
	}

	if(strlen($STYLES) == 0)
		$STYLES = ' ';
	if (!$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/tmp/templates/__bx_preview/styles.css", $STYLES))
		$strWaring .= GetMessage('MAIN_TP_ERROR_SAVE_FILE', Array('#FILE#' => 'styles.css'));

	if(strlen($TEMPLATE_STYLES) == 0)
		$TEMPLATE_STYLES = ' ';
	if (!$APPLICATION->SaveFileContent($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/tmp/templates/__bx_preview/template_styles.css", $TEMPLATE_STYLES))
