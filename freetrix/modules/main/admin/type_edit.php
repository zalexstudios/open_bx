...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST["save"] <> '' || $_POST["apply"] <> '') && $isAdmin && check_freetrix_sessid())
{
	...
	else 
	{
		$DB->Commit();
		if ($_POST["save"] <> '')
			LocalRedirect(FX_ROOT."/admin/type_admin.php?lang=".LANGUAGE_ID);
		else
			LocalRedirect(FX_ROOT."/admin/type_edit.php?EVENT_NAME=".$res["EVENT_NAME"]."&lang=".LANGUAGE_ID);
	}
}
	...
	$context = new CAdminContextMenu(
		array(
			array(
				...
				"LINK"	=> "/freetrix/admin/type_admin.php?lang=".LANGUAGE_ID,
	...
	$context = new CAdminContextMenu(
		array(
			...
			array(
				...
				"LINK"	=> "/freetrix/admin/type_edit.php?lang=".LANGUAGE_ID,
				"TITLE"	=> GetMessage("MAIN_NEW_RECORD_TITLE"),
				"ICON"	=> "btn_new"
			),
			array(
				"TEXT"	=> GetMessage("MAIN_DELETE_RECORD"),
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/freetrix/admin/type_admin.php?ID=".urlencode($arParams["EVENT_NAME"])."&lang=".LANGUAGE_ID."&".freetrix_sessid_get()."&action=delete';",
...
?><tr>
	<td width="40%" class="adm-detail-valign-top">[<a href="/freetrix/admin/message_edit.php?ID=<?=$v["ID"]?>"><?=$v["ID"]?></a>]<?=(strLen(trim($v["SUBJECT"])) > 0 ? " " : "").htmlspecialcharsEx($v["SUBJECT"])?>:</td>
	<td width="60%"><?
			$arLID = array();
