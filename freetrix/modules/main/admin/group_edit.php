...
for($i = 0, $l = count($modules); $i < $l; $i++)
	IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$modules[$i]."/admin/task_description.php");
...
$FX_GROUP_POLICY_CONTROLS = array(
...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_REQUEST["save"] <> '' || $_REQUEST["apply"] <> '') && $USER->CanDoOperation('edit_groups') && check_freetrix_sessid())
{
	if($ID <= 2 && $ID != 0)
		$ACTIVE = "Y";
	...
	$arGroupPolicy = array();
	foreach ($FX_GROUP_POLICY as $key => $value)
	{
		$curVal = ${"gp_".$key};
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/freetrix/admin/group_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
		"ICON"	=> "btn_list"
...
if($USER->CanDoOperation('edit_groups'))
{
	if(intval($ID)>0 && $COPY_ID<=0)
	{
		...
		$aMenu[] = array(
			...
			"LINK"	=> "/freetrix/admin/group_edit.php?lang=".LANGUAGE_ID,
			"ICON"	=> "btn_new"
		);
		if($ID>1)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "/freetrix/admin/group_edit.php?lang=".LANGUAGE_ID."&amp;COPY_ID=".$ID,
				"ICON"	=> "btn_copy"
			);
		}
		...
		if($ID>2)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".CUtil::JSEscape(GetMessage("MAIN_DELETE_RECORD_CONF"))."')) window.location='/freetrix/admin/group_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".freetrix_sessid_get()."';",
				"ICON"	=> "btn_delete"
			);
		}
	}
		...
		<table border="0" cellpadding="0" cellspacing="0" class="internal">
			...
			while ($arUsers = $dbUsers->Fetch())
			{
				...
				<tr>
					...
					<td align="left"><label for="USER_ID_ACT_ID_<?=$ind?>">[<a href="/freetrix/admin/user_edit.php?ID=<?=$arUsers["ID"]?>&lang=<?=LANGUAGE_ID?>" title="<?=GetMessage("MAIN_VIEW_USER")?>"><?=$arUsers["ID"]?></a>] (<?=htmlspecialcharsbx($arUsers["LOGIN"])?>) <?=htmlspecialcharsbx($arUsers["NAME"])?> <?=htmlspecialcharsbx($arUsers["LAST_NAME"])?></label></td>
					<td>
	...
	foreach ($FX_GROUP_POLICY as $key => $value)
	{
		...
		<tr valign="top">
			...
			<td>
				...
				<?$arControl = $FX_GROUP_POLICY_CONTROLS[$key];
				switch($arControl[0])
				{
				case "checkbox":
					?>
			...
			else
			{
				...
				)
				{
					foreach ($arSites["reference_id"] as $i => $site_id_tmp)
					{
						...
						if (strlen($v) > 0)
						{
							?><tr>
								...
								?></td>
								<td style="padding: 3px;"><a href="javascript:void(0)" onClick="settingsDeleteRow(this)"><img src="/freetrix/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"></a></td>
							</tr><?
						}
					}
					...
					<tr id="hidden-rights-row" style="display: none;">
						...
						<td><a href="javascript:void(0)" onClick="settingsDeleteRow(this)"><img src="/freetrix/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"></a></td>
					</tr>
					<?
				}
