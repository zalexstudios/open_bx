...
if (getenv('FREETRIX_VA_VER'))
	define('VMFREETRIX', 'defined');
...
define("STEP_TIME", defined('VMFREETRIX') ? 30 : 15);
# define("DELAY", defined('VMFREETRIX') ? 0 : 3); // reserved
# xdebug_start_trace();
...
$dbconn = $_SERVER['DOCUMENT_ROOT']."/freetrix/php_interface/dbconn.php";
...
$mArr_ru =  array(
			...
			<b>Документация:</b> <a href='http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=35&LESSON_ID=2031' target='_blank'>http://dev.1c-freetrix.ru/learning/course/index.php?COURSE_ID=35&LESSON_ID=2031</a>
			</p>
			",
			"ARC_DOWN" => "Скачать резервную копию с дальнего сайта",
			"ARC_DOWN_FREETRIXCLOUD" => "Развернуть резервную копию из облака &quot;1С-Битрикс&quot;",
			...
			"DBCONN_WARN" => "Внимание! Данные подключения взяты из dbconn.php. Если их не изменить, будет переписана база данных текущего сайта.",
			"HTACCESS_RENAMED_WARN" => "Внимание! Файл .htaccess из архива был сохранен в корне сайта под именем .htaccess.restore, т.к. он может содержать директивы, недопустимые на данном сервере.",
...
$mArr_en = array(
			...
			<b>Documentation:</b> <a href='http://www.freetrixsoft.com/support/training/course/lesson.php?COURSE_ID=12&ID=441' target='_blank'>learning course</a>
			</p>
			",
			"ARC_DOWN" => "Download from remote server",
			"ARC_DOWN_FREETRIXCLOUD" => "Restore the backup from the Freetrix Cloud",
			...
			"DB_ENV" => "restoring in Freetrix Environment",
			...
			"ERR_LOAD_FILE_LIST" => "Wrong Freetrixsoft server response",
			...
			"DBCONN_WARN" => "Warning! Connection settings are read from dbconn.php. If you don't change them, current database will be overwriten.",
			"HTACCESS_RENAMED_WARN" => "Warning! The file .htaccess was saved as .htaccess.restore, because it may contain directives which are not permitted on this server.",
...
{
	$strLog = '';
	if (LoadFile("http://www.1c-freetrix.ru/buy_tmp/backup.php?license=".md5(trim($_REQUEST['license_key']))."&lang=".LANG."&action=get_info", $file = $_SERVER['DOCUMENT_ROOT'].'/file_list.xml') && ($str = file_get_contents($file)))
	{
		if (preg_match_all('/<file name="([^"]+)" size="([^"]+)".*?\\/>/', $str, $regs))
		{
			...
			echo getMsg('SELECT_ARC').':&nbsp;<select name="freetrixcloud_backup">';
			foreach($arFiles as $name => $size)
				echo '<option value="'.htmlspecialcharsbx($name).'" '.($_REQUEST['freetrixcloud_backup'] == $name ? 'selected' : '').'>'.htmlspecialcharsbx($name).' ('.floor($size/1024/1024), ' Mb'.($arParts[$name] > 1 ? ', '.getMsg('CNT_PARTS').': '.$arParts[$name] : '').')</option>';
			echo '</select><br>';
...
//			$port = preg_match('#<port>([^<]+)</port>#i',$str,$regs) ? $regs[1] : false;
		...
		if (!$_REQUEST['try_next'] && $strErrMsg)
		{
			...
			'<input type="hidden" name="source" value="freetrixcloud">'.
			'<input type="hidden" name="freetrixcloud_backup" value="'.htmlspecialcharsbx($_REQUEST['freetrixcloud_backup']).'">'.
	...
	if ($source == 'download')
	{
		...
		if ($res)
		{
			...
			else
			{
				...
				if (count($arHeaders)) // freetrixcloud
				{
					$text .= '<input type=hidden name=source value=freetrixcloud>';
					$text .= '<input type=hidden name="freetrixcloud_backup" value="'.htmlspecialcharsbx(CTar::getNextName($_REQUEST['freetrixcloud_backup'])).'">';
				}
				else
				{
					if ($size < 1024 * 1024)
...
if(!$Step)
{
	$ar = array(
		...
		'TEXT' =>
			...
			getMsg('BEGIN'),
		'BOTTOM' =>
		(defined('VMFREETRIX') ? '<input type=button value="'.getMsg('BUT_TEXT_BACK').'" onClick="document.location=\'/\'"> ' : '').
...
elseif($Step == 1)
{
	...
	$ar = array(
		...
		'TEXT' =>
				...
				'<input type="hidden" name="Step" value="2">'.
				'<div class=t_div>
					<label><input type=radio name=x_source onclick="div_show(0)" '.($_REQUEST['freetrixcloud_backup'] ? 'checked' : '').'>'.getMsg("ARC_DOWN_FREETRIXCLOUD", LANG).'</label>
	...
	<script>
		function div_show(i)
		{
			...
			arSources = [ 'freetrixcloud','download','upload','local','dump' ];
		...
		function LoadFileList()
		{
			...
			query = 'LoadFileList=Y&freetrixcloud_backup=<?=htmlspecialcharsbx($_REQUEST['freetrixcloud_backup'])?>&license_key=' + document.getElementById('license_key').value;
		...
		elseif ($_REQUEST['freetrixcloud_backup'])
		{
			?>
...
elseif($Step == 2)
{
	if(!$bSelectDumpStep)
	{
		...
		if ($rs = $tar->openRead($file1 = $file = $_SERVER['DOCUMENT_ROOT'].'/'.$arc_name))
		{
			...
			else
			{
				...
				$skip = ' <input type=button value="'.getMsg('SKIP').'" onClick="reloadPage(2, \''. LANG.'\')">';
	...
	if ($bSelectDumpStep)
	{
		...
		{
			$bUTF_conf = preg_match('#^[ \t]*define\(.FX_UTF.+true\)#mi', $strFile);
		...
		else
		{
			...
			if (!$_REQUEST['DBName'])
			{
				...
				{
					$strWarning = '<div style="color:red">'.getMsg('DBCONN_WARN').'</div><br>';
					$create_db = false;
				}
				else
				{
					$DBHost = 'localhost'.(@file_exists($_SERVER['DOCUMENT_ROOT'].'/../FreetrixEnv.exe') ? ':31006' : '');
					$DBLogin = 'root';
					$DBPassword = '';
					$DBName = 'freetrix_'.(rand(11,99));
					$create_db = "Y";
				}
			}
			else
			...
			if(count($arDName))
			{
				$ar = array(
					...
					'TEXT' =>
						...
						(
						file_exists($_SERVER['DOCUMENT_ROOT'].'/freetrix/backup/clouds') ?
...
elseif($Step == 3)
{
	...
	if (!isset($_REQUEST['d_pos'])) // start
	{
		...
		if (defined('VMFREETRIX') && !preg_match('#^[ \t]*define..FX_CRONTAB_SUPPORT#mi', $strFile))
			$strFile = '<'.'?define("FX_CRONTAB_SUPPORT", true);?'.'>'.$strFile;

		file_put_contents($dbconn, $strFile);

		if (file_exists($config = $_SERVER['DOCUMENT_ROOT']."/freetrix/.settings.php"))
		{
			ob_start();
			$ar = include($config);
			ob_end_clean();
...
elseif($Step == 5)
{
	@unlink($_SERVER['DOCUMENT_ROOT'].'/freetrixsetup.php');
	$ok = unlink($_SERVER["DOCUMENT_ROOT"]."/restore.php");

	if ($_REQUEST['dump_name'])
	{
		$ok = unlink($_SERVER["DOCUMENT_ROOT"]."/freetrix/backup/".$_REQUEST["dump_name"]) && $ok;
		$ok = unlink($_SERVER["DOCUMENT_ROOT"]."/freetrix/backup/".str_replace('.sql','_after_connect.sql',$_REQUEST["dump_name"])) && $ok;
	}
...
class CDBRestore
{
	...
	function CDBRestore($DBHost, $DBName, $DBLogin, $DBPassword, $DBdump, $d_pos)
	{
		...
		$this->DBName = $DBName;
		$this->DBdump = $_SERVER["DOCUMENT_ROOT"]."/freetrix/backup/".$DBdump;
		$this->d_pos = $d_pos;
	}
...
{
	$arDump = array();
	if (is_dir($back_dir = $_SERVER["DOCUMENT_ROOT"]."/freetrix/backup"))
	{
...
class CTar
{
	...
	const FX_EXTRA = 'BX0000';
	const FX_SIGNATURE = 'Freetrix Encrypted File';
	var $BufferSize;
	var $Buffer;
	...
	function openRead($file)
	{
		...
		if ($this->open($file, 'r'))
		{
			if ('' !== $str = $this->gzip ? gzread($this->res,512) : fread($this->res,512))
			{
				...
				if (trim($data['signature']) != self::FX_SIGNATURE)
				{
...
//		$header['link'] = $data['link'];
	...
	function openWrite($file)
	{
		...
		{
			$enc = pack("a100a90a10a56",md5(uniqid(rand(), true)), self::FX_SIGNATURE, "1.0", "");
	...
	function createEmptyGzipExtra($file)
	{
		...
		$EXTRA = "\x00\x00\x00\x00".self::FX_EXTRA; // 10 байт
...
//						$this->Error('Wrong GZIP Extra Field');
	...
	{
		return md5('FREETRIXCLOUDSERVICE'.$key);
	}
	...
	function DecodeFileName($str)
	{
		if (!$this->EncCurrent)
		{
			...
			elseif (preg_match("/[\xC0-\xFF]/",$str))
				...
				return $this->Error(getMsg('ERR_CANT_DETECT_ENC'));
		}
...
function img($name)
{
	if (file_exists($_SERVER['DOCUMENT_ROOT'].'/images/'.$name))
		return '/images/'.$name;
	return 'http://www.1c-freetrix.ru/images/freetrix_setup/'.$name;
}
...
function CheckHtaccessAndWarn()
{
	...
	if (file_exists($tmp)) // существует какой-то .htaccess в корне
	{
		if (trim(file_get_contents($tmp)) == trim(file_get_contents($tmp1)))
		{
			...
			return '<div style="color:red">'.getMsg('HTACCESS_RENAMED_WARN').'</div>';
	}
	else
	{
...
<IfModule mod_rewrite.c>
	...
	RewriteCond %{REQUEST_FILENAME} !/freetrix/urlrewrite.php$
	RewriteRule ^(.*)$ /freetrix/urlrewrite.php [L]
...
<IfModule mod_expires.c>
	...
	ExpiresByType image/gif "access plus 3 day"
			...
			return '<div style="color:red">'.getMsg('HTACCESS_WARN').'</div>';
		else
			return '<div style="color:red">'.getMsg('HTACCESS_ERR_WARN').'</div>';
	}
}
