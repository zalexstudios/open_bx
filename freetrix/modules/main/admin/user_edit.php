...
$strRedirect_admin = FX_ROOT."/admin/user_admin.php?lang=".LANG;
$strRedirect = FX_ROOT."/admin/user_edit.php?lang=".LANG;
...
if($USER->CanDoOperation('edit_subordinate_users') && !$USER->CanDoOperation('edit_all_users'))
{
	...
	if (count(array_diff($arUserGroups, $arUserSubordinateGroups)) > 0 && !$selfEdit)
		LocalRedirect(FX_ROOT."/admin/user_admin.php?lang=".LANG);
}
...
{
	$mdir = $opt_res["ID"];
	if (file_exists($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$mdir) && is_dir($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$mdir))
	{
		$ofile = $_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$mdir."/options_user_settings.php";
		...
		if(is_array($_REQUEST["profile_module_id"]) && count($_REQUEST["profile_module_id"])>0)
		{
			...
			while ($opt_res = $db_opt_res->Fetch())
			{
				if (in_array($opt_res["ID"], $_REQUEST["profile_module_id"]))
				{
					...
					if (file_exists($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$mdir) && is_dir($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$mdir))
					{
						$ofile = $_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/".$mdir."/options_user_settings_set.php";
						if (file_exists($ofile))
						{
...
if($canViewUserList)
{
	$aMenu[] = array(
		...
		"LINK"	=> "/freetrix/admin/user_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
		"ICON"	=> "btn_list",
...
if($USER->CanDoOperation('edit_all_users') || $USER->CanDoOperation('edit_subordinate_users'))
{
	if ($ID>0 && $COPY_ID<=0)
	{
		...
		$aMenu[] = array(
			...
			"LINK"	=> "/freetrix/admin/user_edit.php?lang=".LANGUAGE_ID,
			"ICON"	=> "btn_new",
			"TITLE"	=> GetMessage("MAIN_NEW_RECORD_TITLE"),
		);
		$aMenu[] = array(
			"TEXT"	=> GetMessage("MAIN_COPY_RECORD"),
			"LINK"	=> "/freetrix/admin/user_edit.php?lang=".LANGUAGE_ID.htmlspecialcharsbx("&COPY_ID=").$ID,
			"ICON"	=> "btn_copy",
		...
		if ($ID!=1)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/freetrix/admin/user_admin.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".freetrix_sessid_get()."';",
				"ICON"	=> "btn_delete",
		...
		<td colspan="2" align="center">
			...
			while ($arGroups = $dbGroups->Fetch())
			{
				...
				<tr>
					...
					<td class="align-left">
						<label for="GROUP_ID_ACT_ID_<?= $ind ?>"><?=htmlspecialcharsbx($arGroups["NAME"])?> [<a href="/freetrix/admin/group_edit.php?ID=<?=$arGroups["ID"]?>&lang=<?=LANGUAGE_ID?>" title="<?=GetMessage("MAIN_VIEW_GROUP")?>"><?echo intval($arGroups["ID"])?></a>]</label>
					</td>
					<td>
