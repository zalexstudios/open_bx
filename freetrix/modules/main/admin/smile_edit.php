...
if ($REQUEST_METHOD == "POST" && (strlen($save) > 0 || strlen($apply) > 0 || strlen($save_and_add) > 0))
{
	...
	elseif (!empty($_FILES["IMAGE"]["tmp_name"]))
	{
		...
		$res = CFile::CheckImageFile($_FILES["IMAGE"], 50000, 0, 0);
		if (strLen($res) > 0)
		{
			$arError[] = array(
				"id" => "IMAGE", 
		...
		{
			@chmod($_SERVER["DOCUMENT_ROOT"].$sUploadDir.$fileName, FX_FILE_PERMISSIONS);
	...
	if (empty($arError))
	{
		...
		if (!empty($_FILES["IMAGE"]["tmp_name"]))
		{
			...
			$arFields["IMAGE_HR"] = isset($_REQUEST["IMAGE_HR"])? 'Y':'N';
		}
		...
		else
		{
			...
			LocalRedirect(strlen($apply) > 0?
				...
				(strlen($save_and_add) > 0 ?
					...
					"smile.php?lang=".LANG."&".GetFilterParams("filter_", false))
			);
		}
	}
...
if ($bInitVars && !empty($arFields))
{
	...
	$arSmile = array(
		...
		"IMAGE" => "",
		"IMAGE_HR" => isset($arFields['IMAGE_HR'])? "Y": "N",
...
else 
{
	...
	$arSmile = array(
		...
		"IMAGE" => "",
		"IMAGE_HR" => isset($_REQUEST['IMAGE_HR'])? "Y": "N",
...
$aMenu = array(
	array(
		...
		"LINK" => "/freetrix/admin/smile.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_list",
	)
);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/freetrix/admin/smile_edit.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_new",
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("SMILE_BTN_DELETE"),
		"LINK" => "javascript:if(confirm('".GetMessage("SMILE_BTN_DELETE_CONFIRM")."')) window.location='/freetrix/admin/smile.php?action=delete&ID[]=".$ID."&lang=".LANG."&".freetrix_sessid_get()."#tb';",
		"ICON" => "btn_delete",
	);
}
...
?>
	...
	<tr<?if ($ID <= 0){ ?> class="adm-detail-required-field"<? }?>>
		<td>
			<?=GetMessage(($ID <= 0)?"SMILE_IMAGE" :"SMILE_IMAGE_UPLOAD")?>:<br><small><?=GetMessage("SMILE_IMAGE_NOTE")?></small></td>
		<td>
			<input type="file" name="IMAGE" size="30" />
			<div style="margin-top: 10px"><label title="<?=GetMessage('SMILE_IMAGE_HR_TITLE')?>"><input type="checkbox" name="IMAGE_HR" /> <?=GetMessage('SMILE_IMAGE_HR')?> (?)</label></div>
		</td>
	</tr>
	<tr>
...
$tabControl->Buttons(array(
	"btnSaveAndAdd" => true,
	"back_url" => "/freetrix/admin/smile.php?lang=".LANG."&".GetFilterParams("filter_", false)));
?>
</form>
<?
$tabControl->End();
...
?>
<?require($DOCUMENT_ROOT."/freetrix/modules/main/include/epilog_admin.php");?>
