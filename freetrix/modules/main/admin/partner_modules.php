...
require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/classes/general/update_client_partner.php");
...
if($isAdmin && $_SERVER["REQUEST_METHOD"] == "POST" && strlen($_POST["module"]) > 0 && check_freetrix_sessid())
{
...
while($info = $rsData->Fetch())
{
	...
	if(!empty($modules[$info["MODULE_ID"]]["VERSION"]))
		$status .= "<br /><a href=\"/freetrix/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=".$info["MODULE_ID"]."\" style=\"color:green;\">".GetMessage("MOD_SMP_NEW_UPDATES")."</a>";
	...
	if(!empty($modules[$info["MODULE_ID"]]) && !empty($modules[$info["MODULE_ID"]]["VERSION"]))
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect("/freetrix/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=".$info["MODULE_ID"]),
		);
	}
	...
	if($info["IsInstalled"])
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?id=".htmlspecialcharsbx($info["MODULE_ID"])."&lang=".LANG."&uninstall=Y&".freetrix_sessid_get()),
		);
	}
	else
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?id=".htmlspecialcharsbx($info["MODULE_ID"])."&lang=".LANG."&install=Y&".freetrix_sessid_get()),
		);
		$arActions[] = array(
			"ICON" => "delete",
			"DEFAULT" => false,
			"TEXT" => GetMessage("MOD_SMP_DELETE"),
			"ACTION" => "if(confirm('".GetMessageJS('MOD_CLEAR_CONFIRM', Array("#NAME#" => htmlspecialcharsbx($info["MODULE_NAME"])))."')) ".$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?id=".htmlspecialcharsbx($info["MODULE_ID"])."&lang=".LANG."&clear=Y&".freetrix_sessid_get()),
		);
	}
...
while($info = $rsData->Fetch())
{
	...
	if($info["UPDATE_END"] != "Y")
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin1->ActionRedirect("/freetrix/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=".$info["MODULE_ID"]),
		);
	}
