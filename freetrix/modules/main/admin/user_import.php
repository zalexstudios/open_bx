...
require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/general/csv_user_import.php");

if(!$USER->IsAdmin())
	$APPLICATION->AuthForm(GetMessage("ACCESS_DENIED"));

//Download sample
$filename = $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/admin/sample.csv";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $tabStep > 2 && check_freetrix_sessid())
{
	...
	if (isset($_REQUEST["action"]) && $_REQUEST["action"] == "import" && $strError === false)
	{
		...
		elseif ($ldp)
		{
			...
			// selecting all users from Freetrix CMS for this LDAP
			CTimeZone::Disable();
...
?>
	<tr class="adm-detail-required-field">
		...
		<td>
			...
			?>
			<?if (is_file($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/admin/sample.csv")):?>
...
<?endif?>
	...
	<tr>
		...
		<td>
			...
			&nbsp;<a href="/freetrix/admin/ldap_server_edit.php?lang=<?=LANGUAGE_ID?>"><?=GetMessage("USER_IMPORT_NEW_LDAP_SERVER")?></a>
		</td>
	</tr>
	<?endif?>
...
<?elseif ($tabStep == 2 && $dataSource == "ldap"):?>
	...
	<tr class="adm-detail-required-field">
		...
		<td width="50%">
			...
			&nbsp;<a href="/freetrix/admin/ldap_server_edit.php?lang=<?=LANGUAGE_ID?>&back_url=<?=urlencode('/freetrix/admin/user_import.php?lang='.$lang.'&dataSource=ldap&tabStep=2')?>"><?=GetMessage("USER_IMPORT_NEW_LDAP_SERVER")?></a>
		</td>
	</tr>
	<tr id="predef">
