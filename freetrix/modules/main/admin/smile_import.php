...
if ($REQUEST_METHOD == "POST" && (strlen($save) > 0 || strlen($apply) > 0))
{
	...
	elseif (!empty($_FILES["IMPORT"]["tmp_name"]))
	{
		...
		{
			@chmod($sUploadDir.$fileName, FX_FILE_PERMISSIONS);
		}
	}
	elseif (empty($_FILES["IMPORT"]["tmp_name"]))
	{
...
?>
<?=BeginNote();?>
<div><?=GetMessage('IM_IMPORT_HELP_1', Array('#LINK_START#'=>'<a href="/freetrix/admin/fileman_admin.php?lang='.LANG.'&path=%2Ffreetrix%2Fmodules%2Fmain%2Finstall%2Fsmiles">', '#LINK_END#'=>'</a>'))?></div>
