...
if (!function_exists("__GroupRightsShowRow"))
{
	...
	function __GroupRightsShowRowGroup($module_id, $ar, $value, $arSites, $arRightsUseSites, $site_id_tmp)
	{
		$v = $GLOBALS["APPLICATION"]->GetGroupRight($module_id, array($value["ID"]), "N", "N", $site_id_tmp);
		if($v == '')
			return;

		$arUsedGroups[$value["ID"]] = true;
		
		$titleCol = $value["NAME"]." [<a title=\"".GetMessage("MAIN_USER_GROUP_TITLE")."\" href=\"/freetrix/admin/group_edit.php?ID=".$value["ID"]."&amp;lang=".LANGUAGE_ID."\">".$value["ID"]."</a>]:".(($value["ID"]==1 && $md->SHOW_SUPER_ADMIN_GROUP_RIGHTS=="Y") ? "<br><small>".GetMessage("MAIN_SUPER_ADMIN_RIGHTS_COMMENT")."</small>" : "");
	...
	function __GroupRightsShowRow($titleCol, $groupID, $group_id, $ar, $v, $site_id_tmp, $arRightsUseSites, $arSites, $useDefault = true)
	{
		...
		<tr>
			...
			?></span></td>
			<td width="0%"><a href="javascript:void(0)" onClick="__GroupRightsDeleteRow(this)"><img src="/freetrix/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"></a></td>
		</tr>
		<?
	}
...
if($REQUEST_METHOD=="POST" && strlen($Update)>0 && $MODULE_RIGHT=="W" && check_freetrix_sessid())
{
	if (count($GROUPS)>0)
	{
...
__GroupRightsShowRowDefault($module_id, $ar, $value, $arSites, $arRightsUseSites, "");
foreach ($arSites["reference_id"] as $site_id_tmp)
	__GroupRightsShowRowDefault($module_id, $ar, $value, $arSites, $arRightsUseSites, $site_id_tmp);

$arUsedGroups = array();
foreach($arGROUPS as $value):
	__GroupRightsShowRowGroup($module_id, $ar, $value, $arSites, $arRightsUseSites, false);
	foreach ($arSites["reference_id"] as $site_id_tmp)
		__GroupRightsShowRowGroup($module_id, $ar, $value, $arSites, $arRightsUseSites, $site_id_tmp);
endforeach; ?>
