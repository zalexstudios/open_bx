...
require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/include/condition.php");
...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST["save"] <> '' || $_POST["apply"] <> '') && $isAdmin && check_freetrix_sessid())
{
	$arFields = array(
	...
	else
	{
		...
		if ($bNew && $_POST["START_SITE_WIZARD"] == "Y")
		{
			...
			$indexContent = '<'.'?'.
				...
				'include_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/install/wizard/wizard.php");'.
				'?'.'>';
		...
		if ($_POST["save"] <> '')
			LocalRedirect(FX_ROOT."/admin/site_admin.php?lang=".LANGUAGE_ID);
		else
			LocalRedirect(FX_ROOT."/admin/site_edit.php?lang=".LANGUAGE_ID."&LID=".$LID."&".$tabControl->ActiveTabParam());
	}
}
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/freetrix/admin/site_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
...
if(!$bNew)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/freetrix/admin/site_edit.php?lang=".LANGUAGE_ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/freetrix/admin/site_edit.php?lang=".LANGUAGE_ID."&amp;COPY_ID=".urlencode($str_LID),
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".CUtil::JSEscape(GetMessage("MAIN_DELETE_RECORD_CONF"))."')) window.location='/freetrix/admin/site_admin.php?ID=".urlencode($str_LID)."&lang=".LANGUAGE_ID."&action=delete&".freetrix_sessid_get()."';",
	...
	<tr id="ID_HIDDENABLE_TR"<?= ($bNew && ($str_START_SITE_WIZARD != "N")) ? "style='display:none'" : ""?>>
		<td colspan="2" align="center">
			...
			<tr>
				...
				<td><a title="<?=GetMessage('MAIN_PREVIEW_TEMPLATE')?>" href="javascript:void(0)" onClick="if(document.getElementById('SITE_TEMPLATE[<?=$i?>][TEMPLATE]').selectedIndex>0)window.open((document.bform.SERVER_NAME.value?'http://'+document.bform.SERVER_NAME.value:'') + document.bform.DIR.value+'?freetrix_preview_site_template='+document.getElementById('SITE_TEMPLATE[<?=$i?>][TEMPLATE]')[document.getElementById('SITE_TEMPLATE[<?=$i?>][TEMPLATE]').selectedIndex].value);return false;"><img src="/freetrix/images/main/preview.gif" width="16" height="16" border="0"></a></td>
