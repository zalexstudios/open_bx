...
elseif (!check_freetrix_sessid()):?>
	<span style="color:red"><?=GetMessage("MAIN_WIZARD_INSTALL_SESSION_EXPIRED")?></span>
	<form action="<?=$APPLICATION->GetCurPageParam(freetrix_sessid_get(), Array("sessid"))?>" method="post">
...
<?
	...
	{
		$path = $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/".$arWizardName[0]."/install/wizards/".$arWizardName[1]."/".$arWizardName[2];

		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/".$arWizardName[0]."/install/wizards/".$arWizardName[1]."/".$arWizardName[2],
			$_SERVER["DOCUMENT_ROOT"]."/freetrix/wizards/".$arWizardName[1]."/".$arWizardName[2],
			true,
			true,
			false,
			""
