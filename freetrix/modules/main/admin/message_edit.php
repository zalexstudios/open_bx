...
if($REQUEST_METHOD=="POST" && (strlen($save)>0 || strlen($apply)>0)&& $isAdmin && check_freetrix_sessid())
{
	...
	else
	{
		if (strlen($save)>0)
		{
			if (!empty($_REQUEST["type"]))
				LocalRedirect(FX_ROOT."/admin/type_edit.php?EVENT_NAME=".$EVENT_NAME."&lang=".LANGUAGE_ID);
			else
				LocalRedirect(FX_ROOT."/admin/message_admin.php?lang=".LANGUAGE_ID);
		}
		else
			LocalRedirect(FX_ROOT."/admin/message_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&type=".$_REQUEST["type"]."&".$tabControl->ActiveTabParam());
	}
}
$str_ACTIVE="Y";
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/freetrix/admin/message_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
...
if (intval($ID)>0 && $COPY_ID<=0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/freetrix/admin/message_edit.php?lang=".LANGUAGE_ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/freetrix/admin/message_edit.php?lang=".LANGUAGE_ID.htmlspecialcharsbx("&COPY_ID=").$ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/freetrix/admin/message_admin.php?ID=".$ID."&lang=".LANGUAGE_ID."&".freetrix_sessid_get()."&action=delete';",
