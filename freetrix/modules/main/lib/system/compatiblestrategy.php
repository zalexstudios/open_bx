	...
	implements IApplicationStrategy
{
	...
	{
		if (($includePath = \Freetrix\Main\Loader::getLocal("init.php")) !== false)
			require_once($includePath);

		if (($includePath = \Freetrix\Main\Loader::getPersonal("php_interface/init.php")) !== false)
			require_once($includePath);

		if (($includePath = \Freetrix\Main\Loader::getPersonal("php_interface/".SITE_ID."/init.php")) !== false)
			require_once($includePath);
	}

	public function initializeDispatcher()
	{
		$dispatcher = \Freetrix\Main\Application::getInstance()->getDispatcher();
	...
	public function preInitialize()
	{
		...
		if(isset($_REQUEST["FX_STATE"])) unset($_REQUEST["FX_STATE"]);
		if(isset($_GET["FX_STATE"])) unset($_GET["FX_STATE"]);
		if(isset($_POST["FX_STATE"])) unset($_POST["FX_STATE"]);
		if(isset($_COOKIE["FX_STATE"])) unset($_COOKIE["FX_STATE"]);
		if(isset($_FILES["FX_STATE"])) unset($_FILES["FX_STATE"]);
		...
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/general/version.php");
	...
	public function initializeBasicKernel()
	{
		...
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/".$GLOBALS["DBType"]."/main.php");	//main class
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/".$GLOBALS["DBType"]."/option.php");	//options and settings class
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/general/cache.php");	//various cache classes
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/general/cache_html.php");	//html cache class support
		...
		if(!defined("FX_COMP_MANAGED_CACHE") && \COption::getOptionString("main", "component_managed_cache_on", "Y") <> "N")
			define("FX_COMP_MANAGED_CACHE", true);
		...
		\CModule::addAutoloadClasses(
			...
			array(
				...
				"Freetrix\\Main\\Entity\\Base" => "lib/entity/base.php",
				"Freetrix\\Main\\Entity\\DataManager" => "lib/entity/base.php",
				"Freetrix\\Main\\Entity\\Field" => "lib/entity/field.php",
				"Freetrix\\Main\\Entity\\ScalarField" => "lib/entity/scalarfield.php",
				"Freetrix\\Main\\Entity\\IntegerField" => "lib/entity/integerfield.php",
				"Freetrix\\Main\\Entity\\FloatField" => "lib/entity/floatfield.php",
				"Freetrix\\Main\\Entity\\StringField" => "lib/entity/stringfield.php",
				"Freetrix\\Main\\Entity\\TextField" => "lib/entity/textfield.php",
				"Freetrix\\Main\\Entity\\BooleanField" => "lib/entity/booleanfield.php",
				"Freetrix\\Main\\Entity\\DateField" => "lib/entity/datefield.php",
				"Freetrix\\Main\\Entity\\DatetimeField" => "lib/entity/datetimefield.php",
				"Freetrix\\Main\\Entity\\EnumField" => "lib/entity/enumfield.php",
				"Freetrix\\Main\\Entity\\ExpressionField" => "lib/entity/expressionfield.php",
				"Freetrix\\Main\\Entity\\UField" => "lib/entity/ufield.php",
				"WorkgroupEntity" => "lib/workgroup.php",
				"Freetrix\\Main\\Entity\\ReferenceField" => "lib/entity/referencefield.php",
				"Freetrix\\Main\\Entity\\Query" => "lib/entity/query.php",
				"Freetrix\\Main\\Entity\\QueryChain" => "lib/entity/querychain.php",
				"Freetrix\\Main\\Entity\\QueryChainElement" => "lib/entity/querychainelement.php",
		...
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/".$GLOBALS["DBType"]."/agent.php");
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/".$GLOBALS["DBType"]."/user.php");
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/".$GLOBALS["DBType"]."/event.php");
		...
		require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/".$GLOBALS["DBType"]."/usertype.php");
	...
	public function authenticateUser()
	{
		...
		//\Freetrix\Main\Security\Authentication::copyToSession($currentUser);

		// раскомментировать когда уберем из CurrentUser вызовы USER
		$GLOBALS["USER"] = new \CUser;
		define("FX_STARTED", true); // нужен для инфоблоков чтобы понять - определен ли сайт???
...
// этот редирект нужно убирать
	...
	public function createDatabaseConnection()
	{
		...
		if(isset($DBSQLServerType) && $DBSQLServerType == "NATIVE")
			require_once($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/classes/".$DBType."/database_ms.php");
		else
		...
		{
			if(file_exists(($fname = $_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/dbconn_error.php")))
				include($fname);
			else
