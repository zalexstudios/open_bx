...
final class Loader
{
	...
	const FREETRIX_HOLDER = "freetrix";
	...
	private static $arLoadedModulesHolders = array("main" => self::FREETRIX_HOLDER);
	...
	/**
	 * \Freetrix\Main\IO\File -> /main/lib/io/file.php
	 * \Freetrix\IBlock\Type -> /iblock/lib/type.php
	 * \Freetrix\IBlock\Section\Type -> /iblock/lib/section/type.php
	...
	public static function autoLoad($className)
	{
		...
		if (isset(self::$arAutoLoadClasses[$file]))
		{
			...
			{
				$m = $pathInfo["module"];
				$h = isset(self::$arLoadedModulesHolders[$m]) ? self::$arLoadedModulesHolders[$m] : 'freetrix';
	...
	/**
	 * Checks if file exists in /local or /freetrix directories
	 *
	 * @param string $path File path relative to /local/ or /freetrix/
	...
	public static function getLocal($path, $root = null)
	{
		...
		elseif (file_exists($root."/freetrix/".$path))
			return $root."/freetrix/".$path;
		else
			return false;
	}
	 ...
	 * If $_SERVER["FX_PERSONAL_ROOT"] is not set than personal directory is equal to /freetrix/
	 *
	...
	{
		$root = static::getDocumentRoot();
		$personal = isset($_SERVER["FX_PERSONAL_ROOT"]) ? $_SERVER["FX_PERSONAL_ROOT"] : "";
...
if (!function_exists("__autoload"))
{
	...
	{
		\spl_autoload_register('\Freetrix\Main\Loader::autoLoad');
	}
	else
	{
