	...
	implements ICacheEngine, ICacheEngineStat
{
	...
	private static function unlink($fileName)
	{
		...
		{
			@chmod($fileName, FX_FILE_PERMISSIONS);
			if (@unlink($fileName))
				return true;
		}
		else
	...
	private static function addAgent()
	{
		...
		{
			$bAgentAdded = true;
			$rsAgents = \CAgent::GetList(array("ID"=>"DESC"), array("NAME" => "\\Freetrix\\Main\\Data\\CacheEngineFiles::delayedDelete(%"));
			if (!$rsAgents->Fetch())
			{
				$res = \CAgent::AddAgent(
					"\\Freetrix\\Main\\Data\\CacheEngineFiles::delayedDelete();",
					"main", //module
					"Y", //period
					1 //interval
				);
