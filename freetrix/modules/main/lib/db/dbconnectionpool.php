...
class DbConnectionPool
{
	...
	private function getDbConnectionClassNameByType($connectionType)
	{
		...
		{
			case 'mysql':
				$className = "\\Freetrix\\Main\\DB\\MysqliDbConnection";
				if (!\extension_loaded("mysqli"))
					$className = "\\Freetrix\\Main\\DB\\MysqlDbConnection";
				break;
			case 'oracle':
				$className = "\\Freetrix\\Main\\DB\\OracleDbConnection";
				break;
			case 'mssql':
				$className = "\\Freetrix\\Main\\DB\\MssqlDbConnection";
				break;
			case 'mssql_native':
				$className = "\\Freetrix\\Main\\DB\\MssqlNativeDbConnection";
				break;
			default:
				throw new \Freetrix\Main\Config\ConfigurationException("Database type mismatch");
		}
	 ...
	 * @return DbConnection
	 * @throws \Freetrix\Main\Config\ConfigurationException If connection with specified name does not exist
	 */
	public function getConnection($name = "")
	{
		...
		if (!isset($this->arConnections[$name]))
		{
			...
			{
				throw new \Freetrix\Main\Config\ConfigurationException(sprintf("Database connection '%s' is not found", $name));
			}
		}
	...
	/*public static function getDbNodeConnection($nodeId, $checkStatus = true)
		{
			...
			{
				if(file_exists($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/dbconn_error.php"))
					include($_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/dbconn_error.php");
				else
