...
{
	require($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/main/public/component_props.php");
	die();
}
...
if($localPath !== false && $arComponentDescription["ICON"] <> "" && $io->FileExists($io->RelativeToAbsolutePath($localPath."/".$arComponentDescription["ICON"])))
	...
	$sIcon = "/freetrix/images/fileman/htmledit2/component.gif";
?>
<?
...
<?endif;?>
<p class="note" title="<?echo GetMessage("comp_prop_path")?>"><a href="/freetrix/admin/fileman_admin.php?lang=<?echo LANGUAGE_ID?>&amp;path=<?echo urlencode($localPath)?>"><?echo htmlspecialcharsbx($_GET["component_name"])?></a></p>
<?
if($strWarning <> "")
{
		...
		else
		{
			...
			else
			{
				if ($prop["TYPE"] == 'FILE')
				{
					...
					(
						"event" => "FX_FD_".$ID,
						"arResultDest" => Array("FUNCTION_NAME" => "FX_FD_ONRESULT_".$ID),
						"arPath" => Array(),
					...
					else
					{
						...
						$res .= '<input size='.$prop["COLS"].' value="..." type="button" onclick="window.FX_FD_'.$ID.'();">';
					}

					$res .= '<script>
					setTimeout(function(){
						if (BX("bx_fd_input_'.strtolower($ID).'"))
							BX("bx_fd_input_'.strtolower($ID).'").onclick = window.FX_FD_'.$ID.';
					}, 200);

					window.FX_FD_ONRESULT_'.$ID.' = function(filename, filepath)
					{
