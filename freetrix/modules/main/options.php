...
if($_SERVER["REQUEST_METHOD"] == "GET" && $USER->IsAdmin() && $_REQUEST["RestoreDefaults"] <> '' && check_freetrix_sessid())
{
	$aSaveVal = array(
		...
		array("NAME"=>"vendor", "DEF"=>"1c_freetrix"),
...
if($_SERVER["REQUEST_METHOD"] == "GET" && $USER->CanDoOperation('edit_other_settings') && $_REQUEST["GenKey"] <> '' && check_freetrix_sessid())
{
...
$arAllOptions = array(
	"main" => Array(
		...
		Array("bx_fast_download", GetMessage("MAIN_OPT_FX_FAST_DOWNLOAD"), "N", Array("checkbox", "N")),
		Array("note" => GetMessage("MAIN_OPT_FX_FAST_DOWNLOAD_HINT")),
...
if(CRsaSecurity::Possible())
{
	...
	$mess .= '<br><br><input type="button" name="" value="'.GetMessage("MAIN_OPT_SECURE_GENKEY").'" onclick="window.location=\'/freetrix/admin/settings.php?GenKey=Y&lang='.LANGUAGE_ID.'&mid='.urlencode($mid).'&'.freetrix_sessid_get().'&tabControl_active_tab=edit6\'">';
...
$arAllOptions["auth"][] = Array("new_user_registration_email_confirmation", GetMessage("MAIN_REGISTER_EMAIL_CONFIRMATION", array("#EMAIL_TEMPLATES_URL#" => "/freetrix/admin/message_admin.php?lang=".LANGUAGE_ID."&set_filter=Y&find_type_id=NEW_USER_CONFIRM")), "N", Array("checkbox", "Y"));
...
?>
	<tr valign="top">
		<td><?=$value["NAME"]." [<a title=\"".GetMessage("MAIN_USER_GROUP_TITLE")."\" href=\"/freetrix/admin/group_edit.php?ID=".$value["ID"]."&amp;lang=".LANGUAGE_ID."\">".$value["ID"]."</a>]:"?></td>
		<td>
		<?
...
function RestoreDefaults()
{
	if(confirm('<?echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
		window.location = "<?echo $APPLICATION->GetCurPage()?>?RestoreDefaults=Y&lang=<?=LANGUAGE_ID?>&mid=<?echo urlencode($mid)?>&<?echo freetrix_sessid_get()?>";
}
...
function DoNext(name, id, recount)
{
	...
	if (result['stop'] == false)
	{
		if (result['done'] == true)
		{
			...
			if (recount == 'begin')
				str = '&recount=begin';
			document.getElementById('frame_disk_quota').src='/freetrix/admin/quota.php?name=' + id + '&id=' + name + str + '&<?echo freetrix_sessid_get()?>';
		}
	}
	else
	{
