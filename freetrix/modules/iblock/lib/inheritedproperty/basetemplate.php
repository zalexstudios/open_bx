...
class BaseTemplate
{
	/** @var \Freetrix\Iblock\InheritedProperty\BaseValues|null */
	...
	{
		$templateList = \Freetrix\Iblock\InheritedPropertyTable::getList(array(
		...
		while ($row = $templateList->fetch())
		{
			...
			if (array_key_exists($CODE, $templates))
			{
				...
				{
					if ($templates[$CODE] != "")
						\Freetrix\Iblock\InheritedPropertyTable::update($row["ID"], array(
							"TEMPLATE" => $templates[$CODE],
						));
					else
						\Freetrix\Iblock\InheritedPropertyTable::delete($row["ID"]);
		...
		if (!empty($templates))
		{
			foreach ($templates as $CODE => $TEMPLATE)
			{
				if ($TEMPLATE != "")
				{
					\Freetrix\Iblock\InheritedPropertyTable::add(array(
						"IBLOCK_ID" => $this->entity->getIblockId(),
						"CODE" => $CODE,
						"ENTITY_TYPE" => $this->entity->getType(),
						"ENTITY_ID" => $this->entity->getId(),
