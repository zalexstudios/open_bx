...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/main/interface/admin_lib.php");
...
$dsc_cookie_name = COption::GetOptionString('main', 'cookie_name', 'FREETRIX_SM')."_DSC";
...
if($bCatalog)
{
	...
	$arProductTypeList = array(
		CCatalogProduct::TYPE_PRODUCT => GetMessage('IBLIST_A_CATALOG_TYPE_MESS_PRODUCT'),
		CCatalogProduct::TYPE_SET => GetMessage('IBLIST_A_CATALOG_TYPE_MESS_SET'),
			...
			if($bCatalog)
			{
				if ($boolCatalogPrice && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_edit_price"))
				{
					...
					$boolProductExists = ($boolMethodExist
						? CCatalogProduct::IsExistProduct($ID)
...
if ($bCatalog)
{
	...
	if (!empty($arProductGroupIDs))
	{
		...
		{
			$arSet['OWNER_ID'] = intval($arSet['OWNER_ID']);
			$arSet['ITEM_ID'] = intval($arSet['ITEM_ID']);
