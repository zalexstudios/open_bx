...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/iblock/admin/iblock_element_edit.php");
IncludeModuleLangFile(__FILE__);

require_once($_SERVER['DOCUMENT_ROOT'].'/freetrix/modules/iblock/classes/general/subelement.php');
...
define("FX_SUB_SETTINGS",(array_key_exists('bxsku', $_REQUEST) && 'Y' == $_REQUEST['bxsku']));
...
$bWorkflow = ('N' != $arIBlock['WORKFLOW']) && CModule::IncludeModule("workflow");
$bBizproc = ('N' != $arIBlock['BIZPROC']) && CModule::IncludeModule("bizproc");
...
//			"ONSELECT" => "InheritedPropertiesTemplates.onTabSelect();",
	...
	)
	{
		...
		else
		{
			if (
				$bCatalog
				&& file_exists($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/subproduct_edit_validator.php")
			)
			{
				// errors'll be appended to $strWarning;
				include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/subproduct_edit_validator.php");
			}
			...
			if (strlen($strWarning) <= 0)
			{
				...
				{
					include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/subproduct_edit_action.php");
				}
			} // if ($strWarning)
...
$strFormAction = "/freetrix/admin/iblock_subelement_edit.php?type=".urlencode($type)."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section);
...
if ($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	<td style="white-space: nowrap;">
		<input type="text" size="50" name="SUB_NAME" id="SUB_NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="sub_name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/freetrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
	</td>
</tr><?
	...
	<td><?echo $tabControl->GetCustomLabelHTML()?></td>
	<td style="white-space: nowrap;">
		<input type="text" size="50" name="SUB_CODE" id="SUB_CODE" maxlength="255" value="<?echo $str_CODE?>"><image id="sub_code_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/freetrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
	</td>
</tr><?
...
?>
	...
	<tr id="tr_SUB_PREVIEW_TEXT_EDITOR">
		<td colspan="2" align="center">
			...
			false,
			array(
				'toolbarConfig' => CFileman::GetEditorToolbarConfig("iblock_".(defined('FX_SUB_SETTINGS') && FX_SUB_SETTINGS == true ? 'admin' : 'public')),
...
if ($arShowTabs['catalog'])
{
	...
	include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/subproduct_edit.php");
				...
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					<tr>
						...
						<td width="1%" align="right">
							...
							<a href="iblock_element_edit.php?WF=<?= $WF ?>&ID=<?= $ID ?>&type=<?= $type ?>&lang=<?= $lang ?>&IBLOCK_ID=<?= $IBLOCK_ID ?>&find_section_section=<?= $find_section_section ?>&stop_bizproc=<?= $arDocumentState["ID"] ?>&<?= freetrix_sessid_get() ?>"><?echo GetMessage("IBEL_BIZPROC_STOP")?></a>
							<?endif;?>
						</td>
					</tr>
				</table>
