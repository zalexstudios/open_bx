...
if($back_url=='')
	$back_url = '/freetrix/admin/iblock_type_admin.php?lang='.$lang;
...
$bVarsFromForm = false;
if($REQUEST_METHOD=="POST" && strlen($Update)>0 && check_freetrix_sessid())
{
	$arFields = Array();
	if(strlen($ID)<=0)
		$arFields["ID"] = $NEW_ID;
...
if(strlen($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessageJS("IBTYPE_E_DEL_CONF")."')) window.location='/freetrix/admin/iblock_type_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".freetrix_sessid_get()."';",
		"ICON" => "btn_delete"
		);
}
