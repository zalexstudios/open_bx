...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/iblock/admin/iblock_element_admin.php");
...
if (!(false == B_ADMIN_SUBELEMENTS_LIST && $bCopy))
{
	...
	while ($arRes = $rsData->NavNext(true, "f_"))
	{
		...
		$edit_url = '/freetrix/admin/iblock_subelement_edit.php?WF=Y&type='.urlencode($strSubIBlockType).'&IBLOCK_ID='.$intSubIBlockID.'&lang='.LANGUAGE_ID.'&PRODUCT_ID='.$ID.'&ID='.$arRes_orig['ID'].'&TMP_ID='.$strSubTMP_ID.$sThisSectionUrl;
		...
		if ($f_LOCKED_USER_NAME)
			$row->AddViewField("LOCKED_USER_NAME", '<a href="/freetrix/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$f_WF_LOCKED_BY.'" title="'.GetMessage("IBEL_A_USERINFO").'">'.$f_LOCKED_USER_NAME.'</a>');
		if ($f_USER_NAME)
			$row->AddViewField("USER_NAME", '<a href="/freetrix/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$f_MODIFIED_BY.'" title="'.GetMessage("IBEL_A_USERINFO").'">'.$f_USER_NAME.'</a>');
		$row->AddViewField("CREATED_USER_NAME", '<a href="/freetrix/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$f_CREATED_BY.'" title="'.GetMessage("IBEL_A_USERINFO").'">'.$f_CREATED_USER_NAME.'</a>');

		if ($bSubWorkFlow || $bSubBizproc)
		{
			$lamp = "/freetrix/images/workflow/".$lockStatus.".gif";
			if ($lockStatus=="green")
		...
		if ($boolSubBizproc)
		{
			...
			foreach ($arDocumentStates as $kk => $vv)
			{
				...
				$arStr[$vv["TEMPLATE_ID"]] .= "<a href=\"/freetrix/admin/bizproc_log.php?ID=".$kk."\">".(strlen($vv["STATE_TITLE"]) > 0 ? $vv["STATE_TITLE"] : $vv["STATE_NAME"])."</a><br />";
...
if (!empty($arRows))
{
	...
	{
		$edit_url = '/freetrix/admin/iblock_subelement_edit.php?WF=Y&type='.urlencode($strSubIBlockType).'&IBLOCK_ID='.$intSubIBlockID.'&lang='.LANGUAGE_ID.'&PRODUCT_ID='.$ID.'&ID='.$row->arRes['orig']['ID'].'&TMP_ID='.$strSubTMP_ID.$sThisSectionUrl;
			...
			else
			{
				...
				)
				{
					...
					$arActions[] = array(
						...
						"ACTION"=>"(new BX.CAdminDialog({
							'content_url': '/freetrix/admin/iblock_subelement_edit.php?WF=Y&type=".CUtil::JSEscape(htmlspecialcharsbx($strSubIBlockType))."&IBLOCK_ID=".$intSubIBlockID."&lang=".LANGUAGE_ID."&PRODUCT_ID=".$ID."&ID=".$row->arRes['orig']['ID']."&TMP_ID=".urlencode($strSubTMP_ID).$sThisSectionUrl.'&'.freetrix_sessid_get()."',
							'content_post': '".(!(defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1) ? '&bxsku=Y' : '')."&bxpublic=Y',
							'draggable': true,
							'resizable': true,
				...
				)
				{
					$arActions[] = array(
						...
						"ACTION"=>"(new BX.CAdminDialog({
							'content_url': '/freetrix/admin/iblock_subelement_edit.php?WF=Y&type=".CUtil::JSEscape(htmlspecialcharsbx($strSubIBlockType))."&IBLOCK_ID=".$intSubIBlockID."&lang=".LANGUAGE_ID."&PRODUCT_ID=".$ID."&ID=".$row->arRes['orig']['ID']."&TMP_ID=".urlencode($strSubTMP_ID)."&action=copy".$sThisSectionUrl.'&'.freetrix_sessid_get()."',
							'content_post': '".(!(defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1) ? '&bxsku=Y' : '')."&bxpublic=Y',
							'draggable': true,
							'resizable': true,
							'buttons': [BX.CAdminDialog.btnSave, BX.CAdminDialog.btnCancel]
...
function ShowNewOffer(id)
{
	...
	else
	{
		...
		PostParams.sessid = BX.freetrix_sessid();
		(new BX.CAdminDialog({
			'content_url': '/freetrix/admin/iblock_subelement_edit.php?WF=Y&type=<? echo CUtil::JSEscape(htmlspecialcharsbx($strSubIBlockType)); ?>&IBLOCK_ID=<? echo $intSubIBlockID; ?>&lang=<? echo LANGUAGE_ID; ?>&PRODUCT_ID=<? echo $intSubPropValue; ?>&ID=0&TMP_ID=<? echo urlencode($strSubTMP_ID).$sThisSectionUrl; ?>',
...
function ShowSkuGenerator(id)
{
	...
	if(is_array($arIBlock) && isset($arIBlock['FIELDS']) && is_array($arIBlock['FIELDS']))
	{
...
?>
	...
	(new BX.CAdminDialog({
		'content_url': '/freetrix/admin/iblock_subelement_generator.php?subIBlockId=<? echo $intSubIBlockID; ?>&subPropValue=<? echo $intSubPropValue; ?>&subTmpId=<? echo $strSubTMP_ID; ?>&iBlockId=<? echo $arSubCatalog['PRODUCT_IBLOCK_ID']; ?>',
		'content_post': PostParams,
		'draggable': true,
		'resizable': true,
