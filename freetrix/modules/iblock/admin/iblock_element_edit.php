...
require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/iblock/prolog.php");
...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].FX_ROOT."/modules/iblock/admin/iblock_element_edit_compat.php");

IncludeModuleLangFile(__FILE__);
$IBLOCK_ID = 0;
if (isset($_REQUEST['IBLOCK_ID']))
	$IBLOCK_ID = intval($_REQUEST["IBLOCK_ID"]); //information block ID
...
$APPLICATION->AddHeadScript('/freetrix/js/iblock/iblock_edit.js');
...
do{ //one iteration loop
	...
	if($bCustomForm)
	{
		...
		{
			// need return OFFERS fields for compatible with custom forms
		}
	}
	...
	)
	{
		if (CCatalogAdminTools::changeTabs($IBLOCK_ID, $ID, $arMainCatalog))
		{
			...
			LocalRedirect("/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, $arUrlParams, "&".$tabControl->ActiveTabParam()));
		}
	}
	...
	)
	{
		...
		if(!(check_freetrix_sessid() || $_SESSION['IBLOCK_CUSTOM_FORM']===true))
		{
		...
		else
		{
			if ($bCatalog)
			{
				...
				if (
					$arShowTabs['catalog']
					&& file_exists($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/product_edit_validator.php")
				)
				{
				// errors'll be appended to $strWarning;
					...
					include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/product_edit_validator.php");
				}
				if ($arShowTabs['product_set'])
				{
			...
			if (strlen($strWarning) <= 0)
			{
				...
				{
					$ELEMENT_PREVIEW_PICTURE_FILE_NAME = \Freetrix\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["ELEMENT_PREVIEW_PICTURE_FILE_NAME"]);
					$ELEMENT_DETAIL_PICTURE_FILE_NAME = \Freetrix\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["ELEMENT_DETAIL_PICTURE_FILE_NAME"]);
				...
				{
					$ipropValues = new \Freetrix\Iblock\InheritedProperty\ElementValues($IBLOCK_ID, $ID);
				...
				if ('' == $strWarning && $bCatalog)
				{
					...
					if ($arShowTabs['catalog'])
					{
						include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/product_edit_action.php");
					}
		...
		else
		{
			...
			if(strlen($apply) <= 0 && strlen($save_and_add) <= 0)
			{
				...
				{
					LocalRedirect("/freetrix/admin/".CIBlock::GetAdminElementListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section))));
				}
			}
			elseif(strlen($save_and_add) > 0)
			{
				...
				{
					LocalRedirect("/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, 0, $params, "&".$tabControl->ActiveTabParam()));
				}
			}
			else
			{
				LocalRedirect("/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
					"WF" => ($WF=="Y"? "Y": null),
					"find_section_section" => intval($find_section_section),
					"return_url" => (strlen($return_url) > 0? $return_url: null),
	...
	if(!empty($dontsave) && check_freetrix_sessid())
	{
		if($bWorkflow)
			CIBlockElement::WF_UnLock($ID);
		...
		if(strlen($return_url)>0)
		{
			...
			elseif(defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1)
			{
...
else
{
	...
	else
	{
		...
		if($arElement = $result->ExtractFields("str_"))
		{
			...
			$ipropTemlates = new \Freetrix\Iblock\InheritedProperty\ElementTemplates($IBLOCK_ID, $WF_ID);
		}
		else
		{
			$WF_ID=0;
			...
			$ipropTemlates = new \Freetrix\Iblock\InheritedProperty\ElementTemplates($IBLOCK_ID, 0);
			$ipropTemlates->getValuesEntity()->setParents($str_IBLOCK_ELEMENT_SECTION);
		}
		$str_IPROPERTY_TEMPLATES = $ipropTemlates->findTemplates();
		$str_IPROPERTY_TEMPLATES["ELEMENT_PREVIEW_PICTURE_FILE_NAME"] = \Freetrix\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["ELEMENT_PREVIEW_PICTURE_FILE_NAME"]);
		$str_IPROPERTY_TEMPLATES["ELEMENT_DETAIL_PICTURE_FILE_NAME"] = \Freetrix\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["ELEMENT_DETAIL_PICTURE_FILE_NAME"]);
	}
	...
	if ( !$bAutocomplete && !$bPropertyAjax )
	{
		...
		$aMenu[] = array(
			...
			"LINK"=>"/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
		...
		if($ID > 0 && !$bCopy)
		{
			...
			$arSubMenu[] = array(
				...
				"LINK" => "/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, 0, array(
					"IBLOCK_SECTION_ID" => $MENU_SECTION_ID,
					"find_section_section" => intval($find_section_section),
				)),
				'ICON' => '',
			...
			if($bWorkflow && !defined("CATALOG_PRODUCT"))
			{
				$arSubMenu[] = array(
					...
					"LINK" => '/freetrix/admin/iblock_history_list.php?ELEMENT_ID='.$ID.'&type='.urlencode($arIBlock["IBLOCK_TYPE_ID"]).'&lang='.LANGUAGE_ID.'&IBLOCK_ID='.$IBLOCK_ID.'&find_section_section='.intval($find_section_section),
				);
			}
...
<?
	...
	if($arTranslit["TRANSLITERATION"] == "Y")
	{
		...
		function set_linked()
		{
			...
			{
				if(linked)
					name_link.src='/freetrix/themes/.default/icons/iblock/link.gif';
				else
					name_link.src='/freetrix/themes/.default/icons/iblock/unlink.gif';
			}
			var code_link = document.getElementById('code_link');
			if(code_link)
			{
				if(linked)
					code_link.src='/freetrix/themes/.default/icons/iblock/link.gif';
				else
					code_link.src='/freetrix/themes/.default/icons/iblock/unlink.gif';
			}
	...
	<script type="text/javascript">
		var InheritedPropertiesTemplates = new JCInheritedPropertiesTemplates(
			...
			'/freetrix/admin/iblock_templates.ajax.php?ENTITY_TYPE=E&IBLOCK_ID=<?echo intval($IBLOCK_ID)?>&ENTITY_ID=<?echo intval($ID)?>'
		);
		BX.ready(function(){
			setTimeout(function(){
...
$tabControl->Begin(array(
	"FORM_ACTION" => "/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, null, $arEditLinkParams)
));
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	?>
		<tr id="tr_NAME">
			...
			<td style="white-space: nowrap;">
				<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/freetrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
	...
	?>
		<tr id="tr_CODE">
			<td><?echo $tabControl->GetCustomLabelHTML()?></td>
			<td style="white-space: nowrap;">
				<input type="text" size="50" name="CODE" id="CODE" maxlength="255" value="<?echo $str_CODE?>"><image id="code_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/freetrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
...
?>
	...
	<tr id="tr_PREVIEW_TEXT_EDITOR">
		<td colspan="2" align="center">
			...
			false,
			array(
				'toolbarConfig' => CFileman::GetEditorToolbarConfig("iblock_".(defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1 ? 'public' : 'admin')),
	...
	<tr id="tr_DETAIL_TEXT_EDITOR">
		<td colspan="2" align="center">
			<?CFileMan::AddHTMLEditorFrame(
				...
				true,
				false,
				array('toolbarConfig' => CFileman::GetEditorToolbarConfig("iblock_".(defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1 ? 'public' : 'admin')), 'saveEditorKey' => $IBLOCK_ID)
			);
		?></td>
	</tr>
	<?else:?>
...
$tabControl->EndCustomField("DETAIL_TEXT",
	...
	?>
	<?
	$tabControl->BeginCustomField("IPROPERTY_TEMPLATES_ELEMENT_PAGE_TITLE", GetMessage("IBEL_E_SEO_PAGE_TITLE"));
	?>
	<tr class="adm-detail-valign-top">
		<td width="40%"><?echo $tabControl->GetCustomLabelHTML()?></td>
	...
	<?else:?>
		...
		<td width="60%">
			...
			?>
				...
				<input type="button" value="<?echo GetMessage("IBLOCK_ELEMENT_EDIT_PROP_ADD")?>..." onClick="jsUtils.OpenWindow('/freetrix/admin/iblock_section_search.php?lang=<?echo LANGUAGE_ID?>&amp;IBLOCK_ID=<?echo $IBLOCK_ID?>&amp;n=input_IBLOCK_SECTION&amp;m=y', 600, 500);">
		</td>
	<?endif;?>
	</tr>
...
if ($arShowTabs['catalog'])
{
	...
	include($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/catalog/admin/templates/product_edit.php");
...
if ($arShowTabs['sku'])
{
	...
	$strSubElementAjaxPath = '/freetrix/admin/iblock_subelement_admin.php?WF=Y&IBLOCK_ID='.$intSubIBlockID.'&type='.urlencode($strSubIBlockType).'&lang='.LANGUAGE_ID.'&find_section_section=0&find_el_property_'.$arSubCatalog['SKU_PROPERTY_ID'].'='.((0 == $ID) || ($bCopy) ? '-'.$TMP_ID : $ID).'&TMP_ID='.urlencode($strSubTMP_ID);
	if ($boolIncludeOffers && file_exists($_SERVER["DOCUMENT_ROOT"].'/freetrix/modules/iblock/admin/templates/iblock_subelement_list.php'))
	{
		require($_SERVER["DOCUMENT_ROOT"].'/freetrix/modules/iblock/admin/templates/iblock_subelement_list.php');
	}
	else
	{
...
if ($arShowTabs['bizproc']):
	...
	foreach ($arDocumentStates as $arDocumentState)
	{
		...
		<tr class="heading">
			<td colspan="2">
				...
				<?if (strlen($arDocumentState["ID"]) > 0 && strlen($arDocumentState["WORKFLOW_STATUS"]) > 0):?>
					(<a href="<?echo htmlspecialcharsbx("/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
						"WF"=>$WF,
						"find_section_section" => $find_section_section,
						"stop_bizproc" => $arDocumentState["ID"],
					),  "&".freetrix_sessid_get()))?>"><?echo GetMessage("IBEL_BIZPROC_STOP")?></a>)
				<?endif;?>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td width="60%"><?if (strlen($arDocumentState["ID"]) > 0):?><a href="/freetrix/admin/bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>"><?endif;?><?= strlen($arDocumentState["STATE_TITLE"]) > 0 ? $arDocumentState["STATE_TITLE"] : $arDocumentState["STATE_NAME"] ?><?if (strlen($arDocumentState["ID"]) > 0):?></a><?endif;?></td>
		</tr>
		<?endif;?>
		<?
		if (strlen($arDocumentState["ID"]) <= 0)
			...
			if (count($arTasks) > 0)
			{
				...
				<tr>
					...
					<td width="60%">
						...
						{
							?><a href="/freetrix/admin/bizproc_task.php?id=<?= $arTask["ID"] ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>" title="<?= strip_tags($arTask["DESCRIPTION"]) ?>"><?= $arTask["NAME"] ?></a><br /><?
						}
						?>
					</td>
				</tr>
	...
	if ($ID > 0):
		...
		if ($bStartWorkflowPermission):
			...
			<tr>
				<td colspan="2" align="center">
					<a href="/freetrix/admin/<?=MODULE_ID?>_start_bizproc.php?document_id=<?= $ID ?>&document_type=<?= DOCUMENT_TYPE ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>"><?echo GetMessage("IBEL_BIZPROC_START")?></a>
				</td>
			</tr>
			<?
		endif;
...
$tabControl->Show();
if (
	(!defined('FX_PUBLIC_MODE') || FX_PUBLIC_MODE != 1)
...
)
{
	...
	echo
		...
		' <a href="/freetrix/admin/iblock_edit.php?type='.htmlspecialcharsbx($type).'&amp;lang='.LANGUAGE_ID.'&amp;ID='.$IBLOCK_ID.'&amp;admin=Y&amp;return_url='.urlencode("/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array("WF" => ($WF=="Y"? "Y": null), "find_section_section" => intval($find_section_section), "return_url" => (strlen($return_url)>0? $return_url: null)))).'">',
