...
$APPLICATION->AddHeadScript('/freetrix/js/catalog/tbl_edit.js');

$arJSDescription = array(
	'js' => '/freetrix/js/iblock/sub_generator.js',
	'css' => '/freetrix/panel/iblock/sub-generator.css',
	'lang' => '/freetrix/modules/iblock/lang/'.LANGUAGE_ID.'/admin/iblock_subelement_generator.php'
);
CJSCore::RegisterExt('iblock_generator', $arJSDescription);
CJSCore::Init(array('iblock_generator'));

use Freetrix\Highloadblock as HL;
...
{
	$arIBlockProperty['ID'] = intval($arIBlockProperty['ID']);
	$propertyType = $arIBlockProperty["PROPERTY_TYPE"];
	$userType = (string)$arIBlockProperty["USER_TYPE"];
	$isMultiply = ($arIBlockProperty["MULTIPLE"] == 'Y') ? true : false;
...
if(!$bReadOnly && check_freetrix_sessid())
{
	...
	if(strlen($_POST['save']) > 0)
	{
		...
		$parentElement = new \Freetrix\Iblock\Template\Entity\Element($parentElementId);
		...
		foreach($arCombinationResult as $arPropertySaveValues)
		{
			...
			{
				if(count(array_diff(array_diff_assoc($arPropertyValueCombinationResult[$keyRow], $arPropertySaveValues), array(-1))) == 0)
				{
					$imageRowId = $keyRow;
				}
			}
