...
elseif(!$bReload && $_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST["save"]) || isset($_POST["apply"])))
{
	...
	{
		$res = $ibp->Update($str_PROPERTY_ID, $arFields);
	}
	else
	{
...
<?
	...
	$bShow = true;
	if(is_array($arPropertyFields["SHOW"]) && in_array("MULTIPLE", $arPropertyFields["SHOW"]))
		$bShow = true;
	elseif(is_array($arPropertyFields["HIDE"]) && in_array("MULTIPLE", $arPropertyFields["HIDE"]))
		$bShow = false;
	...
	<td>
		...
		is_array($arPropertyFields["SET"]) && array_key_exists("MULTIPLE", $arPropertyFields["SET"])
	)
	{
