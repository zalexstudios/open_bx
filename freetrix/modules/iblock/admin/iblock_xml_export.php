	...
	elseif($ABS_FILE_NAME && (count($arErrors) == 0))
	{
		if($NS["STEP"] < 1)
		{
			...
			if($fp = fopen($ABS_FILE_NAME, "wb"))
			{
				if(strtolower(substr($ABS_FILE_NAME, -4)) == ".xml")
				{
					if(@mkdir($DIR_NAME = substr($ABS_FILE_NAME, 0, -4)."_files", FX_DIR_PERMISSIONS));
					{
						$_SESSION["FX_CML2_EXPORT"]["work_dir"] = $WORK_DIR_NAME;
						$_SESSION["FX_CML2_EXPORT"]["file_dir"] = substr($DIR_NAME."/", strlen($WORK_DIR_NAME));
					}
				}
			}
			else
		...
		elseif($NS["STEP"] < 4)
		{
			if($fp = fopen($ABS_FILE_NAME, "ab"))
			{
				...
				if($obExport->Init($fp, $NS["IBLOCK_ID"], $NS["next_step"], true, $_SESSION["FX_CML2_EXPORT"]["work_dir"], $_SESSION["FX_CML2_EXPORT"]["file_dir"]))
				{
					...
					if($NS["STEP"]==1)
					{
						...
						$obExport->ExportProperties($_SESSION["FX_CML2_EXPORT"]["PROPERTY_MAP"]);
						$NS["STEP"]++;
					}
					elseif($NS["STEP"]==2)
					{
						$result = $obExport->ExportSections(
							$_SESSION["FX_CML2_EXPORT"]["SECTION_MAP"],
							$start_time,
							$INTERVAL,
							$NS["SECTIONS_FILTER"],
							$_SESSION["FX_CML2_EXPORT"]["PROPERTY_MAP"]
						);
						if($result)
						{
							$NS["SECTIONS"] += $result;
