...
if($STEP == 0)
{
	$_SESSION["FX_IBLOCK_CONV"] = false;
}
elseif(is_array($_SESSION["FX_IBLOCK_CONV"]))
{
	$ar = array(
		"DONE" => intval($_SESSION["FX_IBLOCK_CONV"]["DONE"]),
		"TODO" => intval($_SESSION["FX_IBLOCK_CONV"]["TODO"]),
		"arMultiple" => array(),
		"arSingle" => array(),
		"arNumber" => array(),
	);
	if(is_array($_SESSION["FX_IBLOCK_CONV"]["arMultiple"]))
	{
		foreach($_SESSION["FX_IBLOCK_CONV"]["arMultiple"] as $id)
		{
			$id = intval($id);
			$ar["arMultiple"][$id] = $id;
		}
	}
	if(is_array($_SESSION["FX_IBLOCK_CONV"]["arSingle"]))
	{
		foreach($_SESSION["FX_IBLOCK_CONV"]["arSingle"] as $id)
		{
			$id = intval($id);
			$ar["arSingle"][$id] = $id;
		}
	}
	if(is_array($_SESSION["FX_IBLOCK_CONV"]["arNumber"]))
	{
		foreach($_SESSION["FX_IBLOCK_CONV"]["arNumber"] as $id)
		{
			$id = intval($id);
			$ar["arNumber"][$id] = $id;
		}
	}
	$_SESSION["FX_IBLOCK_CONV"] = $ar;
}
else
{
	$_SESSION["FX_IBLOCK_CONV"] = false;
...
if(!CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_edit") || (!check_freetrix_sessid() && $STEP > 0))
{
...
if(count($arErrors)==0):?>
	...
	<?elseif($STEP>=1 && $STEP<=5):?>
		<?if($WAY==12):?>
			...
			<?if(is_array($_SESSION["FX_IBLOCK_CONV"])):?>
				<li><?=$STEP==3?'<b>':''?><?=GetMessage("IBCONV_PROGRESS",array("#DONE#"=>$_SESSION["FX_IBLOCK_CONV"]["DONE"],"#TODO#"=>$_SESSION["FX_IBLOCK_CONV"]["TODO"]))?><?=$STEP==3?'</b>':''?></li>
			<?else:?>
...
function DoNext(way, step){
	...
	queryString+='&<?echo freetrix_sessid_get()?>';
...
function NextStep12($arIBlock)
{
	...
	while($arID = $rsID->Fetch())
	{
		...
		if(count($_SESSION["FX_IBLOCK_CONV"]["arMultiple"]) > 0)
		{
			$strSql = "
				...
				AND IBLOCK_PROPERTY_ID IN (".implode(", ", $_SESSION["FX_IBLOCK_CONV"]["arMultiple"]).")
			";
			$rs = $DB->Query($strSql);
		}

		if(count($_SESSION["FX_IBLOCK_CONV"]["arSingle"]))
		{
			$arFields=array();
			$strSql = "
				SELECT *
				FROM b_iblock_element_property
				WHERE IBLOCK_ELEMENT_ID = ".$ELEMENT_ID."
				AND IBLOCK_PROPERTY_ID IN (".implode(", ", $_SESSION["FX_IBLOCK_CONV"]["arSingle"]).")
			";
			$rs = $DB->Query($strSql);
			while($ar = $rs->Fetch())
			{
	...
	$_SESSION["FX_IBLOCK_CONV"]["DONE"]+=$i;
	if($_SESSION["FX_IBLOCK_CONV"]["TODO"]<$_SESSION["FX_IBLOCK_CONV"]["DONE"])
		$_SESSION["FX_IBLOCK_CONV"]["TODO"]=$_SESSION["FX_IBLOCK_CONV"]["DONE"];
	//$_SESSION["FX_IBLOCK_CONV"]["STAT"][]=$i;$arMessages[]=implode(', ',$_SESSION["FX_IBLOCK_CONV"]["STAT"]);
	if($arID)
		return 3;
	else
		return 4;
	...
	while($arElement = $rsElement->Fetch())
	{
		...
		foreach($arElement as $key=>$value)
		{
			if(substr($key, 0 ,9)=="PROPERTY_" && strlen($value)>0)
			{
				...
				if($ID > 0 && array_key_exists($ID, $_SESSION["FX_IBLOCK_CONV"]["arSingle"]))
				{
					$arFields = array(
