...
)
{
	...
	{
		$SECTION_PICTURE_FILE_NAME = \Freetrix\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["SECTION_PICTURE_FILE_NAME"]);
		$SECTION_DETAIL_PICTURE_FILE_NAME = \Freetrix\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["SECTION_DETAIL_PICTURE_FILE_NAME"]);
	...
	else
	{
		...
		{
			$ipropValues = new \Freetrix\Iblock\InheritedProperty\SectionValues($IBLOCK_ID, $ID);
		...
		if(strlen($apply) <= 0 && strlen($save_and_add) <= 0)
		{
			...
			{
				LocalRedirect("/freetrix/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section))));
			}
		}
		elseif(strlen($save_and_add) > 0)
		{
			if (defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1)
			{
				...
				?>
					<script type="text/javascript">
						top.BX.ajax.post(
							'/freetrix/admin/<?echo $l = CUtil::JSEscape(CIBlock::GetAdminSectionEditLink($IBLOCK_ID, 0, array(
			...
			{
				LocalRedirect("/freetrix/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, 0, array(
					"find_section_section" => intval($find_section_section),
					"IBLOCK_SECTION_ID" => $IBLOCK_SECTION_ID,
					"return_url" => (strlen($return_url) > 0? $return_url: null),
				), "&".$tabControl->ActiveTabParam()));
		...
		{
			LocalRedirect("/freetrix/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, $ID, array(
				'find_section_section' => intval($find_section_section),
				'return_url' => strlen($return_url) > 0? $return_url: null,
			), "&".$tabControl->ActiveTabParam()));
		}
...
{
	$ID = 0;
	$ipropTemlates = new \Freetrix\Iblock\InheritedProperty\SectionTemplates($IBLOCK_ID, 0);
}
else
{
	$ipropTemlates = new \Freetrix\Iblock\InheritedProperty\SectionTemplates($IBLOCK_ID, $ID);
}
...
else
{
	...
	$str_IPROPERTY_TEMPLATES["SECTION_PICTURE_FILE_NAME"] = \Freetrix\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["SECTION_PICTURE_FILE_NAME"]);
	$str_IPROPERTY_TEMPLATES["SECTION_DETAIL_PICTURE_FILE_NAME"] = \Freetrix\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["SECTION_DETAIL_PICTURE_FILE_NAME"]);
...
<script type="text/javascript">
	var InheritedPropertiesTemplates = new JCInheritedPropertiesTemplates(
		...
		'/freetrix/admin/iblock_templates.ajax.php?ENTITY_TYPE=S&IBLOCK_ID=<?echo intval($IBLOCK_ID)?>&ENTITY_ID=<?echo intval($ID)?>'
	);
	BX.ready(function(){
		setTimeout(function(){
...
$tabControl->Begin(array(
	"FORM_ACTION" => "/freetrix/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, null, array("find_section_section" => intval($find_section_section))),
));
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	?>
		<tr id="tr_NAME">
			<td><?echo $tabControl->GetCustomLabelHTML()?></td>
			<td nowrap>
				<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="name_link" title="<?echo GetMessage("IBSEC_E_LINK_TIP")?>" class="linked" src="/freetrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
	...
	?>
		<tr id="tr_CODE">
			...
			<td nowrap>
				...
				<input type="text" size="50" name="CODE" id="CODE" maxlength="255" value="<?echo $str_CODE?>"><image id="code_link" title="<?echo GetMessage("IBSEC_E_LINK_TIP")?>" class="linked" src="/freetrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
...
)
{
	...
	if($USER_FIELD_MANAGER->GetRights($entity_id) >= "W")
	{
		...
		?>
			<tr colspan="2">
				<td align="left">
					<a href="/freetrix/admin/userfield_edit.php?lang=<?echo LANGUAGE_ID?>&amp;ENTITY_ID=<?echo urlencode($entity_id)?>&amp;back_url=<?echo urlencode($APPLICATION->GetCurPageParam('', array('bxpublic'))."&tabControl_active_tab=user_fields_tab")?>"><?echo $tabControl->GetCustomLabelHTML()?></a>
				</td>
			</tr>
		<?
		...
		<tr><td align="center" colspan="2">
			...
			function addSectionProperty(iblock_id, select_id, shadow_id, table_id)
			{
				...
				if(select && select.value == 0)
				{
					...
					(new BX.CDialog({
						'content_url' : '/freetrix/admin/iblock_edit_property.php?lang=<?echo LANGUAGE_ID?>&IBLOCK_ID='+iblock_id+'&ID=n0&bxpublic=Y&from_module=iblock&return_url=section_edit',
						'width' : 700,
						'height' : 400,
...
if(strlen($return_url)>0)
	...
	$bu = "/freetrix/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section)));
...
if(CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_edit") && (!defined('FX_PUBLIC_MODE') || FX_PUBLIC_MODE != 1))
{
	echo
		BeginNote(),
