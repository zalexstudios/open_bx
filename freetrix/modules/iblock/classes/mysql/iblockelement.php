...
class CIBlockElement extends CAllIBlockElement
{
	...
	function Update($ID, $arFields, $bWorkFlow=false, $bUpdateSearch=true, $bResizePictures=false, $bCheckDiskQuota=true)
	{
		...
		if($bWorkFlow)
		{
			...
			{
				$pr_val_id = $arProp['PROPERTY_VALUE_ID'];var_dump($pr_val_id);
		...
		$ipropTemplates = new \Freetrix\Iblock\InheritedProperty\ElementTemplates($ar_element["IBLOCK_ID"], $ar_element["ID"]);
		if(array_key_exists("PREVIEW_PICTURE", $arFields) && is_array($arFields["PREVIEW_PICTURE"]))
		{
			...
			else
			{
				...
				$arFields["PREVIEW_PICTURE"]["name"] = \Freetrix\Iblock\Template\Helper::makeFileName(
					$ipropTemplates
		...
		else
		{
			...
			{
				$ipropTemplates = new \Freetrix\Iblock\InheritedProperty\ElementTemplates($arIBlock["ID"], $ID);
	...
	function SetPropertyValues($ELEMENT_ID, $IBLOCK_ID, $PROPERTY_VALUES, $PROPERTY_CODE = false)
	{
		...
		global $FX_IBLOCK_PROP_CACHE;
		if(!is_set($FX_IBLOCK_PROP_CACHE, $IBLOCK_ID))
			$FX_IBLOCK_PROP_CACHE[$IBLOCK_ID] = Array();
		if(is_set($FX_IBLOCK_PROP_CACHE[$IBLOCK_ID], $uniq_flt))
			$ar_prop = &$FX_IBLOCK_PROP_CACHE[$IBLOCK_ID][$uniq_flt];
		else
		{
			...
			$FX_IBLOCK_PROP_CACHE[$IBLOCK_ID][$uniq_flt] = &$ar_prop;
		}
		Reset($ar_prop);
