 ...
 */
	...
	function CAdminSubList($table_id, $sort=false,$list_url,$arHideHeaders = false)
	{
		...
		$arJSDescr = array(
			'js' => '/freetrix/js/iblock/subelement.js',
			'rel' => array('admin_interface')
		);
		if ($this->bPublicMode)
		{
			$arJSDescr['css'] = '/freetrix/panel/iblock/sub-public.css';
		}
			...
			if(!empty($this->link))
			{
				if (true == $this->boolBX)
					$sDefAction = "(new BX.CAdminDialog({
						'content_url': '".$this->link.(!(defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1) ? '&bxsku=Y' : '')."&bxpublic=Y&".freetrix_sessid_get()."',
			...
			else
			{
				...
				if (true == $this->boolBX)
					$sDefAction = "(new BX.CAdminDialog({
						...
						'content_post': '".(!(defined('FX_PUBLIC_MODE') && FX_PUBLIC_MODE == 1) ? '&bxsku=Y' : '')."&bxpublic=Y&from_module=iblock&".freetrix_sessid_get()."',
						'draggable': true,
						'resizable': true,
						'buttons': [BX.CAdminDialog.btnSave, BX.CAdminDialog.btnCancel]
					})).Show();";
 ...
 */
	...
	function CAdminSubForm($name, $tabs, $bCanExpand = true, $bDenyAutosave = false, $list_url = '', $boolShowSettings = false)
	{
		...
		$arJSDescr = array(
			'js' => '/freetrix/js/iblock/subelementdet.js',
			'css' => '/freetrix/panel/iblock/sub-detail-public.css',
			'rel' => array('admin_interface')
		);
...
class CAdminSubResult extends CAdminResult
{
	...
	function GetNavPrint($title, $show_allways=true, $StyleText="", $template_path=false, $arDeleteParam=false)
	{
		if($template_path === false)
			$template_path = $_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/iblock/admin/templates/navigation.php";
