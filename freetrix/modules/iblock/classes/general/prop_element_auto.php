<?
use Freetrix\Main\Localization\Loc as Loc;
	...
	public function GetPropertyFieldHtml($arProperty, $arValue, $strHTMLControlName)
	{
		...
		else
		{
			...
			elseif ('E' == $arSettings['VIEW'])
			{
				...
				onclick="jsUtils.OpenWindow('/freetrix/admin/iblock_element_search.php?lang=<? echo LANGUAGE_ID; ?>&IBLOCK_ID=<? echo $arProperty["LINK_IBLOCK_ID"]; ?>&n=&k=&lookup=<? echo 'jsMLI_'.$control_id; ?>', 900, 600);"><?
			}
		...
		else
		{
			...
			elseif ('E' == $arSettings['VIEW'])
			{
				?><input
					...
					onclick="jsUtils.OpenWindow('/freetrix/admin/iblock_element_search.php?lang=<? echo LANGUAGE_ID; ?>&IBLOCK_ID=<? echo $arProperty["LINK_IBLOCK_ID"]; ?>&n=&k=&m=y&lookup=<? echo 'jsMLI_'.$control_id; ?>', 900, 600);"><?
			}
			if ('Y' == $arProperty['USER_TYPE_SETTINGS']['SHOW_ADD'])
			{
				if ('Y' == $arSettings['IBLOCK_MESS'])
