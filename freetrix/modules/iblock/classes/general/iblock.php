...
class CAllIBlock
{
	...
	function AddPanelButtons($mode, $componentName, $arButtons)
	{
		...
		$arImages = array(
			"add_element" => (defined("PANEL_ADD_ELEMENT_BTN")) ? PANEL_ADD_ELEMENT_BTN : "/freetrix/images/iblock/icons/new_element.gif",
			"edit_element" => (defined("PANEL_EDIT_ELEMENT_BTN")) ? PANEL_EDIT_ELEMENT_BTN : "/freetrix/images/iblock/icons/edit_element.gif",
			"edit_iblock" => (defined("PANEL_EDIT_IBLOCK_BTN")) ? PANEL_EDIT_IBLOCK_BTN : "/freetrix/images/iblock/icons/edit_iblock.gif",
			"history_element" => (defined("PANEL_HISTORY_ELEMENT_BTN")) ? PANEL_HISTORY_ELEMENT_BTN : "/freetrix/images/iblock/icons/history.gif",
			"edit_section" => (defined("PANEL_EDIT_SECTION_BTN")) ? PANEL_EDIT_SECTION_BTN : "/freetrix/images/iblock/icons/edit_section.gif",
			"add_section" => (defined("PANEL_ADD_SECTION_BTN")) ? PANEL_ADD_SECTION_BTN : "/freetrix/images/iblock/icons/new_section.gif",
			"element_list" => "/freetrix/themes/.default/icons/iblock/mnu_iblock_el.gif",
			"section_list" => "/freetrix/themes/.default/icons/iblock/mnu_iblock_sec.gif",
		);
		...
		if(count($arButtons[$mode]) > 0)
		{
			...
			if(strlen($componentName) <= 0 && function_exists("debug_backtrace"))
			{
				...
				foreach($arTrace as $arCallInfo)
				{
					if(array_key_exists("file", $arCallInfo))
					{
						...
						if(preg_match("#.*/freetrix/components/(.+?)/(.+?)/#", $file, $match))
						{
			...
			$arPanelButton = array(
				"SRC" => "/freetrix/images/iblock/icons/iblock.gif",
				"ALT" => $componentName,
				"TEXT" => $componentName,
				"MAIN_SORT" => 300,
				"SORT" => 30,
	...
	{
		$arImages = array(
			"add_element" => "/freetrix/images/iblock/icons/new_element.gif",
			"edit_element" => "/freetrix/images/iblock/icons/edit_element.gif",
			"edit_iblock" => "/freetrix/images/iblock/icons/edit_iblock.gif",
			"history_element" => "/freetrix/images/iblock/icons/history.gif",
			"edit_section" => "/freetrix/images/iblock/icons/edit_section.gif",
			"add_section" => "/freetrix/images/iblock/icons/new_section.gif",
			"element_list" => "/freetrix/themes/.default/icons/iblock/mnu_iblock_el.gif",
			"section_list" => "/freetrix/themes/.default/icons/iblock/mnu_iblock_sec.gif",
		);
	...
	public static function GetPanelButtons($IBLOCK_ID=0, $ELEMENT_ID=0, $SECTION_ID=0, $arOptions=array())
	{
		...
		foreach($return_url as $key => $url)
		{
			if(empty($url))
			{
				if(empty($str))
				{
					if(defined("FX_AJAX_PARAM_ID"))
						$str = $APPLICATION->GetCurPageParam("", array(FX_AJAX_PARAM_ID));
					else
		...
		{
			$url = "/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ELEMENT_ID, array(
			...
			if($bWorkflow)
			{
				$url = "/freetrix/admin/iblock_history_list.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&ELEMENT_ID=".$ELEMENT_ID."&filter_section=".$SECTION_ID."&return_url=".UrlEncode($return_url["history_element"]);
				$arButton = array(
		...
		{
			$url = "/freetrix/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, null, array(
				"force_catalog" => $bCatalog,
				"filter_section" => $SECTION_ID,
		...
		if($ELEMENT_ID <= 0 && $bSectionButtons)
		{
			...
			if(($arIBTYPE = $rsIBTYPE->Fetch()) && ($arIBTYPE["SECTIONS"] == "Y"))
			{
				if($SECTION_ID > 0 && CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $SECTION_ID, "section_edit"))
				{
					...
					$url = "/freetrix/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, $SECTION_ID, array(
						"force_catalog" => $bCatalog,
						"filter_section" => $SECTION_ID,
						"bxpublic" => "Y",
						"from_module" => "iblock",
				...
				{
					$url = "/freetrix/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, null, array(
						"force_catalog" => $bCatalog,
						"IBLOCK_SECTION_ID" => $SECTION_ID,
						"filter_section" => $SECTION_ID,
						"bxpublic" => "Y",
		...
		if( ($IBLOCK_ID > 0) && CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_admin_display") )
		{
			...
			if($SECTION_ID > 0)
				$url = "/freetrix/admin/".CIBlock::GetAdminElementListLink($IBLOCK_ID , array('find_section_section'=>$SECTION_ID));
			else
				$url = "/freetrix/admin/".CIBlock::GetAdminElementListLink($IBLOCK_ID , array('find_el_y'=>'Y'));
			...
			$url = "/freetrix/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>$SECTION_ID));
			$arButton = array(
			...
			{
				$url = "/freetrix/admin/iblock_edit.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&ID=".$IBLOCK_ID."&return_url=".UrlEncode($return_url["edit_iblock"]);
				$arButton = array(
	...
	function Add($arFields)
	{
		...
		else
		{
			...
			{
				$ipropTemplates = new \Freetrix\Iblock\InheritedProperty\IblockTemplates($ID);
...
**************/
	...
	public static function ReplaceDetailUrl($url, $arr, $server_name = false, $arrType = false)
	{
		...
		if($server_name)
		{
			...
			if((defined("ADMIN_SECTION") && ADMIN_SECTION===true) || !defined("FX_STARTED"))
			{
				static $cache = array();
