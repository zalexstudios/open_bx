...
class CIBlockPropertyEnumResult extends CDBResult
{
	...
	{
		$a = parent::Fetch();
		if($a && defined("FX_COMP_MANAGED_CACHE"))
		{
...
class CIBlockPropertyEnum
{
	...
	function Update($ID, $arFields)
	{
		...
		if (defined("FX_COMP_MANAGED_CACHE") && IntVal($arFields["PROPERTY_ID"]) > 0)
			$CACHE_MANAGER->ClearByTag("iblock_property_enum_".$arFields["PROPERTY_ID"]);
	...
	{
		global $DB, $CACHE_MANAGER;
		static $FX_IBLOCK_ENUM_CACHE = array();

		$ID=intval($ID);

		if(!array_key_exists($ID, $FX_IBLOCK_ENUM_CACHE))
		{
			if(CACHED_b_iblock_property_enum===false)
			{
				$rs = $DB->Query("SELECT * from b_iblock_property_enum WHERE ID=".$ID);
				$FX_IBLOCK_ENUM_CACHE[$ID] = $rs->Fetch();
			}
			else
			{
				...
				for($i=$bucket*$bucket_size;$i<$max;$i++)
				{
					if(array_key_exists($i, $arEnums))
						$FX_IBLOCK_ENUM_CACHE[$i]=$arEnums[$i];
					else
						$FX_IBLOCK_ENUM_CACHE[$i]=false;
				}
			}
		}

		return $FX_IBLOCK_ENUM_CACHE[$ID];
	}
}
?>