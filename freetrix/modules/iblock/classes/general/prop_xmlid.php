...
class CIBlockPropertyXmlID
{
		...
		if(strlen($value["VALUE"])>0)
		{
			if(!array_key_exists($value["VALUE"], $cache))
			{
				...
				if($ar_res)
					...
					' [<a href="'.
					'/freetrix/admin/iblock_element_edit.php?'.
	...
	function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		...
		return  '<input name="'.htmlspecialcharsbx($strHTMLControlName["VALUE"]).'" id="'.htmlspecialcharsbx($strHTMLControlName["VALUE"]).'" value="'.htmlspecialcharsex($value["VALUE"]).'" size="20" type="text">'.
			'<input type="button" value="..." onClick="jsUtils.OpenWindow(\''.CUtil::JSEscape('/freetrix/admin/iblock_element_search.php?lang='.LANG.'&n='.urlencode($strHTMLControlName["VALUE"]).'&get_xml_id=Y&a=b').'\', 600, 500);">'.
