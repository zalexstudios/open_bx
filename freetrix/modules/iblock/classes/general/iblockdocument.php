				...
				if (is_array($fieldValue))
				{
					...
					foreach ($fieldValue as $r)
					{
						...
						if ($arImg = $dbImg->Fetch())
							$result[] = "[url=/freetrix/tools/bizproc_show_file.php?f=".htmlspecialcharsbx($arImg["FILE_NAME"])."&i=".$r."&h=".md5($arImg["SUBDIR"])."]".htmlspecialcharsbx($arImg["ORIGINAL_NAME"])."[/url]";
					}
				}
				else
				{
					...
					if ($arImg = $dbImg->Fetch())
						$result = "[url=/freetrix/tools/bizproc_show_file.php?f=".htmlspecialcharsbx($arImg["FILE_NAME"])."&i=".$fieldValue."&h=".md5($arImg["SUBDIR"])."]".htmlspecialcharsbx($arImg["ORIGINAL_NAME"])."[/url]";
				}
				break;
	...
	*/
	public function GetDocumentAdminPage($documentId)
	{
		...
		if ($ar = $db->Fetch())
		{
			...
			return "/freetrix/admin/iblock_element_edit.php?view=Y&ID=".$documentId."&IBLOCK_ID=".$ar["IBLOCK_ID"]."&type=".$ar["IBLOCK_TYPE_ID"];
		}
				...
				elseif ($propertyValue["PROPERTY_TYPE"] == "F")
				{
					...
					foreach ($arPropertyValue as $v)
					{
						...
						if ($ar)
						{
							...
							$arResult["PROPERTY_".$propertyKey."_printable"][intval($v)] = "[url=/freetrix/tools/bizproc_show_file.php?f=".htmlspecialcharsbx($ar["FILE_NAME"])."&i=".$v."&h=".md5($ar["SUBDIR"])."]".htmlspecialcharsbx($ar["ORIGINAL_NAME"])."[/url]";
						}
					}
				}
				else
