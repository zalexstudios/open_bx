...
class CIBlockCMLImport
{
	...
	function MakeFileArray($file, $fields = array())
	{
		...
		{
			if(
				array_key_exists($this->mess["IBLOCK_XML2_FX_URL"], $file)
				&& strlen($file[$this->mess["IBLOCK_XML2_FX_URL"]]) > 0
			)
			{
				if (CModule::IncludeModule('clouds'))
				{
					$bucket = CCloudStorage::FindBucketByFile($file[$this->mess["IBLOCK_XML2_FX_URL"]]);
					if(is_object($bucket) && $bucket->READ_ONLY === "Y")
					{
						return array(
							"name" => $file[$this->mess["IBLOCK_XML2_FX_ORIGINAL_NAME"]],
							"description" => $file[$this->mess["IBLOCK_XML2_DESCRIPTION"]],
							"tmp_name" => $file[$this->mess["IBLOCK_XML2_FX_URL"]],
							"file_size" => $file[$this->mess["IBLOCK_XML2_FX_FILE_SIZE"]],
							"width" => $file[$this->mess["IBLOCK_XML2_FX_FILE_WIDTH"]],
							"height" => $file[$this->mess["IBLOCK_XML2_FX_FILE_HEIGHT"]],
							"type" => $file[$this->mess["IBLOCK_XML2_FX_FILE_CONTENT_TYPE"]],
							"content" => "", //Fake field in order to avoid warning
							"bucket" => $bucket,
						);
					}
				}
				return CFile::MakeFileArray($this->URLEncode($file[$this->mess["IBLOCK_XML2_FX_URL"]])); //Download from the cloud
			}
		}
		else
		{
			if (strlen($file) > 0)
			{
				...
				$fileId = $this->CheckFileByName($file, $fields);
				if ($fileId > 0)
	...
	function ResizePicture($file, $resize, $fields = array())
	{
		if(strlen($file) <= 0)
			return array("tmp_name"=>"", "del"=>"Y");
		...
		elseif(file_exists($file) && is_file($file))
			;
		elseif(($fileId = $this->CheckFileByName($file, $fields)) > 0)
			return CFile::MakeFileArray($fileId);
		else
			return array("tmp_name"=>"", "del"=>"Y");
	...
	function ImportMetaData($xml_root_id, $IBLOCK_TYPE, $IBLOCK_LID, $bUpdateIBlock = true)
	{
		...
		if($ar)
		{
			...
			while($ar = $rs->Fetch())
			{
				...
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_CODE"])
					$arIBlock["CODE"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_SORT"])
					$arIBlock["SORT"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_LIST_URL"])
					$arIBlock["LIST_PAGE_URL"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_DETAIL_URL"])
					$arIBlock["DETAIL_PAGE_URL"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_SECTION_URL"])
					$arIBlock["SECTION_PAGE_URL"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_INDEX_ELEMENTS"])
					$arIBlock["INDEX_ELEMENT"] = ($ar["VALUE"]=="true") || intval($ar["VALUE"])? "Y": "N";
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_INDEX_SECTIONS"])
					$arIBlock["INDEX_SECTION"] = ($ar["VALUE"]=="true") || intval($ar["VALUE"])? "Y": "N";
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_SECTIONS_NAME"])
					$arIBlock["SECTIONS_NAME"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_SECTION_NAME"])
					$arIBlock["SECTION_NAME"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_ELEMENTS_NAME"])
					$arIBlock["ELEMENTS_NAME"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_ELEMENT_NAME"])
					$arIBlock["ELEMENT_NAME"] = $ar["VALUE"];
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_PICTURE"])
				{
				...
				elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_FX_WORKFLOW"])
			...
			//Also check for non freetrix xml file
	...
	function ImportSectionsProperties($XML_PARENT, $IBLOCK_ID)
	{
		...
		while($ar = $rs->Fetch())
		{
			...
			while($arP = $rsP->Fetch())
			{
				...
				elseif($arP["NAME"] == $this->mess["IBLOCK_XML2_FX_PROPERTY_TYPE"])
					$arField["USER_TYPE_ID"] = $arP["VALUE"];
				elseif($arP["NAME"] == $this->mess["IBLOCK_XML2_FX_IS_REQUIRED"])
					$arField["MANDATORY"] = ($arP["VALUE"]=="true") || intval($arP["VALUE"])? "Y": "N";
				elseif($arP["NAME"] == $this->mess["IBLOCK_XML2_FX_FILTER"])
					$arField["SHOW_FILTER"] = ($arP["VALUE"]=="true") || intval($arP["VALUE"])? "Y": "N";
				elseif($arP["NAME"] == $this->mess["IBLOCK_XML2_FX_SHOW_IN_LIST"])
					$arField["SHOW_IN_LIST"] = ($arP["VALUE"]=="true") || intval($arP["VALUE"])? "Y": "N";
				elseif($arP["NAME"] == $this->mess["IBLOCK_XML2_FX_EDIT_IN_LIST"])
					$arField["EDIT_IN_LIST"] = ($arP["VALUE"]=="true") || intval($arP["VALUE"])? "Y": "N";
				elseif($arP["NAME"] == $this->mess["IBLOCK_XML2_FX_SEARCH"])
					$arField["IS_SEARCHABLE"] = ($arP["VALUE"]=="true") || intval($arP["VALUE"])? "Y": "N";
				elseif($arP["NAME"] == $this->mess["IBLOCK_XML2_FX_SETTINGS"])
	...
	function ImportProperties($XML_PROPERTIES_PARENT, $IBLOCK_ID)
	{
		...
		$arElementFields = array(
			"CML2_ACTIVE" => $this->mess["IBLOCK_XML2_FX_ACTIVE"],
		...
		while($ar = $rs->Fetch())
		{
			...
			while($arP = $rsP->Fetch())
			{
				...
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_SORT"])
					$arProperty["SORT"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_CODE"])
					$arProperty["CODE"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_DEFAULT_VALUE"])
				...
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_ROWS"])
					$arProperty["ROW_COUNT"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_COLUMNS"])
					$arProperty["COL_COUNT"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_LIST_TYPE"])
					$arProperty["LIST_TYPE"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_FILE_EXT"])
					$arProperty["FILE_TYPE"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_FIELDS_COUNT"])
					$arProperty["MULTIPLE_CNT"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_USER_TYPE"])
					$arProperty["USER_TYPE"] = $arP["VALUE"];
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_WITH_DESCRIPTION"])
				...
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_LINKED_IBLOCK"])
				...
				{
					if($arP["VALUE"] == $this->mess["IBLOCK_XML2_TYPE_LIST"])
					{
				...
				elseif($arP["NAME"]==$this->mess["IBLOCK_XML2_FX_USER_TYPE_SETTINGS"])
				{
	...
	function CheckManufacturer($xml)
	{
		...
		else
		{
			return CIBlockPropertyEnum::Add(array(
				"VALUE" => $xml[$this->mess["IBLOCK_XML2_NAME"]],
				"PROPERTY_ID" => $propertyId,
				"DEF" => "N",
				"XML_ID" => $enumXmlId,
			));
	...
	function ImportElements($start_time, $interval)
	{
		...
		if($this->next_step["XML_ELEMENTS_PARENT"])
		{
			...
			while($arParent = $rsParents->Fetch())
			{
				...
				elseif(array_key_exists($this->mess["IBLOCK_XML2_FX_TAGS"], $arXMLElement))
				{
	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
		{
			...
			if(array_key_exists($this->mess["IBLOCK_XML2_FX_TAGS"], $arXMLElement))
				$arElement["TAGS"] = $arXMLElement[$this->mess["IBLOCK_XML2_FX_TAGS"]];
			...
			if(array_key_exists($this->mess["IBLOCK_XML2_PICTURE"], $arXMLElement))
			{
				...
				if($arFile)
				{
					...
					if(strlen($arFile["VALUE"]) > 0)
					{
						...
						{
							$arElement["PREVIEW_PICTURE"] = $this->ResizePicture($arFile["VALUE"], $this->preview, array("PREVIEW_PICTURE"));
					...
					if($prop_id > 0)
					{
						...
						while($arFile = $rsFiles->Fetch())
						{
							...
							if(strlen($arFile["VALUE"]) > 0)
								$arPropFile = $this->ResizePicture($arFile["VALUE"], $this->detail, array("DETAIL_PICTURE", $this->PROPERTY_MAP["CML2_PICTURES"]));
							else
			...
			if(isset($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]]))
			{
				foreach($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]] as $value)
				{
					...
					//check for freetrix extended format
		...
		elseif(array_key_exists($this->mess["IBLOCK_XML2_STORES"], $arXMLElement))
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				...
				array(
					...
					"NAME" => $this->mess["IBLOCK_XML2_STORES"],
				),
				array("ID", "ATTRIBUTES")
			);
		}
		elseif(array_key_exists($this->mess["IBLOCK_XML2_STORE"], $arXMLElement))
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				array("ID" => "asc"),
	...
	function ImportElementPrices($arXMLElement, &$counter, $arParent = false)
	{
		...
		elseif(array_key_exists($this->mess["IBLOCK_XML2_STORES"], $arXMLElement) && $arParent)
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				array("ID" => "asc"),
		...
		elseif(array_key_exists($this->mess["IBLOCK_XML2_STORE"], $arXMLElement) && $arParent)
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				array("ID" => "asc"),
	...
	function ImportSection($xml_tree_id, $IBLOCK_ID, $parent_section_id)
	{
		...
		while($arS = $rsS->Fetch())
		{
			...
			elseif($arS["NAME"]==$this->mess["IBLOCK_XML2_FX_SORT"])
				$arSection["SORT"] = intval($arS["VALUE"]);
			elseif($arS["NAME"]==$this->mess["IBLOCK_XML2_FX_CODE"])
				$arSection["CODE"] = $arS["VALUE"];
			elseif($arS["NAME"] == $this->mess["IBLOCK_XML2_FX_PICTURE"])
			{
			...
			elseif($arS["NAME"] == $this->mess["IBLOCK_XML2_FX_DETAIL_PICTURE"])
			{
				if(strlen($arS["VALUE"]) > 0)
			...
			elseif($arS["NAME"] == $this->mess["IBLOCK_XML2_FX_ACTIVE"])
...
class CIBlockCMLExport
{
	...
	function ExportFile($FILE_ID)
	{
		if($this->work_dir)
		{
			...
			if($arFile)
			{
				...
				{
					return array(
						GetMessage("IBLOCK_XML2_FX_ORIGINAL_NAME") => $arFile["ORIGINAL_NAME"],
						GetMessage("IBLOCK_XML2_DESCRIPTION") => $arFile["DESCRIPTION"],
						GetMessage("IBLOCK_XML2_FX_URL") => urldecode($arFile["SRC"]),
						GetMessage("IBLOCK_XML2_FX_FILE_SIZE") => $arFile["FILE_SIZE"],
						GetMessage("IBLOCK_XML2_FX_FILE_WIDTH") => $arFile["WIDTH"],
						GetMessage("IBLOCK_XML2_FX_FILE_HEIGHT") => $arFile["HEIGHT"],
						GetMessage("IBLOCK_XML2_FX_FILE_CONTENT_TYPE") => $arFile["CONTENT_TYPE"],
					);
				}
				else
				{
	...
	function ExportSectionsProperties($arUserFields)
	{
		...
		foreach($arUserFields as $FIELD_ID => $arField)
		{
			fwrite($this->fp, $this->formatXMLNode(3, GetMessage("IBLOCK_XML2_PROPERTY"), array(
				...
				GetMessage("IBLOCK_XML2_FX_PROPERTY_TYPE") => $arField["USER_TYPE_ID"],
				GetMessage("IBLOCK_XML2_FX_IS_REQUIRED") => $arField["MANDATORY"] == "Y"? "true": "false",
				GetMessage("IBLOCK_XML2_FX_FILTER") => $arField["SHOW_FILTER"] == "Y"? "true": "false",
				GetMessage("IBLOCK_XML2_FX_SHOW_IN_LIST") => $arField["SHOW_IN_LIST"] == "Y"? "true": "false",
				GetMessage("IBLOCK_XML2_FX_EDIT_IN_LIST") => $arField["EDIT_IN_LIST"] == "Y"? "true": "false",
				GetMessage("IBLOCK_XML2_FX_SEARCH") => $arField["IS_SEARCHABLE"] == "Y"? "true": "false",
				GetMessage("IBLOCK_XML2_FX_SETTINGS") => serialize($arField["SETTINGS"]),
			)));
		}
		fwrite($this->fp, "\t\t</".GetMessage("IBLOCK_XML2_GROUPS_PROPERTIES").">\n");
	}
	...
	function ExportSections(&$SECTION_MAP, $start_time, $INTERVAL, $FILTER = "", $PROPERTY_MAP = array())
	{
		...
		while($arSection = $rsSections->Fetch())
		{
			...
			{
				fwrite($this->fp,
					$this->formatXMLNode($level, GetMessage("IBLOCK_XML2_FX_ACTIVE"), ($arSection["ACTIVE"]=="Y"? "true": "false"))
					.$this->formatXMLNode($level, GetMessage("IBLOCK_XML2_FX_SORT"), intval($arSection["SORT"]))
					.$this->formatXMLNode($level, GetMessage("IBLOCK_XML2_FX_CODE"), $arSection["CODE"])
					.$this->formatXMLNode($level, GetMessage("IBLOCK_XML2_FX_PICTURE"), $this->ExportFile($arSection["PICTURE"]))
					.$this->formatXMLNode($level, GetMessage("IBLOCK_XML2_FX_DETAIL_PICTURE"), $this->ExportFile($arSection["DETAIL_PICTURE"]))
				);
				...
				$sectionTemplates = new \Freetrix\Iblock\InheritedProperty\SectionTemplates($this->arIBlock["ID"], $arSection["ID"]);
	...
	function ExportProperties(&$PROPERTY_MAP)
	{
		...
		{
			$arElementFields = array(
				"CML2_ACTIVE" => GetMessage("IBLOCK_XML2_FX_ACTIVE"),
		...
		while($arProp = $rsProp->Fetch())
		{
			...
			if($this->bExtended)
			{
				...
				if ('' != $arProp["USER_TYPE"])
				{
					...
					{
						$strUserSettings = $this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_USER_TYPE_SETTINGS"), serialize($arProp['USER_TYPE_SETTINGS']));
					}
				}
				fwrite($this->fp,
					$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_SORT"), intval($arProp["SORT"]))
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_CODE"), $arProp["CODE"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_PROPERTY_TYPE"), $arProp["PROPERTY_TYPE"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_ROWS"), $arProp["ROW_COUNT"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_COLUMNS"), $arProp["COL_COUNT"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_LIST_TYPE"), $arProp["LIST_TYPE"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_FILE_EXT"), $arProp["FILE_TYPE"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_FIELDS_COUNT"), $arProp["MULTIPLE_CNT"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_LINKED_IBLOCK"), $this->GetIBlockXML_ID($arProp["LINK_IBLOCK_ID"]))
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_WITH_DESCRIPTION"), ($arProp["WITH_DESCRIPTION"]=="Y"? "true": "false"))
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_SEARCH"), ($arProp["SEARCHABLE"]=="Y"? "true": "false"))
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_FILTER"), ($arProp["FILTRABLE"]=="Y"? "true": "false"))
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_USER_TYPE"), $arProp["USER_TYPE"])
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_IS_REQUIRED"), ($arProp["IS_REQUIRED"]=="Y"? "true": "false"))
					.$this->formatXMLNode(4, GetMessage("IBLOCK_XML2_FX_DEFAULT_VALUE"), serialize($arProp["DEFAULT_VALUE"]))
	...
	function StartExportCatalog($with_metadata = true, $changes_only = false)
	{
		...
		if($with_metadata)
		{
			...
			{
				fwrite($this->fp,
					$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_CODE"), $this->arIBlock["CODE"])
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_SORT"), intval($this->arIBlock["SORT"]))
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_LIST_URL"), $this->arIBlock["LIST_PAGE_URL"])
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_DETAIL_URL"), $this->arIBlock["DETAIL_PAGE_URL"])
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_SECTION_URL"), $this->arIBlock["SECTION_PAGE_URL"])
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_PICTURE"), $this->ExportFile($this->arIBlock["PICTURE"]))
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_INDEX_ELEMENTS"), ($this->arIBlock["INDEX_ELEMENT"]=="Y"? "true": "false"))
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_INDEX_SECTIONS"), ($this->arIBlock["INDEX_SECTION"]=="Y"? "true": "false"))
					.$this->formatXMLNode(2, GetMessage("IBLOCK_XML2_FX_WORKFLOW"), ($this->arIBlock["WORKFLOW"]=="Y"? "true": "false"))
				);
				...
				$iblockTemplates = new \Freetrix\Iblock\InheritedProperty\IblockTemplates($this->arIBlock["ID"]);
	...
	function exportElementFields($arElement, $SECTION_MAP)
	{
		...
		if($this->bExtended)
			fwrite($this->fp, "\t\t\t\t<".GetMessage("IBLOCK_XML2_FX_TAGS").">".htmlspecialcharsbx($arElement["TAGS"])."</".GetMessage("IBLOCK_XML2_FX_TAGS").">\n");
	...
	function exportElement($arElement, $SECTION_MAP, $PROPERTY_MAP)
	{
		...
		if(!$this->only_price)
		{
			...
			if($this->bExtended)
			{
				$elementTemplates = new \Freetrix\Iblock\InheritedProperty\ElementTemplates($this->arIBlock["ID"], $arElement["ID"]);
