<?
require_once($_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/search/classes/general/search.php");
...
class CSearch extends CAllSearch
{
	...
	*/
	function MakeSQL($query, $strSqlWhere, $strSort, $bIncSites, $bStem)
	{
		...
		$bWordPos = FX_SEARCH_VERSION > 1 && COption::GetOptionString("search", "use_word_distance") == "Y";
			...
			$strSql = "
				SELECT
					...
					".(count($this->Query->m_stemmed_words)>1 && strpos($query, "searchable_content")!==false
						?(FX_SEARCH_VERSION > 1? ",sct.SEARCHABLE_CONTENT": ",sc.SEARCHABLE_CONTENT")
						: ""
					)."
		...
		foreach($arLang as $lang=>$value)
		{
			...
			if($docLength > 0)
			{
				...
				$strSqlPrefix = "
						...
						(SEARCH_CONTENT_ID, LANGUAGE_ID, STEM, TF".(FX_SEARCH_VERSION > 1? ",PS": "").")
						values
				";
				$maxValuesLen = 2048;
				$strSqlValues = "";
