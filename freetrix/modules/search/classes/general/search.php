...
class CAllSearch extends CDBResult
{
	...
	function Search($arParams, $aSort=array(), $aParamsEx=array(), $bTagsCloud = false)
	{
		...
		else
		{
			...
			$query = $this->Query->GetQueryString((FX_SEARCH_VERSION > 1? "sct": "sc").".SEARCHABLE_CONTENT", $strQuery, $bTagsSearch, $aParamsEx["STEMMING"], $this->_opt_ERROR_ON_EMPTY_STEM);
	...
	function PrepareSearchResult($str)
	{
		...
		$matches = array();
		if (preg_match_all("/(".$pregMask.")/i".FX_UTF_PCRE_MODIFIER, $strUpp, $matches, PREG_SET_ORDER|PREG_OFFSET_CAPTURE))
		{
			...
			foreach($arExcTmp as $mask)
			{
				...
				if(strlen($mask))
				{
					if(preg_match("#^/[a-z0-9_.\\\\]+/#i", $mask))
						$arFullExc[] = "'^".$mask."$'".FX_UTF_PCRE_MODIFIER;
					else
						$arExc[] = "'^".$mask."$'".FX_UTF_PCRE_MODIFIER;
				}
			}
	...
	function QueryMnogoSearch(&$xml)
	{
		...
		$strRequest.= "User-Agent: FreetrixSM\r\n";
		$strRequest.= "Accept: */*\r\n";
...
class CAllSearchQuery
{
	...
	function ParseStr($qwe)
	{
		...
		$qwe = trim(preg_replace("/[^".$letters."]+/".FX_UTF_PCRE_MODIFIER, " ", $qwe));

		// query language normalizer
		if(!$this->no_bool_lang)
		{
			$qwe=preg_replace("/(\\s+|^|[|&~])or(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1|\\2", $qwe);
			$qwe=preg_replace("/(\\s+|^|[|&~])and(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1&\\2", $qwe);
			$qwe=preg_replace("/(\\s+|^|[|&~])not(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1~\\2", $qwe);
			$qwe=preg_replace("/(\\s+|^|[|&~])without(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1~\\2", $qwe);

			if($this->rus_bool_lang == 'yes')
			{
				$qwe=preg_replace("/(\\s+|^|[|&~])".GetMessage("SEARCH_TERM_OR")."(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1|\\2", $qwe);
				$qwe=preg_replace("/(\\s+|^|[|&~])".GetMessage("SEARCH_TERM_AND")."(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1&\\2", $qwe);
				$qwe=preg_replace("/(\\s+|^|[|&~])".GetMessage("SEARCH_TERM_NOT_1")."(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1~\\2", $qwe);
				$qwe=preg_replace("/(\\s+|^|[|&~])".GetMessage("SEARCH_TERM_NOT_2")."(\\s+|\$|[|&~])/is".FX_UTF_PCRE_MODIFIER, "\\1~\\2", $qwe);
			}
		}

		$qwe=preg_replace("/(\\s*\\|+\\s*)/is".FX_UTF_PCRE_MODIFIER, "|", $qwe);
		$qwe=preg_replace("/(\\s*\\++\\s*|\\s*\\&\\s*)/is".FX_UTF_PCRE_MODIFIER, "&", $qwe);
		$qwe=preg_replace("/(\\s*\\~+\\s*)/is".FX_UTF_PCRE_MODIFIER, "~", $qwe);

		$qwe=preg_replace("/\s*([()])\s*/s".FX_UTF_PCRE_MODIFIER,"\\1",$qwe);
		...
		$qwe=preg_replace("/(\s+|\&\|+|\|\&+)/s".FX_UTF_PCRE_MODIFIER, $default_op, $qwe);
		...
		$qwe=preg_replace("/([^\&\~\|\(\)]+)~([^\&\~\|\(\)]+)/s".FX_UTF_PCRE_MODIFIER,"\\1".$default_op."~\\2", $qwe);
		$qwe=preg_replace("/\)~{1,}/s".FX_UTF_PCRE_MODIFIER,")".$default_op."~", $qwe);
		$qwe=preg_replace("/~{1,}\(/s".FX_UTF_PCRE_MODIFIER, ($default_op=="|"? "~|(": "&~("), $qwe);
		$qwe=preg_replace("/\)([^\&\~\|\(\)]+)/s".FX_UTF_PCRE_MODIFIER, ")".$default_op."\\1", $qwe);
		$qwe=preg_replace("/([^\&\~\|\(\)]+)\(/s".FX_UTF_PCRE_MODIFIER, "\\1".$default_op."(", $qwe);
		$qwe=preg_replace("/\) *\(/s".FX_UTF_PCRE_MODIFIER, ")".$default_op."(", $qwe);
	...
	function StemWord($w)
	{
		...
		if(!$this->no_bool_lang)
		{
			...
			{
				if($preg_ru === false)
					$preg_ru = "/^(".ToUpper(GetMessage("SEARCH_TERM_OR")."|".GetMessage("SEARCH_TERM_AND")."|".GetMessage("SEARCH_TERM_NOT_1")."|".GetMessage("SEARCH_TERM_NOT_2")).")$/".FX_UTF_PCRE_MODIFIER;
	...
	function StemQuery($q, $lang="en")
	{
		...
		return preg_replace_callback("/([".$arStemInfo["pcre_letters"]."]+)/".FX_UTF_PCRE_MODIFIER, array("CAllSearchQuery", "StemWord"), $q);
	}
