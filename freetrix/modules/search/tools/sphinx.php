...
class CSearchSphinx extends CSearchFullText
{
	...
	public function connect($connectionIndex, $indexName = "", $ignoreErrors = false)
	{
		...
		if (!preg_match("/^[a-zA-Z0-9_]+\$/", $indexName))
		{
			if ($ignoreErrors)
				...
				throw new \Freetrix\Main\Db\ConnectionException('Sphinx connect error', GetMessage("SEARCH_SPHINX_CONN_ERROR_INDEX_NAME"));
			return false;
		}
		...
		if (!function_exists("mysql_connect"))
		{
			if ($ignoreErrors)
				...
				throw new \Freetrix\Main\Db\ConnectionException('Sphinx connect error', GetMessage("SEARCH_SPHINX_CONN_EXT_IS_MISSING"));
			return false;
		}
		...
		if (!$this->db)
		{
			if ($ignoreErrors)
				...
				throw new \Freetrix\Main\Db\ConnectionException('Sphinx connect error', GetMessage("SEARCH_SPHINX_CONN_ERROR", array("#ERRSTR#" => mysql_error())));
			return false;
		}
	...
	public function replace($ID, $arFields)
	{
		...
		if ($result)
			...
			throw new \Freetrix\Main\Db\ConnectionException('Sphinx select error', mysql_error($this->db));
	}
...
class CSearchSphinxFormatter
{
	...
	function formatRow($r)
	{
		...
		$rs = $DB->Query("
			select
				...
				,scsite.SITE_ID
				,scsite.URL SITE_URL
				".(FX_SEARCH_VERSION > 1? ",sc.USER_ID": "")."
	...
	public function buildExcerpts($str)
	{
		...
		{
			throw new \Freetrix\Main\Db\ConnectionException('Sphinx select error', mysql_error($this->sphinx->db));
		}
	}
}
