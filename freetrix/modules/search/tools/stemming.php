...
function stemming_init($sLang="ru")
{
	...
	if($sLang !== false && !isset($arStemFunc[$sLang]))
	{
		...
		{
			$strFileName=$_SERVER["DOCUMENT_ROOT"].FX_PERSONAL_ROOT."/php_interface/".$sLang."/search/stemming.php";
			if(file_exists($strFileName))
				@include($strFileName);
			if(!function_exists("stemming_".$sLang))
			{
				$strFileName=$_SERVER["DOCUMENT_ROOT"]."/freetrix/modules/search/tools/".$sLang."/stemming.php";
				if(file_exists($strFileName))
					@include($strFileName);
				if(!function_exists("stemming_".$sLang))
				{
...
function stemming_split($sText, $sLang="ru")
{
	...
	$tok = " ";
	$sText = preg_replace("/[^".$arStemFunc["pcre_letters"]."]/".FX_UTF_PCRE_MODIFIER, $tok, ToUpper($sText));
...
function stemming($sText, $sLang="ru", $bIgnoreStopWords = false, $bReturnPositions = false)
{
	...
	$pcre_abc = "/[^".$arStemInfo[$sLang]["pcre_abc"]."]+/".FX_UTF_PCRE_MODIFIER;
	...
	if($bReturnPositions)
	{
		$sText = preg_replace("/[^".$arStemInfo[$sLang]["pcre_letters"].".!?]+/".FX_UTF_PCRE_MODIFIER, $tok, ToUpper($sText));
		$sText = preg_replace("/[!?]+/".FX_UTF_PCRE_MODIFIER, ".", $sText);
	}
	else
	{
		$sText = preg_replace("/[^".$arStemInfo[$sLang]["pcre_letters"]."]+/".FX_UTF_PCRE_MODIFIER, $tok, ToUpper($sText));
...
function stemming_detect($word, $arStemInfo, $skipLang)
{
	...
	foreach($arStemInfo as $sGuessLang => $arInfo)
	{
		...
		if(preg_match("/[^".$arInfo["pcre_abc"]."]+/".FX_UTF_PCRE_MODIFIER, $word))
			continue;
