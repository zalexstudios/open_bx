...
function perfmonErrorHandler($errno, $errstr, $errfile, $errline)
{
	...
	$uni_file_name = str_replace("\\", "/", substr($errfile, strlen($_SERVER["DOCUMENT_ROOT"].FX_ROOT)));
	$bRecord = false;
	switch($errno)
	{
		case E_WARNING:
...
class CAllPerfomanceKeeper
{
	...
	function setDebugModeOn()
	{
		...
		define("PERFMON_STARTED", $DB->ShowSqlStat."|".\Freetrix\Main\Data\Cache::getShowCacheStat()."|".$APPLICATION->ShowIncludeStat);

		$DB->ShowSqlStat = true;
		\Freetrix\Main\Data\Cache::setShowCacheStat(COption::GetOptionString("perfmon", "cache_log") === "Y");
	...
	function restoreDebugMode()
	{
		...
		\Freetrix\Main\Data\Cache::setShowCacheStat($toRestore[1]);
	...
	function writeToDatabase()
	{
		...
		if($_SERVER["SCRIPT_NAME"] == "/freetrix/urlrewrite.php" && isset($_SERVER["REAL_FILE_PATH"]))
	...
	function findCaller($trace, &$module_id, &$comp_id)
	{
		...
		foreach($trace as $i => $arCallInfo)
		{
			if(array_key_exists("file", $arCallInfo))
			{
				...
				{
					$match = array();
					if(preg_match("#.*/freetrix/modules/(.+?)/#", $file, $match))
					{
						$module_id = $match[1];
					}
				}
				...
				if (
					!$comp_id
					&& preg_match("#.*/(?:freetrix|install)/components/(.+?)/(.+?)/#", $file, $match)
				)
				{
