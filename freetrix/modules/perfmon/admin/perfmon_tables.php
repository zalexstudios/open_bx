	...
	foreach($arTABLES as $table_name)
	{
		...
		case "orm":
			...
			if (count($tableParts) > 1)
				array_shift($tableParts);
			$className = \Freetrix\Main\Entity\Base::snake2camel(implode("_", $tableParts));
			...
			echo "File: /freetrix/modules/".$tableParts[0]."/lib/".strtolower($className).".php";
			echo "<hr>";
			echo "<pre>";
			echo "&lt;", "?", "php\n";
			echo "namespace Freetrix\\".$moduleNamespace.";\n";
			echo "\n";
			echo "use Freetrix\\Main\\Entity;\n";
			echo "use Freetrix\\Main\\Localization\\Loc;\n";
			...
			foreach ($arParents as $columnName => $parentInfo)
			{
				...
				$parentClassName = \Freetrix\Main\Entity\Base::snake2camel(implode("_", $parentTableParts));

				$columnName = preg_replace("/_ID\$/", "", $columnName);
				echo " * &lt;li&gt; ".$columnName
					." reference to {@link \\Freetrix\\".$parentModuleNamespace
					."\\".$parentClassName."Table}"
				."\n";
			}
			echo " * &lt;/ul&gt;\n";
			echo " *\n";
			echo " * @package Freetrix\\".$moduleNamespace."\n";
			echo " *"."*/\n";
			echo "\n";
			echo "class ".$className."Table extends Entity\DataManager\n";
			echo "{\n";
			echo "\tpublic static function getFilePath()\n";
			echo "\t{\n";
			echo "\t\treturn _"."_FILE_"."_;\n";
			echo "\t}\n";
			echo "\n";
			...
			foreach ($arFields as $columnName => $columnInfo)
			{
				...
				{
					$validateFunctionName = "validate".\Freetrix\Main\Entity\Base::snake2camel($columnName);
					echo "\t\t\t\t'validation' => array(_"."_CLASS_"."_, '".$validateFunctionName."'),\n";
					$arValidators[$validateFunctionName] = $columnInfo["length"];
				}
			...
			foreach ($arParents as $columnName => $parentInfo)
			{
				...
				echo "\t\t\t\t'data_type' => 'Freetrix\\".$parentModuleNamespace."\\".$parentClassName."',\n";
				echo "\t\t\t\t'reference' => array('=this.".$columnName."' => 'ref.".$parentInfo["PARENT_COLUMN"]."'),\n";
				echo "\t\t\t),\n";
			}
			echo "\t\t);\n";
			echo "\t}\n";
			foreach ($arValidators as $validateFunctionName => $length)
			{
				echo "\tpublic static function ".$validateFunctionName."()\n";
				echo "\t{\n";
				echo "\t\treturn array(\n";
				echo "\t\t\tnew Entity\\Validator\\Length(null, ".$length."),\n";
				echo "\t\t);\n";
				echo "\t}\n";
			}
			echo "}\n";
			echo "</pre>";
			echo "File: /freetrix/modules/".$tableParts[0]."/lang/ru/lib/".strtolower($className).".php";
			echo "<hr>";
			echo "<pre>";
			echo "&lt;", "?\n";
...
while($arRes = $rsData->GetNext())
{
	...
	$row->AddViewField("TABLE_NAME", '<a class="table_name" id="'.$arRes["TABLE_NAME"].'" href="perfmon_table.php?lang='.LANGUAGE_ID.'&amp;table_name='.urlencode($arRes["TABLE_NAME"]).'">'.$arRes["TABLE_NAME"].'</a>');
...
function filter_rows()
{
	...
	if(input != prev)
	{
		...
		for(var i = 0; i < hrefs.length; i++)
		{
			if(input.length == 0 || match(hrefs[i].id, input))
			{
