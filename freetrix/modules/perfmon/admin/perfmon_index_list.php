...
)
{
	...
	if($go)
	{
		...
		?>
		<script>
			<?echo $lAdmin->ActionDoGroup(0, "analyze_cont", freetrix_sessid_get()."&last_id=".$last_id);?>
		</script>
		<?
...
if($go || !CPerfomanceKeeper::IsActive())
{
	$aContext[] = array(
		...
		"LINK" => "javascript:".$lAdmin->ActionDoGroup(0, "analyze_start", freetrix_sessid_get()),
	);
}
...
function sendRequest()
{
	if(this.toolTip)
		...
		BX.ajax.get(
			'perfmon_sql_list.php?ajax_tooltip=y' + '&sessid=' + BX.message('freetrix_sessid') + '&sql_id=' + this.id,
...
function Analyze()
{
	var url = 'perfmon_index_list.php?lang=<?echo LANGUAGE_ID?>&<?echo freetrix_sessid_get()?>&action=analyze';
	ShowWaitWindow();
	BX.ajax.post(
		url,
		null,
