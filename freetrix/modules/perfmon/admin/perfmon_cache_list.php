...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if ($f_FILE_NAME != "")
	{
		...
		if (
			...
			$row->AddViewField("FILE_NAME", '<a target="blank" href="/freetrix/admin/fileman_file_view.php?path='.urlencode(substr($f_FILE_PATH, $DOCUMENT_ROOT_LEN)).'&lang='.LANGUAGE_ID.'">'.$f_FILE_NAME.'</a>');
	}
	...
	if($f_BASE_DIR === "/freetrix/managed_cache/")
		$BASE_DIR = GetMessage("PERFMON_CACHE_MANAGED");
	elseif($f_BASE_DIR === "/freetrix/cache/")
...
if ($group == "none")
{
	...
	<tr>
		...
		<td>
			<input type="text" size="25" name="find" value="<?echo htmlspecialcharsbx($find)?>" title="<?=GetMessage("PERFMON_CACHE_FIND")?>">
			<?
			$arr = array(
				"reference" => array(
	...
	<tr>
		...
		<td><input type="text" name="find_module_name" size="47" value="<?echo htmlspecialcharsbx($find_module_name)?>"></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><?
			$arr = array(
				"reference"=>array(GetMessage("PERFMON_CACHE_OP_MODE_R"), GetMessage("PERFMON_CACHE_OP_MODE_W"), GetMessage("PERFMON_CACHE_OP_MODE_C")),
				"reference_id"=>array("R", "W", "C"),
			);
	...
	<tr>
		...
		<td><input type="text" name="find_base_dir" size="47" value="<?echo htmlspecialcharsbx($find_base_dir)?>"></td>
	</tr>
	<tr>
		<td><?=GetMessage("PERFMON_CACHE_INIT_DIR")?></td>
		<td><input type="text" name="find_init_dir" size="47" value="<?echo htmlspecialcharsbx($find_init_dir)?>"></td>
	</tr>
	<tr>
		<td><?=GetMessage("PERFMON_CACHE_FILE_NAME")?></td>
		<td><input type="text" name="find_file_name" size="47" value="<?echo htmlspecialcharsbx($find_file_name)?>"></td>
	</tr>
	<?
