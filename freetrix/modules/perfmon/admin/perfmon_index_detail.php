...
/*.require_module 'hash';.*/
/*.require_module 'freetrix_main_include_prolog_admin_before';.*/
...
if($_SERVER["REQUEST_METHOD"] === "POST" && check_freetrix_sessid() && $isAdmin)
{
	if(isset($_REQUEST["create_index"]) && isset($_REQUEST["ddl"]))
	{
		...
		if(is_object($res))
		{
			...
			LocalRedirect("/freetrix/admin/perfmon_index_detail.php?ID=".$ID."&lang=".LANGUAGE_ID."&".$tabControl->ActiveTabParam());
		}
		else
	...
	elseif(isset($_REQUEST["ban_index"]))
	{
		...
		LocalRedirect("/freetrix/admin/perfmon_index_list.php?lang=".LANGUAGE_ID);
	}
}
	...
	<tr>
		...
		<tr>
			...
			<td style="text-align:center"><?
				...
				if($proc > 5)
					echo '<br><font class="errortext">',round($proc, 4),'%</font><br>'.GetMessage("PERFMON_IDETAIL_QUERY_IS_BAD");
				else
					echo '<br><font class="notetext">',round($proc, 4),'%</font><br>'.GetMessage("PERFMON_IDETAIL_QUERY_IS_GOOD");
			?></td>
			<?endforeach?>
		</tr>
		<?endforeach?>
